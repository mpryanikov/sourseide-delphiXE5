...
{$ENDIF}
  ...
  TFDGUIxScriptProgressEvent = procedure (ASender: TObject;
    const AInfoProvider: IFDGUIxScriptDialogInfoProvider) of object;
  ...
  TFDScriptOuputKind = (soSeparator, soEcho, soScript, soInfo, soError, soConnect,
  ...
  IFDGUIxScriptDialog = interface(IUnknown)
    ...
    procedure Output(const AStr: String; AKind: TFDScriptOuputKind);
...
function FDGUIxSilent: Boolean;
begin
  Result := FADGUIxSilentMode or (TThread.CurrentThread.ThreadID <> MainThreadID);
end;

{-------------------------------------------------------------------------------}
initialization

  FADGUIxSilentMode := {$IFDEF MSWINDOWS} IsConsole {$ELSE} False {$ENDIF};
{$IFDEF MSWINDOWS}
  if IsConsole then
    FADGUIxProvider := C_FD_GUIxConsoleProvider
  else
{$ENDIF}
  if FDIsDesignTime then
    FADGUIxProvider := C_FD_GUIxFormsProvider
...
{$ENDIF}
    ...
    FADGUIxProvider := C_FD_GUIxFMXProvider;
