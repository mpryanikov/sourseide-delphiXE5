...
uses
  ...
  FireDAC.Stan.Intf, FireDAC.Stan.Storage;
...
type
  ...
  private
    ...
    FADObjName: String;
  protected
    procedure SetADObjectName(const AValue: IFDStanObject);
    procedure LoadFromStorage(AStorage: TFDStorage); virtual;
    procedure SaveToStorage(AStorage: TFDStorage); virtual;
  public
    ...
    property FDCode: Integer read FADCode write FADCode;
    property FDObjName: String read FADObjName write FADObjName;
  end;
  ...
  private
    ...
    FRetryLevel: Integer;
  public
    constructor Create(ATimes, AOffset: LongInt; AException: EFDDBEngineException;
  ...
  TFDErrorEvent = procedure (ASender: TObject; const AInitiator: IFDStanObject;
...
function ADDBEngineExceptionLoad(AStorage: TFDStorage): TObject;
function ADDBEngineExceptionCreate(AClass: EFDDBEngineExceptionClass;
  ACode: Integer; const AArgs: array of const): EFDDBEngineException;
function FDExceptionLoad(AStorage: TFDStorage): TObject;
procedure FDExceptionSave(AObject: TObject; AStorage: TFDStorage);
...
begin
  ...
  if Supports(AObj, IFDStanObject, oObj) then
    AEx.SetADObjectName(oObj);
  oEx := Exception(AEx);
  if Supports(AObj, IFDStanErrorHandler, oHndlr) then begin
    try
...
{-------------------------------------------------------------------------------}
function ADDBEngineExceptionCreate(AClass: EFDDBEngineExceptionClass; ACode: Integer;
...
{-------------------------------------------------------------------------------}
constructor EFDException.Create(AADCode: Integer; const AMessage: String);
begin
...
{-------------------------------------------------------------------------------}
procedure EFDException.SetADObjectName(const AValue: IFDStanObject);
var
  oObj: IFDStanObject;
begin
  oObj := AValue;
  ...
  repeat
    if FADObjName <> '' then
      FADObjName := '.' + FADObjName;
    FADObjName := oObj.Name + FADObjName;
    oObj := oObj.Parent;
  until oObj = nil;
end;
...
{-------------------------------------------------------------------------------}
procedure EFDException.LoadFromStorage(AStorage: TFDStorage);
begin
...
{-------------------------------------------------------------------------------}
procedure EFDException.SaveToStorage(AStorage: TFDStorage);
begin
...
{-------------------------------------------------------------------------------}
procedure TFDDBError.LoadFromStorage(AStorage: TFDStorage);
begin
...
{-------------------------------------------------------------------------------}
procedure TFDDBError.SaveToStorage(AStorage: TFDStorage);
begin
...
begin
  ...
  EFDDBEngineException(AValue).FParams.Assign(FParams);
...
{-------------------------------------------------------------------------------}
procedure EFDDBEngineException.LoadFromStorage(AStorage: TFDStorage);
var
  oErr: TFDDBError;
begin
  inherited LoadFromStorage(AStorage);
  while not AStorage.IsObjectEnd('DBError') do begin
    AStorage.ReadObjectBegin('DBError');
    try
      oErr := AppendError;
      oErr.LoadFromStorage(AStorage);
    finally
      AStorage.ReadObjectEnd('DBError');
    end;
  end;
end;

{-------------------------------------------------------------------------------}
procedure EFDDBEngineException.SaveToStorage(AStorage: TFDStorage);
var
  i: Integer;
  oErr: TFDDBError;
begin
  ...
  for i := 0 to ErrorCount - 1 do begin
    AStorage.WriteObjectBegin('DBError');
    try
      oErr := Errors[i];
      oErr.SaveToStorage(AStorage);
    finally
      AStorage.WriteObjectEnd('DBError');
    end;
  end;
end;
...
initialization
  ...
  FDStorageManager().RegisterClass(EFDDBEngineException, 'DBEngineException',
    @ADDBEngineExceptionLoad, @FDExceptionSave);
