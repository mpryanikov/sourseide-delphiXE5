...
type
  ...
  public
    ...
    function AppendError(const ASeverity, AErrorCode, AMessage, ADetailedMessage,
      ...
      AContext, ASourceFile: String; ASourceLine: Integer;
      const ASourceFunction: String): TFDDBError; overload;
  end;
...
{$ENDIF}
  ...
  public
    ...
    property FDSize: Integer read FADSize;
  ...
  private
    ...
    function AddBase(AId: OID; const AName: String; AFixedLen, APGSize,
      AADSize: Integer; AAttrs: TPgTypeAttrs): TPgType;
  ...
  public
    ...
    destructor Destroy; override;
    class procedure GetDefaults(ATypeOid: Oid; out ALen, APrec, AScale: Integer);
  ...
  public
    FHandle: PPGconn;
    constructor Create(AEnv: TPgEnv; AOwningObj: TObject);
    constructor CreateUsingHandle(AEnv: TPgEnv; AHandle: PPGconn; AOwningObj: TObject);
    destructor Destroy; override;
  ...
  private
    ...
    FDumpLabel: String;
    function Pg2AdData(AType: TPgType; ASource: Pointer; ASourceLen: Integer;
      var ADest: Pointer; var ADestLen: LongWord; AByRef: Boolean): Boolean;
    function Ad2PgData(AType: TPgType; ASource: Pointer; ASourceLen: Integer;
      var ADest: Pointer; var ADestLen: LongWord; var ADestFormat: Integer;
      AByRef: Boolean): Boolean;
{$IFDEF FireDAC_MONITOR}
    function DumpSQLDataType: UnicodeString;
    function DumpADValue(ABuffer: Pointer; ALen: Integer): UnicodeString;
{$ENDIF}
  protected
...
{$ENDIF}
  ...
  public
    ...
    constructor Create(AConnection: TPgConnection; AReadOnly: Boolean;
    ...
    procedure Write(ABuff: PByte; ABuffLen: Integer);
...
procedure j2date(const AValue: Int64; out AYear, AMonth, FDay: Word);
procedure dtInt2time(AValue: Int64; out AHour, AMin, ASec: Word; out AFsec: Longword);
procedure dtDbl2time(AValue: Double; out AHour, AMin, ASec: Word; out AFsec: Longword);
function date2j(AYear, AMonth, FDay: Word): Int64;
function time2IntT(const AHour, AMin, ASec: Word; const AFsec: Longword): Int64;
function time2DblT(const AHour, AMin, ASec: Word; const AFsec: Longword): Double;
...
procedure SetPgNumeric(const ASource: TBcd; ADest: Pointer; out ALength: LongWord);
...
{-------------------------------------------------------------------------------}
procedure TFDPgError.LoadFromStorage(AStorage: TFDStorage);
begin
...
{-------------------------------------------------------------------------------}
procedure TFDPgError.SaveToStorage(AStorage: TFDStorage);    
begin
...
function EPgNativeException.AppendError(const ASeverity, AErrorCode, AMessage,
  ...
  const AInternalQuery, AContext, ASourceFile: String; ASourceLine: Integer;
...
var
  ...
  begin
    i2 := 0;
    while True do begin
      i1 := FDPosEx('"', AMessage, i2 + 1);
      if i1 = 0 then
        Break;
      i2 := FDPosEx('"', AMessage, i1 + 1);
      if i2 = 0 then
        Break;
...
begin
  Result :=
      ...
      ([paArray, paRecord] * Members[0].TypeRef.Attrs = []);
end;
...
begin
  ...
  AddBase(SQL_TIME,       'time',           0,  SizeOf(TPgTimeStampBuf),  SizeOf(LongInt),    []);
  ...
  AddBase(SQL_TIMETZ,     'timetz',         0,  SizeOf(TPgTimeStampBuf),  SizeOf(LongInt),    []);
...
function TPgTypesManager.AddBase(AId: OID; const AName: String; AFixedLen,
  APGSize, AADSize: Integer; AAttrs: TPgTypeAttrs): TPgType;
begin
  Result := TPgType.Create;
  Result.FId := AId;
  Result.FName := AName;
...
function TPgTypesManager.Describe(AOid: OID): TPgType;
  ...
  procedure DescribeError(const AReason: String);
  begin
    FDException(FConnection.OwningObj, [S_FD_LPhys, S_FD_PGId],
      er_FD_PgCannotDescribeType, [AOid, AReason]);
  end;
...
begin
  ...
  try
    oStmt.PrepareSQL('SELECT TYPNAME, TYPRELID, TYPELEM, TYPARRAY, TYPBASETYPE, TYPTYPE ' +
                     ...
                     'WHERE OID = ' + IntToStr(AOid));
    oStmt.Execute;
    oStmt.DescribeFields;
    if not oStmt.Fetch then
      DescribeError('Type is not found');
    ...
    else if (iRelId = 0) and (iElem = 0) and (sType <> 'e') then
      if FConnection.UnknownFormat = 0 then
        DescribeError('Complex type must be an array, composite or enum type')
      else
...
begin
  Inc(FStmtIDCounter);
  Result := Format('%dSTM', [FStmtIDCounter]);
end;

{-------------------------------------------------------------------------------}
{ Data conversion                                                               }
{-------------------------------------------------------------------------------}
procedure MoveRotate(ASrc, ADest: Pointer; ALen: LongWord);
var
  pSrc, pDest: PByte;
begin
  pSrc := ASrc;
...
{-------------------------------------------------------------------------------}
function ReadWord(ABuffer: Pointer; AOffset: LongWord = 0): Word;
begin
  MoveRotate(Pointer(NativeUInt(ABuffer) + AOffset), @Result, SizeOf(Result));
end;

{-------------------------------------------------------------------------------}
procedure WriteWord(ABuffer: Pointer; AOffset: LongWord; AValue: Word);
begin
  MoveRotate(@AValue, Pointer(NativeUInt(ABuffer) + AOffset), SizeOf(Word));
end;

{-------------------------------------------------------------------------------}
function ReadInteger(ABuffer: Pointer; AOffset: LongWord = 0): Integer;
begin
  MoveRotate(Pointer(NativeUInt(ABuffer) + AOffset), @Result, SizeOf(Result));
end;
...
begin
  ...
  Result := Result + 7834 * AMonth div 256 + FDay;
end;
...
begin
  ...
  ALength := (4 + iWordInd) * SizeOf(Word);
end;
...
{-------------------------------------------------------------------------------}
function TPgVariable.Pg2AdData(AType: TPgType; ASource: Pointer; ASourceLen: Integer;
  var ADest: Pointer; var ADestLen: LongWord; AByRef: Boolean): Boolean;
var
  iId: Oid;
  pWC: Pointer;

  procedure GetPgDate(ASource: Pointer; out ADest: LongInt);
  var
    iDays: Integer;
  begin
  ...
  procedure GetPgTime(ASource: Pointer; out ADest: LongInt);
  var
...
begin
      ...
      begin
        if (paFixed in AType.Attrs) and Statement.StrsTrim then
          while (ADestLen > 0) and (PFDAnsiString(ASource)[ADestLen - 1] = TFDAnsiChar(' ')) do
            Dec(ADestLen);
        if (ADestLen = 0) and Statement.FStrsEmpty2Null then begin
          Result := False;
          if AByRef then
            ADest := nil;
        end
  ...
  SQL_DATE:
    GetPgDate(ASource, PLongInt(ADest)^);
  SQL_TIME,
  SQL_TIMETZ:
    GetPgTime(ASource, PLongInt(ADest)^);
  SQL_TIMESTAMP:
...
{-------------------------------------------------------------------------------}
function TPgVariable.Ad2PgData(AType: TPgType; ASource: Pointer; ASourceLen: Integer;
  var ADest: Pointer; var ADestLen: LongWord; var ADestFormat: Integer;
  AByRef: Boolean): Boolean;
var
  iByteLen: LongWord;
  ...
  procedure SetPgDate(const ASource: LongInt; ADest: Pointer);
  var
    iDays: Integer;
  begin
  ...
  procedure SetPgTime(const ASource: LongInt; ADest: Pointer);
  var
...
begin
  ...
  SQL_DATE:
    SetPgDate(PLongInt(ASource)^, ADest);
  SQL_TIME,
  SQL_TIMETZ:
    SetPgTime(PLongInt(ASource)^, ADest);
  SQL_TIMESTAMP,
  SQL_TIMESTAMPTZ:
...
{$IFDEF FireDAC_MONITOR}
{-------------------------------------------------------------------------------}
function TPgVariable.DumpSQLDataType: UnicodeString;
begin
  Result := TypeRef.DumpType;
end;

{-------------------------------------------------------------------------------}
function TPgVariable.DumpADValue(ABuffer: Pointer; ALen: Integer): UnicodeString;
var
  s: String;
  sw: UnicodeString;
...
begin
  if (ABuffer = nil) and (ALen = 0) then
    ...
    SQL_ACLITEM:
      if Statement.FConnection.Encoder.Encoding = ecANSI then
        if ALen > 1024 then begin
          s := Statement.FConnection.Encoder.Decode(ABuffer, 1024, ecANSI);
          Result := UnicodeString('(truncated at 1024) ''' + s + ' ...''');
        end
        else begin
          s := Statement.FConnection.Encoder.Decode(ABuffer, ALen, ecANSI);
          Result := UnicodeString('''' + s + '''');
        end
      else
        if ALen > 1024 then begin
          SetString(sw, PWideChar(ABuffer), 1024);
    ...
    SQL_BIT,
    SQL_VARBIT:
      if ALen > 512 then begin
        s := FDBin2Hex(ABuffer, 512);
        Result := UnicodeString('(truncated at 512) ' + s + ' ...');
      end
      else begin
        s := FDBin2Hex(ABuffer, ALen);
        Result := UnicodeString(s);
      end;
    SQL_DATE:
      Result := DateToStr(FDDate2DateTime(PLongint(ABuffer)^));
    SQL_TIME,
    SQL_TIMETZ:
      Result := TimeToStr(FDTime2DateTime(PLongint(ABuffer)^));
    SQL_TIMESTAMP,
    SQL_TIMESTAMPTZ,
    SQL_RELTIME,
    SQL_ABSTIME:
      Result := DateTimeToStr(FDSQLTimeStamp2DateTime(PSQLTimeStamp(ABuffer)^));
    SQL_INTERVAL:
      Result := FDSQLTimeInterval2Str(PFDSQLTimeInterval(ABuffer)^);
    SQL_NUMERIC:
      begin
        FDBCD2Str(aBuff, ALen, PBcd(ABuffer)^, FormatSettings.DecimalSeparator);
        SetString(s, aBuff, ALen);
        Result := UnicodeString(s);
      end;
    SQL_BOOL:
...
begin
  iDataSize := 0;
  if paString in TypeRef.Attrs then begin
    if FSize = 0 then
    ...
    iDataSize := TypeRef.PGSize;

  if (FBuffer <> nil) and (iDataSize = FDataSize) then
    Exit;

  FDataSize := iDataSize;
  AllocBuffer(FDataSize);
end;
...
{$IFDEF FireDAC_MONITOR}
{-------------------------------------------------------------------------------}
function TPgParam.DumpValue: UnicodeString;
var
  pBuf: Pointer;
  iLen: LongWord;
begin
  ...
  if (FValueRef <> nil) and not FIsNull then begin
    Pg2AdData(TypeRef, FValueRef, FValueSize, pBuf, iLen, True);
    if (pBuf = nil) and (iLen > 0) then begin
      pBuf := Statement.FConnection.FBuffer.Check(iLen);
      Pg2AdData(TypeRef, FValueRef, FValueSize, pBuf, iLen, False);
    end;
  end;
  Result := DumpADValue(pBuf, iLen);
end;
{$ENDIF}
...
begin
  ...
  FIsNull := not Ad2PgData(FTypeRef, ApData, ALen, FValueRef, FValueSize, FFormat, AByRef);
end;
...
begin
  ...
  for i := 0 to Count - 1 do
    if not Items[i].FIsNull then
      PPointer(NativeUInt(FValueRefs) + i * SizeOf(Pointer))^ := Items[i].FValueRef
    else
      PPointer(NativeUInt(FValueRefs) + i * SizeOf(Pointer))^ := nil;
  Result := FValueRefs;
end;
  ...
  if ParentField <> nil then
      ...
      if Result then begin
        ...
        while True do begin
          ...
          if iIndex = 0 then
            Break;
          if Result then
            APgData := Pointer(NativeUInt(APgData) + Longword(APgDataLen));
          Dec(iIndex);
        end;
      end;
    end
    ...
    pAttr := PPGresAttValue(NativeUInt(
      PPPGresAttValue(NativeUInt(PPGresult(Statement.FHandle)^.tuples) +
        SizeOf(PPPGresAttValue) * LongWord(Statement.FCurrentRow - Statement.FRowsetOffset))^) +
        SizeOf(pgresAttValue) * LongWord(FIndex));
    Result := pAttr^.len <> NULL_LEN;
    if Result then begin
      APgData := pAttr^.value;
...
begin
  if TypeOid = SQL_PATH then begin
    ...
    if (Length(TypeRef.Members) <> 1) or
       ([paArray, paRecord] * TypeRef.Members[0].TypeRef.Attrs <> []) or
    ...
    ApFirstElem := Pointer(NativeUInt(ApData) + 20);
  end;
end;
...
var
  ...
  begin
    if (AValue < iLBound) or (AValue > iHBound) then
      FDException(Statement.OwningObj, [S_FD_LPhys, S_FD_PGId],
        er_FD_PgArrIndexOutOfBound, [DumpLabel, AValue, iLBound, iHBound]);
  end;

begin
  if FArrayIndex <> AValue then begin
    if AValue >= 0 then begin
      if not GetPgData(pPgData, iPgDataLen) then begin
        iLBound := -1;
        iHBound := -1;
        CheckBounds;
      end;
      ...
      if (TypeOid = SQL_PATH) or (TypeOid = SQL_POLYGON) then begin
        iPgDataLen := 16;
        pPgData := Pointer(NativeUInt(pPgData) + Longword(16 * iIndex));
      end
      else if paFixed in TypeRef.Attrs then begin
        iPgDataLen := iPgDataLen div TypeRef.FixedLen;
        pPgData := Pointer(NativeUInt(pPgData) + Longword(iPgDataLen * iIndex));
      end
      else
        while True do begin
          ...
          if iPgDataLen >= 0 then
            pPgData := Pointer(NativeUInt(pPgData) + Longword(iPgDataLen));
          Dec(iIndex);
        end;
    end
    else begin
...
begin
  ...
  if Result then
    if iPgDataLen = -1 then
      ...
      Result := Pg2AdData(TypeRef, pPgData, iPgDataLen, ApData, ALen, AByRef);
end;
...
begin
  ...
  if GetData(pData, uiLen, True) then
    case Statement.FConnection.Encoder.Encoding of
    ecANSI:
      begin
{$IFDEF NEXTGEN}
        Result := TFDEncoder.Deco(pData, uiLen, Statement.FConnection.Encoder.Encoding);
{$ELSE}
        SetString(sa, PAnsiChar(pData), uiLen);
        Result := String(sa);
{$ENDIF}
      end;
    ecUTF8:
      begin
        SetString(sw, PWideChar(pData), uiLen);
        Result := String(sw);
      end;
    end;
end;
...
{-------------------------------------------------------------------------------}
{$IFDEF FireDAC_MONITOR}
function TPgField.DumpValue: UnicodeString;
var
  pBuf: Pointer;
  iLen: LongWord;
...
begin
  ...
  else if paRecord in TypeRef.Attrs then begin
    Result := '(';
...
begin
  ...
  try
    if ((CompareText(Copy(ASQL, 1, 6), 'SELECT') = 0) or
       ...
       (FRowsetSize > 0) and Assigned(Lib.FPQprepare) then begin
      FCursorName := FConnection.GenerateName;
      FStmtName := InternalPrepare('DECLARE "' + FCursorName + '" CURSOR' +
        C_HOLD[FWithHold] + 'FOR ' + ASQL, True);
      FRowsetStmtName := InternalPrepare('FETCH FORWARD ' + IntToStr(FRowsetSize) +
...
{$ENDIF}
  for i := 0 to FFields.Count - 1 do begin
    ...
    oFld.SetDefaults;
    if [paArray, paRecord] * oFld.TypeRef.Attrs = [] then
...
{$IFDEF FireDAC_MONITOR}
  procedure Trace1;
  begin
    Env.Trace(sPQcmdStatus, ['ppgresult', FHandle, 'stmttype', FSQLStmtType,
      'rowsaffected', FRowsAffected, 'oid', FLastInsertOid]);
  end;

  procedure Trace2;
  begin
    Env.Trace(sPQntuples, ['ppgresult', FHandle, 'rowsselected', FRowsSelected]);
  end;
{$ENDIF}
...
begin
  ...
  iPos := Pos(' ', FSQLStmtType);
  if iPos > 0 then
    Delete(FSQLStmtType, iPos, Length(FSQLStmtType) - iPos + 1);
  ...
  FConnection.FLastInsertOid := FLastInsertOid;
{$IFDEF FireDAC_MONITOR}
  if Env.Tracing then Trace1;
{$ENDIF}
...
{$IFDEF FireDAC_MONITOR}
  ...
  begin
    Env.Trace(sPQexecPrepared, ['ppgconn', FConnection.FHandle,
      'stmtname', AStmtName, 'nParams', AnParams, 'resultFormat', FResultFormat]);
  end;
{$ENDIF}
...
{$ENDIF}
  ...
  if FStmtName <> '' then
    try
      InternalExecute(FStmtName, FParams.Count, FParams.GetValueRefs,
...
{$IFDEF FireDAC_MONITOR}
  ...
  begin
    Env.Trace(sPQexecParams, ['ppgconn', FConnection.FHandle, 'sql', ASQL,
      'nParams', FParams.Count, 'resultFormat', FResultFormat]);
  end;
{$ENDIF}
...
{$ENDIF}
      FHandle := Lib.FPQexecParams(FConnection.FHandle, PByte(
        ...
        FParams.GetValueRefs, FParams.GetValueLengths, FParams.GetValueFormats,
        AResultFormat);
    end;
    Check(FHandle);
  finally
...
{$ENDIF}
  ...
  FRowsetOffset := FRowsSelected;
  FRowsSelected := FRowsSelected + iRows;
  FLastRowset := iRows < FRowsetSize;
end;
...
{$ENDIF}
  Inc(FCurrentRow);
  if not FLastRowset and (FCurrentRow = FRowsSelected) and (FRowsSelected > 0) then
    GetNextRowset;

  FEOF := FCurrentRow = FRowsSelected;
  Result := not FEOF;
...
{ TPgLargeObject                                                                }
{-------------------------------------------------------------------------------}
constructor TPgLargeObject.Create(AConnection: TPgConnection; AReadOnly: Boolean;
  AOwningObj: TObject);
begin
  inherited Create(AConnection.Env, AOwningObj);
  FConnection := AConnection;
...
begin
  Create(AConnection, AReadOnly, AOwningObj);
  FObjOid := AObjOid;
end;
...
begin
  ...
  if sMsg = '' then
    sMsg := 'could not ' + AOperation + ' large object';
...
begin
  FObjOid := Lib.Flo_creat(FConnection.FHandle, 0);
{$IFDEF FireDAC_MONITOR}
  if Env.Tracing then Trace1;
{$ENDIF}
...
{-------------------------------------------------------------------------------}
procedure TPgLargeObject.Close;
var
  iResult: Integer;
...
{-------------------------------------------------------------------------------}
procedure TPgLargeObject.Write(ABuff: PByte; ABuffLen: Integer);
var
  iResult: Integer;
...
{$IFDEF FireDAC_MONITOR}
  ...
  begin
    Env.Trace(slo_write, ['conn', FConnection.FHandle, 'objoid', FObjOid,
      'objhandle', FObjHandle, 'buflen', ABuffLen, 'Result', iResult]);
  end;
{$ENDIF}
...
begin
  ASSERT(FIsOpen);
  CheckReadWrite;
  iResult := Lib.Flo_write(FConnection.FHandle, FObjHandle, ABuff, ABuffLen);
{$IFDEF FireDAC_MONITOR}
  if Env.Tracing then Trace1;
{$ENDIF}
  if iResult = -1 then
    Error('write', -1);
  Inc(FPosition, iResult);
end;

{-------------------------------------------------------------------------------}
function PgNativeExceptionLoad(AStorage: TFDStorage): TObject;
begin
