...
{$IFDEF SHOWOLDWIZARDS}
  ...
  public
    ...
    function GetInetPCHSources(SourceIndex: TSourceIndex; APropValues: TStrings = nil): string;
...
{$IFDEF SHOWOLDWIZARDS}
 TModuleCreator = class(TCreator, IOTACreator, IOTAModuleCreator)
  ...
  // Only for C++ usage
  TPCHModuleCreator = class(TModuleCreator, IOTAModuleCreator)
  private
    FProjectType: TProjectType;
    FSourceIndex: TSourceIndex;
    { IOTAModuleCreator }
  ...
  protected
    ...
    function NewFormFileText(const FormIdent, AncestorIdent: string): string; virtual;
  public
    constructor Create(AProjectType: TProjectType);
  end;
...
begin
  ...
  try
    ...
    begin
      ...
      if woVCLFramework in LWebWizardExt.Options then
        Include(LCreateWebProjectOptions, wpVCLFramework);
...
function TInetSources.GetInetPCHSources(SourceIndex: TSourceIndex;
  APropValues: TStrings): string;
var
  S: TStrings;
begin
  ...
  try
    ...
    Result := GetSourceFromTemplate(WebPCHFileName, S, SourceFlags);
  finally
    S.Free;
  end;
end;
...
begin
  ...
  if Personality = sCBuilderPersonality then
    (BorlandIDEServices as IOTAModuleServices).CreateModule(TPCHModuleCreator.Create(FProjectType));
...
begin
  if (FProjectType in [ptCOM, ptIndyForm]) or (TCreateWebProjectOption.wpVCLFramework in FOptions) then
...
constructor TPCHModuleCreator.Create(AProjectType: TProjectType);
begin
  FProjectType := AProjectType;
  case FProjectType of
    ptISAPI: FSourceIndex := stIsapiPCHSource;
    ptCGI: FSourceIndex := stCgiPCHSource;
    ptIndyForm: FSourceIndex := stIndyVclPCHSource;
    ptIndyConsole: FSourceIndex := stIndyConsolePCHSource;
  end;
end;


function TPCHModuleCreator.GetAncestorName: string;
begin
  Result := '';
end;

function TPCHModuleCreator.GetCreatorType: string;
begin
  Result := sText;
end;


function TPCHModuleCreator.GetImplFileName: string;
var
  Project: IOTAProject;
...
function TPCHModuleCreator.GetOwner: IOTAModule;
begin
  Result := GetActiveProject;
end;

function TPCHModuleCreator.GetShowSource: Boolean;
begin
  Result := false;
end;

function TPCHModuleCreator.GetSource(AIndex: TSourceIndex): string;
var
  LPropValues: TStrings;
begin
  LPropValues := TStringList.Create;
  try
    case FProjectType of
      ptISAPI: LPropValues.Add('IsapiPCHSource=TRUE');
      ptCGI: LPropValues.Add('CgiPCHSource=TRUE');
      ptIndyForm: LPropValues.Add('IndyVclPCHSource=TRUE');
      ptIndyConsole: LPropValues.Add('IndyConsolePCHSource=TRUE');
    end;
    Result := InetSources.GetInetPCHSources(AIndex, LPropValues);
  finally
    LPropValues.Free;
  end;
...
function TPCHModuleCreator.NewFormFile(const FormIdent,
  AncestorIdent: string): IOTAFile;
begin
  Result := nil;
end;

function TPCHModuleCreator.NewFormFileText(const FormIdent,
  AncestorIdent: string): string;
begin
end;

function TPCHModuleCreator.NewImplSource(const ModuleIdent, FormIdent,
  AncestorIdent: string): IOTAFile;
var
  Source: String;
  S: string;
begin
  Source := GetSource(FSourceIndex);
  S := Format(Source, [ModuleIdent, FormIdent, AncestorIdent]);
  Result := TOTAFile.Create(S);
end;

function TPCHModuleCreator.NewImplSourceText(const ModuleIdent, FormIdent,
  AncestorIdent: string): string;
var
  Source: string;
begin
  case FProjectType of
    ptISAPI: Source := InetSources.GetInetPCHSources(stIsapiPCHSource);
    ptCGI: Source := InetSources.GetInetPCHSources(stCgiPCHSource);
    ptIndyForm: Source := InetSources.GetInetPCHSources(stIndyVCLPCHSource);
    ptIndyConsole: Source := InetSources.GetInetPCHSources(stIndyConsolePCHSource);
  end;
  Result := Format(Source, [ModuleIdent, FormIdent, AncestorIdent]);
end;

function TPCHModuleCreator.NewIntfSource(const ModuleIdent, FormIdent,
  AncestorIdent: string): IOTAFile;
begin
  Result := nil;
end;

function TPCHModuleCreator.NewIntfSourceText(const ModuleIdent, FormIdent,
  AncestorIdent: string): string;
begin
