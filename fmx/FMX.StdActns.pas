...
uses System.SysUtils, System.UITypes, System.Classes, System.Actions,
     FMX.Platform, FMX.ActnList, FMX.Types, FMX.Forms, FMX.Controls,
     FMX.VirtualKeyboard;
...
{ TViewAction }
  ...
  /// <summary>
  ///   This base class. Only stored values
  /// </summary>
  ...
  /// <summary>
  ///   This class (without published property) implements a change in the range of values
  /// </summary>
  ...
  /// <summary>
  ///   This class with all published property. Not what new functionality has not.
  ///  See <see cref="TCustomValueRange"/>
  /// </summary>
  ...
  /// <summary>
  ///   Base class action (without published property), with the ability to control values
  /// </summary>
  ...
  /// <summary>
  ///   This class with all published property. Not what new functionality has not.
  ///  See <see cref="TCustomValueRangeAction"/>
  /// </summary>
  ...
  /// <summary>
  /// A class for communication the action and control, which may have
  /// properties <c>Value</c>, <c>Max</c>, <c>Min</c>, etc.
  /// This control must support the interface <see cref="IValueRange"/>.
  /// </summary>
  ...
  /// <summary>
  /// This interface is designed so that action could work with the properties
  /// of <c>Value</c>, <c>Min</c>, <c>Max</c>, <c>Frequency</c>, <c>ViewportSize</c>.
  /// </summary>
...
uses System.RTLConsts, System.TypInfo, System.Math, FMX.Consts;
...
begin
  if Supported and
     Assigned(Screen) then
  begin
    Form := Screen.ActiveForm;
    if Assigned(Form) and
       (not (TFmxFormState.fsModal in Form.FormState)) and
       CanActionExec then
      Form.Close;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      Form := Screen.ActiveForm;
      Enabled := Assigned(Form) and
           (not (TFmxFormState.fsModal in Form.FormState));
    end
    else
      Enabled := False;
  end;
...
begin
  ...
  TPlatformServices.Current.SupportsPlatformService(IFMXHideAppService, IInterface(FHideAppService));
end;
...
begin
  Result := Assigned(FHideAppService);
end;

procedure TFileHideApp.ExecuteTarget(Target: TObject);
begin
  if Assigned(FHideAppService) then
...
begin
  Result := TPlatformServices.Current.SupportsPlatformService(IFMXVirtualKeyboardService, IInterface(FService))
end;

procedure TVirtualKeyboard.ExecuteTarget(Target: TObject);
var Obj: TFmxObject;
    State: TVirtualKeyBoardState;
begin
  ...
  try
    ...
    begin
      ...
      if vksError in State then
        Enabled := False
      else
      begin
        if not (vksVisible in State) then
        begin
        ...
        begin
          if (not (vksAutoShow in State)) and (FService.HideVirtualKeyboard) then
            Checked := False;
        end;
      end;
    end;
