...
uses
  ...
  FMX.StdActns, FMX.PixelFormats, FMX.Controls, FMX.Messages, FMX.Graphics;
...
{ TSwitch }
  ...
  IPresentationObject = interface
  ['{59D283E0-8136-430F-98B4-FD02287E8E10}']
  ...
  ISwitchObject = interface(IPresentationObject)
  ['{02D484DD-B7D5-4576-910E-19E52F130B41}']
  ...
  TSwitch = class(TStyledControl, IIsChecked)
  private
    FSwitchObject: ISwitchObject;
...
begin
  if SupportsPlatformService(IFMXDeviceService, IInterface(DeviceSrv)) and (TDeviceFeature.HasTouchScreen in DeviceSrv.GetFeatures) then
    Result := TRectF.Create(DefaultTouchTargetExpansion, DefaultTouchTargetExpansion, DefaultTouchTargetExpansion, DefaultTouchTargetExpansion)
  else
    Result := inherited ;
end;
...
begin
  if (GroupName <> '') and SameText(TSpeedButtonGroupMessage(M).Value, GroupName) and (Sender <> Self) and
     Assigned(Scene) and (not (Sender is TControl) or ((Sender as TControl).Scene = Scene)) then
    IsPressed := False;
end;
...
var
  DefMetricsSrv: IFMXDefaultMetricsService;
begin
  if SupportsPlatformService(IFMXDefaultMetricsService, IInterface(DefMetricsSrv)) and DefMetricsSrv.SupportsDefaultSize(ckCheckBox) then
    Result := TSizeF.Create(DefMetricsSrv.GetDefaultSize(ckCheckBox).Width, DefMetricsSrv.GetDefaultSize(ckCheckBox).Height)
  else
...
var
  DefMetricsSrv: IFMXDefaultMetricsService;
begin
  if SupportsPlatformService(IFMXDefaultMetricsService, IInterface(DefMetricsSrv)) and DefMetricsSrv.SupportsDefaultSize(ckRadioButton) then
    Result := TSizeF.Create(DefMetricsSrv.GetDefaultSize(ckRadioButton).Width, DefMetricsSrv.GetDefaultSize(ckRadioButton).Height)
  else
    Result := TSizeF.Create(120, 19);
end;
...
begin
  if SameText(TSpeedButtonGroupMessage(M).Value, GroupName) and (Sender <> Self) and Assigned(Scene) and
        ...
        if not (csLoading in ComponentState) then
          case Orientation of
            ...
            TOrientation.orVertical:
              begin
                SetAdjustType(TAdjustType.FixedWidth);
  ...
  begin
    ...
    begin
      if AdjustType <> TAdjustType.None then
        SetAdjustSizeValue(TPointF.Create(Height, Width));
...
begin
  if Assigned(AClient) and (not (AClient is TCustomTrack)) then
...
begin
  inherited;
  Supports(ResourceLink, ISwitchObject, FSwitchObject);
  if Assigned(FSwitchObject) then
  begin
    FSwitchObject.Value := IsChecked;
...
function TSwitch.CanObserve(const ID: Integer): Boolean;
begin
  Result := False;
  if ID = TObserverMapping.EditLinkID then
    Result := True;
...
function TSwitch.GetDefaultTouchTargetExpansion: TRectF;
var
  DeviceSrv: IFMXDeviceService;
begin
  if SupportsPlatformService(IFMXDeviceService, IInterface(DeviceSrv)) and (TDeviceFeature.HasTouchScreen in DeviceSrv.GetFeatures) then
...
procedure TSwitch.SwitchObjectChanged(Sender: TObject);
begin
  IsChecked := FSwitchObject.Value;
end;
...
begin
  inherited;
  if Assigned(FSwitchObject) then
    FSwitchObject.MouseDown(Button, Shift, X, Y);
  end;
...
begin
  ...
  if (ssLeft in Shift) and (FPressing) then
  begin
    FNewValue := PointF(1, 0).AngleBetweenTwoSide(PointF(X - (Width / 2), Y - (Height / 2))) * 180 / Pi;
  ...
  begin
    ...
    try
      ...
      FImage.Bitmap.Canvas.DrawBitmap(FBitmap, RectF(0, 0, FBitmap.Width, FBitmap.Height),
        RectF(0, 0, Round(FImage.Bitmap.Width / FImage.Bitmap.BitmapScale),
          Round(FImage.Bitmap.Height / FImage.Bitmap.BitmapScale)), 1);
    finally
