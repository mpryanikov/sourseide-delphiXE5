...
{$I FireDAC.inc}
{$IFDEF WIN32}
  {$HPPEMIT '#pragma link "FireDAC.Phys.MySQL.obj"'}
{$ELSE}
  {$HPPEMIT '#pragma link "FireDAC.Phys.MySQL.o"'}
{$ENDIF}
...
{$ENDIF}
  ...
  FireDAC.Phys.Intf, FireDAC.Phys.SQLGenerator, FireDAC.Phys.MySQLCli,
    FireDAC.Phys.MySQLWrapper, FireDAC.Phys.MySQLMeta;
...
type
  ...
  TFDPhysMySQLResultMode = (hrStore, hrUse, hrChoose);
  ...
  private
    ...
    FResultMode: TFDPhysMySQLResultMode;
...
{$ENDIF}
  ...
  private
    ...
    function GetResultMode: TFDPhysMySQLResultMode;
...
begin
  ...
  FEmbeddedArgs := TFDStringList.Create;
  FEmbeddedGroups := TFDStringList.Create;
end;
...
procedure TFDPhysMySQLDriverLink.ApplyTo(const AParams: IFDStanDefinition);
  ...
  function ToText(AStrs: TStrings): String;
  var
    i: Integer;
  begin
    Result := '';
...
begin
  ...
  if FEmbeddedArgs.Count > 0 then
    AParams.AsString[S_FD_ConnParam_MySQL_EmbeddedArgs] := ToText(FEmbeddedArgs);
  if FEmbeddedArgs.Count > 0 then
    AParams.AsString[S_FD_ConnParam_MySQL_EmbeddedGroups] := ToText(FEmbeddedGroups);
end;
...
{-------------------------------------------------------------------------------}
function TFDPhysMySQLDriver.GetConnParamCount(AKeys: TStrings): Integer;
begin
  Result := inherited GetConnParamCount(AKeys) + 12;
  if (AKeys <> nil) and
     (CompareText(AKeys.Values[S_FD_ConnParam_MySQL_UseSSL], S_FD_True) = 0) then
    Result := Result + 5;
end;

{-------------------------------------------------------------------------------}
procedure TFDPhysMySQLDriver.GetConnParams(AKeys: TStrings; AIndex: Integer;
  ...
  if AIndex < inherited GetConnParamCount(AKeys) then begin
    ...
    1:
      begin
        ...
        AType := '@I';
        ADefVal := IntToStr(MYSQL_PORT);
        ALoginIndex := 3;
      end;
    2:
      begin
        AName := S_FD_ConnParam_MySQL_Compress;
        AType := '@L';
        ADefVal := S_FD_True;
      end;
    3:
      begin
        AName := S_FD_ConnParam_MySQL_UseSSL;
        AType := '@L';
        ADefVal := S_FD_False;
      end;
    4:
    ...
    5:
      begin
        AName := S_FD_ConnParam_MySQL_ReadTimeout;
        AType := '@I';
        ADefVal := '';
      end;
    6:
      begin
        AName := S_FD_ConnParam_MySQL_WriteTimeout;
        AType := '@I';
        ADefVal := '';
      end;
    7:
      begin
        AName := S_FD_ConnParam_MySQL_ResultMode;
        AType := S_FD_StoreResult + ';' + S_FD_UseResult + ';' + S_FD_ChooseResult;
        ADefVal := S_FD_StoreResult;
      end;
    8:
      begin
    ...
    9:
      begin
        AName := S_FD_ConnParam_MySQL_TinyIntFormat;
        AType := S_FD_Boolean + ';' + S_FD_Integer;
        ADefVal := S_FD_Boolean;
      end;
    10:
      begin
    ...
    11:
      ...
      if (AKeys <> nil) and
        ...
        ADefVal := '';
        case AIndex - inherited GetConnParamCount(AKeys) of
       12: AName := S_FD_ConnParam_MySQL_SSL_key;
       13: AName := S_FD_ConnParam_MySQL_SSL_cert;
       14: AName := S_FD_ConnParam_MySQL_SSL_ca;
       15: AName := S_FD_ConnParam_MySQL_SSL_capath;
       16: AName := S_FD_ConnParam_MySQL_SSL_cipher;
        end;
      end;
    end;
    ACaption := AName;
...
var
  sHost, sDatabase, sUserName, sPwd: String;
  s: String;
  iPort: Integer;
  ...
  var
    uiTimeout: LongWord;
  begin
    if GetConnectionDef.HasValue(AParamName) then begin
      uiTimeout := GetConnectionDef.AsInteger[AParamName];
      FSession.Options[AOption] := @uiTimeout;
    end;
  end;

begin
  sHost := GetConnectionDef.Server;
  if CompareText(sHost, S_FD_Local) = 0 then
    sHost := '127.0.0.1';
  if GetConnectionDef.HasValue(S_FD_ConnParam_Common_Port) then
    iPort := GetConnectionDef.Port
  else
    iPort := MYSQL_PORT;
  sDatabase := GetConnectionDef.ExpandedDatabase;
...
{$ENDIF}
  ...
  FResultMode := hrStore;
  if GetConnectionDef.HasValue(S_FD_ConnParam_MySQL_ResultMode) then begin
    s := GetConnectionDef.AsString[S_FD_ConnParam_MySQL_ResultMode];
    if CompareText(s, S_FD_StoreResult) = 0 then
      FResultMode := hrStore
    else if CompareText(s, S_FD_UseResult) = 0 then
      FResultMode := hrUse
    else if CompareText(s, S_FD_ChooseResult) = 0 then
      FResultMode := hrChoose;
  end;
  FTinyIntFormat := dtBoolean;
  if CompareText(GetConnectionDef.AsString[S_FD_ConnParam_MySQL_TinyIntFormat], S_FD_Integer) = 0 then
    FTinyIntFormat := dtSByte;

  if InternalGetSharedCliHandle() = nil then begin
    if GetConnectionDef.AsBoolean[S_FD_ConnParam_MySQL_Compress] then
    ...
    if GetConnectionDef.AsBoolean[S_FD_ConnParam_MySQL_UseSSL] then begin
      uiClientFlag := uiClientFlag or CLIENT_SSL;
      oDef := GetConnectionDef;
      FSession.SSLInit(oDef.AsString[S_FD_ConnParam_MySQL_SSL_key],
                       oDef.AsString[S_FD_ConnParam_MySQL_SSL_cert],
                       oDef.AsString[S_FD_ConnParam_MySQL_SSL_ca],
                       oDef.AsString[S_FD_ConnParam_MySQL_SSL_capath],
                       oDef.AsString[S_FD_ConnParam_MySQL_SSL_cipher]);
    end;
    ...
    if (FClientVersion >= mvMySQL050600) and GetConnectionDef.IsSpecified(S_FD_ConnParam_Common_NewPassword) then
      uiClientFlag := uiClientFlag or CLIENT_CAN_HANDLE_EXPIRED_PASSWORDS;

    GLock.Enter;
    try
      FSession.Connect(sHost, sUserName, sPwd, sDatabase, iPort, uiClientFlag);
      if (FClientVersion >= mvMySQL050600) and GetConnectionDef.IsSpecified(S_FD_ConnParam_Common_NewPassword) then
        InternalExecuteDirect('SET PASSWORD = PASSWORD("' + GetConnectionDef.NewPassword + '")', nil);
    finally
      GLock.Leave;
    end;
  end;
  ...
  if InternalGetSharedCliHandle() = nil then begin
    if GetConnectionDef.HasValue(S_FD_ConnParam_Common_CharacterSet) then
      FSession.CharacterSetName := GetConnectionDef.AsString[S_FD_ConnParam_Common_CharacterSet];
...
var
  ...
  iLen: LongWord;
begin
  InternalExecuteDirect(ACmd, nil);
...
begin
  ...
  if (FServerVersion >= mvMySQL050000) and (FServerVersion < mvMySQL050100) then
    AMessages.Add(Format('Warning: The server [%s] version does not support Foreign ' +
                         'Key/Foreign Key Columns metadata retrieval.', [FDVerInt2Str(FServerVersion)]));
end;

{-------------------------------------------------------------------------------}
procedure TFDPhysMySQLConnection.UpdateServerOutput;
begin
  FSession.ServerOutput := TFDTopResourceOptions(FOptions.ResourceOptions).ServerOutput;
end;
...
begin
  ...
  if GetMetaInfoKind <> FireDAC.Phys.Intf.mkNone then begin
    ...
    // SHOW INDEX FOR dual - always returns error
    if (GetMetaInfoKind in [mkTableFields, mkIndexes, mkIndexFields,
                            mkPrimaryKey, mkPrimaryKeyFields,
                            mkForeignKeys, mkForeignKeyFields]) and
       (CompareText(rName.FBaseObject, 'dual') = 0) then
      Exit;
...
begin
  ...
  if GetMetaInfoKind = mkNone then
    oFmtOpts.ResolveDataType(AColInfo.FSourceName, eType, AColInfo.FLen,
      AColInfo.FPrec, AColInfo.FScale, AColInfo.FType, AColInfo.FLen, True)
  else
  ...
  dtBoolean:
    if TFDPhysMySQLConnection(FConnectionObj).FServerVersion >= mvMySQL050000 then
      ...
      if PWordBool(AData)^ then
        ...
        RetStr('0');
  dtSByte:
    FDInt2Str(AData, SizeOf(ShortInt), pBuff, iSz, 0);
  dtInt16:
    FDInt2Str(AData, SizeOf(SmallInt), pBuff, iSz, 0);
  dtInt32:
    FDInt2Str(AData, SizeOf(Integer), pBuff, iSz, 0);
  dtInt64:
    FDInt2Str(AData, SizeOf(Int64), pBuff, iSz, 0);
  dtByte:
    iSz := WideFormatBuf(aBuff[0], iSz, PChar(SUnsignedFmt)^, Length(SUnsignedFmt),
      [LongWord(PByte(AData)^)]);
  dtUInt16:
    iSz := WideFormatBuf(aBuff[0], iSz, PChar(SUnsignedFmt)^, Length(SUnsignedFmt),
      [LongWord(PWord(AData)^)]);
  dtUInt32:
    iSz := WideFormatBuf(aBuff[0], iSz, PChar(SUnsignedFmt)^, Length(SUnsignedFmt),
      [PLongWord(AData)^]);
  dtUInt64:
...
begin
  ...
  dtUInt32:
    FDStr2Int(pBuff, iSz, ADestData, SizeOf(LongWord), True);
  dtUInt64:
  ...
  dtExtended:
    begin
      FDStr2Float(pBuff, iSz, E, '.');
      case AType of
      dtSingle:   PSingle(ADestData)^ := E;
      dtDouble:   PDouble(ADestData)^ := E;
      dtExtended: PExtended(ADestData)^ := E;
      end;
    end;
  dtCurrency:
  ...
  dtDate:
    begin
      ...
      if (wYear = 0) and (wMonth = 0) and (wDay = 0) then begin
        ...
        PLongint(ADestData)^ := FDDateTime2Date(EncodeDate(wYear, wMonth, wDay));
    end;
  dtTime:
    begin
      if pBuff^ = '-' then begin
      ...
      if pBuff[8] = '.' then begin
        FDStr2Int(pBuff + 9, 6, @lwFrac, SizeOf(LongWord), True);
        wMSec := lwFrac div 1000;
      end
      else
        wMSec := 0;
      PLongint(ADestData)^ := FDDateTime2Time(EncodeTime(wHour, wMin, wSec, wMSec));
      ...
      if (wYear = 0) and (wMonth = 0) and (wDay = 0) then begin
        ...
        if pBuff[10] = #0 then begin
          ...
          if pBuff[19] = '.' then begin
            FDStr2Int(pBuff + 20, 6, @lwFrac, SizeOf(LongWord), True);
            wMSec := lwFrac div 1000;
          end
          else
            wMSec := 0;
        ...
        PDateTimeRec(ADestData)^.DateTime := FDDateTime2MSecs(
          FDEncodeDateTime(wYear, wMonth, wDay, wHour, wMin, wSec, wMSec));
      end;
    end;
  dtDateTimeStamp:
    begin
...
var
  ...
  begin
    ...
    SetLength(Result, FDByteStrLength(Result) + AMinLen);
    pDestCh := PByte(Result) + iPos;
    pDestEnd := PByte(Result) + FDByteStrLength(Result);
  end;
...
begin
  ...
  sEncSQL := TFDPhysMySQLConnection(FConnectionObj).FSession.Encoder.Encode(sSQL);
  oParams := GetParams();
      ...
      if PFDAnsiString(pSrcCh)^ = TFDAnsiChar(#0) then
        ...
        if oParam <> nil then begin
          ...
          oFmtOpts.ResolveFieldType('', oParam.DataType, oParam.ADDataType, oParam.Size,
            oParam.Precision, oParam.NumericScale, iFieldType, iSize, iPrec, iScale,
          ...
          if oParam.IsNulls[iRow] then begin
            if Length(sNULL) = 0 then
              sNULL := TFDPhysMySQLConnection(FConnectionObj).FSession.Encoder.Encode('NULL');
            sSubst := sNULL;
          end
          else begin
          ...
          if Length(sQMark) = 0 then
            sQMark := TFDPhysMySQLConnection(FConnectionObj).FSession.Encoder.Encode('?');
          WriteDest(sQMark);
        end;
      end;
      Inc(pSrcCh);
...
{-------------------------------------------------------------------------------}
function TFDPhysMySQLCommand.GetResultMode: TFDPhysMySQLResultMode;
var
  ...
  if FCursor = nil then begin
    ...
    if not (GetMetaInfoKind in [mkNone, mkForeignKeys, mkForeignKeyFields]) then
      if Length(FMetaInfoSQLs) = 0 then
        ...
        sSQL := oCon.FSession.Encoder.Encode(FMetaInfoSQLs[0])
    else
      sSQL := GetExpandedSQL(1, 0);
    while Length(sSQL) <> 0 do begin
      oCon.FLock.Enter;
      try
        try
          oCon.UpdateServerOutput;
          ...
          on E: EMySQLNativeException do
            if GetMetaInfoKind <> mkNone then begin
              ...
              if iSQL = Length(FMetaInfoSQLs) then
                ...
                sSQL := oCon.FSession.Encoder.Encode(FMetaInfoSQLs[iSQL]);
            end
            else
              raise;
        end;
...
const
  C_Primary: UnicodeString = 'PRIMARY';
var
  pData: Pointer;
  uiLen: LongWord;
  oRow: TFDDatSRow;
...
begin
  ...
  mkTables:
    begin
      oRow.SetData(0, iRecNo);
      oRow.SetData(1, TFDPhysMySQLConnection(FConnectionObj).InternalGetCurrentCatalog);
...
initialization
  FDPhysManager();
  FDPhysManagerObj.RegisterDriverClass(TFDPhysMySQLDriver);
