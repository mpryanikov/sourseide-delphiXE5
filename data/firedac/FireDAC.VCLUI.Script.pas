...
{$I FireDAC.inc}
{$IFDEF WIN32}
  {$HPPEMIT '#pragma link "FireDAC.VCLUI.Script.obj"'}
{$ELSE}
  {$HPPEMIT '#pragma link "FireDAC.VCLUI.Script.o"'}
{$ENDIF}
...
{-------------------------------------------------------------------------------}
procedure TFDGUIxFormsScriptImpl.Output(const AStr: String; AKind: TFDScriptOuputKind);
begin
  if FDGUIxSilent() or (FForm = nil) then
    Exit;
  FForm.Output(AStr);
...
initialization
  oFact := TFDMultyInstanceFactory.Create(TFDGUIxFormsScriptImpl,
    IFDGUIxScriptDialog, C_FD_GUIxFormsProvider, 'FireDAC.VCLUI.Script');
