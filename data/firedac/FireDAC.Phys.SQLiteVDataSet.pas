...
type
  ...
  private
    ...
    function GetObjName(const AName: String): String;
  ...
  protected
    ...
    function FindDataSet(const ASchema, AName: String): IFDPhysLocalQueryAdapter; override;
  public
...
type
  ...
  private
    ...
    function DataSetByName(const ASchema, AName: String): IFDPhysLocalQueryAdapter;
    procedure ReadDataSet(const ASchema, AName: String; ADataSet: TDataSet);
  protected
  ...
  private
    ...
    function PointByVIndex(const AVIndex: Integer;
      out ASavePoint: Integer): Integer;
  ...
  public
    constructor Create(AModule: TSQLiteVModule; const ASchemaName, AName: String;
      const AAdapter: IFDPhysLocalQueryAdapter; AMultipleCursors, ADisableControls,
      AIncludeHiddenFields, AApplyUpdatesOnCommit: Boolean);
    destructor Destroy; override;
  end;
...
{-------------------------------------------------------------------------------}
procedure TFDLocalSQL.SetSchemaName(const AValue: String);
begin
...
{-------------------------------------------------------------------------------}
function TFDLocalSQL.GetObjName(const AName: String): String;
begin
  if SchemaName <> '' then
    Result := '[' + SchemaName + ']'
  else
...
begin
  if (Connection.RDBMSKind <> mkSQLite) or
  ...
  if SchemaName <> '' then
    Connection.ExecSQL('ATTACH DATABASE ":memory:" AS ' + GetObjName(''));
end;
...
begin
  if SchemaName <> '' then
    Connection.ExecSQL('DETACH DATABASE ' + GetObjName(''));
  TSQLiteDatabase(Connection.ConnectionIntf.CliObj).
...
begin
  ...
  Connection.ExecSQL(sCmd + GetObjName(ADataSet.ActualName) +
...
begin
  Connection.ExecSQL('DROP TABLE IF EXISTS ' + GetObjName(ADataSet.ActualName));
end;

{-------------------------------------------------------------------------------}
function TFDLocalSQL.FindDataSet(const ASchema, AName: String): IFDPhysLocalQueryAdapter;
var
...
begin
  sSchema := Unquote(ASchema);
  if CompareText(sSchema, 'main') = 0 then
    sSchema := '';
  sName := Unquote(AName);
  if CompareText(SchemaName, sSchema) = 0 then
    Result := inherited FindDataSet(sSchema, sName)
  else
    Result := nil;
end;
...
var
  ...
  lMultCrs, lDisCtrls, lIncHid, lApplUpd: Boolean;
begin
  // AArgs format:
  // AArgs count >= 4;
  ...
  Result := TSQLiteVDSTable.Create(Self, AArgs[1], AArgs[2], oAdapt,
    lMultCrs, lDisCtrls, lIncHid, lApplUpd);
end;
...
{-------------------------------------------------------------------------------}
function TSQLiteVDSModule.FindDataSet(const ASchema, AName: String): IFDPhysLocalQueryAdapter;
var
  i: Integer;
begin
  Result := nil;
  for i := 0 to Database.SQLHandlerList.Count - 1 do begin
    Result := IFDPhysSQLHandler(Database.SQLHandlerList[i]).FindDataSet(ASchema, AName);
    if Result <> nil then
      Break;
  end;
end;
...
{-------------------------------------------------------------------------------}
function TSQLiteVDSModule.DataSetByName(const ASchema, AName: String): IFDPhysLocalQueryAdapter;
var
  s: String;
begin
  Result := FindDataSet(ASchema, AName);
...
{-------------------------------------------------------------------------------}
procedure TSQLiteVDSModule.ReadDataSet(const ASchema, AName: String; ADataSet: TDataSet);
var
  i: Integer;
...
begin
  for i := 0 to Database.SQLHandlerList.Count - 1 do begin
    ...
    if oHndl.FindDataSet(ASchema, AName) <> nil then begin
      oHndl.ReadDataSet(ASchema, AName, ADataSet);
      Break;
    end;
  end;
end;
...
begin
  Result := False;
  if (ATable.FAdapter.Conn = nil) or (ATable.FProvider = nil) then
    Exit;
...
begin
  if (FDataSource.State in [dsBrowse, dsInactive]) and
     (FLastState <> FDataSource.State) then begin
...
begin
  ...
  try
    if AStartRead then
      TSQLiteVDSModule(FModule).ReadDataSet(FSchemaName, FName, FDataSet);
  ...
  if not FKeyFieldInit then begin
    ...
    if FKeyFieldNames = '' then
      for i := 0 to FDataSet.FieldCount - 1 do begin
        ...
        if (oFld.FieldKind = fkData) and
           ([pfInKey, pfInWhere] * oFld.ProviderFlags <> []) then begin
          if FKeyFieldNames <> '' then
...
begin
  for i := 0 to FDataSet.FieldCount - 1 do begin
    ...
    if CompareText(oFld.FieldName, AColName) = 0 then begin
      ...
      AAutoInc :=
        (oFld.AutoGenerateValue = arAutoInc) or
        (oFld is TAutoIncField);
...
begin
  Result := (FDataSet is TFDDataSet) and TFDDataSet(FDataSet).ClientCursor and
    TFDDataSet(FDataSet).OptionsIntf.FetchOptions.Unidirectional
    or not (FDataSet is TFDDataSet) and FDataSet.IsUniDirectional
    ;
end;
...
{-------------------------------------------------------------------------------}
procedure TSQLiteVDSTable.VTabRename(const ANewName: UnicodeString);
begin
...
begin
  if OfflineSavepointSupported then
    VTabSavepoint(-1);
  if (not OfflineMode or FApplyUpdatesOnCommit) and
...
procedure TSQLiteVDSTable.CheckSavepoint;
begin
  if not OfflineSavepointSupported then
...
begin
  ...
  else if (AOp = moUpdate) and (ARowid1 <> ARowid2) and
          (ARowid1 <> C_FD_SQLiteUndefRowid) then
    // same as above
    VDSError(Self, er_FD_SQLiteVTabDSNotSupported, []);
end;
...
{-------------------------------------------------------------------------------}
procedure TSQLiteVDSTable.SetFieldData;
var
  i, iSize: Integer;
  oStream: TStream;
  pBuff: Pointer;
...
begin
  for i := 0 to Count - 1 do begin
    oFld := FDataSet.Fields[i];
    // skip updating read-only fields
    // SQLite sets all table fields (modified and not modified)
    if not oFld.ReadOnly then
      case ColsIN[i].IntDataType of
      SQLITE_INTEGER:
        if oFld.DataType = ftLargeInt then
          TLargeIntField(oFld).AsLargeInt := ColsIN[i].AsInteger
        else if oFld.DataType = ftBoolean then
          oFld.AsBoolean := ColsIN[i].AsInteger <> 0
        else
          oFld.AsInteger := ColsIN[i].AsInteger;
      SQLITE_TEXT:
        if oFld.DataType in CWideStringTypes then
          oFld.AsWideString := ColsIN[i].AsWideString
        else if oFld.DataType = ftTime then
          oFld.AsDateTime := ColsIN[i].AsTime
        else if oFld.DataType in CDateTimeTypes then
          oFld.AsDateTime := ColsIN[i].AsDateTime
        else if oFld.DataType = ftBoolean then
          oFld.AsBoolean := ColsIN[i].AsBoolean
        else if oFld.DataType in [ftBCD, ftCurrency, ftFMTBcd] then
          oFld.AsBCD := ColsIN[i].AsNumber
        else
          oFld.AsString := ColsIN[i].AsString;
      SQLITE_FLOAT:
        if oFld.DataType in CDateTimeTypes then
          oFld.AsDateTime := ColsIN[i].AsDateTime
        else if oFld.DataType = ftBoolean then
          oFld.AsBoolean := ColsIN[i].AsFloat <> 0
        else
          oFld.AsFloat := ColsIN[i].AsFloat;
      SQLITE_BLOB:
        begin
          // get blob size 1st
          ColsIN[i].GetData(pBuff, iSize, True, etBlob);
          if iSize > 0 then begin
            ColsIN[i].GetData(pBuff, iSize, False, etBlob);
...
begin
  ...
  if ATable.FMultipleCursors then begin
    ...
    FDataSet := FMemTable;
    FAdapter := IFDPhysLocalQueryAdapter(FMemTable);
...
begin
  ...
  if oFld.IsNull then
    ...
    ftLargeint:
      oCol.AsInteger := TLargeintField(oFld).AsLargeInt;
    ftExtended,
    ftSingle,
    ftFloat:
...
begin
  ...
  if AVar.VType = vtWideString then
    ...
    {$IFDEF NEXTGEN} String {$ELSE} AnsiString {$ENDIF} (AVar.VAnsiString) := '';
end;
  ...
  if FUseRanges then begin
    ...
    if not lHasOrderBy then begin
      ...
      if (j = FRangeCols.Count) and (k <> -1) then begin
        ...
        Inc(iRgLen);
        if FEmptyStart then
          FEmptyStart := PSQLiteVDSIndexRec(FRangeCols.Ptrs[0])^.FOp in [coLess,
            coLessOrEqual];
        if FEmptyEnd then
          FEmptyEnd := PSQLiteVDSIndexRec(FRangeCols.Ptrs[0])^.FOp in [coGreater,
            coGreaterOrEqual];
      end;
    end;
...
var
  ...
  begin
    ...
    if oIn.ExtDataType = etUString then begin
      ...
      UnicodeString(AVar.VUnicodeString) := oIn.AsWideString;
    end
    else begin
      ...
      if oFld.DataType = ftDate then
        ...
        FDBCD2Str(aBuff, iLen, oIn.AsNumber,
          FormatSettings.DecimalSeparator);
        SetString(sVal, aBuff, iLen);
      end
      else if oFld.DataType = ftBoolean then
        sVal := S_FD_Bools[oIn.AsBoolean]
      else
        sVal := oIn.AsString;
{$IFDEF NEXTGEN}
      String(AVar.VAnsiString) := sVal;
{$ELSE}
      AnsiString(AVar.VAnsiString) := AnsiString(sVal);
{$ENDIF}
    end;
  end;
