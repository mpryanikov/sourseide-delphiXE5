...
{$ENDIF}
  System.SysUtils, System.Classes, Vcl.Controls, Vcl.Forms, Vcl.ExtCtrls, Vcl.Grids, Vcl.StdCtrls, Vcl.ComCtrls,
    System.IniFiles, Vcl.Graphics, Vcl.Buttons, Vcl.ToolWin, Vcl.ImgList, Vcl.ActnList, Vcl.Dialogs,
...
type
  ...
  public
    ...
    class function Execute(AConn: TFDCustomConnection; const ACaption: String;
      const ACustomfrmFDGUIxFormsConnEdit: TfrmFDGUIxFormsConnEdit = nil): Boolean; overload;
...
uses
  System.Variants, System.Types, System.UITypes, System.Actions, Data.DB,
...
begin
  if not IsDriverKnown(FDriverID, oDrvMeta) then
    ...
    for i := 0 to sgParams.RowCount - 2 do begin
...
begin
  ...
  if (s <> '') and (FConnectionDefName <> s) then begin
    ...
    FDriverID := FDManager.ConnectionDefs.ConnectionDefByName(s).DriverID;
    cbServerID.Text := '';
    FEdited.Clear;
    FillParamValues(True);
    FillParamGrids(True);
...
begin
  ...
  if (ACol = 1) and (ARow > 1) then begin
    if IsDriverKnown(FDriverID, oDrvMeta) then begin
      ...
      iNewEditor := FDGUIxSetupEditor(cbParams, edtParams, bedtParams, OpenDialog1, sType);
...
begin
  ...
  FDPhysManager.CreateDriver(oConn.ResultConnectionDef.DriverID, oDrv);
...
const
  C_Prefix = C_FD_Docu_Link + 'Connect_to_';
  C_FDS = 'Advantage_Database_Server';
  C_DB2 = 'IBM_DB2_Server';
  C_IB = 'InterBase';
  C_FB = 'Firebird';
  C_MSSQL = 'Microsoft_SQL_Server';
  C_MSAcc = 'Microsoft_Access_Database';
  C_MySQL = 'MySQL_Server';
  C_Ora = 'Oracle_Server';
  C_PG = 'PostgreSQL';
  C_SQLite = 'SQLite_database';
  C_ASA = 'Sybase_SQL_Anywhere';
  C_DBX = 'dbExpress_Data_Source';
  C_ODBC = 'ODBC_Data_Source';
  C_DataSnap = 'DataSnap_Server';
  C_Suff = '_(FireDAC)';
var
  sLink, sId: String;
begin
  sId := FDManager.GetBaseDriverID(FDriverID);
  if CompareText(sId, S_FD_ADSId) = 0 then
    sLink := C_FDS
  else if CompareText(sId, S_FD_DB2Id) = 0 then
    sLink := C_DB2
  else if CompareText(sId, S_FD_IBId) = 0 then
    sLink := C_IB
  else if CompareText(sId, S_FD_FBId) = 0 then
    sLink := C_FB
  else if CompareText(sId, S_FD_MSSQLId) = 0 then
    sLink := C_MSSQL
  else if CompareText(sId, S_FD_MSAccId) = 0 then
    sLink := C_MSAcc
  else if CompareText(sId, S_FD_MySQLId) = 0 then
    sLink := C_MySQL
  else if CompareText(sId, S_FD_OraId) = 0 then
    sLink := C_Ora
  else if CompareText(sId, S_FD_ASAId) = 0 then
    sLink := C_ASA
  else if (CompareText(sId, S_FD_DBXId) = 0) or
          (CompareText(sId, S_FD_TDBXId) = 0) then
    sLink := C_DBX
  else if CompareText(sId, S_FD_ODBCId) = 0 then
    sLink := C_ODBC
  else if CompareText(sId, S_FD_PGId) = 0 then
    sLink := C_PG
  else if CompareText(sId, S_FD_SQLiteId) = 0 then
    sLink := C_SQLite
  else if CompareText(sId, S_FD_DataSnapId) = 0 then
    sLink := C_DataSnap
  else begin
    ShowMessage('Information for the [' + sId + '] driver is not accessible');
    Exit;
  end;
  FDShell(C_Prefix + sLink + C_Suff, S_FD_LComp_Design);
end;
...
class function TfrmFDGUIxFormsConnEdit.Execute(AConn: TFDCustomConnection;
  ...
  const ACustomfrmFDGUIxFormsConnEdit: TfrmFDGUIxFormsConnEdit = nil): Boolean;
var
  oFrm: TfrmFDGUIxFormsConnEdit;
  lFreeFrm: boolean;
begin
  lFreeFrm := False;
  if ACustomfrmFDGUIxFormsConnEdit <> nil then
    oFrm := ACustomfrmFDGUIxFormsConnEdit
  else begin
  ...
  try
    ...
    oFrm.FEdited.Assign(AConn.Params);
...
begin
  ...
  try
    ...
    Result := Execute(oConn, ACaption, ACustomfrmFDGUIxFormsConnEdit);
    if Result then
