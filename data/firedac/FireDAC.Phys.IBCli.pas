...
type
  ...
  api_handle_t = {$IFDEF FireDAC_D64} LongWord {$ELSE} Pointer {$ENDIF};
...
type
  ...
  TISCStatusVector = array[0..19] of ISCStatus;

const
  FB_SQLSTATE_SIZE = 6;
type
  FB_SQLSTATE_STRING = array [0 .. FB_SQLSTATE_SIZE-1] of ISCSChar;
...
type
  ...
  PISCArrayDesc = Pointer;
  PISCBlobDesc = Pointer;

  PISCArrayDescV1 = ^TISCArrayDescV1;
  ISC_ARRAY_DESC_V1 = record
    array_desc_dtype: ISCUChar;
    array_desc_scale: ISCSChar;
    array_desc_length: Word;
    array_desc_field_name: array [0..METADATALENGTH_V1 - 1] of ISCSChar;
    array_desc_relation_name: array [0..METADATALENGTH_V1 - 1] of ISCSChar;
    array_desc_dimensions: Smallint;
    array_desc_flags: Smallint;
    array_desc_bounds: array [0..15] of TISCArrayBound;
  end;
  TISCArrayDescV1 =  ISC_ARRAY_DESC_V1;

  PISCBlobDescV1 = ^TISCBlobDescV1;
  ISC_BLOB_DESC_V1 = record
    blob_desc_subtype: Smallint;
    blob_desc_charset: Smallint;
    blob_desc_segment_size: Smallint;
    blob_desc_field_name: array [0..METADATALENGTH_V1 - 1] of ISCUChar;
    blob_desc_relation_name: array [0..METADATALENGTH_V1 - 1] of ISCUChar;
  end;
  TISCBlobDescV1 =  ISC_BLOB_DESC_V1;
  ...
  ISC_ARRAY_DESC_V2 = record
    ...
    array_desc_scale: UCHAR;
    array_desc_length: USHORT;
    array_desc_field_name: array [0..METADATALENGTH_V2 - 1] of SCHAR;
    array_desc_relation_name: array [0..METADATALENGTH_V2 - 1] of SCHAR;
    array_desc_dimensions: Smallint;
    array_desc_flags: Smallint;
    array_desc_bounds: array [0..15] of TISCArrayBound;
  end;
  ...
  ISC_BLOB_DESC_V2 = record
    ...
    blob_desc_charset: Smallint;
    blob_desc_segment_size: Smallint;
    blob_desc_field_name: array [0..METADATALENGTH_V2 - 1] of UCHAR;
    blob_desc_relation_name: array [0..METADATALENGTH_V2 - 1] of UCHAR;
  end;
...
type
  ...
  ISC_BLOB_CTL = record
    ...
    ctl_bpb: PISCSChar;                // Address of blob parameter block
    ctl_buffer: PISCUChar;             // Address of segment buffer
  ...
  BSTREAM = record
    ...
    bstr_buffer: PISCSChar;     // Address of buffer
    bstr_ptr: PISCSChar;        // Next character
    bstr_length: Smallint;      // Length of buffer
    bstr_cnt: Smallint;         // Characters in buffer
    bstr_mode: ISCSChar;        // (mode) ? OUTPUT : INPUT
  end;
  TBStream = BSTREAM;
...
type
  TBlobGetSegmentFn = function(hnd: Pointer; buffer: PISCUChar; buf_size: ISCUShort;
    var result_len: ISCUShort): Smallint; stdcall;
  TBlobPutSegmentFn = procedure(hnd: Pointer; buffer: PISCUChar; buf_size: ISCUShort); stdcall;
  TBlobLSeekFn = function(hnd: Pointer; mode: ISCUShort; offset: ISCLong): ISCLong; stdcall;
  ...
  PARAMDSC_V1 = record
    ...
    dsc_flags: ISCUShort;
    dsc_address: PISCUChar;
  end;
  TParamDscV1 = PARAMDSC_V1;

  PParamDscV2 = ^TParamDscV2;
  PARAMDSC_V2 = record
    dsc_version: Byte;
    dsc_dtype: Byte;
    dsc_scale: ShortInt;
  ...
  PARAMVARY = record
    ...
    vary_string: array [0..0] of ISCUChar;
  end;
...
type
  ...
  XSQLVAR_V1 = record
    sqltype: ISCShort;                                        // datatype of field
    sqlscale: ISCShort;                                       // scale factor
    sqlsubtype: ISCShort;                                     // datatype subtype
    sqllen: ISCShort;                                         // length of data area
    sqldata: PISCSChar;                                       // address of data
    sqlind: PISCShort;                                         // address of indicator variable
    sqlname_length: ISCShort;                                 // length of sqlname field
    sqlname: array [0..METADATALENGTH_V1 - 1] of ISCSChar;    // name of field, name length + space for NULL
    relname_length: ISCShort;                                 // length of relation name
    relname: array [0..METADATALENGTH_V1 - 1] of ISCSChar;    // field's relation name + space for NULL
    ownname_length: ISCShort;                                 // length of owner name
    ownname: array [0..METADATALENGTH_V1 - 1] of ISCSChar;    // relation's owner name + space for NULL
    aliasname_length: ISCShort;                               // length of alias name
    aliasname: array [0..METADATALENGTH_V1 - 1] of ISCSChar;  // relation's alias name + space for  NULL
  end;
  TXSQLVarV1 = XSQLVAR_V1;

  PXSQLDAV1 = ^TXSQLDAV1;
  XSQLDA_V1 = record
    version: ISCShort;                                // version of this XSQLDA
    sqldaid: array [0..7] of ISCSChar;                // XSQLDA name field          ->  RESERVED
    sqldabc: ISCLong;                                 // length in bytes of SQLDA   ->  RESERVED
    sqln: ISCShort;                                   // number of fields allocated
    sqld: ISCShort;                                   // actual number of fields
    sqlvar: array [0..0] of TXSQLVarV1;               // first field address
  end;
  TXSQLDAV1 = XSQLDA_V1;

  PXSQLVarV2 = ^TXSQLVarV2;
  PPXSQLVarV2 = ^PXSQLVarV2;
  XSQLVAR_V2 = record
    ...
    sqlname: array [0..METADATALENGTH_V2 - 1] of ISCSChar;   // name of field, name length + space  for NULL
    relname_length: Smallint;                                // length of relation name
    relname: array [0..METADATALENGTH_V2 - 1] of ISCSChar;   // field's relation name + space for NULL
    ownname_length: Smallint;                                // length of owner name
    ownname: array [0..METADATALENGTH_V2 - 1] of ISCSChar;   // relation's owner name + space for  NULL
    aliasname_length: Smallint;                              // length of alias name
    aliasname: array [0..METADATALENGTH_V2 - 1] of ISCSChar; // relation's alias name + space for NULL
  end;
  TXSQLVarV2 = XSQLVAR_V2;
  ...
  XSQLDA_V2 = record
    ...
    sqlvar: array [0..0] of TXSQLVarV2;               // first field address
  end;
  TXSQLDAV2 = XSQLDA_V2;
...
type
  USER_SEC_DATA = record
    ...
    server: PISCSChar;        // server to administer
    user_name: PISCSChar;     // the user's name
    password: PISCSChar;      // the user's password
    group_name: PISCSChar;    // the group name
    first_name: PISCSChar;    // the user's first name
    middle_name: PISCSChar;   // the user's middle name
    last_name: PISCSChar;     // the user's last name
    dba_user_name: PISCSChar; // the dba user name
    dba_password: PISCSChar;  // the dba password
  end;
  ...
  // ---------------------------------------------------------------------------
procedure ADD_SPB_LENGTH(var p: PISCUChar; length: Integer);
procedure ADD_SPB_NUMERIC(var p: PISCUChar; data: Integer);
...
{$ENDIF}
  ...
  function BLOB_display(blob_id: PISCQuad; database: IscDbHandle; transaction: IscTrHandle;
    field_name: PISCSChar): Integer; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function BLOB_dump(blob_id: PISCQuad; database: IscDbHandle; transaction: IscTrHandle;
    file_name: PISCSChar): Integer; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  ...
  function BLOB_load(blob_id: PISCQuad; database: IscDbHandle; transaction: IscTrHandle;
    file_name: PISCSChar): Integer; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function BLOB_open(blob: IscBlobHandle; buffer: PISCSChar;
    length: Integer): PBStream; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function BLOB_put(x: ISCSChar; Stream: PBStream): Integer; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  ...
  function Bopen(blob_id: PISCQuad; database: IscDbHandle; transaction: IscTrHandle;
    mode: PISCSChar): PBStream; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  // FB >= 2.0 returns ISCStatus, others - Integer
  function isc_add_user(status: PISCStatus; user_data: PUserSecData): ISCStatus;
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_array_gen_sdl(status: PISCStatus; desc: PISCArrayDesc; sdl_buffer_length: PISCShort;
    sdl_buffer: PISCUChar; sdl_length: PISCShort): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_array_get_slice(status: PISCStatus; db_handle: PIscDbHandle;
    trans_handle: PIscTrHandle; array_id: PISCQuad; desc: PISCArrayDesc; array_: Pointer;
    slice_length: PISCLong): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_array_lookup_bounds(status: PISCStatus; db_handle: PIscDbHandle;
    trans_handle: PIscTrHandle; relation_name, field_name: PISCSChar;
    desc: PISCArrayDesc): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_array_lookup_desc(status: PISCStatus; db_handle: PIscDbHandle;
    trans_handle: PIscTrHandle; relation_name, field_name: PISCSChar;
    desc: PISCArrayDesc): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_array_put_slice(status: PISCStatus; db_handle: PIscDbHandle;
    trans_handle: PIscTrHandle; array_id: PISCQuad; desc: PISCArrayDesc; array_: Pointer;
    slice_length: PISCLong): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_array_set_desc(status: PISCStatus; relation_name, field_name: PISCSChar;
    sql_dtype, sql_length, dimensions: PSmallint; desc: PISCArrayDesc):
      ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_attach_database(user_status: PISCStatus; file_length: Smallint;
    file_name: PISCSChar; handle: PIscDbHandle; dpb_length: Smallint;
    dpb: PISCSChar): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
    external C_FD_IBLib {$IFDEF CPUARM} {$IFDEF IOS} dependency 'stdc++' {$ELSE} dependency 'stlport_static' {$ENDIF} {$ENDIF};
  procedure isc_blob_default_desc(desc: PISCBlobDesc; relation_name,
    field_name: PISCUChar); {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_blob_gen_bpb(status: PISCStatus; to_desc, from_desc: PISCBlobDesc;
    bpb_buffer_length: Word; bpb_buffer: PISCUChar; var bpb_length: Word):
    ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_blob_info(user_status: PISCStatus; blob_handle: PIscBlobHandle;
    item_length: Smallint; items: PISCSChar; buffer_length: Smallint; buffer: PISCSChar):
    ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_blob_lookup_desc(status: PISCStatus; db_handle: PIscDbHandle;
    trans_handle: PIscTrHandle; relation_name, field_name: PISCUChar; desc: PISCBlobDesc;
    global: PISCUChar): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_blob_set_desc(status: PISCStatus; relation_name, field_name: PISCUChar;
    subtype, charset, segment_size: Smallint; desc: PISCBlobDesc): ISCStatus;
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_cancel_blob(user_status: PISCStatus; blob_handle: PIscBlobHandle):
    ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_cancel_events(user_status: PISCStatus; handle: PIscDbHandle;
    id: PISCLong): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_close(user_status: PISCStatus; name: PISCSChar): ISCStatus;
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_close_blob(user_status: PISCStatus;
    blob_handle: PIscBlobHandle): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_commit_retaining(user_status: PISCStatus;
    tra_handle: PIscTrHandle): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_commit_transaction(user_status: PISCStatus;
    tra_handle: PIscTrHandle): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_compile_request(user_status: PISCStatus; db_handle: PIscDbHandle;
    req_handle: PIscReqHandle; blr_length: Smallint; blr: PISCSChar): ISCStatus;
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_compile_request2(user_status: PISCStatus; db_handle: PIscDbHandle;
    req_handle: PIscReqHandle; blr_length: Smallint; blr: PISCSChar): ISCStatus;
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_create_blob(user_status: PISCStatus; db_handle: PIscDbHandle;
    tra_handle: PIscTrHandle; blob_handle: PIscBlobHandle; blob_id: PISCQuad):
    ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_create_blob2(user_status: PISCStatus; db_handle: PIscDbHandle;
    tra_handle: PIscTrHandle; blob_handle: PIscBlobHandle; blob_id: PISCQuad;
    bpb_length: Smallint; bpb: PISCSChar): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_create_database(user_status: PISCStatus; file_length: Smallint;
    file_name: PISCSChar; handle: PIscDbHandle; dpb_length: Smallint; dpb: PISCSChar;
    db_type: Smallint): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_database_info(user_status: PISCStatus; handle: PIscDbHandle;
    item_length: Smallint; items: PISCSChar; buffer_length: Smallint;
    buffer: PISCSChar): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_ddl(user_status: PISCStatus; db_handle: PIscDbHandle; tra_handle: PIscTrHandle;
    length: Smallint; ddl: PISCSChar): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_declare(user_status: PISCStatus; statement, cursor: PISCSChar): ISCStatus;
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  procedure isc_decode_date(date: PISCQuad; times_arg: PCTimeStructure);
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  procedure isc_decode_sql_date(date: PISCDate; times_arg: PCTimeStructure);
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  procedure isc_decode_sql_time(sql_time: PISCTime; times_arg: PCTimeStructure);
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  procedure isc_decode_timestamp(date: PISCTimeStamp; times_arg: PCTimeStructure);
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  // FB >= 2.0 returns ISCStatus, others - Integer
  function isc_delete_user(status: PISCStatus; user_data: PUserSecData): ISCStatus;
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_describe(user_status: PISCStatus; name: PISCSChar;
    sqlda: PXSQLDA): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_describe_bind(user_status: PISCStatus; name: PISCSChar;
    sqlda: PXSQLDA): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_detach_database(user_status: PISCStatus;
    handle: PIscDbHandle): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_drop_database(user_status: PISCStatus; handle: PIscDbHandle):
    ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_dsql_alloc_statement2(user_status: PISCStatus; db_handle: PIscDbHandle;
    stmt_handle: PIscStmtHandle): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  ...
  function isc_dsql_describe(user_status: PISCStatus; stmt_handle: PIscStmtHandle;
    dialect: Word; sqlda: PXSQLDA): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  ...
  function isc_dsql_exec_immed2(user_status: PISCStatus; db_handle: PIscDbHandle;
    tra_handle: PIscTrHandle; length: Word; statement: PISCSChar; dialect: Word;
    in_sqlda, out_sqlda: PXSQLDA): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_dsql_exec_immed3_m(user_status: PISCStatus; db_handle: PIscDbHandle;
    tra_handle: PIscTrHandle; Length: Word; statement: PISCSChar; dialect, in_blr_length: Word;
    in_blr: PISCSChar; in_msg_type, in_msg_length: Word; in_msg: PISCSChar; out_blr_length: Word;
    out_blr: PISCSChar; out_msg_type, out_msg_length: Word; out_msg: PISCSChar):
    ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_dsql_execute(user_status: PISCStatus; tra_handle: PIscTrHandle;
    stmt_handle: PIscStmtHandle; dialect: Word; sqlda: PXSQLDA):
    ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  ...
  function isc_dsql_execute_immediate_m(user_status: PISCStatus; db_handle: PIscDbHandle;
    tra_handle: PIscTrHandle; length: Word; statement: PISCSChar; dialect, blr_length: Word;
    blr: PISCSChar; msg_type, msg_length: Word; msg: PISCSChar): ISCStatus;
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_dsql_execute_m(user_status: PISCStatus; tra_handle: PIscTrHandle;
    stmt_handle: PIscStmtHandle; blr_length: Word; blr: PISCSChar; msg_type, msg_length: Word;
    msg: PISCSChar): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  ...
  function isc_dsql_execute2_m(user_status: PISCStatus; tra_handle: PIscTrHandle;
    stmt_handle: PIscStmtHandle; in_blr_length: Word; in_blr: PISCSChar; in_msg_type,
    in_msg_length: Word; in_msg: PISCSChar; out_blr_length: Word; out_blr: PISCSChar;
    out_msg_type, out_msg_length: Word; out_msg: PISCSChar): ISCStatus;
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_dsql_fetch(user_status: PISCStatus; stmt_handle: PIscStmtHandle;
    dialect: Word; sqlda: PXSQLDA): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_dsql_fetch_m(user_status: PISCStatus; stmt_handle: PIscStmtHandle;
    blr_length: Word; blr: PISCSChar; msg_type, msg_length: Word; msg: PISCSChar):
    ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  ...
  function isc_dsql_free_statement(user_status: PISCStatus; stmt_handle: PIscStmtHandle;
    option: Word): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_dsql_insert(user_status: PISCStatus; stmt_handle: PIscStmtHandle;
    dialect: Word; sqlda: PXSQLDA): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_dsql_insert_m(user_status: PISCStatus; stmt_handle: PIscStmtHandle;
    blr_length: Word; blr: PISCSChar; msg_type, msg_length: Word; msg: PISCSChar):
    ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_dsql_prepare(user_status: PISCStatus; tra_handle: PIscTrHandle;
    stmt_handle: PIscStmtHandle; length: Word; string_: PISCSChar; dialect: Word;
    sqlda: PXSQLDA): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_dsql_prepare_m(user_status: PISCStatus; tra_handle: PIscTrHandle;
    stmt_handle: PIscStmtHandle; length: Word; string_: PISCSChar; dialect, item_length: Word;
    items: PISCSChar; buffer_length: Word; buffer: PISCSChar): ISCStatus;
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_dsql_release(user_status: PISCStatus; name: PISCSChar): ISCStatus;
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_dsql_set_cursor_name(user_status: PISCStatus; stmt_handle: PIscStmtHandle;
    cursor: PISCSChar; type_: Word): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_dsql_sql_info(user_status: PISCStatus; stmt_handle: PIscStmtHandle;
    item_length: Smallint; items: PISCSChar; buffer_length: Smallint; buffer: PISCSChar):
    ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_embed_dsql_close(user_status: PISCStatus; name: PISCSChar):
    ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_embed_dsql_declare(user_status: PISCStatus;
    stmt_name, cursor: PISCSChar): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_embed_dsql_describe(user_status: PISCStatus; stmt_name: PISCSChar;
    dialect: Word; sqlda: PXSQLDA): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_embed_dsql_describe_bind(user_status: PISCStatus; stmt_name: PISCSChar;
    dialect: Word; sqlda: PXSQLDA): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_embed_dsql_execute(user_status: PISCStatus; trans_handle: PIscTrHandle;
    stmt_name: PISCSChar; dialect: Word; sqlda: PXSQLDA): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_embed_dsql_execute_immed(user_status: PISCStatus; db_handle: PIscDbHandle;
    trans_handle: PIscTrHandle; length: Word; string_: PISCSChar; dialect: Word;
    sqlda: PXSQLDA): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_embed_dsql_execute2(user_status: PISCStatus; trans_handle: PIscTrHandle;
    stmt_name: PISCSChar; dialect: Word; in_sqlda, out_sqlda: PXSQLDA): ISCStatus;
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_embed_dsql_fetch(user_status: PISCStatus; cursor_name: PISCSChar;
    dialect: Word; sqlda: PXSQLDA): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_embed_dsql_insert(user_status: PISCStatus; cursor_name: PISCSChar;
    dialect: Word; sqlda: PXSQLDA): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_embed_dsql_open(user_status: PISCStatus; trans_handle: PIscTrHandle;
    cursor_name: PISCSChar; dialect: Word; sqlda: PXSQLDA): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_embed_dsql_open2(user_status: PISCStatus; trans_handle: PIscTrHandle;
    cursor_name: PISCSChar; dialect: Word; in_sqlda, out_sqlda: PXSQLDA): ISCStatus;
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_embed_dsql_prepare(user_status: PISCStatus; db_handle: PIscDbHandle;
    trans_handle: PIscTrHandle; stmt_name: PISCSChar; length: Word; string_: PISCSChar;
    dialect: Word; sqlda: PXSQLDA): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_embed_dsql_release(user_status: PISCStatus;
    stmt_name: PISCSChar): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  procedure isc_encode_date(times_arg: PCTimeStructure; date: PISCQuad);
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  procedure isc_encode_sql_date(times_arg: PCTimeStructure; date: PISCDate);
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  procedure isc_encode_sql_time(times_arg: PCTimeStructure; time: PISCTime);
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  procedure isc_encode_timestamp(times_arg: PCTimeStructure;
    timestamp: PISCTimeStamp); {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_event_block(var event_buffer, result_buffer: PISCUChar; count: ISCUShort;
    v1, v2, v3, v4, v5, v6, v7, v8, v9, v10, v11, v12, v13, v14, v15: PISCUChar): ISCLong; cdecl; external C_FD_IBLib;
  procedure isc_event_counts(ser_status: PISCStatus; buffer_length: ISCShort;
    event_buffer, result_buffer: PISCUChar); {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_execute(user_status: PISCStatus; trans_handle: PIscTrHandle;
    name: PISCSChar; sqlda: PXSQLDA): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_execute_immediate(user_status: PISCStatus; db_handle: PIscDbHandle;
    trans_handle: PIscTrHandle; length: PSmallint; string_: PISCSChar):
    ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  procedure isc_expand_dpb(dpb: PPISCSChar; dpb_size: PSmallint; name_buffer: PPISCSChar); cdecl; external C_FD_IBLib;
  function isc_fetch(user_status: PISCStatus; name: PISCSChar; sqlda: PXSQLDA):
    ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_free(blk: Pointer): ISCLong; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_ftof(string_: PISCSChar; length1: Word; field: PISCSChar;
    length2: Word): ISCLong; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_get_segment(user_status: PISCStatus; blob_handle: PIscBlobHandle; var length: Word;
    buffer_length: Word; buffer: PISCSChar): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_get_slice(user_status: PISCStatus; db_handle: PIscDbHandle; tra_handle: PIscTrHandle;
    array_id: PISCQuad; sdl_length: Smallint; sdl: PISCSChar; param_length: Smallint; param: PISCLong;
    slice_length: ISCLong; slice: Pointer; return_length: PISCLong): ISCStatus;
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  // FB >= 2.0 returns ISCLong, others - ISCStatus
  function isc_interprete(buffer: PISCSChar; var status_vector: PISCStatus):
    ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_modify_dpb(dpb: PPISCSChar; dpb_length: PSmallint; type_: Word;
    str: PISCSChar; str_len: Smallint): Integer; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  // FB >= 2.0 returns ISCStatus, others - Integer
  function isc_modify_user(status: PISCStatus;
    user_data: PUserSecData): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_open(user_status: PISCStatus; trans_handle: PIscTrHandle; name: PISCSChar;
    sqlda: PXSQLDA): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_open_blob(user_status: PISCStatus; db_handle: PIscDbHandle;
    tra_handle: PIscTrHandle; blob_handle: PIscBlobHandle;
    blob_id: PISCQuad): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_open_blob2(user_status: PISCStatus; db_handle: PIscDbHandle;
    tra_handle: PIscTrHandle; blob_handle: PIscBlobHandle; blob_id: PISCQuad; bpb_length: ISCUShort;
    bpb: PISCUChar): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_portable_integer(ptr: PISCUChar; length: Smallint): ISCInt64;
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_prepare(user_status: PISCStatus; db_handle: PIscDbHandle;
    trans_handle: PIscTrHandle; name: PISCSChar; length: PSmallint; string_: PISCSChar;
    sqlda: PXSQLDA): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_prepare_transaction(user_status: PISCStatus;
    tra_handle: PIscTrHandle): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_prepare_transaction2(user_status: PISCStatus; tra_handle: PIscTrHandle;
    msg_length: ISCUShort; msg: PISCUChar): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_print_blr(blr: PISCSChar; callback: IscCallback; callback_argument: Pointer;
    language: Smallint): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  ...
  function isc_put_slice(user_status: PISCStatus; db_handle: PIscDbHandle; tra_handle: PIscTrHandle;
    array_id: PISCQuad; sdl_length: Smallint; sdl: PISCSChar; param_length: Smallint; param: PISCLong;
    slice_length: ISCLong; slice: Pointer): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  procedure isc_qtoq(quad1, quad2: PISCQuad); {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_que_events(user_status: PISCStatus; handle: PIscDbHandle; id: PISCLong;
    length: Smallint; events: PISCUChar; ast: IscCallback; arg: Pointer):
    ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_receive(user_status: PISCStatus; req_handle: PIscReqHandle; msg_type,
    msg_length: Smallint; msg: Pointer; level: Smallint): ISCStatus;
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_reconnect_transaction(user_status: PISCStatus; db_handle: PIscDbHandle;
    tra_handle: PIscTrHandle; length: Smallint; id: PISCShort): ISCStatus;
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_release_request(user_status: PISCStatus;
    req_handle: PIscReqHandle): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_request_info(user_status: PISCStatus; req_handle: PIscReqHandle; level,
    item_length: Smallint; items: PISCSChar; buffer_length: Smallint;
    buffer: PISCSChar): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_rollback_retaining(status_vector: PISCStatus;
    trans_handle: PIscTrHandle): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_rollback_transaction(user_status: PISCStatus;
    tra_handle: PIscTrHandle): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_seek_blob(user_status: PISCStatus; blob_handle: PIscBlobHandle; mode: Smallint;
    offset: ISCLong; Result_: PISCLong): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_send(user_status: PISCStatus; req_handle: PIscReqHandle; msg_type,
    msg_length: Smallint; msg: Pointer; level: Smallint):
    ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_service_attach(status_vector: PISCStatus; service_length: Word;
    service_name: PISCSChar; handle: PIscSvcHandle; spb_length: Word;
    spb: PISCSChar): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
    external C_FD_IBLib {$IFDEF CPUARM} {$IFDEF IOS} dependency 'stdc++' {$ELSE} dependency 'stlport_static' {$ENDIF} {$ENDIF};
  function isc_service_detach(status_vector: PISCStatus;
    handle: PIscSvcHandle): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_service_query(status_vector: PISCStatus; svc_handle: PIscSvcHandle;
    reserved: PIscResvHandle; send_spb_length: Word; send_spb: PISCSChar; request_spb_length: Word;
    request_spb: PISCSChar; buffer_length: Word; buffer: PISCSChar): ISCStatus;
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_service_start(status_vector: PISCStatus; svc_handle: PIscSvcHandle;
    reserved: PIscResvHandle; spb_length: Word; spb: PISCSChar): ISCStatus;
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  procedure isc_set_debug(flag: Integer); {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  procedure isc_sql_interprete(SQLCODE: Smallint; buffer: PISCSChar;
    buffer_length: Smallint); {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_sqlcode(user_status: PISCStatus): ISCLong; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_start_and_send(user_status: PISCStatus; req_handle: PIscReqHandle;
    tra_handle: PIscTrHandle; msg_type, msg_length: Smallint; msg: Pointer;
    level: Smallint): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_start_multiple(user_status: PISCStatus; tra_handle: PIscTrHandle;
    count: Smallint; vector: PISCTEB): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_start_request(user_status: PISCStatus; req_handle: PIscReqHandle;
    tra_handle: PIscTrHandle; level: Smallint): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_start_transaction(user_status: PISCStatus; tra_handle: PIscTrHandle;
    count: Smallint; db_handle: PIscDbHandle; tpb_length: ISCUShort; tpb_ad: PISCSChar): ISCStatus; cdecl; external C_FD_IBLib;
  function isc_transact_request(user_status: PISCStatus; db_handle: PIscDbHandle;
    tra_handle: PIscTrHandle; blr_length: Word; blr: PISCSChar; in_msg_length: Word; in_msg: PISCSChar;
    out_msg_length: Word; out_msg: PISCSChar): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_transaction_info(user_status: PISCStatus; tra_handle: PIscTrHandle;
    item_length: Smallint; items: PISCSChar; buffer_length: Smallint;
    buffer: PISCSChar): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_unwind_request(user_status: PISCStatus; req_handle: PIscTrHandle;
    level: Smallint): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_vax_integer(ptr: PISCSChar; length: Smallint): ISCLong;
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_version(db_handle: PIscDbHandle; callback: IscCallback;
    callback_argument: Pointer): Integer; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  procedure isc_vtof(string1, string2: PISCSChar; length: Word);
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  procedure isc_vtov(string1, string2: PISCSChar; length: Smallint);
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_wait_for_event(user_status: PISCStatus; handle: PIscDbHandle;
    length: Smallint; events, buffer: PISCUChar): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_reset_fpe(fpe_status: ISCUShort): ISCLong; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  procedure isc_get_client_version(version: PISCSChar); {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  ...
  function isc_release_savepoint(status: PISCStatus; TrHandle: PIscTrHandle;
    name: PISCSChar): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_rollback_savepoint(status: PISCStatus; TrHandle: PIscTrHandle;
    name: PISCSChar; Option: Word): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_start_savepoint(status: PISCStatus; TrHandle: PIscTrHandle;
    name: PISCSChar): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function isc_dsql_batch_execute(status: PISCStatus; TrHandle: PIscTrHandle;
    stmt_handle: PIscStmtHandle; dialect: Word; insqlda: PXSQLDA; number_of_rows: Word;
    batch_vars: PXSQLVarV2; rows_affected: PLongWord): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
{$IFDEF FireDAC_IB_STATIC_FBBRAND}
  function fb_interpret(buffer: PISCSChar; v: LongWord; var status_vector: PISCStatus):
    ISCLong; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  procedure fb_sqlstate(sqlstate: PISCSChar; status_vector: PISCStatus);
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function fb_cancel_operation(status: PISCStatus; db_handle: PIscDbHandle;
    options: ISCUShort): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function fb_shutdown(timeout: Longword; reason: Integer): Integer;
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function fb_shutdown_callback(status: PISCStatus; callback_function: FB_SHUTDOWN_CALLBACK_FUNC;
    mask: Integer; arg: Pointer): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function fb_ping(status: PISCStatus; db_handle: PIscDbHandle): ISCStatus;
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function fb_get_database_handle(status: PISCStatus; db_handle: PIscDbHandle;
    data: Pointer): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
  function fb_get_transaction_handle(status: PISCStatus; tr_handle: PIscTrHandle;
    data: Pointer): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF}; external C_FD_IBLib;
{$ENDIF}
{$ENDIF}
...
type
  ...
  TBLOB_display = function(blob_id: PISCQuad; database: IscDbHandle; transaction: IscTrHandle;
    field_name: PISCSChar): Integer; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  TBLOB_dump = function(blob_id: PISCQuad; database: IscDbHandle; transaction: IscTrHandle;
    file_name: PISCSChar): Integer; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  TBLOB_edit = function(blob_id: PISCQuad; database: IscDbHandle; transaction: IscTrHandle;
    field_name: PISCSChar): Integer; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  TBLOB_get = function(Stream: PBStream): Integer; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  TBLOB_load = function(blob_id: PISCQuad; database: IscDbHandle; transaction: IscTrHandle;
    file_name: PISCSChar): Integer; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  TBLOB_open = function(blob: IscBlobHandle; buffer: PISCSChar;
    length: Integer): PBStream; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  TBLOB_put = function(x: ISCSChar; Stream: PBStream): Integer; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  TBLOB_text_dump = function(blob_id: PISCQuad; database: IscDbHandle; transaction: IscTrHandle;
    file_name: PISCSChar): Integer; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  TBLOB_text_load = function(blob_id: PISCQuad; database: IscDbHandle; transaction: IscTrHandle;
    file_name: PISCSChar): Integer; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  TBopen = function(blob_id: PISCQuad; database: IscDbHandle; transaction: IscTrHandle;
    mode: PISCSChar): PBStream; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  // FB >= 2.0 returns ISCStatus, others - Integer
  Tisc_add_user = function(status: PISCStatus; user_data: PUserSecData): ISCStatus;
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_array_gen_sdl = function(status: PISCStatus; desc: PISCArrayDesc; sdl_buffer_length: PISCShort;
    sdl_buffer: PISCUChar; sdl_length: PISCShort): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_array_get_slice = function(status: PISCStatus; db_handle: PIscDbHandle;
    trans_handle: PIscTrHandle; array_id: PISCQuad; desc: PISCArrayDesc; array_: Pointer;
    slice_length: PISCLong): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_array_lookup_bounds = function(status: PISCStatus; db_handle: PIscDbHandle;
    trans_handle: PIscTrHandle; relation_name, field_name: PISCSChar;
    desc: PISCArrayDesc): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_array_lookup_desc = function(status: PISCStatus; db_handle: PIscDbHandle;
    trans_handle: PIscTrHandle; relation_name, field_name: PISCSChar;
    desc: PISCArrayDesc): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_array_put_slice = function(status: PISCStatus; db_handle: PIscDbHandle;
    trans_handle: PIscTrHandle; array_id: PISCQuad; desc: PISCArrayDesc; array_: Pointer;
    slice_length: PISCLong): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_array_set_desc = function(status: PISCStatus; relation_name, field_name: PISCSChar;
    sql_dtype, sql_length, dimensions: PSmallint; desc: PISCArrayDesc):
      ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_attach_database = function(user_status: PISCStatus; file_length: Smallint;
    file_name: PISCSChar; handle: PIscDbHandle; dpb_length: Smallint;
    dpb: PISCSChar): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_blob_default_desc = procedure(desc: PISCBlobDesc; relation_name,
    field_name: PISCUChar); {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_blob_gen_bpb = function(status: PISCStatus; to_desc, from_desc: PISCBlobDesc;
    bpb_buffer_length: Word; bpb_buffer: PISCUChar; var bpb_length: Word):
    ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_blob_info = function(user_status: PISCStatus; blob_handle: PIscBlobHandle;
    item_length: Smallint; items: PISCSChar; buffer_length: Smallint; buffer: PISCSChar):
    ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_blob_lookup_desc = function(status: PISCStatus; db_handle: PIscDbHandle;
    trans_handle: PIscTrHandle; relation_name, field_name: PISCUChar; desc: PISCBlobDesc;
    global: PISCUChar): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_blob_set_desc = function(status: PISCStatus; relation_name, field_name: PISCUChar;
    subtype, charset, segment_size: Smallint; desc: PISCBlobDesc): ISCStatus;
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_cancel_blob = function(user_status: PISCStatus; blob_handle: PIscBlobHandle):
    ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_cancel_events = function(user_status: PISCStatus; handle: PIscDbHandle;
    id: PISCLong): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_close = function(user_status: PISCStatus; name: PISCSChar): ISCStatus;
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_close_blob = function(user_status: PISCStatus;
    blob_handle: PIscBlobHandle): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_commit_retaining = function(user_status: PISCStatus;
    tra_handle: PIscTrHandle): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_commit_transaction = function(user_status: PISCStatus;
    tra_handle: PIscTrHandle): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_compile_request = function(user_status: PISCStatus; db_handle: PIscDbHandle;
    req_handle: PIscReqHandle; blr_length: Smallint; blr: PISCSChar): ISCStatus;
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_compile_request2 = function(user_status: PISCStatus; db_handle: PIscDbHandle;
    req_handle: PIscReqHandle; blr_length: Smallint; blr: PISCSChar): ISCStatus;
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_create_blob = function(user_status: PISCStatus; db_handle: PIscDbHandle;
    tra_handle: PIscTrHandle; blob_handle: PIscBlobHandle; blob_id: PISCQuad):
    ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_create_blob2 = function(user_status: PISCStatus; db_handle: PIscDbHandle;
    tra_handle: PIscTrHandle; blob_handle: PIscBlobHandle; blob_id: PISCQuad;
    bpb_length: Smallint; bpb: PISCSChar): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_create_database = function(user_status: PISCStatus; file_length: Smallint;
    file_name: PISCSChar; handle: PIscDbHandle; dpb_length: Smallint; dpb: PISCSChar;
    db_type: Smallint): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_database_info = function(user_status: PISCStatus; handle: PIscDbHandle;
    item_length: Smallint; items: PISCSChar; buffer_length: Smallint;
    buffer: PISCSChar): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_ddl = function(user_status: PISCStatus; db_handle: PIscDbHandle; tra_handle: PIscTrHandle;
    length: Smallint; ddl: PISCSChar): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_declare = function(user_status: PISCStatus; statement, cursor: PISCSChar): ISCStatus;
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_decode_date = procedure(date: PISCQuad; times_arg: PCTimeStructure);
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_decode_sql_date = procedure(date: PISCDate; times_arg: PCTimeStructure);
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_decode_sql_time = procedure(sql_time: PISCTime; times_arg: PCTimeStructure);
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_decode_timestamp = procedure(date: PISCTimeStamp; times_arg: PCTimeStructure);
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  // FB >= 2.0 returns ISCStatus, others - Integer
  Tisc_delete_user = function(status: PISCStatus; user_data: PUserSecData): ISCStatus;
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_describe = function(user_status: PISCStatus; name: PISCSChar;
    sqlda: PXSQLDA): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_describe_bind = function(user_status: PISCStatus; name: PISCSChar;
    sqlda: PXSQLDA): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_detach_database = function(user_status: PISCStatus;
    handle: PIscDbHandle): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_drop_database = function(user_status: PISCStatus; handle: PIscDbHandle):
    ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_dsql_alloc_statement2 = function(user_status: PISCStatus; db_handle: PIscDbHandle;
    stmt_handle: PIscStmtHandle): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_dsql_allocate_statement = function(user_status: PISCStatus; db_handle: PIscDbHandle;
    stmt_handle: PIscStmtHandle): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_dsql_describe = function(user_status: PISCStatus; stmt_handle: PIscStmtHandle;
    dialect: Word; sqlda: PXSQLDA): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_dsql_describe_bind = function(user_status: PISCStatus; stmt_handle: PIscStmtHandle;
    dialect: Word; sqlda: PXSQLDA): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_dsql_exec_immed2 = function(user_status: PISCStatus; db_handle: PIscDbHandle;
    tra_handle: PIscTrHandle; length: Word; statement: PISCSChar; dialect: Word;
    in_sqlda, out_sqlda: PXSQLDA): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_dsql_exec_immed3_m = function(user_status: PISCStatus; db_handle: PIscDbHandle;
    tra_handle: PIscTrHandle; Length: Word; statement: PISCSChar; dialect, in_blr_length: Word;
    in_blr: PISCSChar; in_msg_type, in_msg_length: Word; in_msg: PISCSChar; out_blr_length: Word;
    out_blr: PISCSChar; out_msg_type, out_msg_length: Word; out_msg: PISCSChar):
    ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_dsql_execute = function(user_status: PISCStatus; tra_handle: PIscTrHandle;
    stmt_handle: PIscStmtHandle; dialect: Word; sqlda: PXSQLDA):
    ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_dsql_execute_immediate = function(user_status: PISCStatus; db_handle: PIscDbHandle;
    tra_handle: PIscTrHandle; length: Word; statement: PISCSChar; dialect: Word;
    sqlda: PXSQLDA): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_dsql_execute_immediate_m = function(user_status: PISCStatus; db_handle: PIscDbHandle;
    tra_handle: PIscTrHandle; length: Word; statement: PISCSChar; dialect, blr_length: Word;
    blr: PISCSChar; msg_type, msg_length: Word; msg: PISCSChar): ISCStatus;
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_dsql_execute_m = function(user_status: PISCStatus; tra_handle: PIscTrHandle;
    stmt_handle: PIscStmtHandle; blr_length: Word; blr: PISCSChar; msg_type, msg_length: Word;
    msg: PISCSChar): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_dsql_execute2 = function(user_status: PISCStatus; tra_handle: PIscTrHandle;
    stmt_handle: PIscStmtHandle; dialect: Word; in_sqlda, out_sqlda: PXSQLDA):
    ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_dsql_execute2_m = function(user_status: PISCStatus; tra_handle: PIscTrHandle;
    stmt_handle: PIscStmtHandle; in_blr_length: Word; in_blr: PISCSChar; in_msg_type,
    in_msg_length: Word; in_msg: PISCSChar; out_blr_length: Word; out_blr: PISCSChar;
    out_msg_type, out_msg_length: Word; out_msg: PISCSChar): ISCStatus;
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_dsql_fetch = function(user_status: PISCStatus; stmt_handle: PIscStmtHandle;
    dialect: Word; sqlda: PXSQLDA): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_dsql_fetch_m = function(user_status: PISCStatus; stmt_handle: PIscStmtHandle;
    blr_length: Word; blr: PISCSChar; msg_type, msg_length: Word; msg: PISCSChar):
    ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_dsql_finish = function(db_handle: PIscDbHandle): ISCStatus;
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_dsql_free_statement = function(user_status: PISCStatus; stmt_handle: PIscStmtHandle;
    option: Word): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_dsql_insert = function(user_status: PISCStatus; stmt_handle: PIscStmtHandle;
    dialect: Word; sqlda: PXSQLDA): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_dsql_insert_m = function(user_status: PISCStatus; stmt_handle: PIscStmtHandle;
    blr_length: Word; blr: PISCSChar; msg_type, msg_length: Word; msg: PISCSChar):
    ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_dsql_prepare = function(user_status: PISCStatus; tra_handle: PIscTrHandle;
    stmt_handle: PIscStmtHandle; length: Word; string_: PISCSChar; dialect: Word;
    sqlda: PXSQLDA): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_dsql_prepare_m = function(user_status: PISCStatus; tra_handle: PIscTrHandle;
    stmt_handle: PIscStmtHandle; length: Word; string_: PISCSChar; dialect, item_length: Word;
    items: PISCSChar; buffer_length: Word; buffer: PISCSChar): ISCStatus;
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_dsql_release = function(user_status: PISCStatus; name: PISCSChar): ISCStatus;
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_dsql_set_cursor_name = function(user_status: PISCStatus; stmt_handle: PIscStmtHandle;
    cursor: PISCSChar; type_: Word): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_dsql_sql_info = function(user_status: PISCStatus; stmt_handle: PIscStmtHandle;
    item_length: Smallint; items: PISCSChar; buffer_length: Smallint; buffer: PISCSChar):
    ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_embed_dsql_close = function(user_status: PISCStatus; name: PISCSChar):
    ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_embed_dsql_declare = function(user_status: PISCStatus;
    stmt_name, cursor: PISCSChar): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_embed_dsql_describe = function(user_status: PISCStatus; stmt_name: PISCSChar;
    dialect: Word; sqlda: PXSQLDA): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_embed_dsql_describe_bind = function(user_status: PISCStatus; stmt_name: PISCSChar;
    dialect: Word; sqlda: PXSQLDA): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_embed_dsql_execute = function(user_status: PISCStatus; trans_handle: PIscTrHandle;
    stmt_name: PISCSChar; dialect: Word; sqlda: PXSQLDA): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_embed_dsql_execute_immed = function(user_status: PISCStatus; db_handle: PIscDbHandle;
    trans_handle: PIscTrHandle; length: Word; string_: PISCSChar; dialect: Word;
    sqlda: PXSQLDA): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_embed_dsql_execute2 = function(user_status: PISCStatus; trans_handle: PIscTrHandle;
    stmt_name: PISCSChar; dialect: Word; in_sqlda, out_sqlda: PXSQLDA): ISCStatus;
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_embed_dsql_fetch = function(user_status: PISCStatus; cursor_name: PISCSChar;
    dialect: Word; sqlda: PXSQLDA): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_embed_dsql_insert = function(user_status: PISCStatus; cursor_name: PISCSChar;
    dialect: Word; sqlda: PXSQLDA): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_embed_dsql_open = function(user_status: PISCStatus; trans_handle: PIscTrHandle;
    cursor_name: PISCSChar; dialect: Word; sqlda: PXSQLDA): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_embed_dsql_open2 = function(user_status: PISCStatus; trans_handle: PIscTrHandle;
    cursor_name: PISCSChar; dialect: Word; in_sqlda, out_sqlda: PXSQLDA): ISCStatus;
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_embed_dsql_prepare = function(user_status: PISCStatus; db_handle: PIscDbHandle;
    trans_handle: PIscTrHandle; stmt_name: PISCSChar; length: Word; string_: PISCSChar;
    dialect: Word; sqlda: PXSQLDA): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_embed_dsql_release = function(user_status: PISCStatus;
    stmt_name: PISCSChar): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_encode_date = procedure(times_arg: PCTimeStructure; date: PISCQuad);
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_encode_sql_date = procedure(times_arg: PCTimeStructure; date: PISCDate);
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_encode_sql_time = procedure(times_arg: PCTimeStructure; time: PISCTime);
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_encode_timestamp = procedure(times_arg: PCTimeStructure;
    timestamp: PISCTimeStamp); {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_event_block = function(var event_buffer, result_buffer: PISCUChar; count: ISCUShort;
    v1, v2, v3, v4, v5, v6, v7, v8, v9, v10, v11, v12, v13, v14, v15: PISCUChar): ISCLong; cdecl;
  Tisc_event_counts = procedure(ser_status: PISCStatus; buffer_length: ISCShort;
    event_buffer, result_buffer: PISCUChar); {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_execute = function(user_status: PISCStatus; trans_handle: PIscTrHandle;
    name: PISCSChar; sqlda: PXSQLDA): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_execute_immediate = function(user_status: PISCStatus; db_handle: PIscDbHandle;
    trans_handle: PIscTrHandle; length: PSmallint; string_: PISCSChar):
    ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_expand_dpb = procedure(dpb: PPISCSChar; dpb_size: PSmallint; name_buffer: PPISCSChar); cdecl;
  Tisc_fetch = function(user_status: PISCStatus; name: PISCSChar; sqlda: PXSQLDA):
    ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_free = function(blk: Pointer): ISCLong; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_ftof = function(string_: PISCSChar; length1: Word; field: PISCSChar;
    length2: Word): ISCLong; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_get_segment = function(user_status: PISCStatus; blob_handle: PIscBlobHandle; var length: Word;
    buffer_length: Word; buffer: PISCSChar): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_get_slice = function(user_status: PISCStatus; db_handle: PIscDbHandle; tra_handle: PIscTrHandle;
    array_id: PISCQuad; sdl_length: Smallint; sdl: PISCSChar; param_length: Smallint; param: PISCLong;
    slice_length: ISCLong; slice: Pointer; return_length: PISCLong): ISCStatus;
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  // FB >= 2.0 returns ISCLong, others - ISCStatus
  Tisc_interprete = function(buffer: PISCSChar; var status_vector: PISCStatus):
    ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tfb_interpret = function(buffer: PISCSChar; v: LongWord; var status_vector: PISCStatus):
    ISCLong; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_modify_dpb = function(dpb: PPISCSChar; dpb_length: PSmallint; type_: Word;
    str: PISCSChar; str_len: Smallint): Integer; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  // FB >= 2.0 returns ISCStatus, others - Integer
  Tisc_modify_user = function(status: PISCStatus;
    user_data: PUserSecData): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_open = function(user_status: PISCStatus; trans_handle: PIscTrHandle; name: PISCSChar;
    sqlda: PXSQLDA): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_open_blob = function(user_status: PISCStatus; db_handle: PIscDbHandle;
    tra_handle: PIscTrHandle; blob_handle: PIscBlobHandle;
    blob_id: PISCQuad): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_open_blob2 = function(user_status: PISCStatus; db_handle: PIscDbHandle;
    tra_handle: PIscTrHandle; blob_handle: PIscBlobHandle; blob_id: PISCQuad; bpb_length: ISCUShort;
    bpb: PISCUChar): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_portable_integer = function(ptr: PISCUChar; length: Smallint): ISCInt64;
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_prepare = function(user_status: PISCStatus; db_handle: PIscDbHandle;
    trans_handle: PIscTrHandle; name: PISCSChar; length: PSmallint; string_: PISCSChar;
    sqlda: PXSQLDA): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_prepare_transaction = function(user_status: PISCStatus;
    tra_handle: PIscTrHandle): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_prepare_transaction2 = function(user_status: PISCStatus; tra_handle: PIscTrHandle;
    msg_length: ISCUShort; msg: PISCUChar): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_print_blr = function(blr: PISCSChar; callback: IscCallback; callback_argument: Pointer;
    language: Smallint): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_print_sqlerror = procedure(sqlcode: ISCShort; status_vector: PISCStatus);
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_print_status = function(status_vector: PISCStatus): ISCStatus;
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_put_segment = function(user_status: PISCStatus; blob_handle: PIscBlobHandle;
    buffer_length: Word; buffer: PISCSChar): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_put_slice = function(user_status: PISCStatus; db_handle: PIscDbHandle; tra_handle: PIscTrHandle;
    array_id: PISCQuad; sdl_length: Smallint; sdl: PISCSChar; param_length: Smallint; param: PISCLong;
    slice_length: ISCLong; slice: Pointer): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_qtoq = procedure(quad1, quad2: PISCQuad); {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_que_events = function(user_status: PISCStatus; handle: PIscDbHandle; id: PISCLong;
    length: Smallint; events: PISCUChar; ast: IscCallback; arg: Pointer):
    ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_receive = function(user_status: PISCStatus; req_handle: PIscReqHandle; msg_type,
    msg_length: Smallint; msg: Pointer; level: Smallint): ISCStatus;
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_reconnect_transaction = function(user_status: PISCStatus; db_handle: PIscDbHandle;
    tra_handle: PIscTrHandle; length: Smallint; id: PISCShort): ISCStatus;
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_release_request = function(user_status: PISCStatus;
    req_handle: PIscReqHandle): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_request_info = function(user_status: PISCStatus; req_handle: PIscReqHandle; level,
    item_length: Smallint; items: PISCSChar; buffer_length: Smallint;
    buffer: PISCSChar): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_rollback_retaining = function(status_vector: PISCStatus;
    trans_handle: PIscTrHandle): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_rollback_transaction = function(user_status: PISCStatus;
    tra_handle: PIscTrHandle): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_seek_blob = function(user_status: PISCStatus; blob_handle: PIscBlobHandle; mode: Smallint;
    offset: ISCLong; Result_: PISCLong): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_send = function(user_status: PISCStatus; req_handle: PIscReqHandle; msg_type,
    msg_length: Smallint; msg: Pointer; level: Smallint):
    ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_service_attach = function(status_vector: PISCStatus; service_length: Word;
    service_name: PISCSChar; handle: PIscSvcHandle; spb_length: Word;
    spb: PISCSChar): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_service_detach = function(status_vector: PISCStatus;
    handle: PIscSvcHandle): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_service_query = function(status_vector: PISCStatus; svc_handle: PIscSvcHandle;
    reserved: PIscResvHandle; send_spb_length: Word; send_spb: PISCSChar; request_spb_length: Word;
    request_spb: PISCSChar; buffer_length: Word; buffer: PISCSChar): ISCStatus;
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_service_start = function(status_vector: PISCStatus; svc_handle: PIscSvcHandle;
    reserved: PIscResvHandle; spb_length: Word; spb: PISCSChar): ISCStatus;
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_set_debug = procedure(flag: Integer); {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_sql_interprete = procedure(SQLCODE: Smallint; buffer: PISCSChar;
    buffer_length: Smallint); {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_sqlcode = function(user_status: PISCStatus): ISCLong; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_start_and_send = function(user_status: PISCStatus; req_handle: PIscReqHandle;
    tra_handle: PIscTrHandle; msg_type, msg_length: Smallint; msg: Pointer;
    level: Smallint): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_start_multiple = function(user_status: PISCStatus; tra_handle: PIscTrHandle;
    count: Smallint; vector: PISCTEB): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_start_request = function(user_status: PISCStatus; req_handle: PIscReqHandle;
    tra_handle: PIscTrHandle; level: Smallint): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_start_transaction = function(user_status: PISCStatus; tra_handle: PIscTrHandle;
    count: Smallint; db_handle: PIscDbHandle; tpb_length: ISCUShort; tpb_ad: PISCSChar): ISCStatus; cdecl;
  Tisc_transact_request = function(user_status: PISCStatus; db_handle: PIscDbHandle;
    tra_handle: PIscTrHandle; blr_length: Word; blr: PISCSChar; in_msg_length: Word; in_msg: PISCSChar;
    out_msg_length: Word; out_msg: PISCSChar): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_transaction_info = function(user_status: PISCStatus; tra_handle: PIscTrHandle;
    item_length: Smallint; items: PISCSChar; buffer_length: Smallint;
    buffer: PISCSChar): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_unwind_request = function(user_status: PISCStatus; req_handle: PIscTrHandle;
    level: Smallint): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_vax_integer = function(ptr: PISCSChar; length: Smallint): ISCLong;
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_version = function(db_handle: PIscDbHandle; callback: IscCallback;
    callback_argument: Pointer): Integer; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_vtof = procedure(string1, string2: PISCSChar; length: Word);
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_vtov = procedure(string1, string2: PISCSChar; length: Smallint);
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_wait_for_event = function(user_status: PISCStatus; handle: PIscDbHandle;
    length: Smallint; events, buffer: PISCUChar): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_reset_fpe = function(fpe_status: ISCUShort): ISCLong; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_get_client_version = procedure(version: PISCSChar); {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  ...
  Tisc_release_savepoint = function(status: PISCStatus; TrHandle: PIscTrHandle;
    name: PISCSChar): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_rollback_savepoint = function(status: PISCStatus; TrHandle: PIscTrHandle;
    name: PISCSChar; Option: Word): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_start_savepoint = function(status: PISCStatus; TrHandle: PIscTrHandle;
    name: PISCSChar): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tisc_dsql_batch_execute = function(status: PISCStatus; TrHandle: PIscTrHandle;
    stmt_handle: PIscStmtHandle; dialect: Word; insqlda: PXSQLDA; number_of_rows: Word;
    batch_vars: PXSQLVarV2; rows_affected: PLongWord): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tfb_sqlstate = procedure (sqlstate: PISCSChar; status_vector: PISCStatus);
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tfb_cancel_operation = function(status: PISCStatus; db_handle: PIscDbHandle;
    options: ISCUShort): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tfb_shutdown = function(timeout: Longword; reason: Integer): Integer;
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tfb_shutdown_callback = function (status: PISCStatus; callback_function: FB_SHUTDOWN_CALLBACK_FUNC;
    mask: Integer; arg: Pointer): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tfb_ping = function(status: PISCStatus; db_handle: PIscDbHandle): ISCStatus;
    {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tfb_get_database_handle = function (status: PISCStatus; db_handle: PIscDbHandle;
    data: Pointer): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
  Tfb_get_transaction_handle = function (status: PISCStatus; tr_handle: PIscTrHandle;
    data: Pointer): ISCStatus; {$IFDEF MSWINDOWS} stdcall {$ELSE} cdecl {$ENDIF};
...
function XSQLDA_LENGTH_V1(n: Integer): Integer;
begin
  Result := SizeOf(TXSQLDAV1) + ((n - 1) * SizeOf(TXSQLVARV1));
end;

{-------------------------------------------------------------------------------}
function XSQLDA_LENGTH_V2(n: Integer): Integer;
begin
  Result := SizeOf(TXSQLDAV2) + ((n - 1) * SizeOf(TXSQLVARV2));
end;

{-------------------------------------------------------------------------------}
function XSQLVAR_LENGTH(num_rows, num_vars: Integer): Integer;
begin
  Result := SizeOf(TXSQLVarV2) * num_rows * num_vars;
end;
...
begin
  p^ := ISCUChar(data);
  Inc(p);
  p^ := ISCUChar(data shr 8);
  Inc(p);
  p^ := ISCUChar(data shr 16);
  Inc(p);
  p^ := ISCUChar(data shr 24);
  Inc(p);
end;
