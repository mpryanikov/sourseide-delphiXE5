...
uses
  ...
  FMX.TabControl, FMX.Edit, FMX.ListBox, FMX.TreeView, FmxDesignWindows;
...
uses System.TypInfo, System.Types, FMX.Design.Bitmap, System.Math, FMX.Consts;
...
procedure TBrushDesigner.FillControls(const Value: TBrush);
  ...
  begin
    ...
    begin
      ...
      gradAngle.Visible := Style = TGradientStyle.gsLinear;
    end;
...
begin
  ...
  begin
    ...
    FillControls(FCurrentBrush);
    case FCurrentBrush.Kind of
      TBrushKind.bkNone     : brushTabControl.TabIndex := tabNone.Index;
      TBrushKind.bkSolid    : brushTabControl.TabIndex := tabSolid.Index;
      TBrushKind.bkGradient : brushTabControl.TabIndex := tabGradient.Index;
      TBrushKind.bkBitmap   : brushTabControl.TabIndex := tabBitmap.Index;
      TBrushKind.bkResource : brushTabControl.TabIndex := tabRes.Index;
    end;
  end;
end;
...
begin
  ...
  try
    ...
    gradEditor.UpdateGradient;
    gradEditor.Repaint;
    gradAngle.Visible := gradEditor.Gradient.Style = TGradientStyle.gsLinear;
...
begin
  ...
  try
    try
      ...
      textGradHex.Text := AlphaColorToString(gradEditor.Gradient.Points[gradEditor.CurrentPoint].IntColor);
      ShowMessage(SInvalidColorString);
    end;
  finally
    FChanged := False;
  end;
...
begin
  ...
  if InputQuery(sMakeResCaption, sMakeResText, S) then
  begin
...
begin
  Dlg := TBrushDialog.Create(nil);
  try
    Dlg.Brush.Kind := TBrushKind.bkGradient;
    Dlg.Brush.Gradient := Gradient;
    Dlg.ShowStyles := [TBrushKind.bkGradient];
...
begin
  Dlg := TBrushDialog.Create(nil);
  try
    Dlg.Brush.Kind := TBrushKind.bkSolid;
    Dlg.Brush.Color := Color;
    Dlg.ShowStyles := [TBrushKind.bkSolid];
    Dlg.ShowBrushList := False;
    if Dlg.Execute then
...
begin
  ...
  FBrush := TBrush.Create(TBrushKind.bkSolid, TAlphaColors.Gray);
  FShowStyles := [TBrushKind.bkNone, TBrushKind.bkSolid, TBrushKind.bkGradient,
                  TBrushKind.bkBitmap, TBrushKind.bkResource];
  FShowBrushList := True;
...
begin
  ...
  try
    ...
    Dialog.tabNone.Visible := TBrushKind.bkNone in ShowStyles;
    Dialog.tabSolid.Visible := TBrushKind.bkSolid in ShowStyles;
    Dialog.tabGradient.Visible := TBrushKind.bkGradient in ShowStyles;
    Dialog.tabBitmap.Visible := TBrushKind.bkBitmap in ShowStyles;
    Dialog.tabRes.Visible := TBrushKind.bkResource in ShowStyles;
