...
{$IFDEF NEXTGEN}
    procedure TransformNode(const stylesheet: IDOMNode; var output: DomString); overload; {$IFDEF WRAPVER1.1} safecall; {$ENDIF}
{$ELSE !NEXTGEN}
...
{$ENDIF NEXTGEN}
    ...
    { IDOMNodeSelect }
{$IFDEF NEXTGEN}
    function selectNode(const nodePath: DomString): IDOMNode; safecall;
    function selectNodes(const nodePath: DomString): IDOMNodeList; safecall;
{$ELSE !NEXTGEN}
...
{$IFDEF NEXTGEN}
function Tox4DOMNode.selectNode(const nodePath: DomString): IDOMNode;
{$ELSE !NEXTGEN}
...
{$IFDEF NEXTGEN}
function Tox4DOMNode.selectNodes(const nodePath: DomString): IDOMNodeList;
{$ELSE !NEXTGEN}
