...
uses
  System.Types, System.UITypes, System.SysUtils, System.Classes, System.Generics.Collections, System.UIConsts,
  FMX.Types, FMX.Controls, FMX.InertialMovement, FMX.TextLayout, FMX.ListView.Types, FMX.Styles, FMX.Objects,
  FMX.StdCtrls, System.Rtti, FMX.Graphics, FMX.Layouts, FMX.Styles.Objects, FMX.Edit, FMX.Platform;
...
type
  ...
  TCustomListView = class(TStyledControl, ISearchResponder, IListItemStyleResources, IListViewController)
  public type
    TDragListMode = (dmNone, dmDrag, dmSwype);
  private type
    TItemHeightSums = TList<Integer>;

    TDelayedIncident = (ChangeRepainted, Invalidate);

    TDelayedIncidentEntry = class
    private
    ...
    TDelayedIncidents = TObjectList<TDelayedIncidentEntry>;

    TTransitionType = (None, EditMode, DeleteMode);
  private const
    DelayedIncidentsTimerInterval = 16; // 16 ms for ~60 frames per second
    ChangeRepaintedIncidentDelay = 0.1; // seconds
    PhysicsProcessingInterval = 8; // 8 ms for ~120 frames per second
    TransitionTimerInterval = 16; // 16 ms for ~60 frames per second

    DefaultDeleteButtonWidth = 72;
    DefaultDeleteRightMargin = 7;

    ItemSeparatorTop = 1;
    ItemSeparatorBottom = 2;

    MultiSelectFillOpacity = 0.5;
  ...
  private
    ...
    DelayedIncidents: TDelayedIncidents;
    DelayedIncidentsTimer: TTimer;
    ...
    [weak] SelectionStyleImage: TStyleObject;
    [weak] HeaderStyleImage: TStyleObject;
    ...
    TransitionTimerStartTime: Extended;
    ...
    procedure DelayedIncidentsTimerNotify(Sender: TObject);
    procedure StartTransitionTimer;
    procedure StopTransitionTimer;
    ...
    procedure DrawIndexFill(const AIndex: Integer; const LocRect: TRectF; const AOpacity: Single);
  ...
  protected
    ...
    procedure PrepareEditModeObjects(const AItem: TListViewItem);
  ...
  private
    ...
    function GetItem(Index: Integer): TListViewItem;
    procedure PrepareEditModeObjects;
  ...
  protected
    ...
    property UnFilteredItems: TItemsList read FUnfilteredItems;
  public
    constructor Create(const AOwner: TCustomListView);
    destructor Destroy; override;
...
{$REGION 'Appearance object declarations'}
  ...
  protected
    ...
    procedure InitEditModeObjects(const AListViewItem: TListViewItem); virtual;
  ...
  protected
    procedure InitEditModeObjects(const AListViewItem: TListViewItem); override;
...
uses
  System.Math, System.RTLConsts, System.TypInfo, FMX.Consts
...
begin
  ...
  TPlatformServices.Current.SupportsPlatformService(IFMXSystemInformationService,
    IInterface(FSystemInformationService));

  TPlatformServices.Current.SupportsPlatformService(IFMXListingService, IInterface(FListingService));

  FDragListMode := TDragListMode.dmNone;
  ...
  DelayedIncidents := TDelayedIncidents.Create;
  ...
  begin
    ...
    if Assigned(FSystemInformationService) then
      FAniCalc.BoundsAnimation := TScrollingBehaviour.sbBoundsAnimation in
        FSystemInformationService.GetScrollingBehaviour;
  end;

  FItemSpaces := TBounds.Create(TRectF.Create(TAppearanceObjects.cDefaultLeftSpaces, 0,
    TAppearanceObjects.cDefaultRightSpaces, 0));
  ...
  TransitionType := TTransitionType.None;
...
begin
  ...
  if Assigned(DelayedIncidentsTimer) then
    FreeAndNil(DelayedIncidentsTimer);

  DelayedIncidents.Free;
  FListViewItems.Free;
...
function TCustomListView.HasTouchTracking: Boolean;
begin
  Result := Assigned(FSystemInformationService) and
    (TScrollingBehaviour.sbTouchTracking in FSystemInformationService.GetScrollingBehaviour);
end;

function TCustomListView.HasSearchFeatures: Boolean;
begin
  Result := (Assigned(FListingService) and (FListingService.GetSearchFeatures <> [])) or
    (csDesigning in ComponentState);
end;

function TCustomListView.HasSearchAsItem: Boolean;
begin
  if FSearchVisible and (csDesigning in ComponentState) then
    Exit(True);

  if FSearchVisible and Assigned(FListingService) and ((not FSearchAlwaysOnTop) or
    (not (TListingSearchFeature.StayOnTop in FListingService.GetSearchFeatures))) and
    (TListingSearchFeature.AsFirstItem in FListingService.GetSearchFeatures) and ((not Assigned(FSearchEdit)) or
    (FSearchEdit.Text.Length < 1)) then
    Result := True
  else
    Result := False;
end;

function TCustomListView.HasCheckBoxMode: Boolean;
begin
  Result := FEditMode and
    ((ItemAppearance.ItemEditAppearance = TAppearanceNames.ListItemRightDetailShowCheck) or
    (ItemAppearance.ItemEditAppearance = TAppearanceNames.ImageListItemShowCheck) or
    (ItemAppearance.ItemEditAppearance = TAppearanceNames.ImageListItemRightButtonShowCheck) or
    (ItemAppearance.ItemEditAppearance = TAppearanceNames.ListItemShowCheck));
end;

procedure TCustomListView.OnSearchEditResize(Sender: TObject);
begin
  InvalidateHeights;
  StartIncident(TDelayedIncident.Invalidate);
end;

procedure TCustomListView.OnSearchEditChange(Sender: TObject);
begin
...
procedure TCustomListView.SelectItemNotify(Sender: TObject);
  ...
  begin
    ...
    if (AValue <> ItemIndex) and (FDeleteButtonIndex = -1) then
      ObserversBeforeSelection(LAllowSelection);
    if LAllowSelection and (FDeleteButtonIndex = -1) then
    begin
...
begin
  ...
  begin
    AnimateFloat('ScrollViewPos', 0, Min(FAutoTapDistance / AutoTapScrollingSpeed, 1), TAnimationType.atOut,
      TInterpolationType.itSinusoidal);
    ItemSelectTimerWorked := True;
  end
  else if (ItemSelectTimerIndex <> -1) and FAllowSelection then
  begin
  ...
  else if (ItemSelectTimerIndex <> -1) and FEditMode then
   begin
     Items[ItemSelectTimerIndex].Checked := not Items[ItemSelectTimerIndex].Checked;
     ItemSelectTimerIndex := -1;
     ItemSelectTimerWorked := True;
   end;
...
procedure TCustomListView.SetAppearanceProperties(const Value: TCustomListView.TPublishedAppearance);
begin
  Assert(False);
  // Do nothing
end;

procedure TCustomListView.SetItemAppearanceObjects(const Value: TPublishedObjects);
begin
  Assert(False);
  // Do nothing
end;
...
procedure TCustomListView.EditModeAppearances;
begin
  Items.PrepareEditModeObjects;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if Assigned(FListingService) and
        (TListingTransitionFeature.EditMode in FListingService.GetTransitionFeatures) then
...
procedure TCustomListView.SetCanSwipeDelete(const Value: Boolean);
begin
  if FCanSwipeDelete <> Value then
    FCanSwipeDelete := Value;
end;

procedure TCustomListView.SetSearchVisible(const Value: Boolean);
begin
  if FSearchVisible <> Value then
  begin
    FSearchVisible := Value;

    if FSearchVisible and HasSearchFeatures and (not Assigned(FSearchEdit)) then
    begin
...
procedure TCustomListView.SetSearchAlwaysOnTop(const Value: Boolean);
begin
...
function TCustomListView.GetEditModeTransitionAlpha: Single;
begin
  Result:= FEditModeTransitionAlpha;
end;

function TCustomListView.GetDeleteModeTransitionAlpha: Single;
begin
  Result:= FDeleteModeTransitionAlpha;
end;

procedure TCustomListView.FeedbackItemClickChecked(const Item: TListItem; const Checked: Boolean);
begin
  if Assigned(Item) then
    if Checked then
      SetDeleteButtonIndex(TListViewItem(Item).Index)
    else
      SetDeleteButtonIndex(-1);
end;

function TCustomListView.GetItemEditOffset(const Item: TListItem): Single;
begin
  if Assigned(Item) and Assigned(TListViewItem(Item).Objects) and Assigned(TListViewItem(Item).Objects.GlyphButton) then
    Result := TListViewItem(Item).Objects.GlyphButton.Width
  else
    Result := 0;
end;

function TCustomListView.GetItemDeleteCutoff(const Item: TListItem): Single;
begin
  if Assigned(Item) and ((TListViewItem(Item).Index = FDeleteButtonIndex) or
    (TListViewItem(Item).Index = FPrevDeleteButtonIndex)) and Assigned(FDeleteLayout) then
    Result := FDeleteLayout.Position.X
  else
    Result := 0;
end;

function TCustomListView.GetClientMargins: TRectF;
begin
  Result := LocalRect;
end;
...
procedure TCustomListView.SetFilterPredicate(const Predicate: TPredicate<string>);
begin
...
procedure TCustomListView.PrepareEditModeObjects(const AItem: TListViewItem);
var
  LHandled: Boolean;
...
begin
  ...
  try
    ...
    begin
      ...
      if not LHandled then
        LItemObjects.InitEditModeObjects(AItem);
...
procedure TCustomListView.StartIncident(const Incident: TDelayedIncident; const Triggered: Boolean;
  const TimeToWait: Single);
var
  Entry: TDelayedIncidentEntry;
begin
  if not Assigned(DelayedIncidentsTimer) then
  begin
    DelayedIncidentsTimer := TTimer.Create(nil);
    DelayedIncidentsTimer.Enabled := False;
    DelayedIncidentsTimer.Interval := DelayedIncidentsTimerInterval;
    DelayedIncidentsTimer.OnTimer := DelayedIncidentsTimerNotify;
    DelayedIncidentsTimer.Enabled := True;
  end;

  Entry := TDelayedIncidentEntry.Create;
  Entry.Incident := Incident;
  Entry.Triggered := Triggered;
  Entry.StartTime := FTimerService.GetTick;
  Entry.TimeToWait := TimeToWait;

  DelayedIncidents.Add(Entry);
end;

procedure TCustomListView.TriggerIncidents(const Incident: TDelayedIncident; const ResetStartupTime: Boolean);
var
  I: Integer;
  CurTime: Extended;
  Entry: TDelayedIncidentEntry;
begin
  ...
  for I := 0 to DelayedIncidents.Count - 1 do
  begin
    Entry := DelayedIncidents[I];
...
procedure TCustomListView.ProcessIncident(const Entry: TDelayedIncidentEntry);
begin
...
procedure TCustomListView.DelayedIncidentsTimerNotify(Sender: TObject);
var
  I: Integer;
  CurTime: Extended;
  Entry: TDelayedIncidentEntry;
begin
  ...
  for I := DelayedIncidents.Count - 1 downto 0 do
  begin
    Entry := DelayedIncidents[I];
    ...
    begin
      ...
      DelayedIncidents.Delete(I);
    end;
  end;

  if (DelayedIncidents.Count < 1) and Assigned(DelayedIncidentsTimer) then
  begin
    DelayedIncidentsTimer.Enabled := False;
    FreeAndNil(DelayedIncidentsTimer);
  end;
...
procedure TCustomListView.StartTransitionTimer;
begin
  if not Assigned(TransitionTimer) then
  begin
    TransitionTimer := TTimer.Create(nil);
    TransitionTimer.Enabled := False;
    TransitionTimer.Interval := TransitionTimerInterval;
    TransitionTimer.OnTimer := TransitionTimerNotify;
  end;

  TransitionTimer.Enabled := True;
  TransitionTimerStartTime := FTimerService.GetTick;
end;

procedure TCustomListView.StopTransitionTimer;
begin
  if Assigned(TransitionTimer) then
  begin
    TransitionTimer.Enabled := False;
...
procedure TCustomListView.TransitionTimerNotify(Sender: TObject);
begin
  case TransitionType of
    ...
    TTransitionType.EditMode:
      if FEditMode then
      begin
        FEditModeTransitionAlpha := Min(Abs(FTimerService.GetTick - TransitionTimerStartTime) /
      ...
      begin
        FEditModeTransitionAlpha := Max(1 - (Abs(FTimerService.GetTick - TransitionTimerStartTime) /
    ...
    TTransitionType.DeleteMode:
      begin
        if FDeleteButtonIndex <> -1 then
        begin
          FDeleteModeTransitionAlpha := Min(Abs(FTimerService.GetTick - TransitionTimerStartTime) /
        ...
        begin
          FDeleteModeTransitionAlpha := Max(1 - (Abs(FTimerService.GetTick - TransitionTimerStartTime) /
  ...
  if TransitionType <> TTransitionType.None then
    Invalidate;
end;

procedure TCustomListView.InitEditModeAnimation;
begin
  if Items.FirstChecked(True) <> -1 then
  begin
    Items.CheckAll(False);
    SetDeleteButtonIndex(-1);
  end;
  ...
  TransitionType := TTransitionType.EditMode;
  StartTransitionTimer;
end;

procedure TCustomListView.ResetEditModeAnimation;
begin
  StopTransitionTimer;
...
procedure TCustomListView.InitDeleteModeAnimation;
begin
  TransitionType := TTransitionType.DeleteMode;
  StartTransitionTimer;

  if not Assigned(FDeleteLayout) then
  begin
...
procedure TCustomListView.ResetDeleteModeAnimation;
begin
  StopTransitionTimer;

  FDeleteButton.Visible := Items.Count > 0;
  Invalidate;
  ...
  begin
    ...
    if Assigned(FDeleteLayout) then
    begin
...
procedure TCustomListView.UpdateDeleteButtonLayout;
var
  RelRect: TRectF;
begin
  if (FListViewItems.Count < 1) or (not Assigned(FDeleteLayout)) or ((FDeleteButtonIndex = -1) and
    (FPrevDeleteButtonIndex = -1)) then
    Exit;

  if Assigned(FListingService) and
    (TListingTransitionFeature.DeleteButtonSlide in FListingService.GetTransitionFeatures) then
  begin
  ...
  FDeleteLayout.Height := FDeleteButton.Height;
  FDeleteLayout.Position.X := Width - (DefaultDeleteRightMargin + FDeleteLayout.Width);
...
procedure TCustomListView.DoDeleteButtonClick(Sender: TObject);
var
  CanDelete: Boolean;
  TentHeaderIndex: Integer;
begin
  if (FDeleteButtonIndex > -1) and (FDeleteButtonIndex < Items.Count) then
  begin
    CanDelete := True;

    if Assigned(FOnDeletingItem) then
      FOnDeletingItem(Self, ItemIndex, CanDelete);
    ...
    begin
      ...
      FListViewItems.Delete(FDeleteButtonIndex);

      // Delete empty header.
      TentHeaderIndex := FDeleteButtonIndex - 1;

      if (TentHeaderIndex >= 0)and(FListViewItems[TentHeaderIndex].Purpose = TListItemPurpose.Header) then
        if (TentHeaderIndex >= FListViewItems.Count - 1) or
          (FListViewItems[TentHeaderIndex + 1].Purpose <> TListItemPurpose.None) then
        begin
          if (TentHeaderIndex < FListViewItems.Count - 1) and
            (FListViewItems[TentHeaderIndex + 1].Purpose = TListItemPurpose.Footer) then
            FListViewItems.Delete(TentHeaderIndex + 1);

          FListViewItems.Delete(TentHeaderIndex);
        end;
...
procedure TCustomListView.ProceedDeleteItem;
begin
  if FDeleteButtonIndex = -1 then
    Exit;
  ...
  Items.Delete(FDeleteButtonIndex);
...
procedure TCustomListView.SetDeleteButtonIndex(const NewItemIndex: Integer);
begin
  if FDeleteButtonIndex <> NewItemIndex then
  begin
    if TransitionType = TTransitionType.DeleteMode then
      ResetDeleteModeAnimation;
...
procedure TCustomListView.ReadCanSwipeDelete(Reader: TReader);
begin
  CanSwipeDelete := Reader.ReadBoolean;
end;

procedure TCustomListView.ReadIsSearchVisible(Reader: TReader);
begin
  SearchVisible := Reader.ReadBoolean;
end;

procedure TCustomListView.ReadIsSearchAlwaysOnTop(Reader: TReader);
begin
  SearchAlwaysOnTop := Reader.ReadBoolean;
end;

procedure TCustomListView.ReadEditModeOptions(Reader: TReader);
begin
  TOpenReader(Reader).ReadSet(TypeInfo(TEditModeOptions));
end;

procedure TCustomListView.DefineProperties(Filer: TFiler);
begin
  inherited;
...
var
  Sender: TListViewItem;
begin
  if Assigned(FOnButtonChange) then
  begin
    if Assigned(AObject) and (AObject.Owner is TListViewItem) then
      Sender := AObject.Owner as TListViewItem
    else
      Sender := nil;

    FOnButtonChange(Self, Sender, AObject);
  end;
end;
...
procedure TCustomListView.UpdateSearchEditPos;
var
  NewPos: Single;
begin
  if Assigned(FSearchEdit) then
...
begin
  Result := Max(Trunc(FSideSpace * 2 + MaxKnownHeight - LocalRect.Height), 0);
end;
...
begin
  ScrollPixelAlign := ScrollingActive;

  ScrollingActive := False;
  FScrollBar.AnimateFloat('Opacity', 0, 0.2);
...
begin
  ...
  try
    ...
    if not (csDesigning in ComponentState) then  // Don't show at design time
      FScrollBar.Visible := (MaxKnownHeight > LocalHeight) or (HasTouchTracking and ScrollingActive)
    else
      FScrollBar.Visible := False;
...
function TCustomListView.GetSeparatorLineHeight: Single;
begin
...
function TCustomListView.AlignValueToPixel(const Value: Single): Single;
begin
  if FScrollScale > TEpsilon.Scale then
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if TOpenListBoxItem(Items[I]).FHeaderRef <> -1 then
            AltIndex := Max((I - TOpenListBoxItem(Items[I]).FHeaderRef) - 1, 0)
          else
            AltIndex := I;
        ...
        begin
          ...
          Canvas.FillRect(SepRect, 0, 0, AllCorners, 1, FBrush);
        end;
  ...
  if Assigned(HeaderStyleImage) then
    for I := StartItem to EndItem do
      ...
      begin
        ...
        if Assigned(ItemObjects) and ((ItemObjects.Count > 0) or
          (Items[Index - 1].Purpose <> TListItemPurpose.None)) then
        begin
          DrawRect := GetItemRelRect(I, LocRect);
          if ItemObjects.Purpose <> TListItemPurpose.None then
          begin
...
procedure TCustomListView.DrawIndexFill(const AIndex: Integer; const LocRect: TRectF; const AOpacity: Single);
var
  DrawRect: TRectF;
  SepHeight: Single;
begin
  ...
  if (AIndex >= Items.Count - 1) or (Items[AIndex + 1].Purpose = TListItemPurpose.None) then
...
{$ENDIF}
  ...
  begin
    ...
    SelectionStyleImage.DrawToCanvas(Canvas, DrawRect, AOpacity);
  end
  else
  begin
    FBrush.Color := SelectionStyleColor;
    Canvas.FillRect(DrawRect, 0, 0, AllCorners, AOpacity, FBrush);
  end;
end;

procedure TCustomListView.DrawSelectedFill(const LocRect: TRectF);
begin
  DrawIndexFill(FItemIndex, LocRect, 1);
end;
...
begin
  ...
  if FSearchVisible and Assigned(FSearchEdit) and Assigned(FListingService) then
  ...
  if Assigned(FListingService) and (TListingHeaderBehavior.Sticky in FListingService.GetHeaderBehaviors) then
  ...
  // Do not render headers/footers with selection
  if FShowSelection and (FItemIndex >= StartItem) and (FItemIndex <= EndItem) and (Items[FItemIndex].Count > 0)
    and (Items[FItemIndex].Purpose = TListItemPurpose.None) and (not FEditMode) and (FDeleteButtonIndex = -1) then
    DrawSelectedFill(LocRect);

  if HasCheckBoxMode then
    for I := StartItem to EndItem do
    begin
      ItemObjects := Items[I];

      if (ItemObjects.Count > 0) and ItemObjects.Checked then
       DrawIndexFill(I, LocRect, MultiSelectFillOpacity);
    end;
      ...
      for I := StartItem to EndItem do
        ...
        begin
          ...
          begin
            ...
            // Do not render headers/footers with selection
            IsCheckboxChecked := FEditMode and Assigned(ItemObjects.Objects.GlyphButton) and
              ItemObjects.Objects.GlyphButton.Checked and
              (ItemObjects.Objects.GlyphButton.ButtonType in [TGlyphButtonType.Checkbox]);

            if ((I = FItemIndex) and (ItemObjects.Purpose = TListItemPurpose.None) and FShowSelection and
              (not FEditMode) and (FDeleteButtonIndex = -1)) or IsCheckboxChecked then
              Include(DrawStates, TListItemDrawState.Selected);

            if (FEditModeTransitionAlpha > 0) and (ItemObjects.Purpose = TListItemPurpose.None) then
              Include(DrawStates, TListItemDrawState.EditMode);

            // During the first sub-pass the designated areas are calculated.
            if SubPassNo = 0 then
              TOpenListItemObject(CurItem).CalculateLocalRect(TRectF.Create(PointF(BorderRect.Left, BorderRect.Top +
                HeightSums[I] - FScrollViewPos), BorderRect.Width, GetItemHeight(I) - VertMarginHeight), SceneScale,
                DrawStates);

            CurItem.Render(Canvas, I, DrawStates, SubPassNo);
          end;
        end;
    ...
    for PassNo := 0 to NumberOfPasses - 1 do
      ...
      begin
        ...
        begin
          ...
          CurItem.Render(Canvas, ItemHeaderIndex, [], SubPassNo);
        end;
      end;
  end;
...
procedure TCustomListView.MouseDown(Button: TMouseButton; Shift: TShiftState; X, Y: Single);
  ...
  begin
    ...
    begin
      ...
      begin
        FirstIndex := Items.FirstChecked(True);
        if (FirstIndex <> -1) and (not ScrollingActive) and Assigned(Items[FirstIndex].Objects.GlyphButton) and
          (Items[FirstIndex].Objects.GlyphButton.ButtonType in [TGlyphButtonType.Delete]) then
        begin
          Items.CheckAll(False);
          SetDeleteButtonIndex(-1);
          Exit(True);
...
begin
  ...
  if Assigned(FAniCalc) and ScrollingActive and FAniCalc.Animation then
  begin
  ...
  if (TransitionType <> TTransitionType.None) or CancelMode then
    Exit;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        else if HasTouchTracking then
          begin
            if Assigned(TOpenListBoxItem(Items[ItemAt]).ObjectAtPoint(PointF(X, Y))) then
              MouseClickIndex := ItemAt;

            if (MouseClickIndex <> -1) and FAllowSelection then
              SetItemIndex(MouseClickIndex);

            IsCheckGlyphVisible := FEditMode and Assigned(Items[ItemAt].Objects.GlyphButton) and
              (Items[ItemAt].Objects.GlyphButton.ButtonType in [TGlyphButtonType.Checkbox]);
            if (MouseClickIndex = -1) or IsCheckGlyphVisible  then
            begin
    ...
    begin
      ...
      MouseDownAt := PointF(X, Y);
      MouseClickPrev := MouseDownAt;
      MouseClickDelta := PointF(0, 0);
      MousePrevScrollPos := FScrollViewPos;
    end;
  end;
end;
...
begin
  ...
  if MouseClicked and (MouseClickIndex = -1) and (MouseEventIndex = -1) then
  begin
    ...
    if FDragListMode = TDragListMode.dmNone then
    begin
      if HasTouchTracking and (Abs(MouseClickDelta.X) > SwypeThreshold) and FCanSwipeDelete and
        (not ItemSelectTimerWorked) then
        FDragListMode := TDragListMode.dmSwype
      else
        if Abs(MouseClickDelta.Y) > ScrollThreshold then
        begin
          FDragListMode := TDragListMode.dmDrag;
          if Assigned(ItemSelectTimer) and ItemSelectTimer.Enabled then
            ItemSelectTimer.Enabled := False;
          ...
          begin
            FAniCalc.Averaging := ssTouch in Shift;
            FAniCalc.Animation := True;
            FAniCalc.MouseDown(MouseClickPrev.X, MouseClickPrev.Y);
          end;
        end;
    end;
  end;

  if ItemSelectTimerWorked and (FDragListMode = TDragListMode.dmDrag) then
  begin
    ItemSelectTimerWorked := False;
    SetItemIndex(-1);
  end;

  if (not FEditMode) and (FDragListMode = TDragListMode.dmSwype) and (not ScrollingMouseTouch) and
  ...
  begin
    ...
    if (NewDeleteIndex <> -1) and (FListViewItems[NewDeleteIndex].Purpose <> TListItemPurpose.None) then
      NewDeleteIndex := -1;

    if NewDeleteIndex <> -1 then
    begin
      SetDeleteButtonIndex(NewDeleteIndex);
      SetItemIndex(NewDeleteIndex);
  ...
  if Assigned(FAniCalc) and FAniCalc.Down and (FDragListMode = TDragListMode.dmDrag) then
    FAniCalc.MouseMove(X, Y);

  if (MouseEventIndex <> -1) and (FDragListMode = TDragListMode.dmDrag) then
...
begin
  ...
  begin
    ...
    ItemSelectTimerWorked := False;
    FDragListMode := TDragListMode.dmNone;
    FAniCalc.MouseUp(MouseDownAt.X, MouseDownAt.Y);
    DoChange;
    Invalidate;
  end;
  ...
  if Assigned(FAniCalc) and ScrollingActive then
    FAniCalc.MouseUp(X, Y);
  ...
  begin
    ...
    begin
      ...
      begin
        if (FAutoTapDistance > 0) and (not ItemSelectTimerWorked) and (FDragListMode = TDragListMode.dmDrag) then
          AnimateFloat('ScrollViewPos', 0, Min(FAutoTapDistance / AutoTapScrollingSpeed, 1.0), TAnimationType.atOut,
            TInterpolationType.itSinusoidal)
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              begin
                if HasTouchTracking and (MouseClickIndex = -1) and
                  TOpenListBoxItem(Items[NewIndex]).HasClickOnSelectItems then
                  FItemIndex := -1;
...
procedure TCustomListView.DoChangeRepainted;
begin
...
begin
  ...
  begin
    ...
    if Assigned(FSearchEdit) and FSearchEdit.Visible then
...
procedure TCustomListView.ApplyStyle;
  ...
  begin
    StyleObject := FindStyleResource(ObjectName);
    if StyleObject is TStyleObject then
      Result := TStyleObject(StyleObject)
    else
      Result := nil;
  end;
...
begin
  ...
  // Style Images
  SelectionStyleImage := GetStyleObjectFromStyle('selection');
  HeaderStyleImage := GetStyleObjectFromStyle('header');
...
begin
  ...
  begin
   NewPos := FSideSpace + HeightSums[AItemIndex];
    ...
    if HasTouchTracking and (TOpenListBoxItem(Items[AItemIndex]).FHeaderRef <> -1) and
...
class procedure TCustomListView.RegisterAppearances(AFactories: TArray<TItemAppearanceObjectsClass>;
...
class procedure TCustomListView.RegisterAppearance(const AFactory: TItemAppearanceObjectsClass;
...
constructor TCustomListView.TPublishedObjects.Create(const AOwner: TCustomListView);
begin
  inherited Create;
  FOwner := AOwner;
end;
...
procedure TCustomListView.TPublishedObjects.SetFooterObjects(const Value: TItemAppearanceObjects);
begin
  FOwner.FFooterAppearanceProperties.Objects := Value;
end;

procedure TCustomListView.TPublishedObjects.SetHeaderObjects(const Value: TItemAppearanceObjects);
begin
  FOwner.FHeaderAppearanceProperties.Objects := Value;
end;

procedure TCustomListView.TPublishedObjects.SetItemEditObjects(const Value: TItemAppearanceObjects);
begin
  FOwner.FItemEditAppearanceProperties.Objects := Value;
end;

procedure TCustomListView.TPublishedObjects.SetItemObjects(const Value: TItemAppearanceObjects);
begin
...
var
  ...
  SaveSelected: TListViewItem;
begin
  SaveSelected := Owner.Selected;
  ...
  if SaveSelected <> Owner.Selected then
  begin
    // Selection has changed, so send notifications
    TLinkObservers.ListSelectionChanged(Owner.Observers);
    Owner.DoChange;
  end;
end;
...
procedure TListViewItems.PrepareEditModeObjects;
...
begin
  if CanResetObjects then
    for I := 0 to FUnfilteredItems.Count - 1 do
      FOwner.PrepareEditModeObjects(FUnfilteredItems[I]);
end;
...
procedure TObjectAppearance.CreateObject(const AListViewItem: TListViewItem);
begin
  Assert(False); // shouldn't get here
end;

procedure TObjectAppearance.ResetObject(const AListViewItem: TListViewItem);
begin
  Assert(False); // shouldn't get here
end;
...
begin
  Result := FInitializing or FIsDefaultValues or not IsActive;
end;
...
begin
  ...
  FInternalPlaceOffset := TPosition.Create(TPointF.Create(0, 0));
...
begin
  ...
  begin
    ...
    begin
      ...
      if csDesigning in Self.Owner.ComponentState then
...
procedure TAppearanceObjects.InitEditModeObjects(const AListViewItem: TListViewItem);
begin
  // Do nothing
end;
...
procedure TPresetItemObjects.InitEditModeObjects(const AListViewItem: TListViewItem);
var
...
begin
  ...
  try
    ...
    if Assigned(LGroupClass) and (AListViewItem.Objects.FAppearance <> nil) and
...
constructor TRegisterAppearanceValue.Create(const AName: string; AOptions: TCustomListView.TRegisterAppearanceOptions;
  const AUnitName: string);
begin
  Name := AName;
  UnitName := AUnitName;
...
begin
  ...
  try
    ...
    begin
      LOuterWidth := Owner.Width - (Owner.ItemSpaces.Left + Owner.ItemSpaces.Right);
      Text.InternalWidth := LOuterWidth * 0.6;
      if Detail.Align = TListItemAlign.Trailing then
      begin
        Detail.InternalPlaceOffset.X := - (Accessory.ActualWidth + cTextMarginAccessory);
        LInternalWidth := (LOuterWidth + Detail.ActualPlaceOffset.X) - (Text.ActualWidth + cTextMarginDetail);
      end
      else
      begin
...
begin
  ...
  try
    ...
    begin
      ...
      Image.InternalPlaceOffset.X := LImagePlaceOffset;
      Text.InternalPlaceOffset.X := Image.ActualPlaceOffset.X +  Image.ActualWidth + LImageTextPlaceOffset;
      LInternalWidth := LOuterWidth - Text.ActualPlaceOffset.X - TextButton.ActualWidth;
