...
{$I FireDAC.inc}
{$IFDEF WIN32}
  {$HPPEMIT '#pragma link "FireDAC.FMXUI.Error.obj"'}
{$ELSE}
  {$HPPEMIT '#pragma link "FireDAC.FMXUI.Error.o"'}
{$ENDIF}
...
begin
  ...
  if FError.ErrorCount > 0 then begin
    ...
    edtMessage.Text := FDFixCRLF(oError.Message);
...
begin
  ...
  for i := 0 to oParams.Count - 1 do begin
    ...
    if oText <> nil then
      oText.Text := FDNameFromIndex(oParams, i);
    oText := TText(oLI.FindStyleResource('Value'));
    if oText <> nil then
      oText.Text := FDValueFromIndex(oParams, i);
  end;
...
begin
  memMsg.Text := FDFixCRLF(FError.Message);
...
begin
  Result := GetForm.FormStyle = TFormStyle.fsStayOnTop;
end;
...
begin
  if AValue then
    GetForm.FormStyle := TFormStyle.fsStayOnTop
  else
    GetForm.FormStyle := TFormStyle.fsNormal;
end;
...
initialization
  oFact := TFDSingletonFactory.Create(TFDGUIxErrorDialogImpl,
    IFDGUIxErrorDialog, C_FD_GUIxFMXProvider, 'FireDAC.FMXUI.Error');
