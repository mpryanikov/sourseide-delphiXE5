...
{                                                                              }
  ...
  TCodecReadEvent = procedure (const Sender: TObject; var Buf; Count: Longint;
                               var Ok: Boolean) of object;
  TCodecWriteEvent = procedure (const Sender: TObject; const Buf; Count: Longint)
                                of object;
...
{$IFDEF NEXTGEN}
    ...
    function  GetCharCount(const S: string): Integer;
{$ELSE !NEXTGEN}
    function  Encode(const S: PWideChar; const Length: Integer;
...
{$IFDEF NEXTGEN}
    property  EncodeReplaceChar: Byte read FEncodeReplaceChar write FEncodeReplaceChar default Byte(#$20);
{$ELSE !NEXTGEN}
    property  EncodeReplaceChar: AnsiChar read FEncodeReplaceChar write FEncodeReplaceChar default AnsiChar(#$20);
{$ENDIF NEXTGEN}
...
{$IFDEF NEXTGEN}
    function  DecodeChar(const P: Byte): WideChar; virtual; abstract;
    function  EncodeChar(const Ch: WideChar): Byte; virtual; abstract;

    function  DecodeUCS4Char(const P: Byte): UCS4Char; virtual;
    function  EncodeUCS4Char(const Ch: UCS4Char): Byte; virtual;
{$ELSE !NEXTGEN}
...
type
  ...
  public
{$IFDEF NEXTGEN}
    function  DecodeChar(const P: Byte): WideChar; override;
    function  EncodeChar(const Ch: WideChar): Byte; override;
{$ELSE !NEXTGEN}
...
{$IFDEF NEXTGEN}
    class function CheckA(const S: string): Boolean;
    class function CheckW(const S: string): Boolean;
{$ELSE !NEXTGEN}
...
{$IFDEF NEXTGEN}
// To avoid ifdefing all the code for the AnsiChar paramaters and the castings,
// we define the AnsiChar as a Byte
type
  AnsiChar = Byte;
{$ENDIF NEXTGEN}
...
begin
  Result := nil;
  PositionBackup := Stream.Position;
  try
    if Stream.Read(A, 2) = 2 then
      case A[0] of
        $00: if A[1] = $00 then
            if Stream.Read(A, 2) = 2 then
...
{$IFDEF NEXTGEN}
function TUnicodeCodec.EncodeStr(const S: string): string;
{$ELSE !NEXTGEN}
...
{$IFDEF NEXTGEN}
  function UTF8ToUCS4Char(const S: string; const Index: Integer;
...
{$IFDEF NEXTGEN}
  function UTF8ToWideChar(const S: string; const Index: Integer;
...
begin
  ...
  repeat
    ...
    try
{$IFDEF NEXTGEN}
      R := UTF8ToWideChar(string(P), L, I, C);
{$ELSE !NEXTGEN}
      R := UTF8ToWideChar(AnsiString(P), L, I, C);
{$ENDIF NEXTGEN}
      case R of
        UTF8ErrorNone :
          begin
...
{$IFDEF NEXTGEN}
function TUTF8Codec.GetCharCount(const S: string): Integer;
{$ELSE !NEXTGEN}
function TUTF8Codec.GetCharCount(const S: AnsiString): Integer;
{$ENDIF NEXTGEN}

{$IFDEF NEXTGEN}
  function UTF8CharSize(const S: string; const Index: Integer): Integer;
{$ELSE !NEXTGEN}
    ...
    if C and $C0 = $80 then // invalid encoding
      ...
      begin
        ...
        for I := Index + 1 to Index + Result - 1 do
          if Ord(S[I-AdjustIndex]) and $C0 <> $80 then
          begin
            Result := 1; // invalid encoding
            Exit;
          end;
      ...
      case FErrorAction of
        ...
        eaReplace :
          begin
            if L >= C then
              Break;
            AnsiChar(Q^) := AnsiChar(Lo(Ord(FDecodeReplaceChar)));
            AnsiChar((Q + 1)^) := AnsiChar(Hi(Ord(FDecodeReplaceChar)));
            Inc(Q, 2);
            Inc(P, 4);
            Inc(L);
          end;
      ...
      begin
        if Ch4 > $FFFF then
          begin
            if L + 1 >= C then
              Break;
            AnsiChar(Q^) := AnsiChar((Ord((P + 1)^) shl 6) + (Ord((P + 2)^) shr 2));
            AnsiChar((Q + 1)^) := AnsiChar($D8 + (Ord((P + 1)^) shr 2));
            (Q + 2)^ := (P + 3)^;
            AnsiChar((Q + 3)^) := AnsiChar($DC + (3 and Ord((P + 2)^)));
            Inc(Q, 4);
            Inc(P, 4);
            Inc(L, 2);
          end
...
begin
  ...
  while P < N do
    case Ord(P^) of
      $D800..$DBFF: // High surrogate of Unicode character [$10000..$10FFFF]
        begin
          ...
          case LowSurrogate of  // Low Surrogate following?
            $DC00..$DF00:
              begin
                AnsiChar(Q^) := AnsiChar(0);
                AnsiChar((Q+1)^) := AnsiChar((HighSurrogate - $D7C0) shr 6);
                AnsiChar((Q+2)^) := AnsiChar(((HighSurrogate and $3F) shl 2) + ((LowSurrogate - $DC00) shr 8));
                AnsiChar((Q+3)^) := AnsiChar(Lo(LowSurrogate));
                Inc(P);
                Inc(Q, 4);
                Inc(M, 4);
              end;
      ...
      AnsiChar(Q^) := AnsiChar(0);
      AnsiChar((Q+1)^) := AnsiChar(0);
      AnsiChar((Q+2)^) := AnsiChar(Hi(Ord(P^)));
      AnsiChar((Q+3)^) := AnsiChar(Lo(Ord(P^)));
      Inc(P);
      Inc(Q, 4);
      Inc(M, 4);
    end;
      ...
      if ((Ch4 >= $D800) and (Ch4 < $E000)) or (Ch4 > $10FFFF) then
        ...
        if Ch4 > $FFFF then
          begin
            if L + 1 >= C then
              Break;
            AnsiChar(Q^) := AnsiChar((Ord((P + 2)^) shl 6) + (Ord((P + 1)^) shr 2));
            AnsiChar((Q + 1)^) := AnsiChar($D8 + (Ord((P + 2)^) shr 2));
            (Q + 2)^ := P^;
            AnsiChar((Q + 3)^) := AnsiChar($DC + (3 and Ord((P + 1)^)));
            Inc(Q, 4);
            Inc(P, 4);
            Inc(L, 2);
          end
      ...
      $D800..$DBFF: // High surrogate of Unicode character [$10000..$10FFFF]
        begin
          ...
          case LowSurrogate of  // Low Surrogate following?
            $DC00..$DF00:
              begin
                AnsiChar(Q^) := AnsiChar(Lo(LowSurrogate));
                AnsiChar((Q + 1)^) := AnsiChar(((HighSurrogate and $3F) shl 2) + ((LowSurrogate - $DC00) shr 8));
                AnsiChar((Q + 2)^) := AnsiChar((HighSurrogate - $D7C0) shr 6);
                AnsiChar((Q + 3)^) := AnsiChar(0);
                Inc(P);
                Inc(Q, 4);
                Inc(M, 4);
              end;
      ...
      if ((Ch4 >= $D800) and (Ch4 < $E000)) or (Ch4 > $10FFFF) then
        ...
        if Ch4 > $FFFF then
          begin
            if L + 1 >= C then
              Break;
            AnsiChar(Q^) := AnsiChar((Ord(P^) shl 6) + (Ord((P + 3)^) shr 2));
            AnsiChar((Q + 1)^) := AnsiChar($D8 + (Ord(P^) shr 2));
            (Q + 2)^ := (P + 2)^;
            AnsiChar((Q + 3)^) := AnsiChar($DC +  (3 and Ord((P + 3)^)));
            Inc(Q, 4);
            Inc(P, 4);
            Inc(L, 2);
          end
  ...
  while P < N do
    begin
      case Ord(P^) of
        $D800..$DBFF: // High surrogate of Unicode character [$10000..$10FFFF]
          begin
            ...
            case LowSurrogate of  // Low Surrogate following?
              $DC00..$DF00:
                begin
                  AnsiChar(Q^) := AnsiChar((HighSurrogate - $D7C0) shr 6);
                  AnsiChar((Q + 1)^) := AnsiChar(0);
                  AnsiChar((Q + 2)^) := AnsiChar(Lo(LowSurrogate));
                  AnsiChar((Q + 3)^) := AnsiChar(((HighSurrogate and $3F) shl 2) + ((LowSurrogate - $DC00) shr 8));
                  Inc(P);
                  Inc(Q, 4);
                  Inc(M, 4);
                end;
        ...
        AnsiChar(Q^) := AnsiChar(0);
        AnsiChar((Q + 1)^) := AnsiChar(0);
        AnsiChar((Q + 2)^) := AnsiChar(Lo(Ord(P^)));
        AnsiChar((Q + 3)^) := AnsiChar(Hi(Ord(P^)));
        Inc(P);
        Inc(Q, 4);
        Inc(M, 4);
      end;
      ...
      if ((Ch4 >= $D800) and (Ch4 < $E000)) or (Ch4 > $10FFFF) then
        ...
        if Ch4 > $FFFF then
          begin
            if L + 1 >= C then
              Break;
            AnsiChar(Q^) := AnsiChar((Ord((P + 3)^) shl 6) + (Ord(P^) shr 2));
            AnsiChar((Q + 1)^) := AnsiChar($D8 + (Ord((P + 3)^) shr 2));
            (Q + 2)^ := (P + 1)^;
            AnsiChar((Q + 3)^) := AnsiChar($DC +  (3 and Ord(P^)));
            Inc(Q, 4);
            Inc(P, 4);
            Inc(L, 2);
          end
...
begin
  ...
  while P < N do
    case Ord(P^) of
      $D800..$DBFF: // High surrogate of Unicode character [$10000..$10FFFF]
        begin
          ...
          case LowSurrogate of  // Low Surrogate following?
            $DC00..$DF00:
              begin
                AnsiChar(Q^) := AnsiChar(((HighSurrogate and $3F) shl 2) + ((LowSurrogate - $DC00) shr 8));
                AnsiChar((Q + 1)^) := AnsiChar(Lo(LowSurrogate));
                AnsiChar((Q + 2)^) := AnsiChar(0);
                AnsiChar((Q + 3)^) := AnsiChar((HighSurrogate - $D7C0) shr 6);
                Inc(P);
                Inc(Q, 4);
                Inc(M, 4);
              end;
      ...
      AnsiChar(Q^) := AnsiChar(Hi(Ord(P^)));
      AnsiChar((Q + 1)^) := AnsiChar(Lo(Ord(P^)));
      AnsiChar((Q + 2)^) := AnsiChar(0);
      AnsiChar((Q + 3)^) := AnsiChar(0);
      Inc(P);
      Inc(Q, 4);
...
const
  Windows1252Map : array[Ord(#$80)..Ord(#$9F)] of WideChar = (
...
{$IFDEF NEXTGEN}
class function TUSASCIICodec.CheckA(const S: string): Boolean;
{$ELSE !NEXTGEN}
...
{$IFDEF NEXTGEN}
class function TUSASCIICodec.CheckW(const S: string): Boolean;
{$ELSE !NEXTGEN}
