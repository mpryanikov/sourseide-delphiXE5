...
uses
  ...
  FMX.Controls, FMX.Styles, FMX.Graphics;
...
begin
  if (FStartCount > 0) or TPlatformServices.Current.SupportsPlatformService(IFMXMouseService, IInterface(MouseSvc)) then
    Inc(FStartCount);
...
begin
  ...
  try
    ...
    if not Assigned(Form) then
      ...
      if Screen.Forms[I].FormStyle = TFormStyle.fsStayOnTop then
      begin
    ...
    begin
      ...
      if Assigned(Control) and (Control.GetObject is TMenuItem) then
      begin
        SubViewFound := False;
        NewItem := TMenuItem(Control.GetObject);
        if Assigned(NewItem) and (not NewItem.IsSelected) then
        begin
          NewView := NewItem.View;
          CurrentView := NewView;
          while Assigned(CurrentView.ChildView) do
          begin
          ...
          begin
            ...
            if Assigned(ViewObject) and (ViewObject.Parent is TPopup) then
    ...
    begin
      if Assigned(FOldItem) and ((Form.FormStyle = TFormStyle.fsPopup) or FActive) then
      begin
        if FActive then
        begin
          if Form.FormStyle = TFormStyle.fsPopup then
            LDelay := LDelay div 3
          else
            LDelay := 0;
        end;
...
begin
  if not Assigned(vAutopopupMenuTimer) then
  begin
...
begin
  if Assigned(vAutopopupMenuTimer) then
  begin
...
begin
  if ([csLoading, csDestroying, csDesigning] * ComponentState = []) and
     Assigned(FMenuService) then
  begin
    FMenuService.UpdateMenuItem(Self, [TMenuItemChange.mcBitmap]);
  end;
end;
...
begin
  ...
  try
    ...
    LOffset := Menu.AbsoluteToLocal(Menu.FContentLayout.LocalToAbsolute(PointF(0, 0)));
    if not View.IsMenuBar then
    begin
      Popup.HorizontalOffset := -LOffset.X;
      Popup.VerticalOffset := -LOffset.Y;
    end;
    Menu.Align := TAlignLayout.alClient;
    // show
    Popup.Popup;
    // start loop
    if Assigned(FMenuService) then
...
procedure TMainMenu.ChangeParent;
var
