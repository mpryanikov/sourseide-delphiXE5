...
begin
  ...
  case AArg^.VType of
    ...
    vtPointer:
      FArgs.WriteBuffer(NativeInt(AArg^.VPointer), SizeOf(AArg^.VPointer));
    vtPChar:
      begin
    ...
    vtPWideChar:
      begin
        iLen := FDWideStrLen(AArg^.VPWideChar) * SizeOf(WideChar);
        FArgs.WriteBuffer(iLen, SizeOf(iLen));
        if iLen > 0 then
    ...
    vtUnicodeString:
      begin
        ...
        FArgs.WriteBuffer(iLen, SizeOf(iLen));
        if iLen > 0 then
          FArgs.WriteBuffer(PWideChar(UnicodeString(AArg^.VUnicodeString))^,
            iLen);
      end;
    else
