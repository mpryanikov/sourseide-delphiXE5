...
{$WARN HIDING_MEMBER ON}
  ...
  protected
    ...
    function ConvertFieldNameToJson(const AFieldName: string): string;
  ...
  private
    ...
    function ConvertFieldNameFromJson(const AFieldName: string): string;
  public
  ...
  public
    ...
    /// </summary>
    /// <param name="AClass">Type of the object to be returned</param>
    /// <param name="JsonObj">JSON object instance</param>
    /// <returns>object instance</returns>
    function CreateObject(AClass: TClass; JsonObj: TJSONObject): TObject;
...
var
  LFieldName: string;
begin
  LFieldName := ConvertFieldNameToJson(FieldName);
...
function TJSONConverter.ConvertFieldNameToJson(const AFieldName: string): string;
var
  LFieldName: string;
begin
  ...
  if AFieldName.StartsWith('F', true) then
...
begin
  ...
  begin
    ...
    begin
      ...
      else if (rttiField.FieldType.TypeKind = tkClass) and (rttiField.GetValue(Data).AsObject <> nil) then
      begin
...
begin
  ...
  if tRtti <> nil then
  begin
    fRtti := tRtti.GetField(ConvertFieldNameFromJson(Field));
...
function TJSONUnMarshal.CreateObject(AClass: TClass; JsonObj: TJSONObject): TObject;
var
  LObjectType: string;
  LObject: TObject;
  LRttiType: TRttiType;
...
begin
  Assert(JsonObj <> nil);
  Assert(JsonObj.Size > 0, 'JsonObj.Size: ' + IntToStr(JsonObj.Size));

  if AClass = nil then
    raise EConversionError.Create(Format(SCannotCreateType, ['nil'])); // Do not localize
  ...
  if LRttiType <> nil then
    for LAttribute in LRttiType.GetAttributes do
      if LAttribute is JsonReflect then
      begin
        LCustomizer := JsonReflect(LAttribute).JSONPopulationCustomizer;
      end;
  ...
  try
    for fieldAttr in Field.GetAttributes do
      if fieldAttr is JsonReflect then
        Exit(JsonReflect(fieldAttr).JSONInterceptor)
  except
...
var
  ...
  LField: string;
begin
  LField := ConvertFieldNameFromJson(Field);
...
function TJSONUnMarshal.ConvertFieldNameFromJson(const AFieldName: string): string;
var
  LFieldName: string;
begin
      ...
      TTypeKind.tkDynArray, TTypeKind.tkArray:
        begin
          ...
          for I := 0 to Length(tvArray) - 1 do
            tvArray[I] := JSONToTValue(TJSONArray(JsonValue).Get(I), elementType);
        ...
        try
          case revAttr.ReverterType of
            ...
            rtTypeObject:
              begin
                if jsonFieldVal is TJSONObject then
                  SetField(Data, FieldName, revAttr.TypeObjectReverter(CreateObject(revEv.FFieldClassType,
                    TJSONObject(jsonFieldVal))))
                else if jsonFieldVal is TJSONNull then
                  SetField(Data, FieldName, nil)
                else
            ...
            rtObject:
              begin
                if jsonFieldVal is TJSONObject then
                  revAttr.ObjectReverter(Data, FieldName, CreateObject(revEv.FFieldClassType,
                    TJSONObject(jsonFieldVal)))
                else if jsonFieldVal is TJSONNull then
                  revAttr.ObjectsReverter(Data, FieldName, nil)
                else
            ...
            rtString:
              begin
                if jsonFieldVal is TJSONString then
                  revAttr.StringReverter(Data, FieldName, TJSONString(jsonFieldVal).Value)
                else if jsonFieldVal is TJSONNull then
                  revAttr.ObjectsReverter(Data, FieldName, nil)
                else
                  raise EConversionError.Create(Format(SObjectExpectedForField, [FieldName, jsonFieldVal.ToString]));
...
constructor JsonReflect.Create(ConverterType: TConverterType; ReverterType: TReverterType; InterceptorType: TClass;
...
begin
  ...
  begin
    if not CanPopulate(Data, rttiField) then
      continue;
    if (rttiField.FieldType <> nil) and (rttiField.FieldType.TypeKind = tkClass) and
      JSONBooleanAttributeValue(rttiField, JSONOwned, true)
...
begin
  ...
  if rttiField.Name = 'FRefCount' then  begin
    result := false;
  end else begin
