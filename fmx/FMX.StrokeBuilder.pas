...
uses
  System.Types, System.UITypes, System.UIConsts, System.Math, FMX.Types,
  FMX.Graphics, FMX.Canvas.GPU.Helpers;
...
begin
  ...
  StepSize := FEllipseCircumf / Max(Sqrt(FEllipseCircumf) * 2, 8);
...
begin
  Path.FlattenToPolygon(Polygon, 1);
  ...
  begin
    if (CurIndex >= Length(Polygon) - 1) and (Path.Count > 0) and
      (Path[Path.Count - 1].Kind <> TPathPointKind.ppClose) then
      Break;
    ...
    begin
      ...
      ThickPerp := PointF(-PieceDirVec.Y, PieceDirVec.X) * FHalfThickness;
...
begin
  ...
  begin
    if (CurIndex >= Length(Polygon) - 1) and (Path.Count > 0) and
      (Path[Path.Count - 1].Kind <> TPathPointKind.ppClose) and (Polygon[0] <> Polygon[Length(Polygon) - 1]) then
      Break;
