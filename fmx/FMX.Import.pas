...
uses
  ...
  System.UIConsts, FMX.Objects3D, FMX.Materials, FMX.MaterialSources, FMX.Graphics;
...
begin
  ...
  LTangent1 := (LPos1 - LPos2).Normalize;
  LTangent2 := (LPos1 - LPos3).Normalize;
  LNormal   := LTangent2.CrossProduct(LTangent1).Normalize;

  Av1.Nor := LNormal.ToPoint3D;
  Av2.Nor := LNormal.ToPoint3D;
  Av3.Nor := LNormal.ToPoint3D;
end;
...
begin
  ...
  begin
    ...
    LVec := LVec * ATransform;
    LMesh.Data.VertexBuffer.Vertices[v] := LVec.ToPoint3D;

    LVec := TVector3D(LNewTriangles[v].Nor);
    LVec := (LVec * ANormalTransform).Normalize;
    LMesh.Data.VertexBuffer.Normals[v] := LVec.ToPoint3D;
