...
{$IFDEF ANDROID}
 {$IFDEF CPUARM}
  ...
  UserContext.uc_mcontext.arm_r2 := MapSignal(SigNum, NativeUInt(UContext));
  ...
  UserContext.uc_mcontext.arm_pc := UIntPtr(@SignalConverter);
 {$ENDIF CPUARM}
{$ENDIF ANDROID}
{$IFDEF MACOS}
 {$IFDEF CPUX86}
...
//  GeneralRegisters^[REG_ECX] := MapSignal(SigNum, IntPtr(GeneralRegisters));
...
{$IFDEF ANDROID}
  if (@Action.sigaction_u.sa_sigaction <> @SignalDispatcher) then
{$ELSE}
...
{$IFDEF ANDROID}
  Action.sigaction_u.sa_sigaction := @SignalDispatcher;
{$ELSE}
