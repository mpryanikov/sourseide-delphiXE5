...
{-------------------------------------------------------------------------------}
function TFDPhysIBMetadata.GetSelectWithoutFrom: Boolean;
begin
  Result := False;
end;
...
{-------------------------------------------------------------------------------}
function TFDPhysIBMetadata.GetNullLocation: TFDPhysNullLocations;
begin
  case FBrand of
  ibYaffil:
...
{-------------------------------------------------------------------------------}
function TFDPhysIBCommandGenerator.GetPing: String;
begin
  Result := 'SELECT 1 FROM ' + GetSingleRowTable;
end;
...
var
  ...
  begin
    Result :=
      'CASE WHEN ' + FDomainField + ' LIKE ''%BOOL%'' THEN ' + DT2Str(dtBoolean) +
      ' ELSE ' +
        'CASE ' + AField +
  ...
  function EncodeTypeName(const AField, ASubField, AScaleField, FDomainField,
  ...
  begin
    Result :=
      'CASE WHEN ' + FDomainField + ' LIKE ''%BOOL%'' THEN ''BOOLEAN''' +
      ' ELSE ' +
        'CASE ' + AField +
...
begin
  ...
  mkTables:
    begin
      ...
      if AWildcard <> '' then
        AddWhere('RDB$RELATION_NAME LIKE ?', 'WIL');
      AddTopScope(True);
  ...
  mkTableFields:
    begin
      ...
      if AWildcard <> '' then
        AddWhere('RF.RDB$FIELD_NAME LIKE ?', 'WIL');
  ...
  mkIndexes:
    begin
      Result := 'SELECT CAST(NULL AS NUMERIC) AS RECNO, ' +
        ...
        'C.RDB$CONSTRAINT_NAME AS PKEY_NAME, ';
      if (oIBConnMeta.Brand = ibInterbase) and (FConnMeta.ServerVersion < ivIB070500) then
        Result := Result +
        ...
        Result := Result +
          ...
          ' WHEN I.RDB$UNIQUE_FLAG = 1 THEN ' +
            'CASE ' +
            ' WHEN C.RDB$CONSTRAINT_NAME IS NULL THEN ' + IntToStr(Integer(ikUnique)) +
            ' ELSE ' + IntToStr(Integer(ikPrimaryKey)) +
            ' END' +
          ' END AS INDEX_TYPE ';
      Result := Result +
        'FROM RDB$INDICES I LEFT OUTER JOIN RDB$RELATION_CONSTRAINTS C ON ' +
        ' I.RDB$RELATION_NAME = C.RDB$RELATION_NAME AND I.RDB$INDEX_NAME = C.RDB$INDEX_NAME AND' +
        ' C.RDB$CONSTRAINT_TYPE = ''PRIMARY KEY''';
      AddWhere('I.RDB$RELATION_NAME = ?', 'OBJ');
      if AWildcard <> '' then
        AddWhere('I.RDB$INDEX_NAME LIKE ?', 'WIL');
  ...
  mkIndexFields:
    begin
      ...
      if AWildcard <> '' then
        AddWhere('S.RDB$FIELD_NAME LIKE ?', 'WIL');
  ...
  mkForeignKeys:
    begin
      ...
      if AWildcard <> '' then
        AddWhere('FK.RDB$CONSTRAINT_NAME LIKE ?', 'WIL');
      Result := Result + ' ORDER BY 5';
    end;
    ...
    begin
      Result := 'SELECT CAST(NULL AS NUMERIC) AS RECNO, ' +
        ...
        'FROM RDB$RELATION_CONSTRAINTS FK LEFT JOIN RDB$REF_CONSTRAINTS R ON FK.RDB$CONSTRAINT_NAME = R.RDB$CONSTRAINT_NAME ' +
                                         ...
                                         'LEFT JOIN RDB$INDEX_SEGMENTS PK_F ON PK.RDB$INDEX_NAME = PK_F.RDB$INDEX_NAME';
      AddWhere('FK.RDB$RELATION_NAME = ?', 'BAS');
      AddWhere('FK.RDB$CONSTRAINT_NAME = ?', 'OBJ');
      if AWildcard <> '' then
        AddWhere('FK_F.RDB$FIELD_NAME LIKE ?', 'WIL');
  ...
  mkPackages:
    if (oIBConnMeta.Brand = ibFirebird) and (FConnMeta.ServerVersion >= ivFB030000) then begin
      ...
      AddTopScope(True);
      if AWildcard <> '' then
        AddWhere('RDB$PACKAGE_NAME LIKE ?', 'WIL');
      Result := Result + ' ORDER BY 4';
    end
    else
      Result := '';
  ...
  mkProcs:
    begin
      ...
      AddTopScope(True);
      if AWildcard <> '' then
        AddWhere('RDB$PROCEDURE_NAME LIKE ?', 'WIL');
      ...
      if (oIBConnMeta.Brand = ibFirebird) and (FConnMeta.ServerVersion >= ivFB030000) then begin
        ...
        AddTopScope(True);
        if AWildcard <> '' then
          AddWhere('RDB$FUNCTION_NAME LIKE ?', 'WIL');
      end;
  ...
  mkProcArgs:
    begin
      ...
      if AWildcard <> '' then
        AddWhere('P.RDB$PARAMETER_NAME LIKE ?', 'WIL');
      ...
      if (oIBConnMeta.Brand = ibFirebird) and (FConnMeta.ServerVersion >= ivFB030000) then begin
        ...
        if AWildcard <> '' then
          AddWhere('P.RDB$ARGUMENT_NAME LIKE ?', 'WIL');
      end;
  ...
  mkGenerators:
    begin
      ...
      if AWildcard <> '' then
        AddWhere('RDB$GENERATOR_NAME LIKE ?', 'WIL');
      Result := Result + ' ORDER BY 4';
    end;
