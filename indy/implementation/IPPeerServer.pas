...
{$HPPEMIT '#pragma link "IPPeerServer"'}    {Do not Localize}
{$IFDEF MACOS}
...
uses
  ...
  IdIOHandlerSocket, IdIOHandler, IdStack, IdHTTP, IdHTTPHeaderInfo,
  IdServerIOHandler, IdContext, IdSchedulerOfThreadPool, IdScheduler,
  IdSSLOpenSSL, IdGlobalProtocols, IdHeaderList, IdSocketHandle,
  IdTCPServer, IdHTTPServer, IdCustomHTTPServer, IdTCPConnection,
  IdStackConsts,
...
begin
  Result := PeerFactory.CreatePeer(GetIPImplementationID, IIPSocketHandle,
              FHandles.Add) as IIPSocketHandle
end;
...
begin
  ...
  try
    ...
    LOnGetPassword := TOnGetPassword.Create(string(AKeyFilePassword));
