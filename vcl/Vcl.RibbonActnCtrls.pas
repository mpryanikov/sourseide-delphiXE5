...
type
  ...
  // This interface is to be used by controls to query their parent to see if
  // the control is actually on a status bar. This is due to Font Color issues
  IRibbonStatusBar = interface
    ['{DD009562-A83B-4B57-B1B2-0A459E06AE17}']
  end;

  IRibbonStatusBarPanel = interface
    ['{ADFDE8BC-A1D8-4D80-AA3A-7A2C39ED27F8}']
  end;
  ...
  protected
    ...
    function HasMultiLineCaption: Boolean; inline;
...
{$ENDIF}
  ...
  public
    constructor Create(AOwner: TComponent); override;
    property IsFilename: Boolean read FIsFilename write FIsFilename;
  end;
...
type
  // The class is used to show a KeyTip for a particular item on the Ribbon.
...
begin
  if Enabled and MouseInControl and not ActionBar.DesignMode then
  begin
...
function TRibbonBaseButtonControl.HasMultiLineCaption: Boolean;
begin
  if Canvas.HandleAllocated then
...
begin
  ...
  begin
    if LImageList.Width < DEF_IMG_LARGE then
    begin
      Inc(LLocation.X, LImageList.Width div 2);
      Inc(LLocation.Y, LImageList.Height div 2);
    end;
  end;

  LImageList.Draw(Canvas, LLocation.X, LLocation.Y, ActionClient.ImageIndex,
...
begin
  ...
  begin
    StartPos := Point(Offset, 2);
    EndPos := Point(ClientWidth - Offset, 2);
    Canvas.Pen.Width := 1;
    Canvas.Pen.Color := $00EBEBEB;
    Canvas.MoveTo(StartPos.X, StartPos.Y);
    Canvas.LineTo(EndPos.X, EndPos.Y);
    Canvas.Pen.Color := $00C5C5C5;
    Canvas.MoveTo(StartPos.X, StartPos.Y + 1);
    Canvas.LineTo(EndPos.X, EndPos.Y + 1);
  end;
end;
...
var
  LText: string;
begin
  if Supports(Parent, IRibbonStatusBarPanel) then
  begin
    if ActionBar.ColorMap is TCustomRibbonColorMap then
    begin
      if MouseInControl or Selected then
        Canvas.Font.Color := TCustomRibbonColorMap(ActionBar.ColorMap).StatusBarHotFontColor
      else
        Canvas.Font.Color := TCustomRibbonColorMap(ActionBar.ColorMap).StatusBarFontColor;
    end
    else
    begin
      if MouseInControl or Selected then
  ...
  else if MouseInControl and (ActionBar.ColorMap.HotFontColor <> clDefault) then
...
begin
  ...
  begin
    case LButtonProps.ButtonSize of
      ...
      bsLarge:
        begin
          result.X := DEF_IMG_LARGE;
          result.Y := DEF_IMG_LARGE;
        end;
    end;
  end
  else
...
begin
  ...
  begin
    LIsMultiLine := HasMultiLineCaption;
    if LIsMultiLine then
      LX := TextBounds.Right + 3
    else
      LX := (Width div 2) - 2;
    LY := ARect.Top + (Canvas.TextHeight(Text) div 2);
    // arrow always appears on the bottom line
...
begin
  ...
  Height := DEF_SEPARATOR_H;
  if ActionClient <> nil then
  begin
    // The first separator in a menu is 2 pixels shorter than a regular separator.
    if (ActionClient.Index = 0) then
      Height := DEF_SEPARATOR_H - 2;
  end
end;
...
begin
  ...
  begin
    if not HasMultilineCaption then
    begin
    ...
    begin
      LY := TextBounds.Bottom - Canvas.TextHeight(Text) + 5;
      LX := TextBounds.Right + 3;
    end;
  end;
  Result := Point(LX, LY);
...
begin
  ...
  if ContainedControl <> nil then
    Height := Min(Max(Height, ContainedControl.Height), DEF_LARGE_HEIGHT);
end;
...
begin
  ...
  begin
    LDetail := StyleServices.GetElementDetails(tsPane);
    LRect := PaintRect;
    LRect.Left := LRect.Right - 2;
    StyleServices.DrawElement(Canvas.Handle, LDetail, LRect);
  end;
end;
...
begin
  ...
  begin
    ...
    else if not LTextProps.ParentFont then
...
begin
  ...
  if not GetShowShortCut then
    Dec(LWidth, ShortCutBounds.Right);
...
const
  ...
  Element: array [Boolean, Boolean] of TSkinMenu =
    ((smArrowLeftDisabledIcon, smArrowRightDisabledIcon),
...
begin
  Result := Point(DEF_IMG_LARGE, DEF_IMG_LARGE);
end;
...
begin
  Height := DEF_APP_BUTTON_H;
  Width := DEF_APP_BUTTON_W;
end;
...
constructor TRibbonRecentButtonControl.Create(AOwner: TComponent);
begin
  inherited;
  FIsFilename := True;
end;

procedure TRibbonRecentButtonControl.DrawText(var ARect: TRect;
...
begin
  ...
  ARect.Right := Width - Margins.Right;
...
begin
  ...
  begin
    ...
    try
      ...
      LOffSet := 8;
      LIconSizeInPixels := APP_ICON_MEDIUM;
      case LIconSize of
        isSmall:
          begin
            LOffset := 12;
            LIconSizeInPixels := APP_ICON_SMALL;
          end;
        isLarge:
          begin
            LOffSet := 4;
            LIconSizeInPixels := APP_ICON_LARGE;
          end;
      end;
...
begin
  ...
  begin
    ...
    begin
      if (FComboControl <> nil) then
      begin
        if not FComboControl.MouseInControl then
...
procedure TRibbonCustomizeItem.DrawText(var ARect: TRect; var Flags: Cardinal; Text: string);
begin
  Inc(ARect.Left, 5);
  inherited;
end;
