...
uses
  System.Classes, System.SysUtils, IBHeader, IB, IBIntf, IBExternals, IBDatabase,
  IBDatabaseInfo;
...
type
  ...
  private
    FVerbose: Boolean;
  protected
    Procedure VersionCheck(IBVersion, ErrorMessage : String);
  public
  published
  ...
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    procedure OnlineDump(Overwrite : Boolean);
...
uses
  Data.Db, IBSQLMonitor, IBSQL, IBXConst, IBUtils,
  System.Character;
...
begin
  ...
  begin
    ...
    if result then
    begin
      Params.Values[SPBConstantNames[isc_spb_user_name]] := Username;
      Params.Values[SPBConstantNames[isc_spb_password]] := Password;
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      try
        FGDSLibrary := IBIntf.GetGDSLibrary(LibString);
...
var
  Len: UShort;
begin
  Len := GDSLibrary.isc_vax_integer(OutputBuffer + RunLen, 2);
  RunLen := RunLen + 2;
  if (Len <> 0) then
  begin
    result := TEncoding.ANSI.GetString(TBytes(Outputbuffer), RunLen, Len);
    RunLen := RunLen + Len;
  end
  else
    result := '';
...
begin
  result := GDSLibrary.isc_vax_integer(OutputBuffer + RunLen, 4);
  RunLen := RunLen + 4;
end;
    ...
    case SPBVal of
      ...
      begin
        if (SPBVal > 0) and
          ...
          IBError(ibxeSPBConstantNotSupported, [SPBConstantNames[SPBVal]])
        else
...
begin
  Inc(RunLen);
  SetLength (FConfigParams.ConfigFileData.ConfigFileValue, Length(FConfigParams.ConfigFileData.ConfigFileValue)+1);
  SetLength (FConfigParams.ConfigFileData.ConfigFileKey, Length(FConfigParams.ConfigFileData.ConfigFileKey)+1);

  FConfigParams.ConfigFileData.ConfigFileKey[High(FConfigParams.ConfigFileData.ConfigFileKey)] := Integer(OutputBuffer[RunLen-1]);
  FConfigParams.ConfigFileData.ConfigFileValue[High(FConfigParams.ConfigFileData.ConfigFileValue)] := ParseInteger(RunLen);
end;
...
begin
  ...
  begin
    if (OutputBuffer[RunLen] <> SPBConstantValues[isc_spb_dbname]) then
      IBError(ibxeOutputParsingError, [nil]);
    Inc(RunLen);
...
begin
  Result := ExecuteSQL('SELECT RDB$FLUSH_INTERVAL from RDB$DATABASE', 'RDB$FLUSH_INTERVAL');  {do not localize}
end;

function TIBConfigService.GetGroupCommit: Boolean;
begin
  Result :=  ExecuteSQL('SELECT RDB$GROUP_COMMIT from RDB$DATABASE', 'RDB$GROUP_COMMIT') = 'Y'; {do not localize}
end;
        ...
        begin
          ...
          begin
            ...
            if S.Contains('CHECK POINT LENGTH') then {do not localize}
              ...
              if S.Contains('NUMBER OF WAL BUFFERS') then    {do not localize}
                FIBJournalInformation.PageCache := GetValue(S)
              else
                if S.Contains('WAL BUFFER SIZE') then        {do not localize}
...
begin
  Result := ExecuteSQL('SELECT RDB$LINGER_INTERVAL from RDB$DATABASE', 'RDB$LINGER_INTERVAL');  {do not localize}
end;
...
begin
  ...
  try
    ...
    svcServer.Fetch;
    if not svcServer.VersionInfo.IsMinimumVersion('11.0') then
...
begin
  ...
  ServiceStartAddParam(FDatabaseName, SPBConstantValues[isc_spb_dbname]);
  ServiceStartAddParam(aOption, SPBConstantValues[isc_spb_options]);
  if Verbose then
    ServiceStartAddParam(SPBConstantValues[isc_spb_verbose]);
...
procedure TIBBackupService.OnlineDump(Overwrite: Boolean);
var
  i: Integer;
begin
...
//    if Overwrite then
//      ServiceStartAddByteParam(1, isc_spb_dmp_overwrite);
    Attach;
    CheckActive;
    InternalServiceStart;
  finally
...
begin
  ...
  try
    VersionCheck('11.0', 'Service API write mode ');
    case FWriteMode of
    ...
    //    jsut skip write mode even if set in this case and eat hte exception;
  end;
  for i := 0 to FBackupFile.Count - 1 do
  begin
    if FBackupFile[i].Trim.IsEmpty then
...
begin
  ...
  if (OutputBuffer[0] <> isc_info_svc_to_eof) then
    IBError(ibxeOutputParsingError, [nil]);
  Len := GDSLibrary.isc_vax_integer(OutputBuffer + 1, 2);
  if (OutputBuffer[3 + Len] = isc_info_truncated) then
    ...
    if (OutputBuffer[3 + Len] = isc_info_end) then
      ...
      IBError(ibxeOutputParsingError, [nil]);
  result := TEncoding.ANSI.GetString(TBytes(OutputBuffer), 3, Len);
end;
...
var
  ...
  begin
    Idx := s.IndexOf('.');  {do not localize}
    if Idx > 0 then
    begin
      Result := s.Substring(0, Idx).ToInteger;
      s := s.Substring(s.IndexOf('.') + 1, s.Length); {do not localize}
    end
    else
    begin
      Result := s.ToInteger;
...
begin
  If PageLength <> 4000 then
    Result := Result + ' LENGTH ' + IntToStr(PageLength);   {do not localize}
end;

{ TIBBackupRestoreService }

procedure TIBBackupRestoreService.VersionCheck(IBVersion, ErrorMessage: String);
var
  svcServer : TIBServerProperties;
begin
  svcServer := TIBServerProperties.Create(nil);
