...
uses
  System.Types, System.UITypes, System.UIConsts, System.Math, System.Classes,
  System.SysUtils, System.StrUtils, System.Generics.Collections, System.Generics.Defaults,
  FMX.Types, FMX.Surfaces, FMX.TextLayout, FMX.FontGlyphs, FMX.Graphics;
...
*)
  ...
  const
    SmoothRenderMargin = 1;
    AntiAliasMargin = SmoothRenderMargin + 1;
  private class var
  ...
  private
    ...
    FScale: Single;
    FScaleFactor: Single;
    FSmoothRenderMarginInPixels: Single;
    FFrame: TGPUFrame;
    FFrame3D: TGPUFrame;
...
uses
  FMX.Consts, FMX.Platform, FMX.Canvas.GPU, FMX.Text, System.Character;
...
begin
  ...
  FSmoothRenderMarginInPixels := SmoothRenderMargin / FScale;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        if Assigned(Rec.Glyph) and Assigned(Rec.Glyph.Path) then
...
begin
  ...
  if NeedPath then
    GlyphSettings := GlyphSettings + [TFontGlyphSetting.gsPath];
...
begin
  ...
  if not HasItem or (NeedBitmap and not Assigned(Result.Bitmap) and not (TFontGlyphStyle.NoGlyph in Result.Glyph.Style)) or
...
var
  ...
  LRun: TGPURun;
  I, J, K: Integer;
  VerticalAligned, HorizonatalAligned, ColoredGlyph: Boolean;
begin
  if Text.IsEmpty then
    Exit;
  ...
  HorizonatalAligned := SameValue(Frac(TopLeft.X), 0.0, TEpsilon.Position) and SameValue(Frac(ACanvas.Matrix.m31), 0.0, TEpsilon.Position);
  ...
  begin
    LLine := FFrame[I];
    Pos := LLine.TopLeft;
    Pos.X := Pos.X + TopLeft.X;
    Pos.Y := Pos.Y + TopLeft.Y;
    for J := 0 to LLine.Count - 1 do
    begin
      LRun := LLine[J];
      CharDic := GetCharDictionary(LRun.Font);
...
begin
  ...
  LLength := Text.Length;
  while ((APos + ALength) < LLength) and IsCombiningCharacter(Text.Chars[APos + ALength]) do //Skipping combinig characters
    Inc(ALength);
