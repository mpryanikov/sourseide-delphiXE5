  ...
  case X.SpecialType of
    ...
    fsNDenormal:
      begin
        Exponent := X.Exponent + 1; // exponent biased to match
        M := X.Mantissa;
        if (M and {$IFDEF EXTENDEDIS10BYTES}$FFFFFFFF00000000{$ELSE}$1FFFFFFFE00000{$ENDIF}) = 0 then
        begin
          M := M shl 32;
          Exponent := Exponent - 32;
        end;
        if (M and {$IFDEF EXTENDEDIS10BYTES}$FFFF000000000000{$ELSE}$1FFFE000000000{$ENDIF}) = 0 then
        begin
          M := M shl 16;
          Exponent := Exponent - 16;
        end;
        if (M and {$IFDEF EXTENDEDIS10BYTES}$FF00000000000000{$ELSE}$1FE00000000000{$ENDIF}) = 0 then
        begin
          M := M shl 8;
          Exponent := Exponent - 8;
        end;
        if (M and {$IFDEF EXTENDEDIS10BYTES}$F000000000000000{$ELSE}$1E000000000000{$ENDIF}) = 0 then
        begin
          M := M shl 4;
          Exponent := Exponent - 4;
        end;
        if (M and {$IFDEF EXTENDEDIS10BYTES}$C000000000000000{$ELSE}$18000000000000{$ENDIF}) = 0 then
        begin
          M := M shl 2;
          Exponent := Exponent - 2;
        end;
        if (M and {$IFDEF EXTENDEDIS10BYTES}$8000000000000000{$ELSE}$10000000000000{$ENDIF}) = 0 then
        begin
          M := M shl 1;
          Exponent := Exponent - 1;
        end;
