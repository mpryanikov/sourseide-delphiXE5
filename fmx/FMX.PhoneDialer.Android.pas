...
uses
  System.SysUtils,
  FMX.Platform, FMX.PhoneDialer, FMX.Helpers.Android, System.Generics.Collections, FMX.Types,
  Androidapi.JNI.JavaTypes, Androidapi.JNI.Telephony, Androidapi.JNI.GraphicsContentViewText, Androidapi.JNI.Net,
  Androidapi.JNIBridge, Androidapi.JNI.Provider, Androidapi.JNI.Os;
...
procedure RegisterPhoneDialerService;
begin
  if HasSystemService(TJPackageManager.JavaClass.FEATURE_TELEPHONY) then
...
begin
  ...
  begin // add call state in the call list
    ...
    begin
      ...
      if Assigned(FParent) and (FParent.DialingNumber <> '') then
      begin
      ...
      // callback
      if Assigned(FParent) and (TempIndex > -1) and (TempIndex < CallCount) then
  ...
  begin // update call state as active
    ...
    begin
      ...
      // update call state
      FCalls.Items[TempIndex].UpdateCallState(TCallState.csDisconnected);
      // callback
    ...
    begin // not found call number (accepted last ringing number)
      ...
      begin
        // add calling number
        LastRingIndex := AddCall(TCallState.csDialing, FParent.DialingNumber);
      end
      else
      begin
      ...
      // callback
      if Assigned(FParent) and (LastRingIndex > -1) and (LastRingIndex < CallCount) then
  ...
  begin // remove call from the active list
    ...
    begin // not found call number
      ...
      begin
        ...
        // if is a valid call index
        if (LastCallIndex > -1) and (LastCallIndex < CallCount) then
        begin
...
var
  ...
  Intent: JIntent;
begin
  FDialingNumber := APhoneNumber;
  PhoneUrl := StrToJURI(Format('tel:%s', [APhoneNumber]));
  Intent := TJIntent.JavaClass.init(TJIntent.JavaClass.ACTION_CALL, PhoneUrl);
  SharedActivityContext.startActivity(Intent);
  Result := Assigned(FTelephonyManager);
end;
...
var
  I: Integer;
begin
  if Assigned(FPhoneStateListener.Calls) then
  begin
