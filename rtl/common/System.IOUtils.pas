...
begin
  ...
  if getcwd(MarshaledAString(LBuffer), SizeOf(LBuffer)) <> nil then
...
{$IFDEF POSIX}
  ...
  if __rename(M.AsAnsi(SourceFileName, CP_UTF8).ToPointer, M.AsAnsi(DestFileName, CP_UTF8).ToPointer) = EXDEV then
{$ENDIF}
  begin
...
//  LTempPath := TEncoding.UTF8.GetBytes(string(tmpnam(nil)));
  LTempPath := TEncoding.UTF8.GetBytes(string(tempnam(MarshaledAString(M.AsUTF8(GetTempPath).ToPointer),nil)));
...
var
  CEnvVars: TArray<TBytes>;
  LTempPathVar: TBytes;
  I: Integer;
begin
  SetLength(CEnvVars, 3);
  CEnvVars[0] := TEncoding.Default.GetBytes('TMPDIR'#0); // Do not localize
  CEnvVars[1] := TEncoding.Default.GetBytes('TMP'#0);
  CEnvVars[2] := TEncoding.Default.GetBytes('TEMP'#0);
  { Lookup env variables, in order: TMPDIR, TMP, TEMP }
  for I := Low(CEnvVars) to High(CEnvVars) do
  begin
    LTempPathVar := TEncoding.UTF8.GetBytes(string(getenv(MarshaledAString(CEnvVars[I]))));

    if (Length(LTempPathVar) > 0) and (LTempPathVar[0] <> 0) then
    begin
  ...
  if LTempPathVar <> nil then
    Result := TEncoding.UTF8.GetString(LTempPathVar)
  else
    Result := CTmpDir;
end;
