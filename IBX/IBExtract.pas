...
uses System.SysUtils, IBUtils, IBXConst, System.Character;
...
var
  ...
  SubType : Short;
  IntChar : Short;
  qryTables, qryPrecision, qryConstraints, qryRelConstraints, qryEncryption : TIBSQL;
  ...
  try
    ...
    begin
      ...
      if DatabaseInfo.FullODS > 11.1 then
        if qryTables.FieldByName('RDB$RELATION_TYPE').AsString.Contains('TEMPORARY') then
    ...
    {  Check first for computed fields, then domains.
      ...
      begin
        ...
        if not ((qryTables.FieldByName('RDB$FIELD_NAME1').AsString.Trim.StartsWith('RDB$')) and {do not localize}
          qryTables.FieldByName('RDB$FIELD_NAME1').AsTrimString[Low(String) + 4].IsNumber) and {do not localize}
      ...
      if (DatabaseInfo.FullODS >= 13.0) then
      begin
        qryEncryption.Params.ByName('ENCRYPTION_ID').AsInteger := qryTables.FieldByName('RDB$ENCRYPTION_ID').AsInteger; {do not localize}
        qryEncryption.ExecQuery;
        if not qryEncryption.Eof then
        begin
          Column := Column + ' ENCRYPT WITH ' + {do not localize}
              QuoteIdentifier(FDatabase.SQLDialect,
                      qryEncryption.FieldByName('RDB$ENCRYPTION_NAME').AsTrimString); {do not localize}
          if not qryTables.FieldByName('RDB$DECRYPT_DEFAULT_SOURCE').IsNull then {do not localize}
            Column := Column + ' ' + qryTables.FieldByName('RDB$DECRYPT_DEFAULT_SOURCE').AsTrimString; {do not localize}
        end;
        qryEncryption.Close;
      end;
   ...
   try
     ...
     begin
       if ((qryTables.FieldByName('RDB$FLAGS').AsInteger <> 1) and  {do not localize}
           (not Flag)) then
         continue;
       if flag or (not qryTables.FieldByName('RDB$SECURITY_CLASS').AsTrimString.StartsWith('SQL$')) then {do not localize}
...
var
  ...
  begin
    _DatabaseInfoCommand := DatabaseInfoCommand;
  ...
  try
    ...
    begin
      ...
      FMetaData.Add(Format('CREATE GENERATOR %s%s',   {do not localize}
        [QuoteIdentifier(FDatabase.SQLDialect, GenName), Term]));
      qryGenerator.Next;
    end;
  finally
    qryGenerator.Free;
...
begin
  ...
  try
    ...
    qryOwner.ExecQuery;
    FMetaData.Add('/* CONNECT ' + QuotedStr(FDatabase.DatabaseName) + {do not localize}
...
var
  ...
  Buffer : String;
begin
  if (FDatabaseInfo.FullODS >= 13.0) then
  begin
