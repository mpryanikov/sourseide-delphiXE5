...
{$I FireDAC.inc}
{$IFDEF WIN32}
  {$HPPEMIT '#pragma link "FireDAC.DApt.obj"'}
{$ELSE}
  {$HPPEMIT '#pragma link "FireDAC.DApt.o"'}
{$ENDIF}
...
{$ENDIF}
    ...
    procedure AttachDatSTable(ATable: TFDDatSTable);
    function MatchRecordSet(const ATable: Variant; ATabNameKind: TFDPhysNameKind): Boolean;
  ...
  private
    ...
    procedure Clear;
    function Find(const ATable: Variant; ANameKind: TFDPhysNameKind): Integer;
  ...
  protected
    ...
    function Lookup(const ATable: Variant; ANameKind: TFDPhysNameKind): IFDDAptTableAdapter;
    ...
    procedure Remove(const ATable: Variant; ANameKind: TFDPhysNameKind); overload;
...
{$ENDIF}
  ...
  FSilentMode := (oCmd <> nil) and oCmd.Options.ResourceOptions.ActualSilentMode;
end;
...
begin
  if FAdapter <> nil then begin
    ...
    Result := FAdapters.Lookup(ATable.Name, nkDatS)
  else
    Result := nil;
end;
...
{-------------------------------------------------------------------------------}
function TFDDAptTableAdapterMappingHandler.MapRecordSet(const ATable: Variant;
  ATabNameKind: TFDPhysNameKind; var ASourceID: Integer; var ASourceName,
  ADatSName, AUpdateName: String; var ADatSTable: TFDDatSTable): TFDPhysMappingResult;
begin
...
{-------------------------------------------------------------------------------}
function TFDDAptTableAdapterMappingHandler.MapRecordSetColumn(
  const ATable: Variant; ATabNameKind: TFDPhysNameKind; const AColumn: Variant;
  AColNameKind: TFDPhysNameKind; var ASourceID: Integer; var ASourceName,
  ADatSName, AUpdateName: String; var ADatSColumn: TFDDatSColumn): TFDPhysMappingResult;
begin
...
{-------------------------------------------------------------------------------}
procedure TFDDAptTableAdapter.AttachDatSTable(ATable: TFDDatSTable);
begin
...
function TFDDAptTableAdapter.MatchRecordSet(const ATable: Variant;
  ATabNameKind: TFDPhysNameKind): Boolean;
var
  oTab: TFDDatSTable;
begin
  case ATabNameKind of
  nkID:
    Result := GetSourceRecordSetID = ATable;
  nkSource:
    Result := {$IFDEF FireDAC_NOLOCALE_META} CompareText {$ELSE} AnsiCompareText {$ENDIF}
      (ATable, GetSourceRecordSetName) = 0;
  nkDatS:
    Result := {$IFDEF FireDAC_NOLOCALE_META} CompareText {$ELSE} AnsiCompareText {$ENDIF}
      (ATable, GetDatSTableName) = 0;
  nkObj:
    begin
...
{-------------------------------------------------------------------------------}
function TFDDAptTableAdapter.MapRecordSet(const ATable: Variant;
  ATabNameKind: TFDPhysNameKind; var ASourceID: Integer; var ASourceName,
  ADatSName, AUpdateName: String; var ADatSTable: TFDDatSTable): TFDPhysMappingResult;
begin
...
{-------------------------------------------------------------------------------}
function TFDDAptTableAdapter.MapRecordSetColumn(const ATable: Variant;
  ATabNameKind: TFDPhysNameKind; const AColumn: Variant; AColNameKind: TFDPhysNameKind;
  var ASourceID: Integer; var ASourceName, ADatSName, AUpdateName: String;
  var ADatSColumn: TFDDatSColumn): TFDPhysMappingResult;
var
  ...
  for i := 0 to AParams.Count - 1 do begin
    ...
    if oPar.ParamType in [ptUnknown, ptInput, ptInputOutput] then begin
      ...
      if oCol <> nil then begin
        ...
        oFmtOpts.ColumnDef2FieldDef(oCol.DataType, oCol.Size,
          oCol.Precision, oCol.Scale, oCol.Attributes, eFldType,
...
{$ENDIF}
  ...
  oConn.CreateCommandGenerator(oCmdGen, nil);
...
begin
  ...
  lAttached := (Result <> nil) and (GetDatSTable = nil);
  if lAttached then
...
{-------------------------------------------------------------------------------}
function TFDDAptTableAdapters.Find(const ATable: Variant; ANameKind: TFDPhysNameKind): Integer;
var
  i: Integer;
begin
  Result := -1;
  for i := 0 to FItems.Count - 1 do
    if TFDDAptTableAdapter(FItems.Items[i]).MatchRecordSet(ATable, ANameKind) then begin
      Result := i;
      Exit;
    end;
end;
...
begin
  for i := 0 to FItems.Count - 1 do begin
    ...
    // Here was call of GetDatSTable, but it leads to TFDDatSTable leak
    // in case, when (FDatSTable = nil) and (GetDatSTableName <> '').
    // Because table will be removed from oTab.Manager.Tables, and
    // TFDDAptTableAdapter ReleaseDatSTable releases only FDatSTable.
    oTab := oAdapt.FDatSTable;
...
{-------------------------------------------------------------------------------}
procedure TFDDAptTableAdapters.Remove(const ATable: Variant; ANameKind: TFDPhysNameKind);
var
  i: Integer;
begin
...
{-------------------------------------------------------------------------------}
function TFDDAptTableAdapters.Lookup(const ATable: Variant;
  ANameKind: TFDPhysNameKind): IFDDAptTableAdapter;
var
  i: Integer;
begin
  i := Find(ATable, ANameKind);
...
{-------------------------------------------------------------------------------}
function TFDDAptSchemaAdapter.MapRecordSet(const ATable: Variant;
  ATabNameKind: TFDPhysNameKind; var ASourceID: Integer; var ASourceName,
  ADatSName, AUpdateName: String; var ADatSTable: TFDDatSTable): TFDPhysMappingResult;
var
...
{-------------------------------------------------------------------------------}
function TFDDAptSchemaAdapter.MapRecordSetColumn(const ATable: Variant;
  ATabNameKind: TFDPhysNameKind; const AColumn: Variant;
  AColNameKind: TFDPhysNameKind; var ASourceID: Integer; var ASourceName,
  ADatSName, AUpdateName: String; var ADatSColumn: TFDDatSColumn): TFDPhysMappingResult;
var
  oTabAdapt: IFDDAptTableAdapter;
