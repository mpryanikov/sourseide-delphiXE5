...
uses
  System.SysUtils,
  FireDAC.Stan.Consts, FireDAC.Stan.Intf, FireDAC.Stan.Storage;
...
var
  ...
  begin
    ...
    if i1 <> 0 then begin
      i2 := FDPosEx('''', ADiagMessage, i1 + Length(C_SyntaxErrMsg1));
      if i2 <> 0 then begin
  ...
  begin
    ...
    if i1 <> 0 then begin
      i2 := FDPosEx('''', ADiagMessage, i1 + Length(C_SyntaxErrMsg2));
      if i2 <> 0 then begin
...
begin
  if AToolLib <> '' then begin
    ...
    for i := 12 downto 5 do
      aTools[12 - i] := Format(C_DBTool, [i]);
    aTools[8] := Format(C_DBTool, [16]);
  end;
...
{-------------------------------------------------------------------------------}
function ASANativeExceptionLoad(AStorage: TFDStorage): TObject;
begin
