...
type
  ...
  TDSRestCallbackLoop = class
  strict private
    [Weak]FClientChannel: TDSRestClientChannel;
...
begin
  ...
  GetCommand(
    ...
    begin
      ...
      begin
        AStream.Position := 0;
        LString := Connection.PeerProcs.ReadStringAsCharset(AStream, AResponseCharSet);
...
var
  Sp, Rp: PChar;
begin
  SetLength(Result, AStr.Length * 3);
  Sp := PChar(AStr);
  Rp := PChar(Result);
  while Sp^ <> #0 do
  begin
...
function EncodeURIComponent(const AStr: string; const APeerProcs: IIPPeerProcs): string; overload;
const
  EncodeURIComponentCharSet = ['@', '$', '&', '=', ':', '/', ',', ';', '?', '+'];
begin
  Result := EncodeCharSet(APeerProcs.ParamsEncode(AStr), EncodeURIComponentCharSet);
end;

function encodeURIComponent(const AValue: TDBXParameter; const APeerProcs: IIPPeerProcs): string; overload;
var
  LStringValue: string;
begin
  ...
  Result := encodeURIComponent(LStringValue, APeerProcs);
end;
...
begin
  LPeerProcs := ACommand.Connection.PeerProcs;
  ...
  LUrl := LProtocol + '://' +
    encodeURIComponent(LHost, LPeerProcs) + LPortString + LpathPrefix + '/' + LContext + LRESTContext + LMethodName  + '/';
...
begin
  LURL := AURL;
  LPeerProcs := IPProcs(AHTTP.IPImplementationID);
  ...
  try
    ...
    begin
      for LParameter in LParametersToSend do
        LUrl := LUrl + encodeURIComponent(LParameter, LPeerProcs) + '/';
    end
    else
    begin
...
begin
  ...
  LUrl := LProtocol + '://' +
    encodeURIComponent(LHost, LPeerProcs) + LPortString + LpathPrefix + '/' + LContext + LCacheContext;
  LSessionID := ACommand.Connection.SessionID;
...
begin
  if (not AContext.IsEmpty) and
     (AContext.Chars[AContext.Length - 1] <> '/') then
      AContext := AContext + '/'
end;
...
function TDSCustomRestConnection.PeerProcs: IIPPeerProcs;
begin
