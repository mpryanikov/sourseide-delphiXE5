...
{$I FireDAC.inc}
{$IFDEF WIN32}
  {$HPPEMIT '#pragma link "FireDAC.Phys.obj"'}
{$ELSE}
  {$HPPEMIT '#pragma link "FireDAC.Phys.o"'}
{$ENDIF}
...
type
  ...
  (*$HPPEMIT '    __interface IFDPhysTransaction2Obj;'*)
  ...
  public
    ...
    procedure RegisterDriverClass(ADriverClass: TFDPhysDriverClass);
    procedure UnRegisterDriverClass(ADriverClass: TFDPhysDriverClass);
  end;
  ...
  TFDPhysConnectionHost = class(TInterfacedObject,
    ...
    IFDStanObjectHost)
  private
    {$IFDEF AUTOREFCOUNT} [Weak] {$ENDIF} FDriverObj: TFDPhysDriver;
...
{$ENDIF}
  ...
  private
    ...
    {$IFDEF AUTOREFCOUNT} [Weak] {$ENDIF} FTransactionObj: TFDPhysTransaction;
    FDontCallStateHandler: Boolean;
...
{$ENDIF}
  ...
  IFDPhysTransaction2Obj = interface(IUnknown)
    ['{3E9B315B-F456-4175-A864-B2573C4A2170}']
    function GetObj: TFDPhysTransaction;
    property Obj: TFDPhysTransaction read GetObj;
  end;
  ...
  TFDPhysTransaction = class(TInterfacedObject,
    ...
    IFDPhysTransaction, IFDPhysTransaction2Obj)
  private
    FConnection: IFDPhysConnection;
    {$IFDEF AUTOREFCOUNT} [Weak] {$ENDIF} FConnectionObj: TFDPhysConnection;
    FOptions,
...
{$ENDIF}
  protected
    ...
    procedure InternalAllocHandle; virtual;
    procedure InternalReleaseHandle; virtual;
    procedure InternalTimeout; virtual;
...
{$ENDIF}
    ...
    procedure InternalExecute(ATimes: LongInt; AOffset: LongInt; var ACount: TFDCounter); virtual; abstract;
...
begin
  ...
  if IsEqualGUID(IID, IFDPhysManagerMetadata) then
    UpdateDriverIDs;
end;
...
{ ----------------------------------------------------------------------------- }
procedure TFDPhysManager.RegisterDriverClass(ADriverClass: TFDPhysDriverClass);
begin
  FDriverClasses.Add(ADriverClass);
end;

{ ----------------------------------------------------------------------------- }
procedure TFDPhysManager.UnRegisterDriverClass(ADriverClass: TFDPhysDriverClass);
begin
  FDriverClasses.Remove(ADriverClass);
end;
...
var
  i: Integer;
  {$IFDEF AUTOREFCOUNT} [Weak] {$ENDIF} oDrv: TFDPhysDriver;
begin
  FLock.Enter;
  try
...
begin
  ...
  try
    ...
    dwStartTime := FDGetTickCount();
    while not lTimeOut and (FDriverList.Count > 0) do begin
      lTimeOut := FDTimeout(dwStartTime, C_FD_PhysManagerShutdownTimeout);
      CleanupDrivers;
      Sleep(1);
    end;
...
begin
  if ABaseDriverID <> '' then
    ...
    sDrvID := ADriverID;
  if (CompareText(sDrvID, S_FD_DBXId) = 0) or
     (CompareText(sDrvID, S_FD_TDBXId) = 0) or
     ...
     (CompareText(sDrvID, S_FD_DataSnapId) = 0) then
  ...
  try
    ...
    if (Result = nil) and ARequired then begin
      ...
      for i := 0 to FDriverClasses.Count - 1 do
        if CompareText(TFDPhysDriverClass(FDriverClasses[i]).GetBaseDriverID, sBaseDriverID) = 0 then begin
          oDrvClass := TFDPhysDriverClass(FDriverClasses[i]);
          Break;
        end;
...
begin
  ...
  try
    oDrv := DriverByID(AConnectionDef.DriverID, CFOMs[AIntfRequired], True);
    if oDrv <> nil then
      try
...
begin
  ASSERT(AConnectionDef <> nil);
  oDrv := DriverByID(AConnectionDef.DriverID, fomIfActive, False);
  if oDrv <> nil then
    try
  ...
  try
    ...
    for i := FDriverIDs.Count - 1 downto 0 do begin
      ...
      if sBaseID = '' then
        sBaseID := sID;
      lHasImpl := False;
      for j := 0 to FDriverClasses.Count - 1 do
        if CompareText(TFDPhysDriverClass(FDriverClasses[j]).GetBaseDriverID, sBaseID) = 0 then begin
          lHasImpl := True;
          Break;
        end;
      if not lHasImpl then
        FDriverIDs.Delete(i)
      else
...
begin
  Result := FDNameFromIndex(FDriverIDs, AIndex);
end;

{ ----------------------------------------------------------------------------- }
function TFDPhysManager.GetBaseDriverID(AIndex: Integer): String;
begin
  Result := FDValueFromIndex(FDriverIDs, AIndex);
end;
...
begin
  ...
  for i := Low(TFDRDBMSKind) to High(TFDRDBMSKind) do
    if CompareText(sId, C_FD_PhysRDBMSIds[i]) = 0 then begin
      Result := i;
      Break;
    end;
end;
...
{ ----------------------------------------------------------------------------- }
function TFDPhysDriver.GetConnParamCount(AKeys: TStrings): Integer;
begin
  Result := 6;
  if (AKeys <> nil) and
     (CompareText(AKeys.Values[S_FD_ConnParam_Common_Pooled], S_FD_True) = 0) then
    Result := Result + 3;
end;

{ ----------------------------------------------------------------------------- }
procedure TFDPhysDriver.GetConnParams(AKeys: TStrings; Index: Integer;
...
begin
  ...
  0:
    begin
      AName := S_FD_ConnParam_Common_DriverID;
      AType := '';
      for i := 0 to FManager.FDriverList.Count - 1 do begin
        if AType <> '' then
          AType := AType + ';';
        AType := AType + TFDPhysDriver(FManager.FDriverList[i]).DriverID;
      end;
      ADefVal := GetDriverID;
      ACaption := AName;
    end;
  1:
    begin
      AName := S_FD_ConnParam_Common_Pooled;
      AType := '@L';
      ADefVal := S_FD_False;
      ACaption := AName;
    end;
  2:
    begin
      AName := S_FD_ConnParam_Common_Database;
      AType := '@S';
      ACaption := AName;
    end;
  3:
    begin
      AName := S_FD_ConnParam_Common_UserName;
      AType := '@S';
      ALoginIndex := 0;
      ACaption := S_FD_ConnParam_Common_BDEStyleUserName;
    end;
  4:
    begin
      AName := S_FD_ConnParam_Common_Password;
      AType := '@P';
      ALoginIndex := 1;
      ACaption := AName;
    end;
  5:
    begin
      AName := S_FD_ConnParam_Common_MonitorBy;
      AType := S_FD_MoniFlatFile + ';' + S_FD_MoniRemote + ';' + S_FD_MoniCustom;
      ADefVal := '';
      ACaption := AName;
    end;
  else
    if (AKeys <> nil) and
      ...
      6:
        begin
          AName := S_FD_PoolParam_CleanupTimeout;
          ADefVal := IntToStr(C_FD_PoolCleanupTimeout);
        end;
      7:
        begin
          AName := S_FD_PoolParam_ExpireTimeout;
          ADefVal := IntToStr(C_FD_PoolExpireTimeout);
        end;
      8:
        begin
          AName := S_FD_PoolParam_MaximumItems;
          ADefVal := IntToStr(C_FD_PoolMaximumItems);
        end;
      end;
      ACaption := AName;
    end;
...
{$IFDEF FireDAC_MONITOR}
  ...
  if FConnectionDef.MonitorBy <> '' then
    AddMoniClient;
{$ENDIF}
  if FConnectionDef.Pooled and not FDIsDesignTime then begin
...
begin
  if AtomicDecrement(FConnectionCount) = 0 then
    if FConnectionDef.Style <> atTemporary then
      FConnectionDef.ToggleUpdates(FPassCode, True, False)
    else if not FConnectionDef.Pooled then
      FDFree(Self);
end;
...
begin
  FMonitor := nil;
  s := Trim(FConnectionDef.MonitorBy);
  if s = '' then
    Exit;
  if FDInSet(s[Length(s)], ['-', '+']) then begin
    FInitialTracing := s[Length(s)] = '+';
    s := Copy(s, 1, Length(s) - 1);
  end;
  if CompareText(s, S_FD_MoniRemote) = 0 then
    rGUID := IFDMoniRemoteClient
  else if CompareText(s, S_FD_MoniFlatFile) = 0 then
    rGUID := IFDMoniFlatFileClient
  else if CompareText(s, S_FD_MoniCustom) = 0 then
    rGUID := IFDMoniCustomClient
  else
    Exit;
...
{$ENDIF}
    ...
    SetOptions(nil);
    GetConnectionDef.ReadOptions(FOptions.FormatOptions, FOptions.UpdateOptions,
...
procedure TFDPhysConnection.GetParentOptions(out AOpts: IFDStanOptions);
begin
...
function GetTxObj(const AIntf: IFDPhysTransaction): TFDPhysTransaction;
var
  oIntf: IFDPhysTransaction2Obj;
begin
  Supports(AIntf, IFDPhysTransaction2Obj, oIntf);
  ASSERT((oIntf <> nil) and (oIntf.Obj <> nil));
  Result := oIntf.Obj;
end;
...
{$IFDEF FireDAC_MONITOR}
  procedure Trace1;
  begin
    Trace(ekLiveCycle, esProgress, 'CreateTransaction', ['ConnectionDef', GetConnectionDef.Name]);
  end;
{$ENDIF}
...
{$IFDEF FireDAC_MONITOR}
  procedure Trace1;
  begin
    Trace(ekLiveCycle, esProgress, 'CreateCommand', ['ConnectionDef', GetConnectionDef.Name]);
  end;
{$ENDIF}
...
{$IFDEF FireDAC_MONITOR}
  procedure Trace1;
  begin
    Trace(ekLiveCycle, esProgress, 'CreateMetaInfoCommand', ['ConnectionDef', GetConnectionDef.Name]);
  end;
{$ENDIF}
...
{$IFDEF FireDAC_MONITOR}
  procedure Trace1;
  begin
    Trace(ekLiveCycle, esProgress, 'BeforeReuse', ['ConnectionDef', GetConnectionDef.Name]);
  end;
{$ENDIF}
...
{$IFDEF FireDAC_MONITOR}
  procedure Trace1;
  begin
    Trace(ekLiveCycle, esProgress, 'AfterReuse', ['ConnectionDef', GetConnectionDef.Name]);
  end;
{$ENDIF}
...
begin
  oConnDef := GetConnectionDef;
  if oConnDef.Pooled and (GetState = csConnected) then begin
...
{$ENDIF}
    ...
    try
      ...
      try
{$IFDEF FireDAC_MONITOR}
        SetTracing((oConnDef.MonitorBy <> '') and FConnHost.FInitialTracing);
{$ENDIF}
        if GetLoginPrompt and not oConnDef.Pooled and not FDGUIxSilent() and
...
begin
  FDriver.Employ;
  try
    InternalChangePassword(GetConnectionDef.UserName, GetConnectionDef.Password,
      ANewPassword);
  finally
    FDriver.Vacate;
  end;
...
begin
  FDriverObj.FManager.CreateConnection(GetConnectionDef, Result);
...
begin
  ...
  if iVer < 0 then
    iVer := -iVer;
  if iVer > 1 then
    AMessages.Add(Format('Warning: The client [%s] and server [%s] major versions difference > 1.',
...
begin
  Result := GetConnectionDef.ExpandedDatabase;
end;
...
begin
  Result := GetConnectionDef.AsString[S_FD_ConnParam_Common_UserName];
end;
...
{$IFDEF FireDAC_MONITOR}
  ...
  procedure Trace5;
  begin
    Trace(ekConnConnect, esProgress, 'HandleConnectionLosted', ['ConnectionDef', sDefName]);
  end;
{$ENDIF}

  procedure ConnectionRecover;
  begin
...
{$ENDIF}
  ...
  procedure ConnectionLosted;
  begin
    if Assigned(GetRecoveryHandler()) then begin
{$IFDEF FireDAC_MONITOR}
      if GetTracing then Trace4;
{$ENDIF}
      GetRecoveryHandler().HandleConnectionLosted();
    end;
  end;
...
begin
  ...
  lRaised := False;
  try
    if (oExc is EFDDBEngineException) and (EFDDBEngineException(oExc).Kind = ekServerGone) and
       (AOnLogin or (GetState = csConnected)) then begin
{$IFDEF FireDAC_MONITOR}
      sDefName := GetConnectionDef.Name;
      if GetTracing then Trace1;
      try
{$ENDIF}
        Lock;
        try
          ...
          try
            for i := 0 to FCommandList.Count - 1 do
              if TFDPhysCommand(FCommandList[i]).GetState <> csInactive then
                oCmdList.Add(FCommandList[i]);
            for i := 0 to FEventList.Count - 1 do
            ...
            FDontCallStateHandler := True;
            try
              ForceDisconnect;
            finally
              FDontCallStateHandler := False;
            end;
            ...
            if GetState = csConnected then begin
              for i := 0 to oCmdList.Count - 1 do
...
{$IFDEF FireDAC_MONITOR}
  procedure Trace1;
  begin
    Trace(ekConnConnect, esStart, 'Ping', ['ConnectionDef', GetConnectionDef.Name]);
  end;

  procedure Trace2;
  begin
    Trace(ekConnConnect, esEnd, 'Ping', ['ConnectionDef', GetConnectionDef.Name]);
  end;
{$ENDIF}
...
begin
  if not (GetState in [tsCommiting, tsRollingback]) and
     (FConnectionObj.FState <> csRecovering) and
...
function TFDPhysTransaction.GetObj: TFDPhysTransaction;
begin
  Result := Self;
end;
...
var
  ...
  oCmd: TFDPhysCommand;
begin
  if not FConnectionObj.FDontCallStateHandler then
...
begin
  if not (GetState in [tsCommiting, tsRollingback]) then
    while GetActive do
      try
...
begin
  ...
  if not AForce then
    for i := FCommandList.Count - 1 downto 0 do begin
      ...
      csFetching,
      csAborting:
        if ASuccess or (oCmd <> ACommandObj) then begin
          lStop := False;
          Break;
        end;
      end;
...
{$IFDEF FireDAC_MONITOR}
  ...
  begin
    ...
    FConnectionObj.Trace(ekConnTransact, esStart, 'StartTransaction',
      ['ConnectionDef', FConnectionObj.GetConnectionDef.Name]);
  end;
...
begin
  if FLockAutoStopCount = 0 then begin
    ...
    oTx.Changed := oTx.Changed - [xoAutoCommit];
  end;
...
begin
  if Active and (FAlerter.GetOptions.Timeout > -1) then
    if FAlerter.GetOptions.Synchronize then
      TFDThread.Synchronize(FAlerter.InternalTimeout)
    else
...
{-------------------------------------------------------------------------------}
procedure TFDPhysEventAlerter.InternalTimeout;
var
  lContinue: Boolean;
begin
  if (GetHandler <> nil) and (GetOptions.Timeout > 0) then begin
    lContinue := True;
    GetHandler.HandleTimeout(lContinue);
...
{-------------------------------------------------------------------------------}
procedure TFDPhysCommand.GetParentOptions(out AOpts: IFDStanOptions);
begin
...
begin
  ...
  try
    ...
    FExecutor.Setup(AOperation, eMode, oRes.CmdExecTimeout, AHandler,
      oRes.ActualSilentMode);
  except
    on E: Exception do begin
...
var
  oConnMeta: IFDPhysConnectionMetadata;
begin
  if (FThreadID <> 0) and (FThreadID = GetCurrentThreadId()) then begin
  ...
  if AWait then begin
    while GetState in [csExecuting, csFetching, csAborting] do
      Sleep(1);
...
begin
  if GetState = csInactive then
    Exit;
  if (GetStateHandler <> nil) and not FConnectionObj.FDontCallStateHandler then begin
    CheckAsyncProgress;
    _AddRef;
    try
...
begin
  ...
  if GetMetaInfoKind() <> mkResultSetFields then begin
    FConnection.CreateMetadata(oConnMeta);
...
var
  ...
  begin
    CreateCommandGenerator(oGen);
    FDbCommandText := oGen.GenerateLimitSelect(FDbCommandText, ASkip, ARows,
...
var
  ...
  pMappedTabId: Variant;
  eMappedTabKind: TFDPhysNameKind;
  ...
  function EnsureTabExistance(ADatSManager: TFDDatSManager; var ASourceID: Integer;
    var ASourceName, FDatSName: String; var ATabCreated: Boolean): TFDDatSTable;
  var
    i: Integer;
  ...
  begin
    ...
    if GetMappingHandler <> nil then begin
      if ASourceID >= 0 then
        eMapResult := GetMappingHandler.MapRecordSet(ASourceID, nkID,
          ASourceID, ASourceName, FDatSName, sTmp, oTab);
      if eMapResult = mrMapped then begin
        pMappedTabId := ASourceID;
        eMappedTabKind := nkId;
      end
      else begin
        eMapResult := GetMappingHandler.MapRecordSet(ASourceName, nkSource,
          ASourceID, ASourceName, FDatSName, sTmp, oTab);
        if eMapResult = mrMapped then begin
          pMappedTabId := ASourceName;
  ...
  function EnsureColExistance(ATable: TFDDatSTable; var ASourceID: Integer;
    var ASourceName, FDatSName: String; var AColCreated: Boolean): TFDDatSColumn;
  var
    i: Integer;
    eMapResult: TFDPhysMappingResult;
    sTmp: String;
    ...
    if (GetMappingHandler <> nil) and (eMappedTabKind <> nkDefault) then begin
      if ASourceID >= 0 then
        eMapResult := GetMappingHandler.MapRecordSetColumn(pMappedTabId, eMappedTabKind,
          ASourceID, nkID, ASourceID, ASourceName, FDatSName, sTmp, oCol);
      if eMapResult <> mrMapped then
        eMapResult := GetMappingHandler.MapRecordSetColumn(pMappedTabId, eMappedTabKind,
          ASourceName, nkSource, ASourceID, ASourceName, FDatSName, sTmp, oCol);
...
begin
  ...
  oOriginCols := TFDStringList.Create;
  oOriginCols.Sorted := True;
  oOriginCols.CaseSensitive := False;
  try
    try
      if lDefPK then
      ...
      if ATable <> nil then begin
        ...
        while InternalColInfoGet(rColInfo) do begin
          if rColInfo.FSourceName = '' then begin
        ...
        for i := 0 to ATable.Columns.Count - 1 do begin
          ...
          if not lExtAttrs then
            if not lOriginProvided then
              ...
              oCol.Attributes := oCol.Attributes + [caExpr, caReadOnly]
...
type
  ...
  protected
    ...
    procedure Execute; override;
  public
    constructor Create(ACmd: TFDPhysCommand; ATimes: Integer; AOffset: Integer);
  end;

{-------------------------------------------------------------------------------}
constructor TFDPhysCommandAsyncExecute.Create(ACmd: TFDPhysCommand;
  ATimes: Integer; AOffset: Integer);
begin
...
{-------------------------------------------------------------------------------}
procedure TFDPhysCommand.CheckExact(AUseRecsMax: Boolean; ATimes, AOffset: LongInt;
  AValue: TFDCounter; AFetch: Boolean);
...
{-------------------------------------------------------------------------------}
procedure TFDPhysCommand.ExecuteBase(ATimes, AOffset: LongInt);
var
...
{$ENDIF}
  ...
  begin
    ...
    lSavepoint := (FTransactionObj <> nil) and FTransactionObj.GetActive and
      oConnMeta.TxAtomic and (oConnMeta.TxNested or oConnMeta.TxSavepoints);
    oConnMeta := nil;
  end;
  ...
  function Process_HandleSystemFailure(ATimes, AOffset: LongInt; out ACount: TFDCounter;
  ...
  var
    iCount: TFDCounter;
    iLastTimes, iLastOffset, iRetryLevel: LongInt;
    iCurTimes, iCurOffset, iArraySize: LongInt;
    oErr: Exception;
  begin
    iLastTimes := 0;
    iLastOffset := 0;
    ...
    repeat
      try
        ...
        on E: EFDDBEngineException do begin
          ...
          if AUndoAll and (iArraySize > 1) then begin
            iArraySize := 1;
            iCurTimes := iCurOffset + 1;
            FErrorAction := eaRetry;
            Continue;
...
{$ENDIF}
    ...
    if eMode in [fmAll, fmExactRecsMax] then
...
begin
  Result := (ADataType <> dtUnknown) and not (
