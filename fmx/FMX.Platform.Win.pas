...
uses
  ...
  FMX.Forms, FMX.Platform, FMX.Types, FMX.Graphics;
...
function DragHBmpFromBitmap(const Bitmap: FMX.Graphics.TBitmap; const RemoveBlackPixel: Boolean = True): THandle;
...
{ TPlatformWin }
  ...
  TPlatformWin = class(TInterfacedObject, IFMXApplicationService, IFMXSystemFontService,
    ...
    IFMXSystemInformationService, IFMXLoggingService, IFMXFullScreenWindowService, IFMXListingService)
  private
    FTitle : string;
...
function FormToHWND(Form: TCommonCustomForm): HWND;
begin
  if Assigned(Form) and (Form.Handle is TWinWindowHandle) then
...
begin
  ...
  LConfigs[0].dwBlock := CPan[not(TInteractiveGesture.igPan in LGestures)] or GC_PAN_WITH_GUTTER;
  ...
  LConfigs[3].dwWant := CTwoFingerTap[TInteractiveGesture.igTwoFingerTap in LGestures];
  LConfigs[3].dwBlock := CTwoFingerTap[not(TInteractiveGesture.igTwoFingerTap in LGestures)];

  // PressAnTap gesture
  LConfigs[4].dwID := GID_PRESSANDTAP;
  LConfigs[4].dwWant := CPressAndTap[TInteractiveGesture.igPressAndTap in LGestures];
  LConfigs[4].dwBlock := CPressAndTap[not(TInteractiveGesture.igPressAndTap in LGestures)];

  SetGestureConfig(FormToHWND(AForm), 0, Length(LConfigs),
...
var
  ...
  begin
    if Assigned(Screen) and Assigned(LForm) and (not WindowHandleToPlatform(LForm.Handle).FDisableDeactivate) then
      Screen.ClosePopupForms(LForm);
  end;
  ...
  begin
    ...
    if Assigned(Screen)then
      ...
      begin
        Screen.PopupForms[I].MouseWheel(KeysToShiftState(wParam),
  ...
  begin
    ...
    try
      case uMsg of
        ...
        WM_ACTIVATE:
          begin
            if not ((TFmxFormState.fsRecreating in LForm.FormState) or
                    (LForm.FormStyle = TFormStyle.fsPopup) or
        ...
        WM_MOUSEACTIVATE:
          begin
            ...
            begin
              if (LForm.FormStyle = TFormStyle.fsPopup) then
        ...
        WM_WINDOWPOSCHANGING:
          begin
            ...
            Result := DefWindowProc(hwnd, uMsg, wParam, lParam);
            if (TFmxFormState.fsInDesigner in LForm.FormState) and (TFmxFormState.fsWasNotShown in LForm.FormState) then
            begin
        ...
        WM_WINDOWPOSCHANGED:
          begin
            ...
            if Assigned(Application.MainForm) and (LForm = Application.MainForm)
              ...
              begin
                ...
                try
                  ...
                  case Placement.showCmd of
                    ...
                    begin
                      if not (TFMXFormState.fsWasNotShown in LForm.FormState) then
                        LForm.WindowState := TWindowState.wsNormal;
                    end;
                  end;
                finally
...
//          end;
  ...
  begin
    if Assigned(PlatformWin) and (hwnd = PlatformWin.FApplicationHWND) then
    begin
      case uMsg of
        WM_DESTROY:
        begin
...
var
  ...
  begin
    Result := 0;
    if Assigned(LForm.ParentForm) then
    begin
      if not Assigned(LForm.ParentForm.Handle) then
...
begin
  ...
  IsDesignerForm := TFMXFormState.fsInDesigner in LForm.FormState;
  ...
  begin
    ...
    begin
      ...
      if LForm.BorderStyle in [TFmxFormBorderStyle.bsToolWindow, TFmxFormBorderStyle.bsSizeToolWin] then
        ExStyle := ExStyle or WS_EX_TOOLWINDOW;
      if LForm.BorderStyle <> TFmxFormBorderStyle.bsNone then
      begin
    ...
    if (ParentWnd = 0) and ((TFmxFormState.fsModal in LForm.FormState) or (LForm.FormStyle = TFormStyle.fsPopup)) then
    begin
    ...
    if (ParentWnd = 0) and Assigned(Screen) and (Screen.FormCount > 0) and (LForm <> Screen.Forms[0]) then
      ParentWnd := FormToHWND(Screen.Forms[0]);
  end;
  Wnd := CreateWindowEx(ExStyle, WindowClass.lpszClassName, PChar(LForm.Caption), Style, Integer(CW_USEDEFAULT),
    Integer(CW_USEDEFAULT), Integer(CW_USEDEFAULT), Integer(CW_USEDEFAULT), ParentWnd, 0, hInstance, nil);
  if Wnd <> 0 then
  try
...
begin
  ...
  if Assigned(Screen) and Assigned(Screen.ActiveForm) then
    Handle := FormToHWND(Screen.ActiveForm)
  else
    Handle := 0;
...
begin
  ...
  try
    ...
    if Assigned(Application.MainForm) then
      SetWindowPos(FApplicationHWND, FormToHWND(Application.MainForm), Application.MainForm.Left, Application.MainForm.Top, Application.MainForm.Width, 0, SWP_SHOWWINDOW);
    DefWindowProc(FApplicationHWND, WM_SYSCOMMAND, SC_MINIMIZE, 0);
  finally
...
var
  WR: TRect;
begin
  Winapi.Windows.GetWindowRect(FormToHWND(AForm), WR);
...
begin
  ...
  if AForm.WindowState in [TWindowState.wsNormal, TWindowState.wsMaximized] then
    SetWindowPos(FormToHWND(AForm), 0, Round(ARect.Left), Round(ARect.Top),
...
var
  CR: TRect;
begin
  if GetClientRect(FormToHWND(AForm), CR) then
...
begin
  Wnd := FormToHWND(AForm);
  Winapi.Windows.GetClientRect(Wnd, CR);
...
begin
  SetWindowPos(FormToHWND(AForm), 0, 0, 0, 0, 0,
...
begin
  ...
  begin
    ...
    try
      if Assigned(OldActiveHandle) then
      begin
  ...
  if AForm.FormStyle in [TFormStyle.fsStayOnTop] then
  begin
...
begin
  ...
  begin
    if AForm.FormStyle = TFormStyle.fsPopup then
      Winapi.Windows.ShowWindow(FormToHWND(AForm), ShowCommands[AState] or SW_SHOWNOACTIVATE)
    else
      Winapi.Windows.ShowWindow(FormToHWND(AForm), ShowCommands[AState]);
  end;
end;
...
begin
  ...
  case Key of
    ...
    vkCamera: Name := 'Camera';
    vkBrowserBack: Name := 'BrowserBack';
    vkHardwareBack: Name := 'HardwareBack';
  else
...
function CreateWinBitmap(Bitmap: TBitmap): HBITMAP;
var
  W, H: Integer;
  ...
  Scale: Double;
  Src, Dst: TRectF;
  function ScaleOfBitmap(const SourceSize, DestinationSize: TSizeF): Double;
  var
...
begin
  ...
  begin
    ...
    H := ((GetSystemMetrics(SM_CYMENU) - 2) div 4) * 4;
    W := H;
    try
      ...
      try
        ...
        try
          if TOSVersion.Major < 6 then
            Color.A := 255;
          TmpBitmap.Clear(Color.Color);
          Scale := ScaleOfBitmap(TSizeF.Create(Bitmap.Width, Bitmap.Height),
                                 TSizeF.Create(W, H));
          Src := TRectF.Create(0, 0, Bitmap.Width, Bitmap.Height);
          Dst.Left := Round((W - Bitmap.Width * Scale) / 2);
          Dst.Top := Round((H - Bitmap.Height * Scale) / 2);
          Dst.Width := Round(Bitmap.Width * Scale);
          Dst.Height := Round(Bitmap.Height * Scale);
          TmpBitmap.Canvas.DrawBitmap(Bitmap, Src, Dst, 1, False);
        finally
          TmpBitmap.Canvas.EndScene;
        end;
        Result := DragHBmpFromBitmap(TmpBitmap, False);
      finally
      ...
      if Result <> 0 then
      begin
        if not DeleteObject(Result) then RaiseLastOSError;
        Result := 0;
      end;
    end;
  end;
...
var
    LPMENUITEMINFO: tagMENUITEMINFOW;
begin
  if Bitmap <> 0 then
  begin
    FillChar(LPMENUITEMINFO, SizeOf(LPMENUITEMINFO), 0);
    LPMENUITEMINFO.cbSize := SizeOf(LPMENUITEMINFO);
    LPMENUITEMINFO.fMask := MIIM_BITMAP;
    if not GetMenuItemInfo(ParentMenu, Menu, False, LPMENUITEMINFO) then
      RaiseLastOSError;
    LPMENUITEMINFO.hbmpItem := Bitmap;
    try
      if not SetMenuItemInfo(ParentMenu, Menu, False, LPMENUITEMINFO) then
        RaiseLastOSError;
    except
      LPMENUITEMINFO.hbmpItem := 0;
      SetMenuItemInfo(ParentMenu, Menu, False, LPMENUITEMINFO);
      raise;
    end;
  end;
end;

procedure RemoveBitmapFromMenu(ParentMenu, Menu: HMENU);
var
  LPMENUITEMINFO: tagMENUITEMINFOW;
begin
  FillChar(LPMENUITEMINFO, SizeOf(LPMENUITEMINFO), 0);
  ...
  if not GetMenuItemInfo(ParentMenu, Menu, False, LPMENUITEMINFO) then
    RaiseLastOSError;
  if (LPMENUITEMINFO.hbmpItem <> HBITMAP(-1)) and (LPMENUITEMINFO.hbmpItem > HBMMENU_POPUP_MINIMIZE) then
  begin
    if not DeleteObject(LPMENUITEMINFO.hbmpItem) then
      RaiseLastOSError;
  end;
end;
    ...
    begin
      ...
      begin
        ...
        try
          ...
          begin
            Bitmap := CreateWinBitmap(Item.Bitmap);
            if Bitmap <> 0 then
              try
          ...
          if Bitmap <> 0 then
            RemoveBitmapFromMenu(Parent, PopupMenu);
...
function TPlatformWin.GetSystemStyle(const Context: TFmxObject): TFmxObject;
begin
  Result := FMX.Controls.Win.GetSystemStyle(Context);
end;
...
begin
  ...
  if (Assigned (Screen) and Assigned(Screen.ActiveForm)) then
    OpenFile.hWndOwner := FormToHWND(Screen.ActiveForm)
  else
...
begin
  ...
  if (Assigned (Screen) and Assigned(Screen.ActiveForm)) then
    SaveFile.hWndOwner := FormToHWND(Screen.ActiveForm)
  else
...
begin
  ...
  if (Assigned (Screen) and Assigned(Screen.ActiveForm)) then
    PrintDlgRec.hWndOwner := FormToHWND(Screen.ActiveForm)
  else
...
begin
  ...
  if Assigned(Screen) and (Screen.FormCount > 0) then
    PageSetupDlgRec.hWndOwner := FormToHWND(Screen.Forms[0])
  else
...
begin
  ...
  if (Assigned(Screen) and Assigned(Screen.ActiveForm)) then
    PageSetupDlgRec.hWndOwner := FormToHWND(Screen.ActiveForm)
  else
    PageSetupDlgRec.hWndOwner := 0;
