...
begin
  ...
  begin
    Value := Trunc(Value);
    Value := Value + StrToDateTime('23:59:59');
...
begin
  if OverridePaintNC then InvalidateNC else Invalidate;
end;
...
begin
  ...
  if (seFont in Control.StyleElements) and (seClient in Control.StyleElements) and
     TCustomRichEdit(Control).PlainText and (Format.dwMask and CFM_COLOR = CFM_COLOR) then
  begin
