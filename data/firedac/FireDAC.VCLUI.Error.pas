...
{$I FireDAC.inc}
{$IFDEF WIN32}
  {$HPPEMIT '#pragma link "FireDAC.VCLUI.Error.obj"'}
{$ELSE}
  {$HPPEMIT '#pragma link "FireDAC.VCLUI.Error.o"'}
{$ENDIF}
...
{$ENDIF}
  System.SysUtils, System.Classes, Vcl.Graphics, Vcl.Forms, Vcl.Controls, Vcl.StdCtrls, Vcl.ExtCtrls, Vcl.ComCtrls,
    Vcl.Clipbrd,
...
begin
  ...
  for i := 0 to oParams.Count - 1 do begin
    ...
    oLI.Caption := FDNameFromIndex(oParams, i);
    oLI.SubItems.Add(FDValueFromIndex(oParams, i));
  end;
...
initialization
  oFact := TFDSingletonFactory.Create(TFDGUIxErrorDialogImpl,
    IFDGUIxErrorDialog, C_FD_GUIxFormsProvider, 'FireDAC.VCLUI.Error');
