...
type
  ...
  private
    FOwnedObjects: TList<TCustomOwnedObject>;
    [Weak] FOwner: TCustomOwnedObject;
    procedure Attach(const Obj: TCustomOwnedObject);
    procedure Detach(const Obj: TCustomOwnedObject);
  ...
  private
    FItems: TList<TCustomOwnedNode>;
    [Weak] FNextSibling: TCustomOwnedNode;
    [Weak] FParent: TCustomOwnedNode;
    [Weak] FPreviousSibling: TCustomOwnedNode;
    FReadOnly: Boolean;
    function GetItem(Index: Integer): TCustomOwnedNode;
    procedure InternalAdopt(const NewOwner: TCustomOwnedNode; const Node: TCustomOwnedNode);
    procedure InternalClear;
    procedure InternalInsertBefore(const NewChild, RefChild: TCustomOwnedNode);
    procedure InternalRemove(const OldChild: TCustomOwnedNode);
  protected
    ...
    function HasAsAncestor(const Node: TCustomOwnedNode): Boolean; virtual;
    ...
    property Items: TList<TCustomOwnedNode> read FItems;
...
procedure TCustomOwnedObject.Attach(const Obj: TCustomOwnedObject);
begin
  DoBeforeAttach(Obj);
  if not Assigned(FOwnedObjects) then
    FOwnedObjects := TList<TCustomOwnedObject>.Create;
...
procedure TCustomOwnedObject.Detach(const Obj: TCustomOwnedObject);
begin
  Obj.FOwner := nil;
...
begin
  inherited;
  FItems := TList<TCustomOwnedNode>.Create;
end;
...
function TCustomOwnedNode.HasAsAncestor(const Node: TCustomOwnedNode): Boolean;
begin
...
procedure TCustomOwnedNode.InternalAdopt(const NewOwner, Node: TCustomOwnedNode);
var
  I: Integer;
begin
...
begin
  ...
  begin
    ...
    while I >= 0 do
    begin
      Node := TCustomOwnedNode(FItems.Items[I]);
      Node.FParent := nil;
...
procedure TCustomOwnedNode.InternalInsertBefore(const NewChild, RefChild: TCustomOwnedNode);
var
  Index: Integer;
begin
...
procedure TCustomOwnedNode.InternalRemove(const OldChild: TCustomOwnedNode);
begin
  FItems.Remove(OldChild);
  OldChild.FParent := nil;

  // Adjust NextSibling / PreviousSibling properties:
  if Assigned(OldChild.FPreviousSibling) then
    OldChild.FPreviousSibling.FNextSibling := OldChild.FNextSibling;
  if Assigned(OldChild.FNextSibling) then
