...
uses
  ...
  FireDAC.Stan.Util, FireDAC.Stan.Intf, FireDAC.Stan.SQLTimeInt, FireDAC.Stan.Option,
    FireDAC.Stan.Error, FireDAC.Stan.Param, FireDAC.Stan.Storage,
  FireDAC.DatS,
  FireDAC.DApt.Intf,
  FireDAC.Phys.Intf;
...
type
  ...
  TFDDataSetFlags = set of (dfOpenBlob, dfOfflining, dfKeyBuffersAllocated,
    ...
    dfPreserveCurrentIndex, dfPostRow, dfDataSetOpened, dfUnidirectional,
  ...
  TFDCopyDataSetOptions = set of (coStructure, coCalcFields,
    ...
    coRestart, coAppend, coEdit, coRefresh);
  ...
  private
    {$IFDEF AUTOREFCOUNT} [Weak] {$ENDIF} FDetailDataSet: TFDDataSet;
  ...
  private
    {$IFDEF AUTOREFCOUNT} [Weak] {$ENDIF} FDataSet: TFDDataSet;
  ...
  TFDDataSet = class (TDataSet, IFDDataSetReference, IFDStanOptions,
    IFDPhysLocalQueryAdapter, IFDDatSTableDataCallback)
  private
    FTable: TFDDatSTable;
    FView: TFDDatSView;
    FBufferSize: Word;
    ...
    {$IFDEF AUTOREFCOUNT} [Weak] {$ENDIF} FLastParentRow: TFDDatSRow;
    ...
    {$IFDEF AUTOREFCOUNT} [Weak] {$ENDIF} FLastField: TField;
    ...
    function InternalFetchNested(ANestedTable: TFDDatSTable;
...
{$WARN SYMBOL_DEPRECATED ON}
    ...
    function GetConn: TObject; virtual;
  ...
  protected
    ...
    procedure DataConvert(AField: TField; ASource: TValueBuffer;
      var ADest: TValueBuffer; AToNative: Boolean); override;
    procedure DataEvent(Event: TDataEvent;
      Info: NativeInt); override;
...
{$ENDIF}
    ...
    procedure InitTable(AWithFields: Boolean);
    ...
    function GetUpdates: TFDDatSUpdatesJournal; virtual;
    ...
    function SaveToDFM: Boolean; virtual;
    procedure StartWait;
    procedure StopWait;
...
{$ENDIF}
    ...
    procedure GetDetailLinkFields(AMasterFields, FDetailFields: TFDFieldList); override;
    ...
    procedure SetPersFieldAttributes;
    ...
    property Aggregates: TFDAggregates read FAggregates write SetAggregates stored IsA;
  ...
  TFDWideMemoField = class(TWideMemoField)
  protected
    function GetAsWideString: UnicodeString; override;
    procedure SetAsWideString(const AValue: UnicodeString); override;
  ...
  TFDXMLField = class(TFDWideMemoField)
  private
...
uses
  Data.DBConsts, System.Variants, Data.FmtBcd, Data.SqlTimSt,
{$IFNDEF NEXTGEN}
  System.WideStrUtils, System.AnsiStrings,
{$ENDIF}
  FireDAC.Stan.Consts, FireDAC.Stan.Factory,
  FireDAC.UI.Intf, System.Math;
...
begin
  if FDetailDataSet <> nil then begin
    if FTimer = nil then begin
      ...
      FTimer.Options := [toSynchronized, toSingleFire];
      FTimer.OnTimer := DoDelayedScroll;
    end;
    FTimer.Schedule(FDetailDataSet.FetchOptions.DetailDelay);
  end;
end;

{-------------------------------------------------------------------------------}
procedure TFDMasterDataLink.DoDelayedScroll;
begin
  if Active then
    try
...
{-------------------------------------------------------------------------------}
procedure TFDMasterDataLink.DataEvent(Event: TDataEvent;
  Info: NativeInt);
begin
...
begin
  ...
  try
    ...
    else if (FTimer <> nil) and FTimer.Enabled and Active then begin
      FTimer.Enabled := False;
      DoDelayedScroll();
    end;
...
{-------------------------------------------------------------------------------}
constructor TFDDatSTableMetadataCallback.Create(ADataSet: TFDDataSet);
begin
  inherited Create;
  FDataSet := ADataSet;
end;
...
{-------------------------------------------------------------------------------}
function TFDDataSet.GetConn: TObject;
begin
  Result := nil;
end;
...
{-------------------------------------------------------------------------------}
procedure TFDDataSet.DataEvent(Event: TDataEvent;
  Info: NativeInt);
begin
  case Event of
...
begin
  if (State in [dsEdit, dsInsert, dsSetKey]) and not AValue and
     (csDestroying in ComponentState) then begin
    // Avoid event handler calls, when a dataset is destroying
    BeforeCancel := nil;
    AfterCancel := nil;
    BeforeScroll := nil;
    AfterScroll := nil;
...
{-------------------------------------------------------------------------------}
procedure TFDDataSet.DoBeforeClose;
begin
...
{-------------------------------------------------------------------------------}
function TFDDataSet.GetParamsCount: Word;
begin
...
{-------------------------------------------------------------------------------}
function TFDDataSet.GetFeatures: TFDPhysLocalQueryAdapterFeatures;
begin
...
{-------------------------------------------------------------------------------}
procedure TFDDataSet.GetDetailLinkFields(AMasterFields, FDetailFields: TFDFieldList);
var
  sFields: String;
begin
  AMasterFields.Clear;
  ...
  if (MasterSource <> nil) and (MasterSource.DataSet <> nil) then begin
    ...
    if sFields <> '' then
      GetFieldList(FDetailFields, sFields);
  end;
end;
  ...
  try
    ...
    for i := 0 to FMasterLink.Fields.Count - 1 do begin
      ...
      if oDestFld is TLargeintField then
        if oSrcFld.IsNull then
          TLargeintField(oDestFld).Clear
        else
  ...
  if IsLinkedDetail then begin
    ...
    if fiDetails in FetchOptions.Cache then begin
      if (FMasterRelation = nil) and (DatSManager <> nil) and
        ...
        BuildMasterRelation;
      if MasterRangeChanged() then begin
        CreateFilterView;
        RelinkViews;
        ResyncViews;
      end;
...
begin
  ...
  if FMasterRelation = nil then begin
    ...
    sDet := '';
    for i := 0 to FMasterLink.Fields.Count - 1 do begin
      AddFld(sMast, TField(FMasterLink.Fields[i]));
...
begin
  if AField is TObjectField then begin
    ...
    if oObjFld.DataType in [ftADT, ftArray] then begin
      if (oObjFld.DataType = ftArray) and SparseArrays and (oObjFld.Fields[0].DataType = ftADT) then
        SetFieldAttributes(TObjectField(oObjFld.Fields[0]).Fields)
      else
        SetFieldAttributes(oObjFld.Fields);
    end;
  end;
  ...
  if __TField(AField).FAutoGenerateValue = TAutoRefreshFlag.arNone then begin
    if caAutoInc in AColumn.Attributes then
      __TField(AField).FAutoGenerateValue := TAutoRefreshFlag.arAutoInc;
    if ([caCalculated, caDefault, caROWID, caVolatile, caRowVersion, caExpr] * AColumn.Attributes <> []) or
      ...
      __TField(AField).FAutoGenerateValue := TAutoRefreshFlag.arDefault;
  end;
...
{-------------------------------------------------------------------------------}
procedure TFDDataSet.SetFieldAttributes(AFields: TFields);
var
  i: Integer;
  oFld: TField;
  oCol: TFDDatSColumn;
...
{-------------------------------------------------------------------------------}
procedure TFDDataSet.SetFieldAttributes;
begin
  SetFieldAttributes(Fields);
end;

{-------------------------------------------------------------------------------}
procedure TFDDataSet.SetPersFieldAttributes;
var
...
begin
  ...
  if oUpdOpts.AutoIncFields <> '' then begin
    for i := 0 to FieldCount - 1 do begin
      oFld := Fields[i];
      if __TField(oFld).FAutoGenerateValue = TAutoRefreshFlag.arAutoInc then
        __TField(oFld).FAutoGenerateValue := TAutoRefreshFlag.arNone;
    end;
    i := 1;
    while i <= Length(oUpdOpts.AutoIncFields) do begin
      ...
      __TField(oFld).FAutoGenerateValue := TAutoRefreshFlag.arAutoInc;
      oFld.Required := False;
    end;
  end;
end;
...
{-------------------------------------------------------------------------------}
procedure TFDDataSet.SetColumnAttributes;
var
  i: Integer;
  oCol: TFDDatSColumn;
  oFld: TField;
...
{-------------------------------------------------------------------------------}
procedure TFDDataSet.SetInternalCalcs;
var
...
begin
  if not DefaultFields and (ParentDataSet = nil) then begin
    FieldDefs.Clear;
...
begin
  ...
  try
    if Designer <> nil then
      DoResetDatSManager;
    FFlags := FFlags - [dfColumnAttributesUpdated, dfDataSetOpened,
      dfDataSetOpening, dfUnidirectional];
    if FetchOptions.Unidirectional then
      Include(FFlags, dfUnidirectional);
    FSourceEOF := False;
    FUnidirRecsPurged := 0;
    FRecordCount := -1;
    FLastSavePoint := 0;
    ...
    if DataSetField = nil then begin
      ...
      if LoadFromFileAtOpen then begin
        FStorage := FDStorageManager().GetStorage(
          ResourceOptions.PersistentFileName, ResourceOptions.DefaultStoreFormat,
          ResourceOptions, FEncoder);
        try
          FStorage.Open(ResourceOptions.PersistentFileName, nil, smRead);
          InternalLoadFromStorage;
        finally
          FDFreeAndNil(FStorage);
        end;
      end
...
begin
  ...
  if (FetchOptions.Mode in [fmOnDemand, fmAll, fmExactRecsMax]) and
     (State <> dsOpening) and not lDeferOpen and not FSourceEOF then
  ...
  if DefaultFields then
    SetFieldAttributes
  else if not lLoadedFromStorage then begin
    SetPersFieldAttributes;
    SetColumnAttributes;
  end;
...
begin
  ...
  if SaveToFileAtClose then begin
    FStorage := FDStorageManager().GetStorage(
      ...
      ResourceOptions, FEncoder);
    try
      FStorage.Open(ResourceOptions.PersistentFileName, nil, smWrite);
...
begin
  ...
  try
    if Active and not SourceEOF and DoIsSourceOpen and not (dfUnidirectional in FFlags) then
...
procedure TFDDataSet.ClearFColumns;
var
  i: Integer;
begin
  for i := 0 to Length(FColumns) - 1 do
    TFDColMapItem(FColumns[i]).Free;
  SetLength(FColumns, 0);
end;
...
begin
  ...
  if AColumn.DataType in [dtRowSetRef, dtCursorRef, dtRowRef, dtArrayRef] then begin
    ...
    oChildTab := nil;
    if (eType in [ftBCD, ftFMTBcd]) and
       (iSize = 0) and (iScale > 0) then
      iSize := iScale;
  end;
...
begin
  ...
  FBufferSize := Word(CalcFieldsSize + SizeOf(TFDRecInfo));
end;
...
begin
  if State = dsInsert then
    if GetBookmarkFlag(ActiveBuffer) = bfEOF then
      ...
      Result := FRecordIndexToInsert
  else
    Result := GetRowIndex();
  Inc(Result, FUnidirRecsPurged + 1);
end;
...
var
  ...
  begin
    ...
    if not DefaultFields then begin
      ...
      if (oFld <> nil) and (ADataLen > oFld.Size) then
        Result := oFld.Size;
    end;
  end;
...
begin
  ...
  dtRowRef, dtArrayRef:
    ...
    if Result and (ABuffer <> nil) then
      ...
      dtUInt32:           PLongWord(ABuffer)^ := PLongWord(pData)^;
      ...
      dtTime:             PDateTimeRec(ABuffer)^.Time := PLongint(pData)^;
      dtDate:             PDateTimeRec(ABuffer)^.Date := PLongint(pData)^;
      ...
      dtObject:           PIUnknown(ABuffer)^ := PIUnknown(pData)^;
      end;
  end;
end;
...
begin
  if (AState in [dsOldValue, dsCurValue]) and __TField(AField).FValidating then begin
    UpdateCursorPos;
    lPrevModified := Modified;
    ePrevState := SetTempState(AState);
    __TField(AField).FValidating := False;
    try
      Result := AField.Value;
    finally
      __TField(AField).FValidating := True;
...
{-------------------------------------------------------------------------------}
procedure TFDDataSet.DataConvert(AField: TField; ASource: TValueBuffer;
  var ADest: TValueBuffer; AToNative: Boolean);
begin
  if AField.DataType in [ftWideString, ftFixedWideChar] then
    {$IFDEF NEXTGEN} StrLCopy {$ELSE} WStrLCopy {$ENDIF}
      (PWideChar(ADest), PWideChar(ASource), AField.Size)
  else
  ...
  if iFieldNo > 0 then begin
    ...
    if GetFieldColumn(pRecBuf, iFieldNo, oColumn, iColIndex, oRow, True) then begin
      ...
      if ABuffer <> nil then
        ...
        dtWideString:
          begin
            iLen := FDWideStrLen(PWideChar(ABuffer));
            if (caFixedLen in oColumn.Attributes) and FormatOptions.StrsTrim then
        ...
        dtDateTimeStamp: pData := ABuffer;
        dtBCD: pData := ABuffer;
        dtFmtBCD: pData := ABuffer;
        dtBoolean,
        dtSByte, dtByte,
        dtInt16, dtInt32, dtInt64,
        dtUInt16, dtUInt32, dtUInt64,
        dtTime, dtDate, dtDateTime,
...
{$ENDIF}
      ...
      if AField.DataSize > 0 then
        Move(PByte(ABuffer)^, pBuff[1], AField.DataSize);
  end;
...
begin
  if dfUnidirectional in FFlags then
    ErrorUnidir;
end;
...
begin
  ...
  if ((FParentDataSet <> nil) or not SourceEOF) and
    ...
    if FParentDataSet <> nil then
      ...
      lUnidir := (dfUnidirectional in FFlags);
      StartWait;
      try
        if DoIsSourceAsync then
...
{-------------------------------------------------------------------------------}
function TFDDataSet.InternalFetchNested(ANestedTable: TFDDatSTable;
  ARowOptions: TFDPhysFillRowOptions): Boolean;
var
...
begin
  if ADataSetField <> nil then
    oTab := (ADataSetField.NestedDataSet as TFDDataSet).FTable
  else
    oTab := nil;
  if InternalFetchNested(oTab, [foDetails]) and not (dfOpenBlob in FFlags) then
    DataEvent(deDataSetChange, 0);
end;
...
begin
  CheckActive;
  if InternalFetchNested(nil, [foBlobs]) and not (dfOpenBlob in FFlags) then
    DataEvent(deDataSetChange, 0);
end;
...
begin
  CheckActive;
  Cancel;
  Result := InternalFetchNested(nil, ARowOptions + FDGetFillRowOptions(GetFetchOptions));
  if Result then begin
...
{-------------------------------------------------------------------------------}
// If to call Refresh and a connection is losted, then a dataset will be
// closed automatically and then Resync will be called. Resync for inactive
// dataset raises an AV.

procedure TFDDataSet.Resync(Mode: TResyncMode);
begin
  if Active then
...
begin
  CheckBrowseMode;
  CheckBidirectional;
  if UpdatesPending or (FDataLoadSave <> nil) then
    ...
    if (MasterSource <> nil) and (MasterSource.DataSet <> nil) and
      ...
      try
        ...
        for i := 0 to iCount - 1 do
          if TField(oDetails[i]).CanModify then
            if TField(oDetails[i]) is TLargeintField then
              if TField(oMasters[i]).IsNull then
                TLargeintField(oDetails[i]).Clear
              else
                TLargeintField(oDetails[i]).Value := TField(oMasters[i]).Value
            else
              TField(oDetails[i]).Assign(TField(oMasters[i]));
      finally
        FDFree(oMasters);
        FDFree(oDetails);
      end;
...
begin
  for i := 0 to ASource.FieldCount - 1 do begin
    ...
    if (oDestField = nil) or not oDestField.CanModify or
...
begin
  ...
  try
    ...
    if coStructure in AOptions then begin
      ...
      if (coIndexesCopy in AOptions) and (ASource is TFDDataSet) then begin
      ...
      try
        ...
        if coCalcFields in AOptions then
          for i := 0 to ASource.FieldCount - 1 do begin
            oSrcFld := ASource.Fields[i];
            if (oSrcFld.FieldKind <> fkData) and (FieldDefs.IndexOf(oSrcFld.FieldName) = -1) then
              FieldDefs.Add(oSrcFld.FieldName, oSrcFld.DataType, oSrcFld.Size);
          end;
        FieldDefList.Update;
      ...
      while not ASource.Eof do begin
        ...
        if [coEdit, coRefresh] * AOptions <> [] then begin
          ...
          if Locate(sKeyFields, vKeyValues, []) then
            eStatus := usModified
          else if coAppend in AOptions then
            eStatus := usInserted;
        end
        else if coAppend in AOptions then
          eStatus := usInserted;
        ...
        if eStatus in [usInserted, usModified] then
          ...
          if coRefresh in AOptions then begin
            if eStatus = usInserted then begin
...
{-------------------------------------------------------------------------------}
function TFDDataSet.IsA: Boolean;
begin
...
begin
  ...
  for i := Low(TFDKeyIndex) to High(TFDKeyIndex) do begin
    ...
    if pBuff <> nil then begin
      ...
      if oRow <> nil then
        FDFree(oRow);
      FreeRecBuf(FKeyBuffers[i].FRecBuff);
...
begin
  GetRecInfo(ABuffer.FRecBuff)^.FRow := ARow;
end;
...
begin
  ...
  for i := 0 to ADefs.Count - 1 do begin
    ...
    if (FDatSManager <> nil) and (eDestDataType in [dtRowSetRef, dtCursorRef, dtRowRef, dtArrayRef]) then begin
      oChildTab := TFDDatSTable.Create;
      oChildTab.CountRef;
      oChildTab.Setup(GetOptionsIntf);
      oChildTab.Name := FDatSManager.Tables.BuildUniqueName(
        C_FD_SysNamePrefix + '#' + ATable.Name + '#' + oCol.Name);
      FDatSManager.Tables.Add(oChildTab);
      InitColumnsFromFieldDefs(oDef.ChildDefs, oChildTab, FDatSManager);

      oChildTabRelCol := TFDDatSColumn.Create;
      oChildTabRelCol.Name := oChildTab.Columns.BuildUniqueName(
        C_FD_SysNamePrefix + '#' + oCol.Name);
      ...
      oRel.Name := FDatSManager.Relations.BuildUniqueName(
        C_FD_SysNamePrefix + '#' + ATable.Name + '#' + oCol.Name);
      oRel.Nested := True;
...
{-------------------------------------------------------------------------------}
procedure TFDDataSet.InitTable(AWithFields: Boolean);
var
  oTab: TFDDatSTable;
  oSch: TFDDatSManager;
begin
...
begin
  ...
  try
    ...
    IndexDefs.Updated := False;
    DataSetField := nil;
    FStorage := FDStorageManager().GetStorage(AFileName, AFormat,
      ResourceOptions, FEncoder);
    try
      FStorage.Open(AFileName, AStream, smRead);
      Active := True;
    finally
      FDFreeAndNil(FStorage);
    end;
...
begin
  ...
  FStorage := FDStorageManager().GetStorage(AFileName, AFormat,
    ResourceOptions, FEncoder);
  try
    FStorage.Open(AFileName, AStream, smWrite);
    InternalSaveToStorage;
  finally
    FDFreeAndNil(FStorage);
  end;
...
begin
  StartWait;
  try
    DoGetDatSManager.LoadFromStorage(FStorage);
    oTabs := DoGetDatSManager.RootTables;
    if Length(oTabs) > 0 then
      AttachTable(oTabs[0], nil);
    FLastSavePoint := DoGetDatSManager.Updates.SavePoint;
    FSourceEOF := True;
  finally
    StopWait;
  end;
...
begin
  StartWait;
  try
    DoGetDatSManager.SaveToStorage(FStorage);
    FLastSavePoint := DoGetDatSManager.Updates.SavePoint;
  finally
    StopWait;
  end;
...
begin
  Result := (DataSetField = nil) and (ParentDataSet = nil) and (DoGetDatSManager <> nil) and
    ...
    ((FLastSavePoint < DoGetDatSManager.Updates.SavePoint) or
     not FileExists(ResourceOptions.ResolveFileName()));
end;

{-------------------------------------------------------------------------------}
function TFDDataSet.SaveToDFM: Boolean;
begin
...
begin
  ...
  AFiler.DefineBinaryProperty('Content', ReadContent, WriteContent, SaveToDFM);
end;
...
begin
  ...
  oCont.Position := 0;
  FStorage := FDStorageManager().GetStorage('', sfBinary,
    ResourceOptions, FEncoder);
  FStorage.OwnStream := True;
  FStorage.Open('', oCont, smRead);
end;
...
type
  ...
  protected
    ...
    function GetDataView: TFDDatSView;
  public
    constructor Create(AView: TFDDatSView; AOwnView: Boolean);
    destructor Destroy; override;
  end;

{-------------------------------------------------------------------------------}
constructor TFDDataSetIntfImpl.Create(AView: TFDDatSView; AOwnView: Boolean);
begin
  inherited Create;
  FView := AView;
  FOwnView := AOwnView;
...
begin
  CheckBrowseMode;
  if FView <> nil then
    Result := TFDDataSetIntfImpl.Create(FView, False)
  else if FTable <> nil then
    Result := TFDDataSetIntfImpl.Create(FTable.FullView, False)
  else
    Result := TFDDataSetIntfImpl.Create(nil, False);
end;
...
begin
  CheckBrowseMode;
  if FSourceView <> nil then
    Result := TFDDataSetIntfImpl.Create(FSourceView, False)
  else
    Result := GetData;
end;
...
begin
  ...
  if FTable <> nil then begin
    ...
    Result := TFDDataSetIntfImpl.Create(oView, True);
  end
  else
    Result := TFDDataSetIntfImpl.Create(nil, False);
end;
...
{-------------------------------------------------------------------------------}
function TFDDataSet.GetSavePoint: Integer;
begin
  CheckBrowseMode;
  CheckCachedUpdatesMode;
  if Updates <> nil then
...
{-------------------------------------------------------------------------------}
procedure TFDDataSet.SetSavePoint(const AValue: Integer);
begin
  Cancel;
  CheckBrowseMode;
  CheckCachedUpdatesMode;
...
{-------------------------------------------------------------------------------}
procedure TFDDataSet.ApplyUpdatesComplete(AErrors: Integer);
begin
...
{-------------------------------------------------------------------------------}
function TFDDataSet.Reconcile: Boolean;
var
  oRow, oNextRow: TFDDatSRow;
...
begin
  CheckCachedUpdatesMode;
  oJournal := Updates;
  if (oJournal = nil) or not Assigned(OnReconcileError) then begin
  ...
  try
    ...
    try
      ...
      if not lPrevActive then
        Close
      else if BookmarkValid(sPrevPos) then
        Bookmark := sPrevPos;
    end;
  finally
    EnableControls;
    ...
    try
      ...
      if FDataSet.GetFieldColumn(FBuffer, FField.FieldNo, oCol, iColIndex, oRow, True) then begin
        if oRow.RowState in [rsInserted, rsModified, rsUnchanged] then
          oRow.BeginEdit;
        iLen := LongWord(Size);
        if (FField is TFDWideMemoField)
           or (FField is TWideStringField)
           or (FField is TWideMemoField)
           then
...
{-------------------------------------------------------------------------------}
function TFDBlobStream.Write(const ABuffer: TBytes; AOffset, ACount: Integer): Longint;
begin
  CheckWrite;
...
{ TFDWideMemoField                                                              }
{-------------------------------------------------------------------------------}
constructor TFDWideMemoField.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
...
{-------------------------------------------------------------------------------}
function TFDWideMemoField.GetClassDesc: string;
begin
  Result := '(WideMemo)';
  if not IsNull then
...
{-------------------------------------------------------------------------------}
function TFDWideMemoField.GetAsString: String;
begin
  Result := GetAsWideString;
end;

{-------------------------------------------------------------------------------}
procedure TFDWideMemoField.SetAsString(const AValue: String);
begin
  SetAsWideString(AValue);
end;

{-------------------------------------------------------------------------------}
function TFDWideMemoField.GetAsWideString: UnicodeString;
var
  iLen: Integer;
  oStr: TStream;
begin
  oStr := DataSet.CreateBlobStream(Self, bmRead);
  try
    iLen := Integer(oStr.Size);
    SetString(Result, nil, iLen div SizeOf(WideChar));
    oStr.ReadBuffer(PWideChar(Result)^, iLen);
  finally
    FDFree(oStr);
  end;
end;
...
{-------------------------------------------------------------------------------}
procedure TFDWideMemoField.SetAsWideString(const AValue: UnicodeString);
var
  oStr: TStream;
begin
  oStr := DataSet.CreateBlobStream(Self, bmWrite);
  try
    oStr.WriteBuffer(PWideChar(AValue)^, Length(AValue) * SizeOf(WideChar));
  finally
    FDFree(oStr);
  end;
end;
...
{-------------------------------------------------------------------------------}
function TFDWideMemoField.GetAsVariant: Variant;
begin
  Result := GetAsWideString;
end;

{-------------------------------------------------------------------------------}
procedure TFDWideMemoField.SetVarValue(const AValue: Variant);
begin
  SetAsWideString(AValue);
end;
