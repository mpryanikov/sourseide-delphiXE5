...
begin
  ...
  begin
    Result := not SameValue(NewValue, FTabControl.TabHeight);
    Value := FTabControl.TabHeight;
  end;
end;
...
begin
  ...
  begin
    ...
    if FFullSize and (CountVisibleTab > 0) then
      AutoWidth := Round(AutoWidth / CountVisibleTab);
    ...
    begin
      ...
      if FFullSize then
      begin
        if CurX + LItem.Margins.Left + L + AutoWidth - R > Width then
          LItem.SetBounds(CurX + LItem.Margins.Left - L, CurY + LItem.Margins.Top, Width - (CurX + LItem.Margins.Left + L - R),
            MaxHeight - LItem.Margins.Top - LItem.Margins.Bottom)
        else
          LItem.SetBounds(CurX + LItem.Margins.Left - L, CurY + LItem.Margins.Top, AutoWidth + R,
            MaxHeight - LItem.Margins.Top - LItem.Margins.Bottom)
      end
      else
        LItem.SetBounds(CurX + LItem.Margins.Left - L, CurY + LItem.Margins.Top, LItem.GetAutoWidth,
          MaxHeight - LItem.Margins.Top - LItem.Margins.Bottom);
