...
{$ENDIF}
  ...
  FireDAC.Stan.Intf, FireDAC.Stan.Expr, FireDAC.Stan.Util, FireDAC.Stan.Error, FireDAC.Stan.Consts,
...
var
  AYear1, AMonth1, FDay1: Word;
  AYear2, AMonth2, FDay2: Word;
begin
  if FDStrIsNull(AArgs[0]) or FDStrIsNull(AArgs[1]) then
    Result := Null
  else begin
    ...
    AMonth1 := 0;
    FDay1 := 0;
    DecodeDate(AArgs[0], AYear1, AMonth1, FDay1);
    AYear2 := 0;
    AMonth2 := 0;
    FDay2 := 0;
    DecodeDate(AArgs[1], AYear2, AMonth2, FDay2);
    Result := (Integer(AYear1 * 12 * 31 + AMonth1 * 31 + FDay1) -
               Integer(AYear2 * 12 * 31 + AMonth2 * 31 + FDay2)) / 31;
  end;
end;
  ...
  if FDStrIsNull(AArgs[0]) or
    ...
    if N > 0 then
      if (VarType(V) and varTypeMask) = varDate then begin
        ...
        Result := VarAsType(V, varADString)
    else
      Result := VarAsType(V, varADString);
  end;
end;
...
begin
  if VarType(AValue) = varOleStr then
    Result := VarAsType(AValue, varADString)
  else
    Result := AValue;
end;
...
var
  iIndex: Integer;
begin
  if FDStrIsNull(AArgs[0]) then begin
    Result := Null;
    exit;
  end
  else
    if FDStrIsNull(AArgs[1]) then
      Result := AArgs[0]
    else begin
      iIndex := VarAsType(AArgs[1], varInteger);
...
begin
  if FDStrIsNull(AArgs[0]) or FDStrIsNull(AArgs[1]) then
    ...
    if (Length(AArgs) > 2) and not FDStrIsNull(AArgs[2]) then
      Result := FDPosEx(VarToStr(AArgs[0]), VarToStr(AArgs[1]), AArgs[2])
    else
      Result := Pos(VarToStr(AArgs[0]), VarToStr(AArgs[1]));
end;
...
begin
  if FDStrIsNull(AArgs[0]) or FDStrIsNull(AArgs[1]) then
    ...
    if sType = 'BIGINT' then
      ...
      Result := VarAsType(V, varADAString)
      ...
      Result := VarAsType(V, varADWString)
