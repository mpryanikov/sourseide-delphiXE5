...
var
  oIni: TFDIniFile;
begin
  oIni := TFDIniFile.Create(ActualFileName);
...
begin
  ...
  TFDStringList(FParams).OnChange := ParamsChanged;
  TFDStringList(FParams).OnChanging := ParamsChanging;
  Clear;
end;
...
begin
  if FParentDefinition <> nil then begin
    ...
    try
      i := FParams.Count - 1;
      while i >= 0 do begin
        sName := FDNameFromIndex(FParams, i);
        if sName <> '' then begin
...
{-------------------------------------------------------------------------------}
procedure TFDDefinition.Migrate;
var
  i: Integer;
begin
  // v 2.0.0
  // changed: MSSQL2000 and MSSQL2005 driver ID's replaced by MSSQL
  i := FParams.IndexOfName(S_FD_ConnParam_Common_DriverID);
  if i <> -1 then
    if (AnsiCompareText(FParams.Values[S_FD_ConnParam_Common_DriverID], S_FD_MSSQLId + '2000') = 0) or
       (AnsiCompareText(FParams.Values[S_FD_ConnParam_Common_DriverID], S_FD_MSSQLId + '2005') = 0) then
    FParams.Values[S_FD_ConnParam_Common_DriverID] := S_FD_MSSQLId;
  // changed: Generic ODBC driver parameter 'Driver' renamed to 'ODBCDriver'
  i := FParams.IndexOfName('Driver');
  if i <> -1 then
    FParams.Values[S_FD_ConnParam_Common_DriverID] := S_FD_ConnParam_ODBC_Driver;
end;
...
begin
  ...
  if ADefinition <> nil then
    for i := 0 to ADefinition.Params.Count - 1 do begin
      sName := FDNameFromIndex(ADefinition.Params, i);
      sValue := FDValueFromIndex(ADefinition.Params, i);
  ...
  while i <= Length(AStr) do begin
    ...
    if (AKeywords <> nil) and (AKeywords.Count > 0) then begin
      ...
      for j := 0 to AKeywords.Count - 1 do begin
        sODBCId := FDValueFromIndex(AKeywords, j);
        ...
        if {$IFDEF FireDAC_NOLOCALE_META} CompareText {$ELSE} AnsiCompareText {$ENDIF}
          ...
          sId := FDNameFromIndex(AKeywords, j);
          lFound := True;
        end
...
begin
  oKeywordValues := TFDStringList.Create;
  ...
  try
    ...
    while oDef <> nil do begin
      for i := 0 to oDef.Params.Count - 1 do begin
        sId := FDNameFromIndex(oDef.Params, i);
        sVal := FDExpandStr(FDValueFromIndex(oDef.Params, i));
          ...
          for j := 0 to AKeywords.Count - 1 do begin
            if {$IFDEF FireDAC_NOLOCALE_META} CompareText {$ELSE} AnsiCompareText {$ENDIF}
              ...
              AddKwdValue(j, sId)
                    ...
                    (FDNameFromIndex(AKeywords, j), sId) = 0 then
              AddKwdValue(j, FDValueFromIndex(AKeywords, j));
          end
        else
...
begin
  ...
  if i = -1 then begin
    ...
    Result := FDValueFromIndex(FParams, i);
...
var
  ...
  begin
    for i := 0 to FParams.Count - 1 do
      if CompareText(FDNameFromIndex(FParams, i), S_FD_DefinitionParam_Common_Name) <> 0 then
        AWriter.WriteString(sName, FDNameFromIndex(FParams, i), FDValueFromIndex(FParams, i));
  end;
...
{-------------------------------------------------------------------------------}
function TFDDefinitionTemporaryFactory.CreateObject: TObject;
begin
...
{-------------------------------------------------------------------------------}
function TFDConnectionDef.GetPooled: Boolean;
begin
  Result := GetAsBoolean(S_FD_ConnParam_Common_Pooled);
end;

{-------------------------------------------------------------------------------}
procedure TFDConnectionDef.SetPooled(AValue: Boolean);
begin
  SetAsBoolean(S_FD_ConnParam_Common_Pooled, AValue);
end;

{-------------------------------------------------------------------------------}
function TFDConnectionDef.GetDriverID: String;
begin
  Result := GetAsString(S_FD_ConnParam_Common_DriverID);
end;

{-------------------------------------------------------------------------------}
procedure TFDConnectionDef.SetDriverID(const AValue: String);
begin
  SetAsString(S_FD_ConnParam_Common_DriverID, AValue);
end;

{-------------------------------------------------------------------------------}
function TFDConnectionDef.GetPassword: String;
begin
  Result := GetAsString(S_FD_ConnParam_Common_Password);
end;

{-------------------------------------------------------------------------------}
procedure TFDConnectionDef.SetPassword(const AValue: String);
begin
  SetAsString(S_FD_ConnParam_Common_Password, AValue);
end;

{-------------------------------------------------------------------------------}
function TFDConnectionDef.GetNewPassword: String;
begin
  Result := GetAsString(S_FD_ConnParam_Common_NewPassword);
end;

{-------------------------------------------------------------------------------}
procedure TFDConnectionDef.SetNewPassword(const AValue: String);
begin
  SetAsString(S_FD_ConnParam_Common_NewPassword, AValue);
end;

{-------------------------------------------------------------------------------}
function TFDConnectionDef.GetOSAuthent: Boolean;
begin
  Result := GetAsBoolean(S_FD_ConnParam_Common_OSAuthent);
end;

{-------------------------------------------------------------------------------}
procedure TFDConnectionDef.SetOSAuthent(const AValue: Boolean);
begin
  SetAsYesNo(S_FD_ConnParam_Common_OSAuthent, AValue);
end;

{-------------------------------------------------------------------------------}
function TFDConnectionDef.GetUserName: String;
var
...
begin
  iLvlStd := 0;
  iLvlBDE := 0;
  lStd := HasValue(S_FD_ConnParam_Common_UserName, iLvlStd);
  lBde := HasValue(S_FD_ConnParam_Common_BDEStyleUserName, iLvlBDE);
  if lStd or lBde then
    if iLvlStd < iLvlBDE then
      Result := GetAsString(S_FD_ConnParam_Common_UserName)
    else
      Result := GetAsString(S_FD_ConnParam_Common_BDEStyleUserName);
end;

{-------------------------------------------------------------------------------}
procedure TFDConnectionDef.SetUserName(const AValue: String);
begin
  SetAsString(S_FD_ConnParam_Common_UserName, AValue);
end;

{-------------------------------------------------------------------------------}
function TFDConnectionDef.GetDatabase: String;
begin
  Result := GetAsString(S_FD_ConnParam_Common_Database);
end;

{-------------------------------------------------------------------------------}
procedure TFDConnectionDef.SetDatabase(const AValue: String);
begin
  if Pos('=', AValue) = 0 then
    SetAsString(S_FD_ConnParam_Common_Database, AValue)
  else
    ParseString(AValue);
end;

{-------------------------------------------------------------------------------}
function TFDConnectionDef.GetExpandedDatabase: String;
begin
  Result := FDExpandStr(GetDatabase);
end;

{-------------------------------------------------------------------------------}
function TFDConnectionDef.GetMonitorBy: String;
begin
  Result := GetAsString(S_FD_ConnParam_Common_MonitorBy);
end;

{-------------------------------------------------------------------------------}
procedure TFDConnectionDef.SetMonitorBy(const AValue: String);
begin
  SetAsString(S_FD_ConnParam_Common_MonitorBy, AValue);
end;

{-------------------------------------------------------------------------------}
function TFDConnectionDef.GetPort: Integer;
begin
  Result := GetAsInteger(S_FD_ConnParam_Common_Port);
end;

{-------------------------------------------------------------------------------}
procedure TFDConnectionDef.SetPortI(const AValue: Integer);
begin
  SetAsInteger(S_FD_ConnParam_Common_Port, AValue);
end;

{-------------------------------------------------------------------------------}
function TFDConnectionDef.GetServer: String;
begin
  Result := GetAsString(S_FD_ConnParam_Common_Server);
end;

{-------------------------------------------------------------------------------}
procedure TFDConnectionDef.SetServer(const AValue: String);
begin
  SetAsString(S_FD_ConnParam_Common_Server, AValue);
end;

{-------------------------------------------------------------------------------}
function TFDConnectionDef.GetMetaDefCatalog: String;
begin
  Result := GetAsString(S_FD_ConnParam_Common_MetaDefCatalog);
end;

{-------------------------------------------------------------------------------}
function TFDConnectionDef.GetMetaDefSchema: String;
begin
  Result := GetAsString(S_FD_ConnParam_Common_MetaDefSchema);
end;

{-------------------------------------------------------------------------------}
procedure TFDConnectionDef.SetMetaDefCatalog(const AValue: String);
begin
  SetAsString(S_FD_ConnParam_Common_MetaDefCatalog, AValue);
end;

{-------------------------------------------------------------------------------}
procedure TFDConnectionDef.SetMetaDefSchema(const AValue: String);
begin
  SetAsString(S_FD_ConnParam_Common_MetaDefSchema, AValue);
end;
...
begin
  ...
  while oDef <> nil do begin
    for i := 0 to oDef.Params.Count - 1 do begin
      sName := LowerCase(FDNameFromIndex(oDef.Params, i));
    ...
    try
      ...
      try
        ...
        while i < GetParams.Count do begin
          sName := FDNameFromIndex(GetParams, i);
...
{-------------------------------------------------------------------------------}
function TFDConnectionDefTemporaryFactory.CreateObject: TObject;
begin
