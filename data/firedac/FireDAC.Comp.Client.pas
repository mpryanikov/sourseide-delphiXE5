...
type
  ...
  TFDConnectionLoginEvent = procedure (AConnection: TFDCustomConnection;
    const AConnectionDef: IFDStanConnectionDef) of object;
  ...
  TFDConnectionRecoverEvent = procedure (ASender: TObject; const AInitiator: IFDStanObject;
  ...
  TFDGetDatasetEvent = procedure (ASender: TObject; const AName: String;
    var ADataSet: TDataSet; var AAdapter: IFDPhysLocalQueryAdapter) of object;
  TFDReadDatasetEvent = procedure (ASender: TObject; const AName: String;
  ...
  private
    ...
    FOfflined: Boolean;
    FOnLosted: TNotifyEvent;
    FOnRestored: TNotifyEvent;
    FOnRecover: TFDConnectionRecoverEvent;
    {$IFDEF AUTOREFCOUNT} [Weak] {$ENDIF} FUpdateTransaction: TFDCustomTransaction;
    {$IFDEF AUTOREFCOUNT} [Weak] {$ENDIF} FTransaction: TFDCustomTransaction;
    FLastUsed: TDateTime;
    ...
    procedure GetParentOptions(out AOpts: IFDStanOptions);
    procedure SetOptionsIntf(const AValue: IFDStanOptions);
    function GetFetchOptions: TFDFetchOptions;
    function GetFormatOptions: TFDFormatOptions;
    function GetUpdateOptions: TFDUpdateOptions;
    ...
    procedure PrepareConnectionDef;
...
{$ENDIF}
    ...
    property OnLosted: TNotifyEvent read FOnLosted write FOnLosted;
  ...
  private
    ...
    {$IFDEF AUTOREFCOUNT} [Weak] {$ENDIF} FOwner: TFDDataSet;
    ...
    FThreadID: LongWord;
    {$IFDEF AUTOREFCOUNT} [Weak] {$ENDIF} FTableAdapter: TFDCustomTableAdapter;
...
{$ENDIF}
  ...
  private
    ...
    {$IFDEF AUTOREFCOUNT} [Weak] {$ENDIF} FAdaptedDataSet: TFDAdaptedDataSet;
    ...
    procedure SetDatSTable(const AValue: TFDDatSTable);
  ...
  private
    ...
    FOnReconcileRow: TFDReconcileRowEvent;
    FOnUpdateRow: TFDUpdateRowEvent;
    function GetTableAdaptersIntf: IFDDAptTableAdapters;
    function GetDatSManager: TFDDatSManager;
    procedure SetDatSManager(const AValue: TFDDatSManager);
    function GetCount: Integer;
    function GetTableAdapters(AIndex: Integer): TFDCustomTableAdapter;
  protected
  ...
  private
    {$IFDEF AUTOREFCOUNT} [Weak] {$ENDIF} FDataSet: TFDAdaptedDataSet;
  ...
  private
    FDataSet: TDataSet;
    FName: String;
    FAdapter: IFDPhysLocalQueryAdapter;
    FTemporary: Boolean;
    procedure SetName(const AValue: String);
    procedure SetDataSet(const AValue: TDataSet);
    procedure SetAdapter(const AValue: IFDPhysLocalQueryAdapter);
    procedure Changing;
    procedure Changed;
    procedure CheckDefaultAdapter;
  ...
  public
    ...
    property Adapter: IFDPhysLocalQueryAdapter read FAdapter write SetAdapter;
  published
  ...
  private
    {$IFDEF AUTOREFCOUNT} [Weak] {$ENDIF} FOwner: TFDCustomLocalSQL;
  ...
  public
    ...
    function Add(ADataSet: TDataSet; const AName: String = '';
      const AAdapter: IFDPhysLocalQueryAdapter = nil): TFDLocalSQLDataSet; overload;
  ...
  private
    ...
    FOnReadDataSet: TFDReadDatasetEvent;
    procedure SetConnection(const AValue: TFDCustomConnection);
    procedure DoConnectChanged(Sender: TObject; Connecting: Boolean);
  ...
  protected
    // IFDPhysSQLHandler
    function FindDataSet(const ASchema, AName: String): IFDPhysLocalQueryAdapter; virtual;
    function DataSetNotFound(const AName: String): Boolean; virtual;
    procedure ReadDataSet(const ASchema, AName: String; ADataSet: TObject); virtual;
    // introduced
  ...
  public
    ...
    property OnReadDataSet: TFDReadDatasetEvent read FOnReadDataSet write FOnReadDataSet;
  end;

  TFDAdaptedDataSet = class(TFDDataSet)
  private
  ...
  protected
    ...
    // IFDPhysLocalQueryAdapter
    function GetConn: TObject; override;
    // IProviderSupport
...
{$WARN SYMBOL_DEPRECATED ON}
  ...
  private
    ...
    function GetCommandText: UnicodeString;
    procedure SetCommandText(const AValue: UnicodeString);
  ...
  protected
    ...
    function SaveToDFM: Boolean; override;
 ...
 public
    ...
    property CommandText: UnicodeString read GetCommandText write SetCommandText;
...
{$IFDEF POSIX}
  Posix.Pthread,
{$ENDIF}
  System.Variants, Data.FmtBcd, Data.SqlTimSt, System.Generics.Collections,
...
{-------------------------------------------------------------------------------}
procedure TFDCustomManager.AddConnectionDef(const AName, ADriver: string; AList: TStrings;
  APersistent: Boolean = False);
var
...
begin
  ...
  try
    oDef.Params.Assign(AList);
    if AName <> '' then
      oDef.Name := AName;
    if ADriver <> '' then
      oDef.DriverID := ADriver;
  finally
...
begin
  ...
  try
    for i := 0 to AList.Count - 1 do begin
      sName := FDNameFromIndex(AList, i);
      sValue := FDValueFromIndex(AList, i);
...
procedure TFDCustomConnection.GetParentOptions(out AOpts: IFDStanOptions);
begin
...
begin
  if Assigned(FOnError) then
    if AInitiator = nil then
      FOnError(Self, Self as IFDStanObject, AException)
    else
...
begin
  if Assigned(FOnRecover) then
    FOnRecover(Self, AInitiator, AException, AAction);
end;
...
{-------------------------------------------------------------------------------}
procedure TFDCustomConnection.DoLosted;
begin
  if Assigned(FOnLosted) then
    FOnLosted(Self);
end;

{-------------------------------------------------------------------------------}
procedure TFDCustomConnection.HandleConnectionLosted;
begin
...
begin
  Result := RDBMSKind in [mkOracle, mkMSSQL, mkMSAccess, mkMySQL, mkDB2, mkASA,
    mkADS, mkInterbase, mkFirebird, mkSQLite, mkPostgreSQL, mkNexus, mkOther];
end;
...
begin
  if ConnectionIntf <> nil then begin
    ...
    PrepareConnectionDef;
    Result := FDManager.GetRDBMSKind(FParams.DriverID);
  end;
end;

{-------------------------------------------------------------------------------}
function TFDCustomConnection.GetParams: TStrings;
begin
...
{-------------------------------------------------------------------------------}
procedure TFDCustomConnection.SetParams(const AValue: TStrings);
begin
  CheckInactive;
...
begin
  Result := FParams.AsString[S_FD_DefinitionParam_Common_ConnectionDef];
end;
...
begin
  if GetConnectionDefName <> AValue then begin
    FParams.DriverID := '';
    FParams.AsString[S_FD_DefinitionParam_Common_ConnectionDef] := AValue;
  end;
end;
...
begin
  if GetDriverName <> AValue then begin
    FParams.AsString[S_FD_DefinitionParam_Common_ConnectionDef] := '';
...
{-------------------------------------------------------------------------------}
procedure TFDCustomConnection.PrepareConnectionDef;
begin
  ...
  try
    ...
    try
      ...
      lErasePassword := not FParams.HasValue(S_FD_ConnParam_Common_Password) and
        not ResourceOptions.AutoReconnect and not FParams.Pooled;
      if LoginPrompt and not FParams.Pooled and (Assigned(FOnLogin) or not FDGUIxSilent()) and
      ...
      if lErasePassword then
        FParams.Password := ''
      else if FParams.NewPassword <> '' then
        FParams.Password := FParams.NewPassword;
      FParams.NewPassword := '';
      FParams.OnChanging := ParamsChanging;
    end;
  except
...
begin
  Connected := False;
  if AUserName <> '' then
    ResultConnectionDef.UserName := AUserName;
  if APassword <> '' then
    ResultConnectionDef.Password := APassword;
end;
  ...
  try
    ...
    try
      ...
      if riConnDef in AItems then begin
        Header('Connection definition parameters');
        oDef := ResultConnectionDef;
        while oDef <> nil do begin
          for i := 0 to oDef.Params.Count - 1 do begin
            sName := FDNameFromIndex(oDef.Params, i);
            sVal := FDValueFromIndex(oDef.Params, i);
      ...
      if riClient in AItems then begin
        ...
        if (riClientLog in AItems) and
          ...
          for i := 0 to oConnIntf.Driver.Messages.Count - 1 do begin
            ...
            if (Pos('Warning:', sMsg) > 0) or (Pos('Error:', sMsg) > 0) then
              Include(Result, rsClientWarning);
            AList.Add(sMsg);
          end;
        end;
        if sClientMsg <> '' then begin
          AList.Add('Failed to load DBMS client !');
...
{-------------------------------------------------------------------------------}
procedure TFDCustomConnection.AttachClient(AObj: TObject);
begin
  RegisterClient(AObj);
  if AObj is TFDCustomCommand then
    FCommands.Add(AObj);
  if FRefCount = 0 then
    Open
...
{-------------------------------------------------------------------------------}
procedure TFDCustomConnection.DetachClient(AObj: TObject);
begin
  UnRegisterClient(AObj);
  if AObj is TFDCustomCommand then
    FCommands.Remove(AObj);
...
begin
  ...
  else if (ACommand.CommandText <> ASQL) and (ASQL <> '') then begin
    Result := True;
    ACommand.CommandKind := skUnknown;
    ACommand.Prepare(ASQL);
  end;
end;
...
begin
  oCmd := BaseCreateSQL;
  try
    BasePrepareSQL(oCmd, ASQL, AParams, ATypes);
    oCmd.Execute();
...
{-------------------------------------------------------------------------------}
function TFDCustomConnection.ExecSQL(const ASQL: String): LongInt;
begin
  Result := ExecSQL(ASQL, [], []);
end;
...
begin
  ...
  if (FOwner <> nil) and (FOwner.State = dsOpening) and
     ((FThreadID = 0) or (FThreadID <> GetCurrentThreadId())) then
...
procedure TFDCustomCommand.GetParentOptions(out AOpts: IFDStanOptions);
var
  oConn: TFDCustomConnection;
begin
  if FConnection <> nil then
...
var
  ...
  oRes: TFDResourceOptions;
begin
  if CheckComponentState([csReading]) or (TFDStringList(CommandText).UpdateCount > 0) or
...
begin
  if AInitiator = nil then begin
    if (FOwner = nil) or not Supports(TObject(FOwner), IFDStanObject, oInit) then
      Supports(TObject(Self), IFDStanObject, oInit);
  end
  else
    oInit := AInitiator;
  if Assigned(FOnError) then
...
begin
  if Assigned(FAdaptedDataSet) then
    FAdaptedDataSet.InternalUpdateErrorHandler(GetSender, AInitiator, AException);
  if (AException <> nil) and Assigned(FOnError) then
    FOnError(GetSender, AInitiator, AException);
end;
...
begin
  if Assigned(FAdaptedDataSet) then
    FAdaptedDataSet.InternalReconcileErrorHandler(GetSender, ARow, AAction);
...
begin
  if Assigned(FAdaptedDataSet) then
    FAdaptedDataSet.InternalUpdateRecordHandler(GetSender, ARow, ARequest,
...
begin
  if FSchemaAdapter <> nil then begin
    if FTableAdapterOwned then
      FSchemaAdapter.FDAptSchemaAdapter.TableAdapters.Remove(FTableAdapterIntf);
    SetTableAdapterIntf(FSchemaAdapter.FDAptSchemaAdapter.TableAdapters.Lookup(AValue, nkID), False);
    if FTableAdapterIntf = nil then begin
...
begin
  if FSchemaAdapter <> nil then begin
    if FTableAdapterOwned then
      FSchemaAdapter.FDAptSchemaAdapter.TableAdapters.Remove(FTableAdapterIntf);
    SetTableAdapterIntf(FSchemaAdapter.FDAptSchemaAdapter.TableAdapters.Lookup(AValue, nkSource), False);
    if FTableAdapterIntf = nil then begin
...
begin
  ...
  for i := 0 to Count - 1 do
    TableAdapters[i].UpdateAdapterCmds([arInsert, arUpdate, arDelete, arLock,
                                        arUnlock, arFetchRow]);
  Result := FDAptSchemaAdapter.Update(AMaxErrors);
  DoAfterApplyUpdate(Result);
  if FDAptSchemaAdapter.DatSManager.HasErrors then
    Reconcile;
end;
...
{-------------------------------------------------------------------------------}
type
  TFDDefaultLocalSQLAdapter = class (TInterfacedObject, IFDPhysLocalQueryAdapter)
  private
    FDataSet: TDataSet;
  protected
    // IFDPhysLocalQueryAdapter
...
{-------------------------------------------------------------------------------}
constructor TFDDefaultLocalSQLAdapter.Create(ADataSet: TDataSet);
begin
  inherited Create;
  FDataSet := ADataSet;
end;
...
{-------------------------------------------------------------------------------}
function TFDDefaultLocalSQLAdapter.GetConn: TObject;
begin
  // nothing
  Result := nil;
end;
...
{-------------------------------------------------------------------------------}
function TFDDefaultLocalSQLAdapter.GetFeatures: TFDPhysLocalQueryAdapterFeatures;
begin
  // nothing
  Result := [];
end;
...
{-------------------------------------------------------------------------------}
function TFDDefaultLocalSQLAdapter.GetSavePoint: Integer;
begin
  // nothing
  Result := 0;
end;
...
{-------------------------------------------------------------------------------}
procedure TFDDefaultLocalSQLAdapter.SetSavePoint(const AValue: Integer);
begin
  // nothing
end;
...
begin
  if ASource is TFDLocalSQLDataSet then begin
    ...
    FAdapter := TFDLocalSQLDataSet(ASource).FAdapter;
    CheckDefaultAdapter;
    Changed;
  end
  else
...
begin
  Result := (ActualName <> '') and (FAdapter <> nil) and
...
{-------------------------------------------------------------------------------}
procedure TFDLocalSQLDataSet.CheckDefaultAdapter;
begin
  if (FAdapter = nil) and (FDataSet <> nil) then
    if not Supports(FDataSet, IFDPhysLocalQueryAdapter, FAdapter) then
      FAdapter := IFDPhysLocalQueryAdapter(TFDDefaultLocalSQLAdapter.Create(FDataSet));
end;

{-------------------------------------------------------------------------------}
procedure TFDLocalSQLDataSet.SetAdapter(const AValue: IFDPhysLocalQueryAdapter);
begin
  if FAdapter <> AValue then begin
    Changing;
    FAdapter := AValue;
    if FAdapter <> nil then
      FDataSet := TDataSet(FAdapter.DataSet)
    else
      CheckDefaultAdapter;
    Changed;
  end;
...
{-------------------------------------------------------------------------------}
function TFDLocalSQLDataSets.Add(ADataSet: TDataSet; const AName: String;
  const AAdapter: IFDPhysLocalQueryAdapter): TFDLocalSQLDataSet;
begin
  Result := Add;
  Result.FDataSet := ADataSet;
  Result.FName := AName;
  Result.FAdapter := AAdapter;
  if ADataSet <> nil then
    ADataSet.FreeNotification(FOwner);
  Result.CheckDefaultAdapter;
  Result.Changed;
end;
...
var
  ...
  sName: String;
begin
  if AItem.ActualName = '' then
    Exit;
  for i := 0 to Count - 1 do
    if (Items[i] <> AItem) and
       (CompareText(Items[i].ActualName, AItem.ActualName) = 0) then begin
      if FOwner <> nil then
        sName := FOwner.Name
      else
        sName := '';
      FDException(Self, [S_FD_LComp, S_FD_LComp_PClnt], er_FD_ClntDSNameNotUnique,
        [AItem.ActualName, sName]);
    end;
end;
...
{-------------------------------------------------------------------------------}
function TFDCustomLocalSQL.FindDataSet(const ASchema, AName: String): IFDPhysLocalQueryAdapter;
var
  i: Integer;
...
begin
  ...
  for i := 0 to DataSets.Count - 1 do begin
    oDS := DataSets[i];
    if oDS.IsValid and (CompareText(oDS.ActualName, AName) = 0) then begin
      Result := oDS.Adapter;
      Break;
    end;
  end;
...
{-------------------------------------------------------------------------------}
function TFDCustomLocalSQL.DataSetNotFound(const AName: String): Boolean;
var
  oDS: TDataSet;
  oAdapt: IFDPhysLocalQueryAdapter;
begin
  Result := False;
  if csDestroying in ComponentState then
    Exit;
  if Assigned(OnGetDataSet) and (FindDataSet('', AName) = nil) then begin
    oDS := nil;
    oAdapt := nil;
    OnGetDataSet(Self, AName, oDS, oAdapt);
    if (oDS <> nil) or (oAdapt <> nil) then begin
      DataSets.Add(oDS, AName, oAdapt).Temporary := True;
      Result := True;
    end;
  end;
end;
...
{-------------------------------------------------------------------------------}
procedure TFDCustomLocalSQL.ReadDataSet(const ASchema, AName: String;
  ADataSet: TObject);
begin
  if csDestroying in ComponentState then
    Exit;
  if Assigned(OnReadDataSet) then
    OnReadDataSet(Self, AName, TDataSet(ADataSet));
end;
...
begin
  if (AAdapter <> nil) and (AAdapter.FAdaptedDataSet <> nil) then
    FDException(Self, [S_FD_LComp, S_FD_LComp_PClnt], er_FD_ClntCantShareAdapt,
      [AAdapter.Name, GetDisplayName, AAdapter.FAdaptedDataSet.GetDisplayName]);
...
{-------------------------------------------------------------------------------}
function TFDAdaptedDataSet.GetConn: TObject;
begin
  Result := PointedConnection;
end;
...
{-------------------------------------------------------------------------------}
procedure TFDCustomMemTable.GetParentOptions(out AOpts: IFDStanOptions);
begin
  if Command <> nil then
...
{-------------------------------------------------------------------------------}
function TFDCustomMemTable.SaveToDFM: Boolean;
begin
  Result := (DataSetField = nil) and (ParentDataSet = nil) and (DoGetDatSManager <> nil) and
    Active and (FStorage = nil) and ResourceOptions.Persistent and (ResourceOptions.PersistentFileName = '');
end;

{-------------------------------------------------------------------------------}
function TFDCustomMemTable.GetCommandText: UnicodeString;
begin
  if Command <> nil then
    Result := Command.CommandText.Text
  else
...
{-------------------------------------------------------------------------------}
procedure TFDCustomMemTable.SetCommandText(const AValue: UnicodeString);
begin
  if Command <> nil then
...
var
  oStr: TStringStream;
begin
  oStr := TStringStream.Create(AValue);
  try
...
{-------------------------------------------------------------------------------}
function TFDRdbmsDataSet.GetMacrosCount: Word;
begin
...
begin
  ...
  try
    ...
    // Text has a SQL command with all limitations applied.
    oQuery.SQL.Text := oGen.GenerateCountSelect(Text);
...
begin
  ...
  // Setup cursor kind
  if CachedUpdates or FetchOptions.Unidirectional or Filtered and Assigned(OnFilterRecord) then
    FClientCursor := True
  else
...
begin
  ...
  try
    ...
    FTableParams.FNullLocation := oConnMeta.NullCollation;
  finally
    Command.ReleaseConnection(oConn);
  end;
end;
...
begin
  if caBlobData in ACol.Attributes then
    Result := SizeOf(Word) + FormatOptions.InlineDataSize
  else if ACol.DataType in C_FD_StrTypes then
    Result := ACol.StorageSize
  else
    Result := SizeOf(Word) + ACol.StorageSize;
end;
...
begin
  ...
  Result := LongWord(SizeOf(LongWord) +    // Str length
    Length(sIndexFields) * SizeOf(Char)) + // str
    Result;                                // data buffer
end;
...
begin
  ...
  PLongWord(pBuff)^ := Length(sIndexFields) * SizeOf(Char);
  // index fields
  pBuff := PByte(pBuff) + SizeOf(LongWord);
  Move(sIndexFields[1], pBuff^, Length(sIndexFields) * SizeOf(Char));
  pBuff := PByte(pBuff) + Length(sIndexFields) * SizeOf(Char);
  ...
  while i <= Length(sIndexFields) do begin
    ...
    iBuffLen := GetBookmarkColSize(oCol) - SizeOf(Word);
    pData := PByte(pBuff) + SizeOf(Word);
    if PFDBookmarkData(Data)^.FRow.GetData(oCol.Index, rvDefault, pData,
                                           iBuffLen, iDataLen, True) then
      PWord(pBuff)^ := iDataLen
    else
      PWord(pBuff)^ := $FFFF;
    pBuff := PByte(pBuff) + SizeOf(Word) + iBuffLen;
  end;
end;
...
begin
  ...
  if ABookmark <> nil then begin
    ...
    iDataLen := PLongWord(pBuff)^;
    pBuff := PByte(pBuff) + SizeOf(LongWord);
    SetString(Result, PChar(pBuff), iDataLen div SizeOf(Char));
    ADataPtr := PByte(pBuff) + iDataLen;
  end;
end;
...
begin
  ...
  try
    ...
    while i <= Length(sIndexFields) do begin
      ...
      iDataLen := PWord(pBuff)^;
      if iDataLen = $FFFF then
        oRow.SetData(oCol.Index, nil, 0)
      else
        oRow.SetData(oCol.Index, PByte(pBuff) + SizeOf(Word), iDataLen);
      pBuff := PByte(pBuff) + iBuffLen;
    end;
    if APKOnly then
      Result := oRow.GetValues(FPrimaryKeyFields, True)
    else
      Result := oRow.GetValues(sIndexFields, True);
  finally
    FDFree(oRow);
  end;
end;
...
begin
  ...
  try
    if AExpression = '' then begin
      Result := FDMergeFieldNames(Result, FPrimaryKeyFields);

      // Set Null location. Null location on server and client MUST BE THE SAME.
      // Without it data window WILL NOT WORK PROPERLY
      oConnMeta := oConn.GetConnectionMetadataIntf;
      eLoc := oConnMeta.NullCollation;
...
begin
  if UpdateCursorKind() then begin
    ...
    oRow := GetRow();
    if oRow <> nil then
      vKeyValues := oRow.GetValues(FPrimaryKeyFields, True);
    Table.Clear;
    First;
...
var
  ...
  function NormName(const AName: Variant): String;
  begin
    Result := oConnMeta.QuoteObjName(oConn.CnvMetaValue(AName));
  end;
        ...
        try
          for i := 0 to oVInds.Rows.Count - 1 do begin
            ...
            oVIndFlds := oConnMeta.GetTableIndexFields(
              oConn.CnvMetaValue(oRow.GetData('CATALOG_NAME')),
              oConn.CnvMetaValue(oRow.GetData('SCHEMA_NAME')),
              NormName(oRow.GetData('TABLE_NAME')),
              NormName(oRow.GetData('INDEX_NAME')), '');
            try
      ...
      if IsLinkedDetail then begin
        ...
        if not FClientCursor then
          for i := 0 to MasterLink.Fields.Count - 1 do
            if TField(MasterLink.Fields[i]).IsNull then
              sMastNullFields := sMastNullFields + ';' + TField(MasterLink.Fields[i]).FieldName;
      end;
      ...
      // all other params must have WINDOW_ prefix or does not have a prefix for master/detail
        ...
        tcLocate:
          ...
          if SourceView.Rows.Count > 0 then begin
            ...
            else if (FRecordIndex > -1) and (FRecordIndex < SourceView.Rows.Count) then
              // use current row
...
begin
  ...
  Result := -1;
  if not FetchOptions.LiveWindowParanoic or
...
{-------------------------------------------------------------------------------}
function TFDTable.GetRecordCount: Longint;
var
  oTable: TFDDatSTable;
  iFetched: Integer;
begin
