...
{$IFDEF MSWINDOWS}
  Winapi.Windows,
{$ENDIF}
 Web.BrkrConst, Web.WebConst, System.Generics.Collections, System.Types;
  ...
  try
    ...
    try
      ...
      if WebAppServices = nil then
        if WebModules.ItemCount = 1 then
        begin
          WebModule := WebModules[0];
          for I := 0 to WebModule.ComponentCount - 1 do
          begin
            LComponent := WebModule.Components[I];
            if Supports(LComponent, IWebAppServices, WebAppServices) then
              if WebAppServices.Active then
                break
              else
                WebAppServices := nil;
          end;
