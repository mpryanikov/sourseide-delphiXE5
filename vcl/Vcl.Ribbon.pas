...
type
  ...
  private
    ...
    FStatusBarFontColor: TColor;
    FStatusBarHotFontColor: TColor;
  public
    ...
    property StatusBarFontColor: TColor read FStatusBarFontColor write FStatusBarFontColor;
    property StatusBarHotFontColor: TColor read FStatusBarHotFontColor write FStatusBarHotFontColor;
  end;
  ...
  published
    ...
    property StatusBarFontColor default clBlack;
    property StatusBarHotFontColor default clWhite;
  end;
  ...
  published
    ...
    property StatusBarFontColor default clWhite;
    property StatusBarHotFontColor default clBlack;
  end;
...
{$ENDIF}
  ...
  private
    ...
    procedure RibbonCustomizeCloseHandler(Sender: TObject);
  protected
    procedure CMFontchanged(var Message: TMessage); message CM_FONTCHANGED;
  ...
  private
    ...
    FOnTabVisibleChanged: TTabVisibleChangedEvent;
  ...
  protected
    ...
    procedure DoTabVisibleChanged(const AIndex: Integer; const AVisible: Boolean);
  ...
  public
    ...
    property OnTabVisibleChanged: TTabVisibleChangedEvent read FOnTabVisibleChanged write FOnTabVisibleChanged;
  end;
...
begin
  ...
  begin
    LCalcRect := Rect;
    LFlags := DT_CALCRECT or DT_CENTER or DT_WORDBREAK or DT_NOPREFIX;
...
begin
  ...
  try
    ...
    try
      ...
      SetBounds(Left, Top, Max(LRect.Right - LRect.Left + 13, 42), Height);
      LCanvas.Handle := 0;
    finally
      ReleaseDC(0, LDC);
    end;
...
// Returns the two lines required to be painted for a multiline caption
function GetLinesToPaint(const ALine: Integer; ACanvas: TCanvas; const AFullCaption: string; const AWidth: Integer): TStringDynArray;
var
  SL: TStringList;
  S: string;
  I, J: Integer;
  LLine: Integer;
begin
  // This method should only called when a multiline caption needs to be drawn
  SetLength(Result, 2);
  SL := TStringList.Create;
  try
    SL.StrictDelimiter := True;
    SL.Delimiter := ' ';
    SL.DelimitedText := AFullCaption;
    I := 0;
    S := '';
    LLine := 0;
    while I < SL.Count do
    begin
      S := TrimLeft(S + ' ' + SL.Strings[I]);
      J := ACanvas.TextWidth(S);
      if J <= AWidth then
      begin
        Result[LLine] := S;
      end
    ...
    // If this line is false, then there is an issue!
    if LLine = high(Result) then
      Result[LLine] := S;
  finally
    SL.Free;
  end;
...
var
  ...
  LLineHeight: Integer;
  LMultiLine: Boolean;
  LCaption: TStringDynArray;
  LTargetWidth: Integer;
  LTextWidth: Integer;
  LUseTextWidth: Boolean;
...
begin
  ...
  LRect := Rect(2, Height div 2, Width - 2, Height);
  LCalcRect := Rect(6, 0, Width - 7, 0);
  Canvas.Brush.Style := bsClear;
  LFlags := DT_CENTER or DT_NOPREFIX or DT_WORDBREAK;
  if BiDiMode <> bdLeftToRight then // For Bidimode support
  ...
  // Initialise variables to keep the compiler happy
  LTargetWidth := 0;
  LUseTextWidth := False;
  LTextWidth := 0;
  if LMultiLine then
  begin
    // To draw the multiline accurately, each line must be drawn separately
    LCaption := GetLinesToPaint(0, Canvas, Caption, Width - 13);
    // The first line is always draw centered, this is because it has no arrow to worry about
    DrawText(Canvas.Handle, LCaption[0], -1, LRect, DT_NOPREFIX or DT_SINGLELINE or DT_CENTER);
    // Increase the top position of the draw rect
    Inc(LRect.Top, Canvas.TextHeight(LCaption[0]));
    // Since this line requires drawing the caption, with line must be extended by the width
    // of the arrow, plus the spacing between the arrow and the last letter
    LTargetWidth := Canvas.TextWidth(LCaption[1]) + 9;
    // Store the original width of the caption to be used for accurate arrow placement
    LTextWidth := Canvas.TextWidth(LCaption[1]);
    // Increase the line to draw up to the target width. This is required for centered captions
    // to look correctly
    while Canvas.TextWidth(LCaption[1]) < LTargetWidth do
      LCaption[1] := LCaption[1] + ' ';
    // if the Width of the caption is less than the border (2 * 2) and the arrow size and spacing (9)
    // then this line can be drawn centered. This means that the first line is the longer of the
    // two lines being drawn
    if Canvas.TextWidth(LCaption[1]) < Width - 13 then
      DrawText(Canvas.Handle, LCaption[1], -1, LRect, DT_NOPREFIX or DT_SINGLELINE or DT_CENTER)
    else
    begin
      LUseTextWidth := True;
      DrawText(Canvas.Handle, LCaption[1], -1, LRect, DT_NOPREFIX or DT_SINGLELINE or DT_LEFT);
    end;
  end
  else
  ...
  LX := (Width div 2) - 3;
  LY := (Height div 2) + LLineHeight + 2;
  if LMultiLine then
  begin
    // If LUseTextWidth is true, then the second line was drawn using DT_LEFT
    if LUseTextWidth then
      LX := LTextWidth + 2 {border} + 3 {spacing}
    else
      LX := (Width div 2) + (LTargetWidth div 2) - 6 {arrow width};
  end;
  LPt := Point(LX, LY + 3);
  // Note: There is a bug in the Obsidian skin, this arrow should have a white shadow, but it is clear
...
procedure TCustomRibbon.DoTabVisibleChanged(const AIndex: Integer; const AVisible: Boolean);
var
  LCurrentPage: TCustomRibbonPage;
begin
  if not AVisible and (ActivePage.Index = AIndex) then
  begin
    Tabs.SelectPrior;
    if ActivePage = nil then
      Tabs.SelectNext;
  ...
  Invalidate;
  LCurrentPage := ActivePage;
  if LCurrentPage <> nil then
    LCurrentPage.Invalidate;
  RebuildRibbonMetrics;
  if Assigned(FOnTabVisibleChanged) then
    FOnTabVisibleChanged(Self, AIndex, AVisible);
end;
...
var
  ...
  begin
    ...
    DrawText(Canvas.Handle,Format(CaptionFmt[LDrawSeparator], [Caption]),
      -1, LCalcRect, TextFlags or DT_CALCRECT);
  ...
  begin
    ...
    GetThemeTextExtent(Theme, canvas.Handle, Parts[FParentForm.WindowState], States[FParentForm.Active],
      PWideChar(Format(GlassCaptionFmt[LDrawSeparator], [DocumentName, Caption])),
...
var
  ...
  HThemeWindow : HTHEME;
  DrawTextOpts: DTTOPTS;
  ThemedCaptionFont: TLogFont;
begin
  LDrawSeparator := (DocumentName <> '') and (Caption <> '');
...
//          -1, TextFlags or DT_END_ELLIPSIS, 0, LRect );
        ...
        // Set glow size
        FillChar (DrawTextOpts, SizeOf(DTTOPTS), 0);
        DrawTextOpts.dwSize := SizeOf(DTTOPTS);
        DrawTextOpts.dwFlags := DTT_COMPOSITED or DTT_GLOWSIZE;
        DrawTextOpts.iGlowSize := 15;

        // Select themed font
        if (FParentForm <> nil) and (FParentForm.WindowState <> wsMaximized) then
          LRect.Top := 0;
        if Succeeded (GetThemeSysFont (HThemeWindow, TMT_CAPTIONFONT, ThemedCaptionFont)) then
          Canvas.Font.Handle := CreateFontIndirect(ThemedCaptionFont);

        DrawThemeTextEx(HThemeWindow, Canvas.Handle, Parts[FParentForm.WindowState], States[FParentForm.Active],
          PWideChar(Format(GlassCaptionFmt[LDrawSeparator], [DocumentName, Caption])), -1, TextFlags or DT_END_ELLIPSIS,
          LRect, DrawTextOpts);
      finally
...
begin
  ...
  begin
    LRect := GetTabRect(0);
    Dec(LRect.Top);
    LRect.Left := cRibbonFirstTabOffSet - 2;
    LRect.Right := Width;
    InvalidateRect(Handle, LRect, True);
  end;
end;
...
begin
  if not Minimized and not KeyTipsVisible and
    (FParentForm.Active or ((FParentForm is TForm) and (TForm(FParentForm).FormStyle = fsMDIForm))) then
  begin
    LAllowChange := TabIndex < Tabs.Count - 1;
    if LAllowChange then
    begin
      DoTabChange(TabIndex + 1, TabIndex, LAllowChange);
      if LAllowChange then
        Tabs.SelectNext;
    end;
...
begin
  inherited;
  if Active and Enabled and not HideTabs then
  begin
...
begin
  ...
  begin
    if AComponent = FActionManager then
      FActionManager := nil
    else if HasQuickAccessToolbar and (AComponent = FQuickAccessToolbar.ActionBar) then
    begin
      ...
      FColorMap := nil
    else if (FApplicationMenu <> nil) and (AComponent = FApplicationMenu.Menu) then
...
begin
  ...
  try
    ...
    if Active then
      ...
      Style.DrawElement(sfBottomFrameDisabled, LCanvas, LRect, -1)
  finally
    LCanvas.Handle := 0;
    LCanvas.Free;
  end;
...
begin
  if (csLoading in ComponentState) or not HasQuickAccessToolbar or HideRibbon then
    Exit;
...
begin
  Result := (Index > -1) and (Index < Tabs.Count) and (Tabs[Index] <> nil) and
    (Tabs[Index].Visible or (csDesigning in ComponentState));
end;
...
begin
  Result := (QuickAccessToolbar <> nil) and (QuickAccessToolbar.ActionBar <> nil) and not HideRibbon;
  if Result then
  begin
...
begin
  inherited;
  AllowKeyboardNavigation := False;
  if Minimized and not KeyTipsVisible and FParentForm.Active then
  begin
    if ActivePage <> nil then
...
begin
  ...
  begin
    ...
    begin
      FParentForm.WindowProc := FParentWndProc;
      FParentWndProc := nil;
      FParentForm.GlassFrame.Enabled := False;
    end;
...
begin
  ...
  if (Value > -1) and not Tabs[Value].Visible then
    Exit; // Don't allow switching to an invisible tab
...
begin
  ...
  begin
    if (ControlCount > 0) and (Controls[0] is TRibbonGroup) then
      Result := TRibbonGroup(Controls[0]).RibbonStyle
    else
      Result := RibbonLunaStyle;
  end;
end;
...
function TOffice2007Button.GetGraphicClass: TGraphicClass;
begin
  Result := TBitmap;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      FCustomizeDlg.OnClose := RibbonCustomizeCloseHandler;
    end;
    FCustomizeDlg.Show;
  end;
end;
...
procedure TCustomRibbonQuickAccessToolbar.RibbonCustomizeCloseHandler(Sender: TObject);
var
  I: Integer;
begin
  for I := 0 to Ribbon.ActionManager.ActionCount - 1 do
  begin
    TCustomAction(Ribbon.ActionManager.Actions[I]).Enabled := False;
    TCustomAction(Ribbon.ActionManager.Actions[I]).Enabled := True;
  end;
  Ribbon.Invalidate;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      FScrollBtn.Top := FScrollBtn.Top + 2;
      FScrollBtn.Left := GetBarWidth - FScrollBtn.Width;
    end
...
begin
  ...
  if Result then
  begin
    LIdx := Ribbon.TabIndex + 1;
    LAllowed := False;
    while LIdx < Count do
    begin
      if Ribbon.ValidTab(LIdx) then
      begin
        LAllowed := True;
        Break;
      end;
...
constructor TRibbonTabItem.Create(Collection: TCollection);
begin
  inherited;
  FVisible := True;
end;
...
type
  TCustomRibbonAccess = class(TCustomRibbon)

  end;

procedure TRibbonTabItem.SetVisible(const Value: Boolean);
begin
  if FVisible <> Value then
  begin
    FVisible := Value;
    TCustomRibbonAccess(FPage.Ribbon).DoTabVisibleChanged(Index, Value);
  end;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (LTop > (Height - GetCaptionHeight)) or (Rows = 1) then
        begin
          LTop := FSpacingVert + FRowStart;
          Inc(LLeft, LPrevWidth + FSpacingHorz);
        end
...
begin
  ...
  begin
    ...
    for ActionClientitem in TCustomRibbonGroup(Source).ActionClient.Items do
      if (TActionClientItem(ActionClientItem).CommandStyle in [csControl, csComboBox]) and
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            if (Items[I].CommandProperties is TButtonProperties) and
              (TButtonProperties(Items[I].CommandProperties).ButtonSize = bsLarge) then
...
var
  ...
  begin
    ...
    begin
      if TItemData(FItems[I]).Tab.Visible then
      begin
...
begin
  ...
  StatusBarFontColor := clBlack;
  StatusBarHotFontColor := clWhite;
end;
...
begin
  ...
  StatusBarFontColor := clWhite;
  StatusBarHotFontColor := clBlack;
end;
