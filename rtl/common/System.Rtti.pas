...
uses
  System.Variants, System.TypInfo, System.SysUtils,
...
{$IFNDEF NEXTGEN}
    ...
    tkLString: AResult := TValue.From<AnsiString>(AnsiChar(value));
    tkWString: AResult := TValue.From<WideString>(WideString(AnsiChar(value)));
    tkUString: AResult := TValue.From<UnicodeString>(UnicodeString(AnsiChar(value)));
{$ELSE NEXTGEN}
                                                                       
    tkUString: AResult := TValue.From<UnicodeString>(UnicodeString(Char(value)));
{$ENDIF NEXTGEN}
  else
    Exit(False);
  end;
  ...
  case ATarget^.Kind of
    ...
    begin
      ...
      if Length(asAnsi) <> 1 then
        Exit(False);
      TValue.Make(PAnsiChar(asAnsi), ATarget, AResult);
    end;
...
begin
  ...
  case TVarData(v).VType of
    ...
    varLongWord: TempVal := TVarData(v).VLongWord;
...
{$ELSE NEXTGEN}
    ...
    var
      ...
      buff: TBytes;
    begin
      // Is the string stored in UTF8? We assume Yes!
      resLen := ReadU16(Data);
      SetLength(buff, resLen);
      Move(Data^, Buff[0], resLen);
      Result := TEncoding.UTF8.GetString(buff);
      Inc(Data, resLen);
      Dec(Len, resLen + SizeOf(Word));
    end;
{$ENDIF !NEXTGEN}
...
{$ELSE NEXTGEN}
        tkUString:
          Result[i] := TValue.From<string>(ReadString).Cast(Params[i].ParamType.Handle);
{$ENDIF !NEXTGEN}
      else
...
{ Raw invocation logic -- 3 CPU based versions follow -- }
{$IF Defined(CPUX86)}
type
  TFloatReg = record
    case TFloatType of
...
{$ELSEIF Defined(CPUX64)} // Raw invocation logic ( 2 of 3 )
...
begin
  ...
  if (CallingConvention <> ccSafeCall) and IsStatic and UseResultPointer(AResultType, IsConstructor) then
  begin
    TValue.Make(nil, AResultType, Result);
    PutRefArg(Result.GetReferenceToRawData);
  end;
  ...
  if (CallingConvention <> ccSafeCall) and not IsStatic and UseResultPointer(AResultType, IsConstructor) then
  begin
    TValue.Make(nil, AResultType, Result);
    PutRefArg(Result.GetReferenceToRawData);
  end;
...
{$ELSEIF Defined(CPUARM)} // Raw invocation logic ( 3 of 3 )
...
{$ENDIF !IOS}
  ...
  begin
    ...
    else if (Arg.GetTypeKind = tkRecord) then
    begin
...
begin
  ...
  FParamList.Free;
  FParamList := nil;
end; // of CPUX64 TMethodImplementation.TInvokeInfo.Seal implementation
...
var
  ...
  begin
    ...
    begin
      ...
      begin
        if Param.FTypeInfo.Kind = tkFloat then
        begin
          if dataSize = 4 then // Single
          begin
            reg := AllocReg(freeFPRegs);
        ...
        else if Param.FTypeInfo.Kind = tkRecord then
        begin
...
{$ELSEIF Defined(CPUARM)}
     cdecl; external 'librtlhelper.a' name 'allocate_first_stage_intercept';
{$ELSE OTHERCPU}
  {$MESSAGE Fatal 'Method not implemented for CPU'}
{$ENDIF OTHERCPU}
...
{$ELSEIF Defined(CPUARM)}
     cdecl; external 'librtlhelper.a' name 'free_first_stage_intercept';
{$ELSE OTHERCPU}
  {$MESSAGE Fatal 'Method not implemented for CPU'}
{$ENDIF OTHERCPU}
  ...
  try
    ...
    begin
      try
        ...
        begin
{$IF     defined(CPUX86)}
          if   ((ii.Method.CallingConvention in [ccCdecl, ccStdCall, ccSafeCall]) and (pfConst in parList[i-1].Flags) and (parList[i-1].ParamType.TypeKind = tkVariant))
            or ((pfConst in parList[i - 1].Flags) and (parList[i - 1].ParamType.TypeSize > SizeOf(Pointer)))
            or ([pfVar, pfOut] * parList[i - 1].Flags <> []) then
{$ELSEIF defined(CPUARM) }
          if   ((ii.Method.CallingConvention in [ccReg]) and (parList[i - 1].ParamType.TYpeKind = tkRecord))
            or ([pfVar, pfOut] * parList[i - 1].Flags <> []) then
{$ELSE}
