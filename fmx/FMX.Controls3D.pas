...
{ TControl3D }
  ...
  protected
    ...
    { ray casting }
    function DoRayCastIntersect(const RayPos, RayDir: TVector3D; var Intersection: TVector3D): Boolean; virtual;
    { IControl }
  ...
  public
    ...
    property IsChildFocused: boolean read GetIsChildFocused;
...
uses
  System.Math,  System.TypInfo, System.RTLConsts, FMX.Platform, FMX.Materials, FMX.Forms, FMX.Effects;
...
begin
  ...
  begin
    if Assigned(FParent) and (FParent is TControl3D) then
...
function TControl3D.GetIsChildFocused: boolean;
begin
  Result := FIsChildFocused;
end;

procedure TControl3D.SetIsChildFocused(const Value: boolean);
begin
    ...
    if Assigned(Children) then
      ...
      begin
        ...
        begin
          ...
          if esDisablePaint in TEffect(Children[I]).EffectStyle then
            FAbsoluteHasDisablePaintEffect := True;
          if esAfterPaint in TEffect(Children[I]).EffectStyle then
...
procedure TControl3D.AddToTabList(const AObject: TFmxObject);
begin
...
begin
  FViewport := AViewport;
  if Assigned(Children) and (Children.Count > 0) then
    for i := 0 to Children.Count - 1 do
...
function TControl3D.DoRayCastIntersect(const RayPos, RayDir: TVector3D; var Intersection: TVector3D): Boolean;
var
...
begin
  ...
  if (not Assigned(Result)) and Assigned(Context) and Visible and (GlobalProjection = Projection) and CheckHitTest(HitTest) then
  begin
    if Assigned(FViewport) then
      P := FViewport.ScreenToLocal(P);
    Context.Pick(P.X, P.Y, FProjection, rPos, rDir);
    if RayCastIntersect(AbsoluteToLocalVector(rPos), (AbsoluteToLocalVector(rDir)).Normalize, IP) then
    begin
      if (Projection = TProjection.pjScreen) and ((IP-rPos).Length < GlobalDistance) then
      begin
        GlobalDistance := (IP-rPos).Length;
        Result := Self;
      end;
      if (Projection = TProjection.pjCamera) and Assigned(FViewport.CurrentCamera) and
        ((IP - FViewport.CurrentCamera.AbsolutePosition).Length < GlobalDistance) then
      begin
        GlobalDistance := (IP - FViewport.CurrentCamera.AbsolutePosition).Length;
        Result := Self;
      end;
    end;
  end;
...
begin
  ...
  if (Not Assigned(Result)) and Assigned(Context) then
  begin
    if Assigned(FViewport) then
      P := FViewport.ScreenToLocal(P);
    Context.Pick(P.X, P.Y, FProjection, rPos, rDir);
    if CheckHitTest(HitTest) and RayCastIntersect(AbsoluteToLocalVector(rPos),
      (AbsoluteToLocalVector(rDir)).Normalize, IP)
    and (GlobalProjection = Projection) then
    begin
      if (Projection = TProjection.pjScreen) and ((IP-rPos).Length < GlobalDistance) then
      begin
        GlobalDistance := (IP-rPos).Length;
...
begin
  ...
  if HasDisablePaintEffect and not FInRenderTo then
    ApplyEffect;
  {$IFDEF MSWINDOWS}
  if not FInRenderTo and Assigned(FRoot) and (FRoot.GetObject is TCommonCustomForm) and
...
begin
  ...
  begin
    Context.SetMatrix(AbsoluteMatrix);
    Context.SetContextState(TContextState.csZWriteOn);
    Context.FillCube(NullVector3D, Vector3D(Width + 0.01, Height + 0.01, Depth + 0.01), 0.4, $B2005ACC);
  end;
end;
  ...
  begin
    for I := 0 to Children.Count - 1 do
      ...
      begin
        ...
        if not Assigned(FEffectTexture) then
        begin
          FEffectTexture := TTexture.Create;
          FEffectTexture.Style := [TTextureStyle.tsRenderTarget];
        ...
        begin
          ...
          try
            FEffectContext.Clear([TClearTarget.ctColor, TClearTarget.ctDepth], 0, 1.0, 0);
...
function TControl3D.AsIControl: IControl;
begin
  Result := Self;
end;
...
begin
  ...
  begin
    ...
    try
      ...
      if B.Map(TMapAccess.maRead, M) then
      try
  ...
  begin
    ...
    try
      ...
      Texture.Style := [TTextureStyle.tsRenderTarget];
...
begin
  ...
  try
    ...
    BitmapContext := TContextManager.CreateFromTexture(Texture, TMultisample.msNone, True);
    if Assigned(FViewport) and Assigned(FViewport.Context) then
    begin
      S := TMatrix3D.Identity;
...
begin
  ...
  begin
    ...
    begin
      ...
      TOpenObject(Parent).FTabList.Insert(Value, AsIControl);
    end;
  end;
end;
...
begin
  ...
  RayDir := (AbsoluteToLocalVector(RayDir)).Normalize;
...
function TControl3D.IsIControl: Boolean;
begin
  Result := True;
end;
...
begin
  ...
  if (csDesigning in ComponentState) then
  begin
    Context.FillCube(Vector3D(0, 0,   0), Vector3D(0.8, 0.8, 0.8), 1, $FF60A799);
    Context.FillCube(Vector3D(0, 0, 0.5), Vector3D(0.3, 0.3, 1.4), 1, $FF9C60A7);
    Context.DrawLine(Vector3D(0, 0,   0), Vector3D(0, 0, 1) * 1000, 1, $FF9C60A7);
  end;
end;

function TCamera.DoRayCastIntersect(const RayPos, RayDir: TVector3D; var Intersection: TVector3D): Boolean;
begin
  Result := False;
  if (csDesigning in ComponentState) then
    Result := inherited;
...
begin
  ...
  begin
    Result := TMatrix3D.CreateLookAtDirRH(AbsolutePosition.ToPoint3D, (AbsolutePosition
      - Target.AbsolutePosition).ToPoint3D, - AbsoluteUp.ToPoint3D);
  end
  else
    Result := TMatrix3D.CreateLookAtDirRH(AbsolutePosition.ToPoint3D, - AbsoluteDirection.ToPoint3D,
      - AbsoluteUp.ToPoint3D);
end;
...
begin
  ...
  begin
    if not Enabled then
    begin
      Context.FillCube(NullVector3D, Vector3D(Width, Height, Depth) * 0.8, 1, TAlphaColors.Gray);
      Context.DrawCube(NullVector3D, Vector3D(Width, Height, Depth) * 0.8, 1, TAlphaColors.Navy);
    end
    else
    begin
      Context.FillCube(NullVector3D, Vector3D(Width, Height, Depth) * 0.8, 1, TAlphaColors.Yellow);
      Context.DrawCube(NullVector3D, Vector3D(Width, Height, Depth) * 0.8, 1, TAlphaColors.Navy);
      case LightType of
        TLightType.ltDirectional:
          begin
            Context.DrawLine(NullVector3D, Vector3D(0, 0, 1) * (Width * 5), 1, TAlphaColors.Navy);
          end;
        TLightType.ltPoint:
          begin
            for i := 1 to 18 do
              Context.DrawLine(NullVector3D,
                Vector3D(cos(DegToRad(i * 20)), sin(DegToRad(i * 20)), 0) * (Width * 2), 1, TAlphaColors.Navy);
            for i := 1 to 18 do
              Context.DrawLine(NullVector3D, Vector3D(cos(DegToRad(i * 20)), 0,
                sin(DegToRad(i * 20))) * (Width * 2), 1, TAlphaColors.Navy);
          end;
        TLightType.ltSpot:
          begin
            Context.DrawLine(NullVector3D, Vector3D( 0.2, -0.2, 1).Normalize * (Width * 5), 1, TAlphaColors.Navy);
            Context.DrawLine(NullVector3D, Vector3D( 0.2,  0.2, 1).Normalize * (Width * 5), 1, TAlphaColors.Navy);
            Context.DrawLine(NullVector3D, Vector3D(-0.2,  0.2, 1).Normalize * (Width * 5), 1, TAlphaColors.Navy);
            Context.DrawLine(NullVector3D, Vector3D(-0.2, -0.2, 1).Normalize * (Width * 5), 1, TAlphaColors.Navy);
          end;
...
function TLight.DoRayCastIntersect(const RayPos, RayDir: TVector3D; var Intersection: TVector3D): Boolean;
begin
  Result := False;
  if (csDesigning in ComponentState) then
    Result := inherited;
...
begin
  Result := TLightDescription.Create(Enabled, Color, LightType, SpotCutOff, SpotExponent, AbsolutePosition.ToPoint3D,
    AbsoluteDirection.ToPoint3D);
end;
...
begin
  ...
  begin
    if (Left.Opacity < 1.0) and (Right.Opacity = 1.0) then
      Result := 1
    else
      if (Left.Opacity = 1.0) and (Right.Opacity < 1.0) then
        Result := -1;
    if (Result = 0) and Assigned(Left.Context) and Assigned(Left.Context) then
    begin
      V1 := Left.AbsolutePosition - Left.Context.CurrentCameraMatrix.M[3];
      V2 := Right.AbsolutePosition - Left.Context.CurrentCameraMatrix.M[3];
...
var
  ...
  SaveS: TVector3D;
begin
  if Assigned(FSourceObject) then
  begin
