...
type
  TStyleDesignerInfo = class (TComponent)
  private
    FStyleDesigner: TStyleDesigner;
    procedure SetStyleDesigner(const Value: TStyleDesigner);
  protected
    procedure Notification(AComponent: TComponent; Operation: TOperation); override;
  public
    property StyleDesigner: TStyleDesigner read FStyleDesigner write SetStyleDesigner;
  end;

var
  StyleDesignerInfo: TStyleDesignerInfo;
...
procedure TStyleDesignerInfo.Notification(AComponent: TComponent; Operation: TOperation);
begin
  inherited;
  if Operation = opRemove then
  begin
    if FStyleDesigner = AComponent then
      FStyleDesigner := nil;
  end;
end;

procedure TStyleDesignerInfo.SetStyleDesigner(const Value: TStyleDesigner);
begin
  if FStyleDesigner <> Value then
  begin
    if Assigned(FStyleDesigner) then
      RemoveFreeNotification(FStyleDesigner);
    FStyleDesigner := Value;
    if Assigned(FStyleDesigner) then
      FreeNotification(FStyleDesigner);
  end;
end;
...
begin
  ...
  begin
    ...
    if (ObjClass <> nil) and (ObjClass.InheritsFrom(TFmxObject)) and (ObjClass.ClassName <> TStyleBook.ClassName) and
       Assigned(StyleDesignerInfo.StyleDesigner) then
    begin
      // make the child object
      NewObj := TFmxObjectClass(ObjClass).Create(StyleDesignerInfo.StyleDesigner.DesignRoot.Root.GetObject);
      NewObj.Parent := FObject;
      NewObj.StyleName := StyleDesignerInfo.StyleDesigner.Designer.UniqueName(NewObj.ClassName);
      // make the child node
      Node := TStyleStructureNode.Create(nil);
      Node.Parent := Self;
      Node.FObject := NewObj;
      Node.FCaption := NewObj.StyleName + ':' + NewObj.ClassName;
      StyleDesignerInfo.StyleDesigner.FStructureView.StructureChanged(True);
      Effect := DROPEFFECT_COPY;
    end;
    StyleDesignerInfo.StyleDesigner.ComponentDesigner.Environment.ResetCompClass;
  end
  else
  if Assigned(StyleDesignerInfo.StyleDesigner) and Assigned(StyleDesignerInfo.StyleDesigner.FDragNode) then
  begin
    Node := TStyleStructureNode(StyleDesignerInfo.StyleDesigner.FDragNode);
    // Check equal
    ...
    Effect := DROPEFFECT_COPY;
    StyleDesignerInfo.StyleDesigner.FDragNode := nil;
  end;
end;
  ...
  begin
    ...
    begin
      ...
      if (Pos('[IOSALTERNATE]', UpperCase(Desc.PlatformTarget)) > 0) and (Pos('ios', LowerCase(GetCurrentPlatform)) <> 1) then
        Result := False;
      if (Pos('[ANDROID]', UpperCase(Desc.PlatformTarget)) > 0) and (Pos('android', LowerCase(GetCurrentPlatform)) <> 1) then
        Result := False;
...
begin
  ...
  if LHost is TFrame then
    if TFrame(LHost).Visible and TFrame(LHost).HandleAllocated and TFrame(LHost).CanFocus then
      TFrame(LHost).SetFocus;
...
begin
  if Assigned(Handle) then
    ...
    raise EArgumentException.CreateFMT(SNotInstance, ['Handle']) at ReturnAddress;
end;
...
procedure TStyleDesigner.SetStyleBook(const Value: TStyleBook);
begin
...
var
  ...
  IdeHost: TFrame;
begin
  if (not FInResize) and (([TFmxFormState.fsReleased] * FormState) = []) then
  begin
    FInResize := True;
    try
...
procedure TStyleDesigner.CloseDesigner;
begin
  if not FClosed then
  begin
    FClosed := True;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Designer.SelectComponent(TStyleStructureNode(N.Parent).FObject);
      end;
...
initialization
  StyleDesignerInfo := TStyleDesignerInfo.Create(nil);

finalization
  FreeAndNil(StyleDesignerInfo);
