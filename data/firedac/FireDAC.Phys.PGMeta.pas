...
type
  ...
  public
    constructor Create(const AConnection: TFDPhysConnection;
      ...
      AColumnOriginProvided, AUseMonthsBetween: Boolean);
  end;
...
{-------------------------------------------------------------------------------}
function TFDPhysPgMetadata.GetSelectWithoutFrom: Boolean;
begin
  Result := True;
end;
...
{-------------------------------------------------------------------------------}
function TFDPhysPgMetadata.GetNullLocation: TFDPhysNullLocations;
begin
...
begin
  Result := AValue;
  // this is enum, it requires casting to enum type
  if (ACol.DataType in [dtAnsiString, dtWideString]) and (ACol.SourceDataTypeName <> '') then
...
{-------------------------------------------------------------------------------}
function TFDPhysPgCommandGenerator.GetPing: String;
begin
  Result := 'SELECT 1';
end;
...
begin
  ...
  for i := 0 to FParams.Count - 1 do begin
    ...
    if oParam.ParamType in [ptUnknown, ptInput, ptInputOutput] then begin
      if sInPar <> '' then
        sInPar := sInPar + ', ';
      if (FParams.BindMode = pbByName) and (FConnMeta.ServerVersion >= svPGSQL090000) then
  ...
  if FParams.Count = iInParIndex - 1 then begin
    ...
    if bHasCrs and (FCommandKind = skStoredProc) then
    ...
    mkPrimaryKey:
      begin
        Result := 'SELECT NULL::INTEGER AS RECNO, CURRENT_DATABASE() AS CATALOG_NAME,' +
          ...
          'CASE' +
          ' WHEN I.INDISPRIMARY THEN IC.RELNAME' +
          ' ELSE NULL ' +
