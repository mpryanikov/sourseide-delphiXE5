...
uses
  ...
  FireDAC.Stan.Consts, FireDAC.DatS, FireDAC.Stan.Util, FireDAC.Stan.Param;
...
{-------------------------------------------------------------------------------}
function TFDPhysOraMetadata.GetSelectWithoutFrom: Boolean;
begin
  Result := False;
end;
...
{-------------------------------------------------------------------------------}
function TFDPhysOraMetadata.GetNullLocation: TFDPhysNullLocations;
begin
...
{-------------------------------------------------------------------------------}
function TFDPhysOraCommandGenerator.GetPing: String;
begin
  Result := 'SELECT * FROM ' + GetSingleRowTable;
end;
...
begin
  ...
  mkForeignKeyFields:
    begin
      ...
      AddWhere('C.R_OWNER = C2.OWNER');
      AddWhere('C.R_CONSTRAINT_NAME = C2.CONSTRAINT_NAME');
      AddWhere('L.OWNER = C.OWNER');
      AddWhere('L.TABLE_NAME = C.TABLE_NAME');
      AddWhere('L.CONSTRAINT_NAME = C.CONSTRAINT_NAME');
      AddWhere('L2.OWNER = C2.OWNER');
      AddWhere('L2.TABLE_NAME = C2.TABLE_NAME');
      AddWhere('L2.CONSTRAINT_NAME = C2.CONSTRAINT_NAME');
