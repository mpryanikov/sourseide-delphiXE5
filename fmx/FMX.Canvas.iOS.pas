...
uses
  ...
  FMX.Platform.iOS, System.Character, FMX.Graphics;
...
begin
  ...
  try
    ...
    begin
      ...
      if Assigned(img) then
      try
        if SameText(ExtractFileExt(AFileName), '.jpg') or SameText(ExtractFileExt(AFileName), '.jpeg') then
        begin
          if Assigned(SaveParams) then
            data := TNSData.Wrap(UIImageJPEGRepresentation((img as ILocalObject).GetObjectID, SaveParams.Quality / 100))
          else
