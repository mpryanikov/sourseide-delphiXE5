...
{ Shaders }
  ...
  TContextShaderArch = (saUndefined, saDX9, saDX10, saGLSL, saMac, saIOS, saAndroid);
...
{ Texture }
  ...
  TTextureStyle = (tsMipMaps, tsDynamic, tsRenderTarget, tsVolatile);
...
{ Context }
  ...
  protected
    ...
    class var FMaxLightCount: Integer;
  ...
  public
    ...
    class property MaxLightCount: Integer read FMaxLightCount;
...
uses
  System.Math,  System.TypInfo, System.RTLConsts, FMX.Platform, FMX.Materials, FMX.Surfaces, FMX.Consts;
...
begin
  ...
  begin
    ...
    FFaceNormals[Index div 3] := (Term1.CrossProduct(Term2)).Normalize;
...
begin
  if not (TTextureStyle.tsVolatile in Style) then
  begin
    if FHandle <> 0 then
...
begin
  if FPixelFormat = TPixelFormat.pfUnknown then
...
begin
  ...
  begin
    if SameValue(FHeight, 0.0, Epsilon) then
      Result := TMatrix3D.CreatePerspectiveFovRH(cPI / 4, 1.0, 1.0, 1000.0)
    else
      Result := TMatrix3D.CreatePerspectiveFovRH(cPI / 4, FWidth / FHeight, 1.0, 1000.0);
...
begin
  ...
  begin
    ...
    if Assigned(FTimerService) then
...
begin
  ...
  if (FGlobalBeginSceneCount = 0) and Assigned(FTimerService) then
  begin
...
begin
  if FCurrentStates[TContextState.csScissorOn] then
    FCurrentStates[TContextState.csScissorOn] := True;
...
begin
  ...
  begin
    ...
    vPos := vPos * CurrentCameraInvMatrix;
    vNear := vNear * CurrentCameraInvMatrix;
    //
    RayPos := vPos;
    RayDir := (vPos-vNear).Normalize;
    RayDir.W := 0.0;
  end
  else
  begin
...
begin
  ...
  begin
    ...
    Result := Point3D(Vector3D(P) * matProj);
...
begin
  if (Length(Points) = 0) or SameValue(Size.X, 0.0, TEpsilon.Scale) or SameValue(Size.Y, 0.0, TEpsilon.Scale) then
    Exit;
  { calc bounds }
  ...
  if Abs(a.X - b.X) < TEpsilon.Position then Exit;
  if Abs(a.Y - b.Y) < TEpsilon.Position then Exit;

  Ver := TVertexBuffer.Create([TVertexFormat.vfVertex, TVertexFormat.vfNormal, TVertexFormat.vfTexCoord0], 0);
  Idx := TIndexBuffer.Create(0, TIndexFormat.ifUInt32);
  ...
  begin
    ...
    begin
      ...
      N := (Term1.CrossProduct(Term2)).Normalize;

      Ver.Normals[Vtx1] := N.ToPoint3D;
      Ver.Normals[Vtx2] := N.ToPoint3D;
      Ver.Normals[Vtx3] := N.ToPoint3D;
