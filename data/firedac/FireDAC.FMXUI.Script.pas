...
{$I FireDAC.inc}
{$IFDEF WIN32}
  {$HPPEMIT '#pragma link "FireDAC.FMXUI.Script.obj"'}
{$ELSE}
  {$HPPEMIT '#pragma link "FireDAC.FMXUI.Script.o"'}
{$ENDIF}
...
begin
  ...
  ShowMessage('SQL Script paused.' + C_FD_EOL + s);
  Output('');
end;
...
begin
  if FIsRunning and not FRequestStop then begin
    if MessageDlg('Do you really want to cancel script execution ?',
                  TMsgDlgType.mtConfirmation, [TMsgDlgBtn.mbYes,
                  TMsgDlgBtn.mbNo], -1) = mrYes then
      FRequestStop := True;
  end
...
{-------------------------------------------------------------------------------}
procedure TFDGUIxFMXScriptImpl.Output(const AStr: String; AKind: TFDScriptOuputKind);
begin
  if FDGUIxSilent() or (FForm = nil) then
    Exit;
  FForm.Output(AStr);
...
initialization
  oFact := TFDMultyInstanceFactory.Create(TFDGUIxFMXScriptImpl,
    IFDGUIxScriptDialog, C_FD_GUIxFMXProvider, 'FireDAC.FMXUI.Script');
