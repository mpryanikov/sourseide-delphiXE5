...
{$ENDIF}
    ...
    procedure DoError(ADBXError: TDBXError);
    function GetIsDataSnap: Boolean;
...
{$ENDIF}
  ...
  public
    ...
    property IsDataSnap: Boolean read GetIsDataSnap;
...
{$ENDIF}
  ...
  FireDAC.Stan.Consts, FireDAC.Stan.Option, FireDAC.Stan.Factory, FireDAC.Stan.ResStrs,
    FireDAC.Stan.Storage, FireDAC.Stan.SQLTimeInt,
...
type
  ...
  TDBXMetaDataTableTypesEx = TDBXMetaDataTableTypes;
...
{-------------------------------------------------------------------------------}
function TFDPhysTDBXDriverBase.GetConnParamCount(AKeys: TStrings): Integer;
begin
  Result := inherited GetConnParamCount(AKeys) + 1;
end;

{-------------------------------------------------------------------------------}
procedure TFDPhysTDBXDriverBase.GetConnParams(AKeys: TStrings; Index: Integer;
...
begin
  ...
  if Index < inherited GetConnParamCount(AKeys) then
    ...
    0:
      begin
        AName := S_FD_ConnParam_TDBX_Advanced;
        AType := '@S';
      end;
    end;
    ACaption := AName;
...
{-------------------------------------------------------------------------------}
function TFDPhysTDBXConnectionBase.GetIsDataSnap: Boolean;
begin
...
begin
  ...
  if FDriverName = '' then
    FDException(Self, [S_FD_LPhys, S_FD_DBXId], er_FD_DBXParMBNotEmpty,
...
begin
  ...
  AConnProps.Values[TDBXPropertyNames.Database] := AConnectionDef.ExpandedDatabase;
  AConnProps.Values[TDBXPropertyNames.UserName] := AConnectionDef.UserName;
  AConnProps.Values[TDBXPropertyNames.Password] := AConnectionDef.Password;
end;
...
{$ENDIF}
    ...
    if GetConnectionDef.HasValue(S_FD_ConnParam_TDBX_Advanced) then begin
      s := GetConnectionDef.AsString[S_FD_ConnParam_TDBX_Advanced];
      i := 1;
      while i <= Length(s) do
...
{$ENDIF}
  ...
  FDBX4 := (FDbxConnection.DatabaseMetaData is TDBXDatabaseMetaDataEx) and
    (TDBXDatabaseMetaDataEx(FDbxConnection.DatabaseMetaData).MetaDataVersion >= DBXVersion40);
end;
...
begin
  ...
  if (FDbxConnection <> nil) and (FDbxConnection is TDBXConnectionEx) then
    Inc(Result, 2);
end;
  ...
  if AIndex < inherited GetItemCount then
    ...
    0:
      begin
        ...
        AKind := ikClientInfo;
        AValue := TDBXConnectionEx(FDbxConnection).ProductName;
      end;
    1:
      begin
        AName := 'DBX product version';
        AKind := ikClientInfo;
        AValue := TDBXConnectionEx(FDbxConnection).ProductVersion;
      end;
    end;
end;
...
begin
  ...
  for i := 0 to AParams.Count - 1 do begin
    ...
    oFmtOpts.ResolveFieldType('', oParam.DataType, oParam.ADDataType, oParam.Size,
      oParam.Precision, oParam.NumericScale, eFieldType, iSize, iPrec, iScale,
    ...
    if eParamType in [ptUnknown, ptInput, ptInputOutput] then begin
      if iInd <> 0 then
        ...
        dtUInt32:
          oDbxParam.Value.SetInt32(PLongWord(FBuffer.Ptr)^);
        dtUInt64:
        ...
        dtDateTime:
          oDbxParam.Value.SetTimestamp(FDDateTime2SQLTimeStamp(TimeStampToDateTime(
...
{$ENDIF}
        ...
        dtWideHMemo:
          begin
            SetString(sw, PWideChar(FBuffer.Ptr), iDestDataLen div SizeOf(WideChar));
            oDbxParam.Value.SetWideString(sw);
          end;
        dtByteString,
        dtBlob,
        dtHBlob,
...
{$ENDIF}
        ...
        ftDBaseOle:
          begin
            sw := oDbxParam.Value.GetWideString;
            iLen := Length(sw);
            if iLen <> 0 then
              Move(sw[1], FBuffer.Ptr^, iLen * SizeOf(WideChar));
            PWideChar(LongWord(FBuffer.Ptr) + LongWord(iLen) * LongWord(SizeOf(WideChar)))^ := #0;
          end;
        ftSmallInt,
        ftWord:
        ...
        ftOraBlob:
          begin
            ...
            oDbxParam.Value.GetBytes(0, aBytes, 0, FBuffer.Size);
            Move(aBytes[1], FBuffer.Ptr^, FBuffer.Size);
          end;
        ftDataSet:
        ...
        if oParam.DataType <> ftDataSet then begin
          GetOptions.FormatOptions.ResolveFieldType('', oParam.DataType,
            oParam.ADDataType, oParam.Size, oParam.Precision, oParam.NumericScale,
...
begin
  ...
  if GetMetaInfoKind = mkNone then
    oFmtOpts.ResolveDataType(AColInfo.FSourceName, AColInfo.FSourceType,
      ...
      // TDBXTableTypeParser.ParseTableTypes does not support SystemView
{
      if (osSystem in GetObjectScopes) and (tkView in GetTableKinds) then
        sCmd := sCmd + TDBXMetaDataTableTypesEx.SystemView + ';';
}
    end;
  mkTableFields:
    if TDBXConn.IsDataSnap then
  ...
  mkProcs:
    begin
      ...
      if FMetaForPackage then
        sCmd := TDBXMetaDataCommandsEx.GetPackageProcedures + ' ' + sCmd
      else
  ...
  mkProcArgs:
    begin
      ...
      FMetaForPackage := rName.FBaseObject <> '';
      if FMetaForPackage then
        sCmd := TDBXMetaDataCommandsEx.GetPackageProcedureParameters + ' ' + sCmd
      else
...
const
  C_Asc: UnicodeString = 'ASC';
  C_Desc: UnicodeString = 'DESC';
var
  oRow: TFDDatSRow;
  iDbxPrec: Integer;
  siDbxScale: SmallInt;
    ...
    if oVal.IsNull then
      ...
      dtInt16:
        begin
          ...
          TDBXDataTypes.Uint32Type: PSmallInt(FBuffer.Ptr)^ := LongWord(oVal.GetInt32);
          else ASSERT(False);
          end;
      ...
      dtInt32:
        begin
          ...
          TDBXDataTypes.Uint32Type: PInteger(FBuffer.Ptr)^ := LongWord(oVal.GetInt32);
          else ASSERT(False);
          end;
          oRow.SetData(ARowColIndex, FBuffer.Ptr, 0);
        end;
      dtWideString:
        begin
          case oVal.ValueType.DataType of
          TDBXDataTypes.WideStringType: sw := oVal.GetWideString;
          TDBXDataTypes.AnsiStringType: sw := UnicodeString(oVal.
            {$IFDEF NEXTGEN} GetString {$ELSE} GetAnsiString {$ENDIF});
          else ASSERT(False);
          end;
          oRow.SetData(ARowColIndex, PWideChar(sw), Length(sw));
        end;
      else
        ASSERT(False);
      end;
  ...
  begin
    ...
    if TDBXConn.IsDataSnap and
       ((CompareText(sSchema, 'DSAdmin') = 0) or
        (CompareText(sSchema, 'DSMetadata') = 0)) then
      AScope := osSystem
    else if not (npSchema in oConnMeta.NameParts) or TDBXConn.IsDataSnap or
  ...
  begin
    ...
    if TDBXConn.IsDataSnap then begin
    ...
    mkPrimaryKey:
      begin
        ...
        if GetMetaInfoKind = mkPrimaryKey then
          if TDBXConn.GetRDBMSKindFromAlias = mkMSAccess then begin
...
{-------------------------------------------------------------------------------}
function TDBXNativeExceptionLoad(AStorage: TFDStorage): TObject;
begin
