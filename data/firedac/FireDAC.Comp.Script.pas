...
type
  ...
  TFDConsolePutEvent = procedure (AEngine: TFDScript;
    const AMessage: String; AKind: TFDScriptOuputKind) of object;
  ...
  IFDScriptEngineIntf = interface (IUnknown)
    ...
    procedure Progress;
    procedure ConPut(const AMsg: String; AKind: TFDScriptOuputKind);
...
{$ENDIF}
  ...
  published
    ...
    property FileEncoding: TFDEncoding read FFileEncoding write FFileEncoding default ecANSI;
  ...
  protected
    ...
    procedure Progress;
    procedure ConPut(const AMsg: String; AKind: TFDScriptOuputKind); virtual;
...
{-------------------------------------------------------------------------------}
type
  TFDScriptCommandRegItem = class (TObject)
    FClass: TFDScriptCommandClass;
    constructor Create(AClass: TFDScriptCommandClass);
  end;

constructor TFDScriptCommandRegItem.Create(AClass: TFDScriptCommandClass);
begin
  inherited Create;
  FClass := AClass;
end;
...
begin
  ...
  if AAllowDoubleQuote and (FCh = '"') then begin
    ...
    iQuoted := 0;
    iSep := 1;
    while not FDInSet(FCh, ASet) do begin
      if ARequired then
        GetChar;
  ...
  while True do begin
    ...
    '/':
      if (iInComment1 = 0) and not lInComment2 and not lInStr and not lInStr2 and not lInName then begin
        ...
        else if (Length(ucCS) > 1) and (ucCS[1] = '/') then
          // protection for // separator or similar
          SetBookmark(iLastBmk);
      end;
    #0:
      begin
        if ((Result = nil) or (Result.ClassType = FScriptCommandRegistry.DefaultSQLCommandClass) or
                              (Result.ClassType = FScriptCommandRegistry.DefaultPLSQLCommandClass)) then
          // if SQL or PL/SQL command, then extract it
    ...
    '$':
      if (iInComment1 = 0) and not lInComment2 and not lInName and not lInStr and
         (AEngine.GetRDBMSKind = mkPostgreSQL) then begin
        GetChar;
        if not FDInSet(FCh, ['0' .. '9']) then begin
          s := InternalGetString(False, False, False, True, ['$']);
    ...
    '\':
      if (iInComment1 = 0) and not lInComment2 and lInStr and
         ...
         // in string literals
         (AEngine.GetRDBMSKind in [mkMySQL, mkASA]) then begin
        GetChar;
        Continue;
      end;
    end;
    ...
    if (iInComment1 = 0) and not lInComment2 and not lInStr and not lInStr2 and not lInName and (iBraceLevel = 0) then begin
      ...
      if not lCmdSep and (Result = nil) and ((ucS <> '') or FDInSet(FCh, ['A'..'Z', 'a'..'z'])) then begin
        if ucS = '' then
          ...
          GetChar;
        if (AEngine.GetRDBMSKind = mkOracle) and ((ucS = 'BEGIN') or (ucS = 'DECLARE')) then
          Result := FScriptCommandRegistry.DefaultPLSQLCommandClass.Create(Self, AEngine)
        else if (AEngine.GetRDBMSKind = mkFirebird) and (ucS = 'EXECUTE') then begin
          SkipWSLF;
          ucS := GetUCWord;
          if ucS = 'BLOCK' then
            Result := FScriptCommandRegistry.DefaultPLSQLCommandClass.Create(Self, AEngine);
        end
        else if not (AEngine.GetRDBMSKind in [mkMSSQL, mkASA]) and
                    ((ucS = 'CREATE') or (ucS = 'RECREATE')) then begin
          SkipWSLF;
          ucS := GetUCWord;
          if (AEngine.GetRDBMSKind in [mkInterbase, mkFirebird]) and (ucS = 'DATABASE') then begin
...
begin
  if FDriverID <> AValue then begin
    ...
    if (FOwner <> nil) and (FOwner is TFDScript) then
      TFDScript(FOwner).FRDBMSKind := mkUnknown;
  end;
end;
...
begin
  if ASource is TFDSQLScript then begin
    ...
    FSQL.Assign(TFDSQLScript(ASource).SQL);
  end
  else
...
{-------------------------------------------------------------------------------}
// IFDStanOptions
procedure TFDScript.GetParentOptions(out AOpts: IFDStanOptions);
begin
  if FConnection <> nil then
...
type
  __TADCustomConnection = class(TFDCustomConnection)
  end;

procedure TFDScript.SetConnection(const AValue: TFDCustomConnection);
begin
  if FConnection <> nil then
    __TADCustomConnection(FConnection).UnRegisterClient(Self);
  InternalReleaseConnection;
  FConnection := AValue;
  if FConnection <> nil then begin
    __TADCustomConnection(FConnection).RegisterClient(Self, DoConnectChanged);
...
{-------------------------------------------------------------------------------}
procedure TFDScript.ConPut(const AMsg: String; AKind: TFDScriptOuputKind);
var
  sCon: String;
  sSpool: String;
  oDlg: IFDGUIxScriptDialog;
...
begin
  ...
  for i := 0 to AArgs.Count - 1 do begin
    oMac := Macros.Add;
    oMac.Name := FDNameFromIndex(AArgs, i);
    oMac.AsRaw := FDValueFromIndex(AArgs, i);
  end;
  FDFree(AArgs);
end;
{$HINTS ON}
...
begin
  ...
  if ScriptOptions.ClientLib <> '' then begin
    ...
    oDef := FDPhysManager.DriverDefs.FindDefinition(Connection.ResultConnectionDef.DriverID);
    if oDef = nil then begin
      oDef := FDPhysManager.DriverDefs.Add;
      oDef.Name := Connection.ResultConnectionDef.DriverID;
    end;
...
begin
  ...
  if GetRDBMSKind = mkOracle then begin
