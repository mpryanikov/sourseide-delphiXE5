...
{-------------------------------------------------------------------------------}
function TFDPhysMySQLMetadata.InternalEscapeBoolean(
...
{-------------------------------------------------------------------------------}
function TFDPhysMySQLCommandGenerator.GetPing: String;
begin
  Result := 'SELECT 1';
end;
  ...
  mkProcs:
    if FConnMeta.ServerVersion < mvMySQL050000 then
      ...
      Result := 'SELECT CAST(NULL AS UNSIGNED) AS RECNO, ROUTINE_SCHEMA AS CATALOG_NAME, ' +
        ...
        'ROUTINE_NAME AS PROC_NAME, CAST(NULL AS UNSIGNED) AS OVERLOAD, CASE ROUTINE_TYPE ' +
      ...
      if ACatalog <> '' then
        AddWhere('UPPER(ROUTINE_SCHEMA) = UPPER(''' + ACatalog + ''')');
      if AWildcard <> '' then
