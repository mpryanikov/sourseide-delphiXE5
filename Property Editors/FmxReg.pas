...
uses Vcl.Graphics, Vcl.Forms, System.SysUtils, System.Classes, System.Actions, System.UITypes, System.Types, System.Math,
  ...
  FmxEditors, FMX.WebBrowser, FMX.Gestures, FMX.Printer, Generics.Collections, Generics.Defaults,
  FMX.PhoneDialer.Actions, FmxStyleEditors, FMX.Notification,
  FMX.MultiResBitmap, {Fmx.ImgList,} FMX.DateTimeCtrls, FMX.BehaviorManager,
  // Designers
  FmxMultiResBitmapEditors, FMX.Design.Brush, FMX.Design.Bitmap, FMX.Design.Style, FMX.Design.Items,FMX.Design.Path, FMX.Design.Lang,
...
begin
  ...
  StyleBehavior := BehaviorServices.GetBehaviorService(IStyleBehavior, Context);
  if Assigned(StyleBehavior) then
  begin
...
begin
  ...
  if Assigned(StyleDesc) and StyleDesc.PlatformTarget.Contains('[ANDROID]') then
  begin
    if ControlStyleName.Contains('button') and (StyleName.Contains('bordered')) then Exit;
  end;
...
type
 TCustomSyntheticProperty = class(TBasePropertyEditor, IProperty, IPropertyKind)
  ...
  TLocationSensorEditor = class(TSelectionEditor)
  public
    procedure RequiresUnits(Proc: TGetStrProc); override;
  end;

  TMotionSensorEditor = class(TSelectionEditor)
  public
    procedure RequiresUnits(Proc: TGetStrProc); override;
  end;
...
//  RegisterClass(TMenuItem);
  ...
  {$WARNINGS ON}
  RegisterComponents(srAdditional, [TCalendar, TCalendarEdit, TTimeEdit,
...
//                         TCustomOptionsProperty);
  ...
  RegisterComponents(srLayouts, [TLayout, TScaledLayout, TGridLayout, TGridPanelLayout, TFlowLayoutBreak, TFlowLayout,
  ...
  RegisterComponents(srService, [TNotificationCenter]);
...
//  RegisterPropertyEditor(TypeInfo(TBounds), TFmxObject, '', TRectAnimationProperty);
  ...
  RegisterSelectionEditor(TLocationSensor, TLocationSensorEditor);
  RegisterSelectionEditor(TMotionSensor, TMotionSensorEditor);
  ...
  RegisterActions(srPhoneCategory, [TPhoneCallAction], nil);
  RegisterActions(srMediaLibraryCategory, [TTakePhotoFromLibraryAction, TTakePhotoFromCameraAction, TShowShareSheetAction], nil);
...
//  RegisterSelectionEditor(TCustomSourceItem, TMultiResBitmapEditor);

  {RegisterComponents('Cep', [FMX.ImgList.TImageList]);
  RegisterComponents('Cep', [FMX.ImgList.TGlyph]);      }
end;
...
var
  ...
  FmxFont: FMX.Graphics.TFont;
  TmpVclFont: Vcl.Graphics.TFont;
begin
  FontDialog := TFontDialog.Create(Application);
  try
    FmxFont := FMX.Graphics.TFont(GetOrdValue);
...
{ TLocationSensorEditor }

procedure TLocationSensorEditor.RequiresUnits(Proc: TGetStrProc);
begin
  inherited;
  Proc('System.Sensors');
end;
...
procedure TMotionSensorEditor.RequiresUnits(Proc: TGetStrProc);
begin
  inherited;
  Proc('System.Sensors');
end;
...
function TListViewSprig.DeleteStyle: TSprigDeleteStyle;
begin
  Result := dsAbort;
end;
