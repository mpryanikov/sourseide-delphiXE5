...
{$ENDIF}
  ...
  FireDAC.Stan.Consts, FireDAC.Stan.Error, FireDAC.Stan.Util, FireDAC.Stan.Option, 
    FireDAC.Stan.Param, FireDAC.Stan.Factory,
        ...
        try
          ...
          try
            ...
            for i := 0 to oList.Count - 1 do
              AType := AType + oList[i] + ';';
            AType := Copy(AType, 1, Length(AType) - 1);
          finally
            FDFree(oList);
          end;
        finally
...
begin
  ...
  if (oConnMeta.ServerVersion > 0) and (oConnMeta.ServerVersion < caADS10) then
    AMessages.Add(Format('Warning: invalid Advantage version [%s]. Minimal version is [10.0]',
      [FDVerInt2Str(oConnMeta.ServerVersion)]));

  // 3. The minimal ACE version is 10.0
  if (oConnMeta.ClientVersion > 0) and (oConnMeta.ClientVersion < caADS10) then
    AMessages.Add(Format('Warning: invalid ACE version [%s]. Minimal version is [10.0]',
      [FDVerInt2Str(oConnMeta.ClientVersion)]));
end;
...
begin
  ...
  if ADSConnection.ConnectionDef.HasValue(S_FD_ConnParam_ADS_TablePassword) then
    ACrs.EnableEncryption(ADSConnection.ConnectionDef.AsString[S_FD_ConnParam_ADS_TablePassword]);
end;
...
begin
  if ATimes - AOffset > 1 then begin
    ...
    ADSConnection.FConnection.EnableAbort;
    try
      try
      ...
      ADSConnection.FConnection.DisableAbort;
...
initialization
  FDPhysManager();
  FDPhysManagerObj.RegisterDriverClass(TFDPhysADSDriver);
