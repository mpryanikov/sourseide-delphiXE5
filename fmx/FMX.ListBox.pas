...
uses
  ...
  FMX.Objects, FMX.Platform, FMX.Pickers, FMX.Messages, FMX.Controls, FMX.Graphics;
...
begin
  ...
  try
    ...
    begin
      ...
      if Assigned(ContentLayout.ParentControl) then
      begin
        OverlayPadding := ContentLayout.Padding.Rect;
        Inset(OverlayPadding, ContentLayout.Margins.Rect);
        Inset(OverlayPadding, ContentLayout.ParentControl.Padding.Rect);
        FContentOverlay.Padding.Rect := OverlayPadding;
      end;
    end;
...
var
  ...
  FMobj : TFmxObject;
begin
  if not Assigned(Selection) or (FUpdating > 0) then
    Exit;
  ...
  begin
    ...
    if Assigned(Item) then
      ...
      begin
        P := Item.LocalToAbsolute(PointF(0, 0));
        if Assigned(Selection.ParentControl) then
  ...
  begin
    if Self.IsFocused and Assigned(FFocusedSelection) then
      ...
      Clone := TControl(Selection.Clone(Self));
...
begin
  ...
  if Assigned(ItemDown) and (ItemDown.Index <> ItemIndex) then
    TLinkObservers.PositionLinkPosChanging(Observers);
  if Assigned(ItemDown) then
  begin
    P := TPointF.Create(X, Y);
  ...
  if (Button = TMouseButton.mbLeft)
    ...
    and Assigned(ItemDown) then
  begin
    InitiateSelectionTimer(250, ItemDown);
  end
  else
...
{$ENDIF}
          ItemDown.IsSelected := not ItemDown.IsSelected
        else if ssShift in Shift then
        begin
          SelectRange(Selected, ItemDown);
          ItemIndex := ItemDown.Index;
        end
        else
        begin
          SelectRange(ItemDown, ItemDown);
          ItemIndex := ItemDown.Index;
        end;
        FFirstSelect := ItemDown;
      end
      else
      begin
        if ItemIndex <> ItemDown.Index then
          ItemIndex := ItemDown.Index
        else
          if AllowDrag then
          begin
            LBitMap := ItemDown.MakeScreenshot;
            try
...
begin
  ...
  begin
    if Assigned(Item) and Assigned(ItemDown) then
    begin
...
begin
  ...
  try
    ...
    begin
      ...
      Item := ItemByPoint(X, Y);
      if Assigned(Item) and (Item = ItemDown) then
      begin
...
begin
  ...
  begin
    ...
    UpdateSelection;
    if (FUpdating = 0) and (not (csLoading in ComponentState)) then
    begin
      Repaint;
      DoChange;
    end;
...
begin
  Result := Assigned(SourceItem) and Assigned(DestItem);
...
begin
  ...
  if (Count > 0) and
     ((Key = vkEscape) or
      ([ssAlt, ssCtrl, ssShift, ssCommand] * Shift = []) or
    ((([ssAlt, ssCtrl, ssShift, ssCommand] * Shift = [ssAlt]) and (Key = vkDown)))) then
  begin
    if KeyChar <> #0 then
    begin
      for i := 0 to Count - 1 do
    ...
    case Key of
      ...
      vkEscape:
        ...
        begin
          DropDown;
          if (FOldItemIndex >= 0) and (FOldItemIndex < Count) then
            LItemIndex := FOldItemIndex
          else
            LItemIndex := -1;
        end
        else
