...
type
  ...
  public
    ...
    class function IsGesture(const Points: array of TPointF; const Items: TGestureList; GestureTypes: TGestureTypes; var EventInfo: TGestureEventInfo): Boolean;
  ...
  public
    ...
    function IsPointsGesture(const Points, GesturePoints: array of TPointF; GestureID: TGestureID; Deviation: Integer; ErrorMargin: Integer; var Percentages: array of Double): Integer;
    function Match(const Points, GesturePoints: array of TPointF; const Options: TGestureOptions; GestureID: TGestureID; Deviation, ErrorMargin: Integer): Single; override;
    function NormalizePoints(const Points: array of TPointF): TGesturePointArray; override;
    function ScalePoints(const Points, GesturePoints: array of TPointF; MinScale: Integer; Scaling: Boolean = False): TGesturePointArray; override;
    procedure Reset; override;
    function RotatePoints(const Points, GesturePoints: array of TPointF; ErrorMargin: Integer; var Angle: Double): TGesturePointArray;
    function RemoveDuplicates(const Points: array of TPointF): TGesturePointArray;
  end;

implementation

uses System.Math, System.Classes, System.SysUtils, FMX.Consts, System.UITypes, System.Generics.Collections;
  ...
  begin
    ...
    begin
      ...
      for I := 1 to Count - 1 do
        FPoints.Add(PointF(LastPoint.X + Round(StepX * I), LastPoint.Y + Round(StepY * I)));
    end;
...
begin
  ...
  if not(csDesigning in AControl.ComponentState) then
    ...
    begin
      AControl.Touch.GestureEngine := TAndroidGestureEngine.GetGestureEngine(AControl);
    end;
end;
...
class function TPlatformGestureEngine.IsGesture(const Points: array of TPointF;
  const Items: TGestureList; GestureTypes: TGestureTypes;
...
class procedure TPlatformGestureEngine.SetDefaultRecognizerClass(const Value: TCustomGestureRecognizerClass);
begin
...
constructor TAndroidGestureEngine.Create(const AControl: TFmxObject);
begin
  inherited;
...
begin
  if Radius > 0 then
  begin
    Result := Sqr((Point.X - Center.X) / Radius) + Sqr((Point.Y - Center.Y) / Radius) <= 1;
  end
  else
  begin
    Result := False;
...
function CheckPoint(const Point, Source: TPointF; Deviation, ErrorMargin: Integer): Double;
var
  Distance: Double;
...
begin
  ...
  ErrorMarginDistance := Deviation * ErrorMargin / 100;
  // MulDiv(Deviation, ErrorMargin, 100);
...
function CheckPointInGestureWindow(const Point: TPointF; const SourcePoints: array of TPointF; var GestureIndex: Integer; var Verification: TDoubleDynArray; ADeviation, ErrorMargin: Integer): Boolean;
var
  Index: Integer;
  Percentage: Double;
begin
...
function Line(const Point1, Point2: TPointF): TLine; inline;
begin
  Result.Point1 := Point1;
  Result.Point2 := Point2;
end;

function Circle(const Point: TPointF; Radius: Integer): TCircle; inline;
begin
  Result.Point := Point;
  Result.Radius := Radius;
end;

function LineIntersectsCircle(const Line: TLine; Circle: TCircle): Boolean;
var
  A, B, C: Single;
begin
  A := Sqr(Line.Point2.X - Line.Point1.X) + Sqr(Line.Point2.Y - Line.Point1.Y);

  B := 2 * ((Line.Point2.X - Line.Point1.X) * (Line.Point1.X - Circle.Point.X) + (Line.Point2.Y - Line.Point1.Y) * (Line.Point1.Y - Circle.Point.Y));

  C := Sqr(Circle.Point.X) + Sqr(Circle.Point.Y) + Sqr(Line.Point1.X) + Sqr(Line.Point1.Y) - 2 * (Circle.Point.X * Line.Point1.X + Circle.Point.Y * Line.Point1.Y) - Sqr(Circle.Radius);

  Result := (B * B - 4 * A * C) > 0;
end;

function CheckLineIntersectsCircle(const Line: TLine; const SourcePoints: array of TPointF; GestureIndex, Radius: Integer; var Verification: TIntegerDynArray): Boolean;
var
  Index: Integer;
begin
  Result := False;
...
function Distance(const Point1, Point2: TPointF): Single;
begin
  Result := Sqrt(Sqr(Point1.X - Point2.X) + Sqr(Point1.Y - Point2.Y));
end;


function TGestureRecognizer.CopyPoints(const Points: TGesturePoints): TGesturePointArray;
var
  Index: Integer;
begin
...
function TGestureRecognizer.InterpolatePoints(const Points: array of TPointF): TGesturePointArray;
var
  ...
  try
    ...
    begin
      ...
      begin
        ...
        for I := 1 to Count - 1 do
        begin
          ResultPoints.Add(PointF(CurrentPoint.X + Trunc(StepX * I), CurrentPoint.Y + Trunc(StepY * I)));
        end;
      end;
...
function TGestureRecognizer.IsPointsGesture(const Points, GesturePoints: array of TPointF; GestureID: TGestureID; Deviation,
...
begin
  ...
  begin
    ...
    while (PointsIndex + 1 < PointsCount) and PointInCircle(GesturePoints[Index], Points[PointsIndex + 1], Deviation) do
      Inc(PointsIndex);
  ...
  begin
    ...
    while (GestureIndex + 1 < GestureCount) and PointInCircle(Points[Index], GesturePoints[GestureIndex + 1], Deviation) do
      Inc(GestureIndex);
    ...
    if Index + 1 < PointsCount - 1 then
      CheckLineIntersectsCircle(Line(Points[Index], Points[Index + 1]), GesturePoints, GestureIndex, Deviation, LinesMatch);
  end;
  ...
  // MulDiv(100, AnglesMatched, GestureCount);
  Percentages[3] := 100 * LinesMatched / GestureCount;
  // MulDiv(100, LinesMatched, GestureCount);
...
function TGestureRecognizer.Match(const Points, GesturePoints: array of TPointF; const Options: TGestureOptions; GestureID: TGestureID; Deviation,
  ErrorMargin: Integer): Single;
var
...
begin
  ...
  try
    ...
    if (Result < LErrorMargin) and not(TGestureOption.goUniDirectional in Options) and PointInCircle(LPoints[0], GesturePoints[GesturePointCount - 1], Deviation) then
    begin
      Result := IsPointsGesture(LPoints, ReversePoints(LGesturePoints), GestureID, Deviation, LErrorMargin, Percentages);
    end;
  finally
...
function TGestureRecognizer.RotatePoints(const Points, GesturePoints: array of TPointF; ErrorMargin: Integer;
...
function TGestureRecognizer.ScalePoints(const Points, GesturePoints: array of TPointF; MinScale: Integer;
