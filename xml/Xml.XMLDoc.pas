...
{$IFDEF NEXTGEN}
    procedure TransformNode(const stylesheet: IXMLNode; var output: DomString); overload;
{$ELSE !NEXTGEN}
...
begin
  case SourceNode.nodeType of
    ...
    ntText, ntCData, ntComment:
        Result := TargetDoc.CreateNode(SourceNode.NodeValue, SourceNode.NodeType);
    ntEntityRef:
...
begin
  ...
  begin
    if ChangeType = ncAddAttribute then
      DOMElement.setAttribute(AttrName, NewValue)
    else
...
{ This method uses an explicit namespaceURI. }
  ...
  begin
    ...
    if UseXSDBooleanStrings and (VarType(Value) = varBoolean) then
