...
var
  ...
  function ProcessQuotedDelim(FDelim1, FDelim2: Char; var AName: String): Boolean;
  begin
    Result := False;
    if not FDInSet(FDelim1, [#0, ' ']) and (FCh = FDelim1) then begin
      AIsQuoted := True;
      repeat
        Inc(i);
        if i = 256 then
        ...
        aBuff[i] := GetChar;
      until FDInSet(aBuff[i], [#0, FDelim2]);
...
var
  ...
  begin
    ...
    if lIsQuoted then
      sSubst := ':' + ConnMetadata.QuoteObjName(sSubst)
    else
      sSubst := ':' + sSubst;
  end;
...
begin
  ...
  if FDInSet(cCh, ['=', ' ', #13, #10, #9, #0]) or
    ...
    // - TSQL label - name:
    // - if before ':' is identifier (eg Informix - catalog:schema.object)
  end
  else if cCh = ':' then
    if (ConnMetadata.Kind in [mkPostgreSQL, mkMSSQL, mkADS]) or
       (ConnMetadata.Kind = mkSQLite) and FNested then begin
      // skip:
      // - PostgreSQL ::
    ...
    PutBack;
    lIsQuoted := False;
    if ConnMetadata.Kind = mkOracle then begin
...
begin
  ...
  repeat
    ...
    '\':
      ...
      if not (piExpandEscapes in Instrs) or FInComment1 or FInComment2 or FInStr1 or FInStr2 or FInStr3 or (FInNames <> []) then
        GetChar
      else begin
        Commit(-1);
        GetChar;
    ...
    #13, #10:
      begin
        ...
        if piTransformEOLs in Instrs then
          case FConnMetadata.LineSeparator of
          elUnix
          {$IFDEF UNIX}{$IFNDEF MACOS}, elDefault {$ENDIF}{$ENDIF}
          {$IFDEF ANDROID}, elDefault {$ENDIF}: // #10
          ...
          elMac
          {$IFDEF MACOS}, elDefault {$ENDIF}: // #13
            if FCh = #10 then begin
              Commit(-1);
              if GetChar <> #13 then begin
                PutBack;
                WriteStr(#13);
              end;
            end
    ...
    '!', '&':
      if Instrs * [piExpandMacros, piCreateMacros] <> [] then
        ProcessMacro(FCh);
    '$':
      if FConnMetadata.Kind = mkPostgreSQL then
        ProcessQuoteTag;
    'a'..'z', 'A'..'Z':
      if (ConnMetadata <> nil) and (ConnMetadata.Kind in [mkInterbase, mkFirebird]) and
...
var
  ...
  function ProcessQuotedDelim(FDelim1, FDelim2: Char): Boolean;
  begin
    Result := False;
    if not FDInSet(FDelim1, [#0, ' ']) then
      while FDestination[iPos] = FDelim1 do begin
        Result := True;
        repeat
          Inc(iPos);
        until (iPos > Length(FDestination)) or
              FDInSet(FDestination[iPos], [FDelim2, #0]);
        if (iPos <= Length(FDestination)) and
           (FDestination[iPos] = FDelim2) then begin
          Inc(iPos);
      ...
      if not FInComment1 and not FInComment2 and not FInStr1 and not FInStr2 and (FInNames = []) and
        ...
        if lFirstWord then begin
          ...
          repeat
            Inc(iPos);
          until (iPos > Length(FDestination)) or
                not FDInSet(FDestination[iPos], ['A' .. 'Z', 'a' .. 'z', '_']);
          FTokens.Add(UpperCase(Copy(FDestination, iCmdStartedPos, iPos - iCmdStartedPos)));
          FSQLCommandKind := FConnMetadata.GetSQLCommandKind(FTokens);
          Dec(iPos);
        end
          ...
          if CheckKeyword('INTO') then begin
            ...
            FSQLCommandKind := FConnMetadata.GetSQLCommandKind(FTokens);
                    ...
                    (iFromStartedPos <> 0) and (ConnMetadata <> nil) and
                      (ConnMetadata.Kind = mkPostgreSQL) and CheckKeyword('ONLY')) then begin
            SkipKeyword(4);
