...
type
  ...
  public
    ...
    class procedure NeedScenes; static;
    ...
    // Set system style
    class procedure SetSystemStyle(const Context: TFmxObject); overload;
    ...
    // Return system style
    class function SystemStyle(const Context: TFmxObject): TFmxObject; static;
...
uses FMX.Forms, FMX.Platform, FMX.Dialogs, FMX.Controls, FMX.Styles.Objects,
  FMX.Styles.Switch;
...
begin
  ...
  {$IFDEF MSWINDOWS}
  if (Pos('[MSWINDOWS]', UpperCase(FPlatformTarget)) = 0) and (Pos('[IOSALTERNATE]', UpperCase(FPlatformTarget)) = 0) and
     (Pos('[ANDROID]', UpperCase(FPlatformTarget)) = 0) then
    Result := False;
  {$ENDIF}
  {$IFDEF MACOS}
  {$IFNDEF NEXTGEN}
  ...
  {$ENDIF}
  {$IFDEF ANDROID}
  if (Pos('[ANDROID]', UpperCase(FPlatformTarget)) = 0) then
    Result := False;
  {$ENDIF}
end;
...
begin
  ...
  begin
    FActiveStyle := SystemStyle(Context);
...
class function TStyleManager.SystemStyle(const Context: TFmxObject): TFmxObject;
var
...
begin
  if TPlatformServices.Current.SupportsPlatformService(IFMXStyleService, IInterface(StyleService)) then
    Result := StyleService.GetSystemStyle(Context)
  else
    Result := nil;
end;
...
class procedure TStyleManager.SetSystemStyle(const Context: TFmxObject);
begin
  SetStyle(SystemStyle(Context));
end;
...
class procedure TStyleManager.NeedScenes;
begin
  if not FNeedScenes then
  begin
    FNeedScenes := True;
    UpdateScenes;
  end;
