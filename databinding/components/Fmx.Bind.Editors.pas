...
uses
  System.Classes, System.Bindings.EvalProtocol, Data.Bind.Components, Data.Bind.Editors, System.Bindings.ObjEval,
    FMX.ListBox, FMX.ListView, FMX.Grid, FMX.Controls, FMX.Types, FMX.Memo, FMX.StdCtrls, System.RTTI, FMX.Graphics,
    Generics.Collections, System.SysUtils, Data.Bind.ObjectScope, System.UITypes, FMX.DateTimeCtrls;
...
type
  ...
  TBindListViewEditor = class(TBindListViewLookupEditorImpl, IBindFillControlMembers,
    IBindListGroupEditor, IBindListSynchEditor)
  private type
...
type
  ...
  private
    ...
    function GetData: TValue;
  public
    constructor Create(AOwner: TGrid; AColumn: Integer);
...
constructor TGridDataObject.Create(AOwner: TGrid; AColumn: Integer);
begin
  FColumn := AColumn;
  FOwner := AOwner;
end;
...
type
  TOpenListViewItems = class(TListViewItems);
...
begin
  Assert(CheckRange);
  if CheckRange then
    if FIndexType = TItemIndexType.Unfiltered then
      Result := TOpenListViewItems(FListView.Items).UnFilteredItems[FItemIndex]
    else
...
begin
  if FIndexType = TItemIndexType.Unfiltered then
    Result := (FItemIndex >= 0) and (FItemIndex < TOpenListViewItems(FListView.Items).UnFilteredItems.Count)
  else
...
var
  ...
  LUnfilteredItems: TListViewItems.TItemsList;
  LIndex: Integer;
begin
  FApplyFilter := True;
  ...
  if IsFiltered then
  begin
    LUnfilteredItems := TOpenListViewItems(FEditorObject.FListView.Items).UnFilteredItems;
...
begin
  ...
  begin
    ...
    if FEditorObject.FListView.Items.Filtered then
      // Reapply filter
      FEditorObject.FListView.Items.Filter :=
        FEditorObject.FListView.Items.Filter;
  end;
end;
...
procedure TBindListViewEditor.SetSynchIndex(AIndex: Integer);
  ...
  function SynchOffsetToIndex(AOffset: Integer; AItems: TListViewItems.TItemsList): Integer;
  var
    I: Integer;
    LItem: TListViewItem;
    LCounter: Integer;
...
function TBindListViewEditor.GetSynchIndex: Integer;
  ...
  function GetIndexAndOffset(const AItem: TListViewItem; AItems: TListViewItems.TItemsList; out AItemIndex: Integer): Integer;
  var
    I: Integer;
    LItem: TListViewItem;
