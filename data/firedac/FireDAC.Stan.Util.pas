...
{$ENDIF}
  ...
  TFDTimerOption = (toSynchronized, toSingleFire, toImmediateFire);
  TFDTimerOptions = set of TFDTimerOption;
  TFDTimerState = (tsDisabled, tsWaiting, tsRaised, tsReqRemove);
  TFDTimer = class(TComponent)
  private
    FInterval: LongWord;
    FEnabled: Boolean;
    FOptions: TFDTimerOptions;
    FOnTimer: TThreadMethod;
    FData: TObject;
    procedure SetInterval(AValue: Longword);
    procedure SetEnabled(AValue: Boolean);
    procedure Update;
    function GetState: TFDTimerState;
    class procedure CheckPool;
  protected
  ...
  public
    ...
    destructor Destroy; override;
    procedure Schedule(AInterval: Longword);
    procedure Fire;
    class procedure LockTimers;
    class procedure UnLockTimers;
    class procedure Shutdown;
    property State: TFDTimerState read GetState;
  published
    property Interval: Longword read FInterval write SetInterval default 1000;
    property Enabled: Boolean read FEnabled write SetEnabled default False;
    property Options: TFDTimerOptions read FOptions write FOptions default [];
...
*)
  public
    ...
    class function Deco(const ASrc: Pointer; ASrcLen: Integer = -1; ASrcEncoding: TFDEncoding = ecDefault): String; overload;
    class function Deco(const ASrc: TFDByteString; ASrcEncoding: TFDEncoding = ecDefault): String; overload;
...
{$ENDIF}
  ...
  TFDIntfCodePart = (cpHeader, cpPrivate, cpProtected, cpPublic, cpPublished);
  TFDImplCodePart = (ipHeader, ipConstr, ipDestr, ipMethod);
  TFDCodeGenerator = class
  private
    FIntfStreams: array[TFDIntfCodePart] of TStream;
    FImplStreams: array[TFDImplCodePart] of TStream;
    FMyClassName, FParentClassName, FOriginalObject: String;
    FIsComponent: Boolean;
  public
    constructor Create;
    destructor Destroy; override;
    procedure WriteIntf(APart: TFDIntfCodePart; const AStr: String);
    procedure WriteImplNewMethod(ASourceName: String);
    procedure WriteImpl(APart: TFDImplCodePart; const AStr: String);
...
function FDPosEx(const ASub, AStr: String; AFrom: Integer = 1): Integer;
...
function FDMergeFieldNames(const AFieldNames1, AFieldNames2: String): String;
...
function FDStrToken(const AString: String; AChars: String; var AFrom: Integer;
...
function FDByteStrLength(const AStr: TFDByteString): Integer; inline;
...
function FDToolName: String;

// Date & time
// All time-interval related code is in the FireDAC.Stan.SQLTimeInt
function FDIsSQLTimeStampBlank(const ATimeStamp: TSQLTimeStamp): Boolean;
function FDDateTime2SQLTimeStamp(const ADateTime: TDateTime): TSQLTimeStamp;
function FDSQLTimeStamp2DateTime(const ADateTime: TSQLTimeStamp): TDateTime;
function FDSQLTimeStamp2Time(const AValue: TSQLTimeStamp): Longint;
function FDSQLTimeStamp2Date(const AValue: TSQLTimeStamp): Longint;
function FDTime2SQLTimeStamp(const AValue: Longint): TSQLTimeStamp;
function FDDate2SQLTimeStamp(const AValue: Longint): TSQLTimeStamp;
function FDTime2DateTime(const AValue: Longint): TDateTime;
function FDDateTime2Time(const AValue: TDateTime): Longint;
function FDDate2DateTime(const AValue: Longint): TDateTime;
function FDDateTime2Date(const AValue: TDateTime): Longint;
function FDVar2SQLTimeStamp(const AValue: Variant): TSQLTimeStamp;
function FDGetTickCount: LongWord;
function FDGetPerformanceCounter: Int64;
function FDEncodeDateTime(const AYear, AMonth, FDay, AHour, AMin, ASec, AMSec: Word): TDateTime;
...
procedure ADDouble2BCD(const AExt: Extended; out AVal: TBcd);
procedure FDInt2Str(ASrcData: Pointer; ASrcLen: Integer; var ADestData: PChar;
  var ADestLen: Integer; AScale: Integer);
...
procedure FDStr2Float(pIn: PChar; ASize: Integer; out AExt: Extended; ADot: Char = '.'); overload;
...
function FDTimeout(AStartTicks, ATimeout: LongWord): Boolean;
...
uses
{$IFDEF POSIX}
  Posix.Pthread, Posix.Dlfcn, Posix.Stdio, Posix.Unistd, Posix.Stdlib,
    Posix.Signal, Posix.Time, Posix.SysTime,
{$ENDIF}
{$IFDEF MACOS}
...
{ TFDFileStream                                                                 }
{-------------------------------------------------------------------------------}
constructor TFDFileStream.Create(const AFileName: string; AMode: Word; AStreamBufferSize: Word = $7FFF);
begin
...
{-------------------------------------------------------------------------------}
destructor TFDFileStream.Destroy;
begin
  SyncBuffer(False);
...
{-------------------------------------------------------------------------------}
procedure TFDFileStream.SyncBuffer(AReRead: boolean);
begin
...
{-------------------------------------------------------------------------------}
function TFDFileStream.Read(var Buffer; Count: Longint): Longint;
var
  pSrc: PByte;
begin
  if Count >= FStreamBufferSize then begin
    ...
    pSrc := FBuffer + (FPos - FBufStart);
    case Result of
    SizeOf(Byte):     PByte(@Buffer)^ := PByte(pSrc)^;
    SizeOf(Word):     PWord(@Buffer)^ := PWord(pSrc)^;
    SizeOf(LongWord): PLongWord(@Buffer)^ := PLongWord(pSrc)^;
    SizeOf(Int64):    PInt64(@Buffer)^ := PInt64(pSrc)^;
    else              Move(pSrc^, Buffer, Result);
    end;
  end;
  FPos := FPos + Result;
end;
...
{-------------------------------------------------------------------------------}
function TFDFileStream.Write(const Buffer; Count: Longint): Longint;
var
  pDest: PByte;
begin
  if Count >= FStreamBufferSize then begin
    ...
    if (FBufStart > FPos) or (FPos + Count > FBufStart + FStreamBufferSize) then
      SyncBuffer(True);
    Result := Count;
    pDest := FBuffer + (FPos - FBufStart);
    case Result of
    SizeOf(Byte):     PByte(pDest)^ := PByte(@Buffer)^;
    SizeOf(Word):     PWord(pDest)^ := PWord(@Buffer)^;
    SizeOf(LongWord): PLongWord(pDest)^ := PLongWord(@Buffer)^;
    SizeOf(Int64):    PInt64(pDest)^ := PInt64(@Buffer)^;
    else              Move(Buffer, pDest^, Result);
    end;
    FModified := True;
    FPos := FPos + Result;
...
{-------------------------------------------------------------------------------}
function TFDFileStream.Read(Buffer: TBytes; Offset, Count: Longint): Longint;
begin
  Result := Read(Buffer[Offset], Count);
end;

{-------------------------------------------------------------------------------}
function TFDFileStream.Write(const Buffer: TBytes; Offset, Count: Longint): Longint;
begin
  Result := Write(Buffer[Offset], Count);
end;

{-------------------------------------------------------------------------------}
function TFDFileStream.Seek(Offset: Longint; Origin: Word): Longint;
begin
  case Origin of
...
{-------------------------------------------------------------------------------}
function TFDFileStream.Seek(const Offset: Int64; Origin: TSeekOrigin): Int64;
begin
  case Origin of
...
{-------------------------------------------------------------------------------}
destructor TFDReadSharedMemoryStream.Destroy;
begin
  SetPointer(nil, 0);
  inherited Destroy;
end;
...
{-------------------------------------------------------------------------------}
procedure TFDReadSharedMemoryStream.SetData(APtr: Pointer; ASize: Integer);
begin
...
var
  ...
  i: Integer;
begin
  if (GetCurrentThreadId <> ThreadID) and not Finished then begin
...
begin
  if @FCallAddress <> nil then begin
    VirtualProtect(PFDThunk(@FCallAddress), SizeOf(TFDThunk),
      FSavedFlag, @SaveFlag);
...
{-------------------------------------------------------------------------------}
type
  TFDTimerData = class(TObject)
    {$IFDEF AUTOREFCOUNT} [Weak] {$ENDIF} FTimer: TFDTimer;
    FNextFire: Int64;
    FPrev, FNext: TFDTimerData;
    FState: TFDTimerState;
    procedure DoFire;
  end;

  TFDTimerPool = class(TThread)
  private
    FTimerDatas: TFDTimerData;
    FLock: TCriticalSection;
    FEvent: TEvent;
    FLastTickCount: LongWord;
    FTickCountBase: Int64;
    function GetTickCount64: Int64;
    procedure BeginUpdate;
    procedure EndUpdate;
    procedure ExtractTimer(AData: TFDTimerData);
    procedure Schedule(AData: TFDTimerData; ANewNextFire: Int64);
  protected
    procedure Execute; override;
  public
    constructor Create;
    destructor Destroy; override;
    procedure Update(ATimer: TFDTimer);
    procedure Remove(ATimer: TFDTimer);
  end;

var
  GTimerPool: TFDTimerPool;

{-------------------------------------------------------------------------------}
procedure TFDTimerData.DoFire;
begin
  if (FTimer <> nil) and (FState = tsRaised) then
    FTimer.DoFire;
end;

{-------------------------------------------------------------------------------}
{$WARNINGS OFF}
constructor TFDTimerPool.Create;
begin
  GTimerPool := Self;
  inherited Create(True);
...
{-------------------------------------------------------------------------------}
destructor TFDTimerPool.Destroy;
begin
  if GTimerPool = Self then
    GTimerPool := nil;
  Terminate;
  FEvent.SetEvent;
  WaitFor;
...
{-------------------------------------------------------------------------------}
procedure TFDTimerPool.BeginUpdate;
begin
  FLock.Enter;
end;

{-------------------------------------------------------------------------------}
procedure TFDTimerPool.EndUpdate;
begin
  if FTimerDatas <> nil then
    FEvent.SetEvent;
  FLock.Leave;
end;

{-------------------------------------------------------------------------------}
function TFDTimerPool.GetTickCount64: Int64;
var
  iTicks: LongWord;
begin
  iTicks := FDGetTickCount();
  if iTicks < FLastTickCount then
    FTickCountBase := FTickCountBase + $FFFFFFFF;
  FLastTickCount := iTicks;
  Result := iTicks + FTickCountBase;
end;

{-------------------------------------------------------------------------------}
procedure TFDTimerPool.Schedule(AData: TFDTimerData; ANewNextFire: Int64);
var
  pCur, p1: TFDTimerData;
begin
  AData.FNextFire := ANewNextFire;
  if AData.FState <> tsDisabled then
    ExtractTimer(AData);
  if FTimerDatas = nil then begin
    FTimerDatas := AData;
    AData.FNext := nil;
    AData.FPrev := nil;
  end
    ...
    pCur := FTimerDatas;
    while (pCur.FNext <> nil) and (pCur.FNextFire < AData.FNextFire) do
      pCur := pCur.FNext;
    if pCur.FNextFire < AData.FNextFire then begin
      pCur.FNext := AData;
      AData.FPrev := pCur;
      AData.FNext := nil;
    end
...
{-------------------------------------------------------------------------------}
procedure TFDTimerPool.ExtractTimer(AData: TFDTimerData);
var
  p1, p2: TFDTimerData;
begin
  p1 := AData.FNext;
...
{-------------------------------------------------------------------------------}
procedure TFDTimerPool.Update(ATimer: TFDTimer);
var
  oData: TFDTimerData;
  iInterval: Int64;
begin
  ...
  BeginUpdate;
  try
    if ATimer.FData = nil then begin
      oData := TFDTimerData.Create;
      ATimer.FData := oData;
      oData.FTimer := ATimer;
    end
    else
      oData := TFDTimerData(ATimer.FData);
    if oData.FState in [tsDisabled, tsWaiting, tsReqRemove] then begin
      if toImmediateFire in ATimer.Options then begin
        Exclude(ATimer.FOptions, toImmediateFire);
        iInterval := 0;
      end
      else
        iInterval := ATimer.Interval;
      Schedule(oData, GetTickCount64() + iInterval);
      oData.FState := tsWaiting;
    end;
  finally
    EndUpdate;
  end;
...
{-------------------------------------------------------------------------------}
procedure TFDTimerPool.Remove(ATimer: TFDTimer);
var
  oData: TFDTimerData;
begin
  ASSERT((ATimer <> nil) and (ATimer.FData <> nil));
  BeginUpdate;
  try
    oData := TFDTimerData(ATimer.FData);
    ATimer.FData := nil;
    oData.FTimer := nil;
    if oData.FState = tsWaiting then begin
      ExtractTimer(oData);
      FDFreeAndNil(oData);
    end
    else if oData.FState = tsRaised then
      oData.FState := tsReqRemove;
  finally
    EndUpdate;
  end;
end;
...
{-------------------------------------------------------------------------------}
procedure TFDTimerPool.Execute;
var
  iLastTime: Int64;
  iSleep: Int64;
  oData: TFDTimerData;
begin
  repeat
    ...
    while True do begin
      ...
      FLock.Enter;
      try
        iLastTime := GetTickCount64();
        if FTimerDatas <> nil then begin
          if iLastTime > FTimerDatas.FNextFire then
            Break;
          iSleep := FTimerDatas.FNextFire - iLastTime;
        end;
      finally
        FLock.Leave;
      end;
      FEvent.WaitFor(iSleep);
      if Terminated then
        Exit;
    end;
    FLock.Enter;
    ...
    try
      if (toSynchronized in oData.FTimer.Options) and Assigned(GCheckGuiRunning) then begin
        if GCheckGuiRunning() then
          Synchronize(oData.DoFire);
      end
      else
        oData.DoFire;
    except
    ...
    FLock.Enter;
    try
      if oData.FState in [tsRaised, tsReqRemove] then
        if oData.FState = tsReqRemove then begin
          ExtractTimer(oData);
          FDFreeAndNil(oData);
        end
        else begin
          oData.FState := tsWaiting;
          if toSingleFire in oData.FTimer.Options then
            oData.FTimer.Enabled := False
          else
            Schedule(oData, iLastTime + oData.FTimer.Interval);
        end;
    finally
      FLock.Leave;
    end;
...
{ TFDTimer                                                                      }
{-------------------------------------------------------------------------------}
constructor TFDTimer.Create(AOwner: TComponent);
begin
...
{-------------------------------------------------------------------------------}
function TFDTimer.GetState: TFDTimerState;
begin
  if FData = nil then
    Result := tsDisabled
  else
    Result := TFDTimerData(FData).FState;
end;

{-------------------------------------------------------------------------------}
procedure TFDTimer.SetInterval(AValue: Longword);
begin
...
{-------------------------------------------------------------------------------}
procedure TFDTimer.SetEnabled(AValue: Boolean);
begin
...
{-------------------------------------------------------------------------------}
class procedure TFDTimer.CheckPool;
begin
  FLock.Enter;
  try
    if GTimerPool = nil then
      TFDTimerPool.Create;
  finally
    FLock.Leave;
  end;
...
{-------------------------------------------------------------------------------}
procedure TFDTimer.Update;
begin
  if [csDesigning, csLoading] * ComponentState <> [] then
    Exit;
  if not ((FLock = nil) and (GTimerPool = nil)) then begin
    CheckPool;
    if Enabled then
      GTimerPool.Update(Self)
    else if State <> tsDisabled then
      GTimerPool.Remove(Self);
  end
  else
    FDFreeAndNil(FData);
end;
...
{-------------------------------------------------------------------------------}
procedure TFDTimer.Schedule(AInterval: Longword);
begin
  FInterval := AInterval;
  FEnabled := True;
  Update;
...
{-------------------------------------------------------------------------------}
procedure TFDTimer.Fire;
begin
  Include(FOptions, toImmediateFire);
  FEnabled := True;
  Update;
end;

{-------------------------------------------------------------------------------}
class procedure TFDTimer.LockTimers;
begin
  CheckPool;
  GTimerPool.BeginUpdate;
end;

{-------------------------------------------------------------------------------}
class procedure TFDTimer.UnLockTimers;
begin
  CheckPool;
  GTimerPool.EndUpdate;
end;

{-------------------------------------------------------------------------------}
class procedure TFDTimer.Shutdown;
begin
  FDFreeAndNil(GTimerPool);
end;
    ...
    case ADestEncoding of
      ecANSI:
        begin
          ...
          WideCharToMultiByte(CP_THREAD_ACP, 0, PWideChar(AStr),
    ...
    case ASrcEncoding of
      ecANSI:
        begin
          ...
          MultiByteToWideChar(CP_THREAD_ACP, 0, LPCSTR(PByte(AStr)),
...
{$IFDEF POSIX}
  {$IFDEF NEXTGEN}
    case ASrcEncoding of
      ecANSI:
        Result := TEncoding.ANSI.GetString(AStr);
      ecUTF8:
        Result := TEncoding.UTF8.GetString(AStr);
      ecUTF16:
        Result := TEncoding.Unicode.GetString(AStr);
    end;
  {$ELSE}
    SetLength(sb, Length(AStr));
    Move(PByte(AStr)^, PByte(sb)^, Length(AStr));
    case ASrcEncoding of
      ecANSI:
        Result := TEncoding.ANSI.GetString(sb);
      ecUTF8:
        Result := TEncoding.UTF8.GetString(sb);
      ecUTF16:
        Result := TEncoding.Unicode.GetString(sb);
    end;
  {$ENDIF}
{$ENDIF}
  end;
...
begin
  ...
  if ASrcEncoding = ecUTF16 then begin
    if ASrcLen = -1 then
      ASrcLen := FDWideStrLen(PWideChar(ASrc));
    case ADestEncoding of
      ecANSI:
        begin
          iMaxLen := ASrcLen * SizeOf(TFDAnsiChar) + 1;
          if ADest = nil then
            ADest := Alloc(iMaxLen);
          Result := LocaleCharsFromUnicode(CP_THREAD_ACP, 0, PWideChar(ASrc),
    ...
    case ADestEncoding of
      ...
      ecUTF8:
        begin
          ...
          iLen := UnicodeFromLocaleChars(CP_THREAD_ACP, 0, MarshaledAString(ASrc),
            ASrcLen, pWC, ASrcLen);
      ...
      ecUTF16:
        begin
          iMaxLen := (ASrcLen + 1) * SizeOf(WideChar);
          if ADest = nil then
            ADest := Alloc(iMaxLen);
          Result := UnicodeFromLocaleChars(CP_THREAD_ACP, 0, MarshaledAString(ASrc),
    ...
    case ASrcEncoding of
      ...
      ecUTF8:
        begin
          if ASrcLen = -1 then
            ASrcLen := FDAnsiStrLen(PFDAnsiString(ASrc));
          iMaxLen := (ASrcLen + 1) * SizeOf(TFDAnsiChar);
          if Odd(iMaxLen) then
            Inc(iMaxLen);
          if ADest = nil then begin
            ADest := Alloc(iMaxLen + (ASrcLen + 1) * SizeOf(WideChar));
          ...
          iLen := UnicodeFromLocaleChars(CP_UTF8, 0, MarshaledAString(ASrc),
            ASrcLen, pWC, ASrcLen);
          Result := LocaleCharsFromUnicode(CP_THREAD_ACP, 0, pWC, iLen,
...
{-------------------------------------------------------------------------------}
class function TFDEncoder.Deco(const ASrc: Pointer; ASrcLen: Integer = -1;
  ASrcEncoding: TFDEncoding = ecDefault): String;
begin
  Result := GEncoder.Decode(ASrc, ASrcLen, ASrcEncoding);
end;
...
begin
  ...
  if not ARead then begin
    ...
    oStream := TFDFileStream.Create(AFileName, fmOpenWrite or fmShareDenyWrite);
  end
  else
    oStream := TFDFileStream.Create(AFileName, fmOpenRead or fmShareDenyWrite);
...
begin
  ...
  if ARead then begin
    ...
    if Length(sb1) <> 0 then begin
      SetLength(sb2, Length(sb1));
      FStream.Read(PByte(sb2)^, Length(sb2));
      if sb1 <> sb2 then
        FStream.Position := 0;
    end;
  end
    ...
    if AAppend and (FStream.Size > 0) then begin
      ...
      if FStream is TFDFileStream then
        TFDFileStream(FStream).SyncBuffer(True);
      ...
      if FStream is TFDFileStream then
        TFDFileStream(FStream).SyncBuffer(True);
      sb1 := FEncoder.Preamble;
      if Length(sb1) <> 0 then
        FStream.Write(PByte(sb1)^, Length(sb1));
    end;
end;
...
begin
  if Length(AText) > 0 then begin
    ...
    FStream.Write(PByte(sb)^, FDByteStrLength(sb));
  end;
end;
...
begin
  ...
  elDefault: {$IFDEF MSWINDOWS} WriteText(C_FD_WinEOL) {$ENDIF}
             {$IFDEF MACOS}     WriteText(C_FD_MacEOL) {$ENDIF}
             {$IFDEF UNIX}
               {$IFNDEF MACOS}  WriteText(C_FD_UnixEOL) {$ENDIF}{$ENDIF}
             {$IFDEF ANDROID}   WriteText(C_FD_UnixEOL) {$ENDIF};
  elWindows: WriteText(C_FD_WinEOL);
  elUnix:    WriteText(C_FD_UnixEOL);
  elMac:     WriteText(C_FD_MacEOL);
  end;
end;
...
begin
  ...
  until lEOF or (aBuff[i] = #13) or (aBuff[i] = #10);
...
{$IFDEF MSWINDOWS}
  ...
  hKernel: HMODULE;
  pSetDllDirectory: TSetDllDirectory;
  iBinType: Longword;
{$ENDIF}
begin
  FhDLL := 0;
...
{$IFDEF MSWINDOWS}
  hKernel := Winapi.Windows.LoadLibrary(PChar(kernel32));
  pSetDllDirectory := GetProcAddress(hKernel, PChar('SetDllDirectoryW'));
  if not Assigned(pSetDllDirectory) then
  ...
  try
    for i := Low(ADLLNames) to High(ADLLNames) do
      if ADLLNames[i] <> '' then begin
        ...
        if sPath <> '' then
          if Assigned(pSetDllDirectory) then
            pSetDllDirectory(PChar(sPath))
          else
            SetCurrentDir(sPath);
        try
...
{$IFDEF MSWINDOWS}
      if GetBinaryType(PChar(ADLLNames[i]), iBinType) and
         (iBinType = {$IFDEF FireDAC_32} 6 {SCS_64BIT_BINARY} {$ELSE} SCS_32BIT_BINARY {$ENDIF}) then
        sErr := sErr + Format('%s has not supported architecture [%s]. Required [%s].', [ADLLNames[i],
          {$IFDEF FireDAC_32} 'x64' {$ELSE} 'x86' {$ENDIF},
          {$IFDEF FireDAC_32} 'x86' {$ELSE} 'x64' {$ENDIF}]) + C_FD_EOL;
{$ENDIF}
    end;
    sErr := sErr + FDLastSystemErrorMsg;
...
{$IFDEF POSIX}
  SetLength(FDLLName, 255);
  SetLength(FDLLName, GetModuleFileName(FhDLL, PChar(FDLLName), 255) - 1);
{$ENDIF}
end;
...
{-------------------------------------------------------------------------------}
procedure TFDIniFile.WriteString(const Section, Ident, Value: String);
begin
...
{-------------------------------------------------------------------------------}
procedure TFDIniFile.DeleteKey(const Section, Ident: String);
begin
...
{-------------------------------------------------------------------------------}
procedure TFDIniFile.EraseSection(const Section: string);
begin
...
{-------------------------------------------------------------------------------}
procedure TFDIniFile.UpdateFile;
begin
  if FModified then begin
    FModified := False;
...
var
  sb: TFDByteString;
begin
  sb := TFDEncoder.Enco(AStr + #13#10, ecANSI);
  AStream.Write(PByte(sb)^, FDByteStrLength(sb));
end;
...
begin
  ...
  if AStr <> '' then
    if Length(AStr) > Length(CCommentBody) - 6 then
      Move(AStr[1], S[3], (Length(CCommentBody) - 6) * SizeOf(Char))
    else
      Move(AStr[1], S[3], Length(AStr) * SizeOf(Char));
  WriteStr(AStream, S);
end;
...
begin
  WriteStr(AStream, '');
  if ASourceName <> '' then
    WriteStr(AStream, '// Generated for: Stored Procedure ' + ASourceName);
end;

{-------------------------------------------------------------------------------}
constructor TFDCodeGenerator.Create;
var
  intf: TFDIntfCodePart;
  impl: TFDImplCodePart;
begin
  inherited Create;
  for intf := Low(TFDIntfCodePart) to High(TFDIntfCodePart) do
    FIntfStreams[intf] := TMemoryStream.Create;
  for impl := Low(TFDImplCodePart) to High(TFDImplCodePart) do
    FImplStreams[impl] := TMemoryStream.Create;
end;

{-------------------------------------------------------------------------------}
destructor TFDCodeGenerator.Destroy;
var
  intf: TFDIntfCodePart;
  impl: TFDImplCodePart;
begin
...
{-------------------------------------------------------------------------------}
procedure TFDCodeGenerator.WriteImpl(APart: TFDImplCodePart; const AStr: String);
begin
  WriteStr(FImplStreams[APart], AStr);
end;

{-------------------------------------------------------------------------------}
procedure TFDCodeGenerator.WriteImplNewMethod(ASourceName: String);
begin
  StartNewMethod(FImplStreams[ipMethod], ASourceName);
end;

{-------------------------------------------------------------------------------}
procedure TFDCodeGenerator.WriteIntf(APart: TFDIntfCodePart; const AStr: String);
begin
  WriteStr(FIntfStreams[APart], AStr);
end;

{-------------------------------------------------------------------------------}
procedure TFDCodeGenerator.SaveImpl(AStream: TStream);
var
  lNeedDelim: Boolean;
begin
...
{-------------------------------------------------------------------------------}
procedure TFDCodeGenerator.SaveIntf(AStream: TStream);
begin
  WriteStr(AStream, '  // ' + MyClassName);
  WriteStr(AStream, '  // Generated for: ' + OriginalObject);
  ...
  if FIntfStreams[cpPublished].Size > 0 then begin
    WriteStr(AStream, '  published');
    AStream.CopyFrom(FIntfStreams[cpPublished], 0);
  end;
  WriteStr(AStream, '  end;');
  WriteStr(AStream, '');
end;
...
{-------------------------------------------------------------------------------}
function FDByteStrLength(const AStr: TFDByteString): Integer;
begin
  Result := Length(AStr);
  if Result > 0 then
    Dec(Result);
...
begin
  ...
  while i <= Length(AFieldNames2) do begin
    ...
    if sField <> '' then
      if Pos(';' + UpperCase(sField) + ';', sUCResult) = 0 then begin
...
begin
  ...
  if AIndex >= 0 then begin
    ...
    i := Pos('=', s);
    if i > 0 then
      Result := Copy(s, i + 1, MAXINT div 2);
  end;
end;
...
begin
  if AFrom <= Length(AString) then begin
    ...
    pFrom := pS;
    while (pS^ <> #0) and (StrScan(PChar(AChars), pS^) = nil) do
      Inc(pS);
    Inc(AFrom, pS - pFrom + 1);
    SetString(Result, pFrom, pS - pFrom);
    if ASqueeze then
      while (AFrom <= Length(AString)) and (StrScan(PChar(AChars), AString[AFrom]) <> nil) do
        Inc(AFrom);
  end
  else
    Result := '';
...
function FDSQLTimeStamp2Time(const AValue: TSQLTimeStamp): Longint;
begin
  Result := LongInt(LongWord(((AValue.Hour * 60 + AValue.Minute) * 60 +
...
function FDTime2SQLTimeStamp(const AValue: Longint): TSQLTimeStamp;
begin
  Result := FDDateTime2SQLTimeStamp(FDTime2DateTime(AValue));
end;

{-------------------------------------------------------------------------------}
function FDSQLTimeStamp2Date(const AValue: TSQLTimeStamp): Longint;
begin
  Result := Longint(Trunc(FDSQLTimeStamp2DateTime(AValue) - C_Date_1_1_1 + 1.0));
end;

{-------------------------------------------------------------------------------}
function FDDate2SQLTimeStamp(const AValue: Longint): TSQLTimeStamp;
begin
  Result := FDDateTime2SQLTimeStamp(FDDate2DateTime(AValue));
end;
...
begin
{$IFNDEF NEXTGEN}
  // workaround for QC 72462
  if VarType(AValue) = varUString then
    Result := VarToSQLTimeStamp(VarAsType(AValue, varString))
  else
{$ENDIF}
    Result := VarToSQLTimeStamp(AValue);
end;
...
{-------------------------------------------------------------------------------}
{$IFDEF UNIX}
function gettimeofday(tp: ptimeval; tzp: ptimezone): cint; cdecl; external clib name 'gettimeofday';
{$ENDIF}
...
{$ENDIF}
{$IFDEF ANDROID}
  gettimeofday(rTimeval, nil);
  Result := Int64(rTimeval.tv_sec) * 100000 + Int64(rTimeval.tv_usec);
{$ENDIF}
{$IFDEF UNIX}
  GetTimeOfDay(@rTimeval, nil);
  Result := Int64(rTimeval.tv_sec) * 100000 + Int64(rTimeval.tv_usec);
...
var
  tm: TDateTime;
begin
  Result := EncodeDate(AYear, AMonth, FDay);
  tm := EncodeTime(AHour, AMin, ASec, AMSec);
  if Result >= 0 then
    Result := Result + tm
  else
    Result := Result - tm;
...
{$ELSE}
  {$IFDEF IOS}
  Result := FDGetAppPath + 'tmp';
  {$ELSE}
    {$IFDEF POSIX}
  Result := FDReadEnvValue('TMPDIR');
  if Result = '' then
    {$ENDIF}
    Result := FDReadEnvValue('TEMP');
  if Result = '' then
    Result := FDReadEnvValue('TMP');
  {$ENDIF}
{$ENDIF}
end;
...
begin
  ...
  lNext := False;
  while True do begin
    i1 := FDPosEx('$(', Result, i1);
    if i1 = 0 then
      Break;
    i2 := FDPosEx(')', Result, i1 + 2);
    if i2 = 0 then
      Break;
...
function FDIsDesignTime: Boolean;
begin
  Result := (CompareText(ExtractFileName(ParamStr(0)),
    {$IFDEF BCB} 'bcb.exe' {$ELSE} 'bds.exe' {$ENDIF}) = 0);
end;

{-------------------------------------------------------------------------------}
function FDToolName: String;
begin
  Result := 'RAD Studio XE5';
end;
...
begin
  ...
  // Workaround for Delphi XE2, where StrToBcd always returns even precision
...
var
  ...
  iLen: Integer;
begin
  FDBCD2Str(aBuff, iLen, ABcd, Char(FormatSettings.DecimalSeparator));
...
procedure FDInt2Str(ASrcData: Pointer; ASrcLen: Integer;
  var ADestData: PChar; var ADestLen: Integer; AScale: Integer);
var
  i: LongInt;
  i64: Int64;
  lNeg: Boolean;
...
begin
  ...
  if ASrcLen = SizeOf(Int64) then begin
    ...
    lNeg := i64 < 0;
    repeat
      pBuff^ := Char(Abs(i64 mod 10) + Ord('0'));
      Dec(pBuff);
      Inc(iDigs);
      if iDigs = AScale then begin
        pBuff^ := Char(FormatSettings.DecimalSeparator);
        Dec(pBuff);
        iDigs := $FF;
      end;
      i64 := i64 div 10;
    ...
    case ASrcLen of
    SizeOf(ShortInt): i := PShortInt(ASrcData)^;
    SizeOf(SmallInt): i := PSmallInt(ASrcData)^;
    SizeOf(Integer):  i := PInteger(ASrcData)^;
    else              i := 0; ASSERT(False);
    end;
    lNeg := i < 0;
    repeat
      pBuff^ := Char(Abs(i mod 10) + Ord('0'));
      Dec(pBuff);
      Inc(iDigs);
      if iDigs = AScale then begin
        pBuff^ := Char(FormatSettings.DecimalSeparator);
...
var
  ...
  begin
    ...
    if ANoSign then
      ...
      SizeOf(LongWord): sType := 'LongWord';
  ...
  if AScale = 0 then begin
    if ADestLen = SizeOf(Int64) then begin
      ...
      while ASrcData <= pEnd do begin
        if ANoSign and (lw > $FFFFFFFF div 10) then
          RangeError;
        lw := lw * 10 + Longword(Ord(ASrcData^) - Ord('0'));
        Inc(ASrcData);
      end;
      if lNeg then
    ...
    if ADestLen = SizeOf(Int64) then begin
      ...
      while (ASrcData <= pEnd) and (iDecs < AScale) do begin
        if ASrcData^ = Char(FormatSettings.DecimalSeparator) then
          lHasPoint := True
        else begin
          if lHasPoint then
            Inc(iDecs);
...
{------------------------------------------------------------------------------}
procedure FDStr2Float(pIn: PChar; ASize: Integer; out AExt: Extended; ADot: Char = '.');
...
begin
  if (pIn + ASize)^ <> #0 then
    (pIn + ASize)^ := #0;
  pFS := @GCachedNumFS;
  pFS^.DecimalSeparator := Char(ADot);
  if not TextToFloat(pIn, AExt, fvExtended, pFS^) then
    Error;
end;
...
{$IFDEF POSIX}
  ...
  __write(2, PByte(s), FDByteStrLength(s));
{$ENDIF}
end;
...
{$ENDIF}
  ...
  FLock := TCriticalSection.Create;
  FExpandStrCache := TFDStringList.Create;
  ...
  GEncoder := TFDEncoder.Create(nil);
