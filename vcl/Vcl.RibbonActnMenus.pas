...
type
  ...
  protected
    ...
    function HasGripItem: Boolean; inline;
    procedure WMEntersizemove(var Message: TMessage); message WM_ENTERSIZEMOVE;
    procedure WMExitsizemove(var Message: TMessage); message WM_EXITSIZEMOVE;
    procedure WMNCHitTest(var Message: TWMNCHitTest); message WM_NCHITTEST;
    procedure WMSize(var Message: TWMSize); message WM_SIZE;
    procedure CMShowingchanged(var Message: TMessage); message CM_SHOWINGCHANGED;
    procedure ResizingFinished; virtual;
    ...
    begin
      ...
      begin
        ...
        if (LTop - FRecentDocuments.Top) + LCtrl.Height > FRecentDocuments.Height then
          LCtrl.SetBounds(cAlignOffScreen, LTop, FRecentDocuments.Width - 2, LCtrl.Height)
        else
...
type
  TRibbonApplicationMenuButtonAccess = class(TRibbonApplicationMenuButton)

  end;

procedure TRibbonApplicationPopupMenu.CloseMenu;
var
...
begin
  inherited;
  LRibbon := TCustomRibbon(GetRibbon);
  if (LRibbon <> nil) and (LRibbon.ApplicationMenu <> nil) and
    (LRibbon.ApplicationMenu.Menu <> nil) then
  begin
    LRibbon.ApplicationMenu.Menu.InMenuLoop := False;
    LItem := LRibbon.ApplicationMenu.Menu.FindFirstVisibleItem;
    if LItem <> nil then
    begin
      LControl := LItem.Control;
      if (LControl <> nil) and (LControl is TRibbonApplicationMenuButton) then
      begin
        TRibbonApplicationMenuButton(LControl).Selected := False;
        TRibbonApplicationMenuButtonAccess(LControl).MouseInControl := True;
        TRibbonApplicationMenuButtonAccess(LControl).MouseInControl := False;
      end;
    end;
  end;
end;
...
procedure TRibbonStylePopupMenu.CMShowingchanged(var Message: TMessage);
begin
  inherited;
  if FFirstShow then
  begin
    // Need to setup the popup menus constraints once the menu has calculated
    // the required dimensions to be able to show the necessary controls.
...
function TRibbonStylePopupMenu.CreateControl(AnItem: TActionClientItem): TCustomActionControl;
var
  LControl: TControl;
begin
...
function TRibbonStylePopupMenu.HasGripItem: Boolean;
begin
  Result := (FGripItem <> nil) and FGripItem.Visible;
end;

procedure TRibbonStylePopupMenu.ResizingFinished;
begin
...
procedure TRibbonStylePopupMenu.WMEntersizemove(var Message: TMessage);
begin
  FResizing := True;
  inherited;
end;
...
procedure TRibbonStylePopupMenu.WMSize(var Message: TWMSize);
begin
  if (Message.Width > Constraints.MinWidth) and
    (Message.Height > Constraints.MinHeight) then
    inherited
  else
    Message.Result := 1;
end;
...
var
  ...
  I: Integer;
begin
  // Force the application menu button to fade back to its unselected state
  for I := 0 to ControlCount - 1 do
  begin
    if Controls[I] is TRibbonApplicationMenuButton then
      TRibbonApplicationMenuButtonAccess(Controls[I]).MouseInControl := False;
  end;
  inherited;
