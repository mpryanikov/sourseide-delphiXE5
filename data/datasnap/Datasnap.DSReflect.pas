...
begin
  case DBXParameter.DataType of
    ...
    begin
      if TVarData(Value).VType and varByRef <> 0 then
        DBXParameter.Value.SetWideString(string(TVarData(Value).VPointer^))
      else
        DBXParameter.Value.SetWideString(string(TVarData(Value).VUString));
    end;
    TDBXDataTypes.TableType:
...
begin
  ...
  try
   infoHeader := GetMethodInfo(instance, methodName);
