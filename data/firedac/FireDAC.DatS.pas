...
{$ENDIF}
  System.Classes,
  FireDAC.Stan.Intf, FireDAC.Stan.Error, FireDAC.Stan.Util, FireDAC.Stan.Storage,
    FireDAC.Stan.Consts, FireDAC.Stan.Option;
...
type
  ...
  TFDDatSMergeOption = (goNone);
  TFDDatSMergeOptions = set of TFDDatSMergeOption;

  TFDDatSViewState = (vsOutdated);
  ...
  TFDDatSHandleMode = (lmStandard, lmMove, lmHavyMove, lmFetching,
    lmHavyFetching, lmDestroying, lmRefreshing, lmHavyLoading);
  ...
  private
    {$IFDEF AUTOREFCOUNT} [weak] {$ENDIF} FOwner: TFDDatSObject;
  ...
  protected
    procedure InternalSaveToStorage(AStorage: TFDStorage); override;
    procedure InternalLoadFromStorage(AStorage: TFDStorage); override;
  ...
  private
    ...
    function GetInvariantObject(AColumn: Integer): TFDDatSObject;
    procedure SetInvariantObject(AColumn: Integer; AObj: TFDDatSObject);
    ...
    procedure SaveRowVersion(AStorage: TFDStorage; const AName: String;
      ABuff: PByte; ABuffKind: TFDDatSRowState);
    procedure LoadRowVersion(AStorage: TFDStorage; const AName: String;
  ...
  protected
    ...
    function SetBlobLength(ABuffer: PByte; AColumn: Integer;
      ApData: PByte; ALength: Integer; out AZeroAddOn, AUnitSize: Integer;
      AForceNotNull: Boolean = False): PByte;
    ...
    class function GetStreamingName(AStorage: TFDStorage): String; override;
    procedure InternalSaveToStorage(AStorage: TFDStorage); override;
    procedure InternalLoadFromStorage(AStorage: TFDStorage); override;
  public
    ...
    function GetValues(const ANames: String; ADataSetNative: Boolean): Variant;
...
{$WARNINGS ON}
  ...
  public
    ...
    procedure EndLoadData(const AState: TFDDatSLoadState);
    ...
    procedure MakeColumnMap(ASrcRow: TFDDatSRow; out AColumnMap: TFDArrayOfInteger);
  ...
  public
    ...
    procedure RemRef;
    procedure AcceptChanges;
    procedure BeginLoadData(var AStates: TFDDatSLoadStates; ADataHandleMode: TFDDatSHandleMode = lmHavyMove);
    procedure Clear;
    function Copy: TFDDatSManager;
    procedure EndLoadData(const AStates: TFDDatSLoadStates);
    function HasChanges(ARowStates: TFDDatSRowStates): Boolean;
    procedure Merge(ARow: TFDDatSRow; AOptions: TFDDatSMergeOptions); overload;
    procedure Merge(AManager: TFDDatSManager; AOptions: TFDDatSMergeOptions); overload;
    procedure Merge(ATable: TFDDatSTable; AOptions: TFDDatSMergeOptions); overload;
    procedure RejectChanges;
    procedure Reset;
    procedure Setup(const AOptions: IFDStanOptions);
    // ro
...
{- TFDDatSObject                                                               -}
{-------------------------------------------------------------------------------}
constructor TFDDatSObject.Create;
begin
  inherited Create;
end;
...
{-------------------------------------------------------------------------------}
procedure TFDDatSList.MoveTo(AFromIndex, AToIndex: Integer);
var
  oObj: TFDDatSObject;
begin
...
{-------------------------------------------------------------------------------}
procedure TFDDatSList.InternalLoadFromStorage(AStorage: TFDStorage);
var
  sName: String;
begin
...
{-------------------------------------------------------------------------------}
procedure TFDDatSList.InternalSaveToStorage(AStorage: TFDStorage);
var
  i: Integer;
begin
  for i := 0 to Count - 1 do
...
begin
  if AObj is TFDDatSNamedObject then begin
    oList := NamedList;
    if oList <> nil then
...
{-------------------------------------------------------------------------------}
procedure TFDDatSNamedObject.InternalLoadFromStorage(AStorage: TFDStorage);
begin
  Name := AStorage.ReadString('Name', Name);
end;

{-------------------------------------------------------------------------------}
procedure TFDDatSNamedObject.InternalSaveToStorage(AStorage: TFDStorage);
begin
...
{-------------------------------------------------------------------------------}
procedure TFDDatSNamedList.InternalLoadFromStorage(AStorage: TFDStorage);
var
...
{-------------------------------------------------------------------------------}
procedure TFDDatSBindedObject.InternalLoadFromStorage(AStorage: TFDStorage);
begin
  inherited InternalLoadFromStorage(AStorage);
  SourceName := AStorage.ReadString('SourceName', Name);
  SourceID := AStorage.ReadInteger('SourceID', 0);
end;

{-------------------------------------------------------------------------------}
procedure TFDDatSBindedObject.InternalSaveToStorage(AStorage: TFDStorage);
begin
  inherited InternalSaveToStorage(AStorage);
  if (SourceName <> '') and (SourceName <> DefaultName) and
     (SourceName <> Name) then
...
begin
  ...
  if caBlobData in Attributes then
    ...
    dtAnsiString:       Result := SizeOf(Word) + Size * SizeOf(TFDAnsiChar) + SizeOf(TFDAnsiChar);
    dtByteString:       Result := SizeOf(Word) + Size;
...
{-------------------------------------------------------------------------------}
procedure TFDDatSColumn.InternalLoadFromStorage(AStorage: TFDStorage);
var
...
{-------------------------------------------------------------------------------}
procedure TFDDatSColumn.InternalSaveToStorage(AStorage: TFDStorage);
var
  eAttr: TFDDataAttribute;
  eAttrs: TFDDataAttributes;
  eOpt: TFDDataOption;
...
const
  C_Alignment = {$IFDEF CPUARM} 4 {$ELSE} 2 {$ENDIF};
var
  iOffset: Integer;
  i: Integer;
...
begin
  ...
  try
    ...
    for i := 0 to Count - 1 do begin
      ...
      Inc(iOffset, (oCol.StorageSize + C_Alignment - 1) and not (C_Alignment - 1));
...
{-------------------------------------------------------------------------------}
procedure TFDDatSColumnList.InternalLoadFromStorage(AStorage: TFDStorage);
begin
...
{-------------------------------------------------------------------------------}
procedure TFDDatSColumnList.InternalSaveToStorage(AStorage: TFDStorage);
begin
  Include(FHasThings, ctSerializing);
  try
...
{-------------------------------------------------------------------------------}
procedure TFDDatSConstraintBase.InternalLoadFromStorage(AStorage: TFDStorage);
var
  lEnforce: Boolean;
begin
  Enforce := False;
...
{-------------------------------------------------------------------------------}
procedure TFDDatSConstraintBase.InternalSaveToStorage(AStorage: TFDStorage);
begin
  inherited InternalSaveToStorage(AStorage);
...
begin
  ...
  for i := 0 to Count - 1 do
    if ItemsI[i] is TFDDatSUniqueConstraint then
      if {$IFDEF FireDAC_NOLOCALE_META} CompareText {$ELSE} AnsiCompareText {$ENDIF}
         (AFields, TFDDatSUniqueConstraint(ItemsI[i]).ColumnNames) = 0 then begin
...
{-------------------------------------------------------------------------------}
procedure TFDDatSConstraintList.InternalLoadFromStorage(AStorage: TFDStorage);
var
  lEnforce: Boolean;
begin
  Enforce := False;
...
{-------------------------------------------------------------------------------}
procedure TFDDatSConstraintList.InternalSaveToStorage(AStorage: TFDStorage);
begin
  AStorage.WriteBoolean('Enforce', Enforce, True);
  inherited InternalSaveToStorage(AStorage);
end;
...
{-------------------------------------------------------------------------------}
procedure TFDDatSUniqueConstraint.InternalLoadFromStorage(AStorage: TFDStorage);
var
  eOpt: TFDSortOption;
...
{-------------------------------------------------------------------------------}
procedure TFDDatSUniqueConstraint.InternalSaveToStorage(AStorage: TFDStorage);
var
  eOpt: TFDSortOption;
begin
  inherited InternalSaveToStorage(AStorage);
  ...
  begin
    ...
    until (iPos >= oChildView.Rows.Count) or
          (ARow.CompareRows(FRelatedColumns, nil, nil, -1, oChildView.Rows.ItemsI[iPos],
                            FColumns, rvPrevious, [], FDEmptyCC) <> 0);
  end;
...
{-------------------------------------------------------------------------------}
procedure TFDDatSForeignKeyConstraint.InternalLoadFromStorage(AStorage: TFDStorage);
begin
  inherited InternalLoadFromStorage(AStorage);
  ColumnNames := AStorage.ReadString('ColumnNames', '');
...
{-------------------------------------------------------------------------------}
procedure TFDDatSForeignKeyConstraint.InternalSaveToStorage(AStorage: TFDStorage);
begin
  inherited InternalSaveToStorage(AStorage);
  AStorage.WriteString('ColumnNames', ColumnNames, '');
...
{-------------------------------------------------------------------------------}
procedure TFDDatSCheckConstraint.InternalLoadFromStorage(AStorage: TFDStorage);
begin
  inherited InternalLoadFromStorage(AStorage);
  Expression := AStorage.ReadString('Expression', '');
end;
...
{-------------------------------------------------------------------------------}
procedure TFDDatSCheckConstraint.InternalSaveToStorage(AStorage: TFDStorage);
begin
  inherited InternalSaveToStorage(AStorage);
  AStorage.WriteString('Expression', Expression, '');
end;
...
{-------------------------------------------------------------------------------}
procedure TFDDatSRelation.InternalLoadFromStorage(AStorage: TFDStorage);
begin
  inherited InternalLoadFromStorage(AStorage);
...
{-------------------------------------------------------------------------------}
procedure TFDDatSRelation.InternalSaveToStorage(AStorage: TFDStorage);
begin
  inherited InternalSaveToStorage(AStorage);
  if ChildTable <> nil then
...
{-------------------------------------------------------------------------------}
procedure TFDDatSMechBase.InternalLoadFromStorage(AStorage: TFDStorage);
var
  lActive: Boolean;
begin
  Active := False;
...
{-------------------------------------------------------------------------------}
procedure TFDDatSMechBase.InternalSaveToStorage(AStorage: TFDStorage);
begin
  inherited InternalSaveToStorage(AStorage);
...
{-------------------------------------------------------------------------------}
procedure TFDDatSMechSort.InternalLoadFromStorage(AStorage: TFDStorage);
var
  eOpt: TFDSortOption;
  eOpts: TFDSortOptions;
begin
...
{-------------------------------------------------------------------------------}
procedure TFDDatSMechSort.InternalSaveToStorage(AStorage: TFDStorage);
var
  eOpt: TFDSortOption;
begin
  inherited InternalSaveToStorage(AStorage);
...
{-------------------------------------------------------------------------------}
procedure TFDDatSMechRowState.InternalLoadFromStorage(AStorage: TFDStorage);
var
...
{-------------------------------------------------------------------------------}
procedure TFDDatSMechRowState.InternalSaveToStorage(AStorage: TFDStorage);
var
  eState: TFDDatSRowState;
begin
  inherited InternalSaveToStorage(AStorage);
...
{-------------------------------------------------------------------------------}
procedure TFDDatSMechRange.InternalLoadFromStorage(AStorage: TFDStorage);
begin
  inherited InternalLoadFromStorage(AStorage);
...
{-------------------------------------------------------------------------------}
procedure TFDDatSMechRange.InternalSaveToStorage(AStorage: TFDStorage);
begin
  inherited InternalSaveToStorage(AStorage);
...
{-------------------------------------------------------------------------------}
procedure TFDDatSMechDistinct.InternalLoadFromStorage(AStorage: TFDStorage);
begin
  inherited InternalLoadFromStorage(AStorage);
...
{-------------------------------------------------------------------------------}
procedure TFDDatSMechDistinct.InternalSaveToStorage(AStorage: TFDStorage);
begin
  inherited InternalSaveToStorage(AStorage);
...
{-------------------------------------------------------------------------------}
procedure TFDDatSMechFilter.InternalLoadFromStorage(AStorage: TFDStorage);
var
...
{-------------------------------------------------------------------------------}
procedure TFDDatSMechFilter.InternalSaveToStorage(AStorage: TFDStorage);
var
  eOpt: TFDExpressionOption;
begin
  inherited InternalLoadFromStorage(AStorage);
...
{-------------------------------------------------------------------------------}
procedure TFDDatSMechDetails.InternalLoadFromStorage(AStorage: TFDStorage);
begin
  inherited InternalLoadFromStorage(AStorage);
...
{-------------------------------------------------------------------------------}
procedure TFDDatSMechDetails.InternalSaveToStorage(AStorage: TFDStorage);
begin
  inherited InternalSaveToStorage(AStorage);
...
{-------------------------------------------------------------------------------}
procedure TFDDatSMechMaster.InternalLoadFromStorage(AStorage: TFDStorage);
begin
  inherited InternalLoadFromStorage(AStorage);
...
{-------------------------------------------------------------------------------}
procedure TFDDatSMechMaster.InternalSaveToStorage(AStorage: TFDStorage);
begin
  inherited InternalSaveToStorage(AStorage);
...
begin
  ...
  Result := TFDDatSObject(PNativeInt(FpInfo + oCols.FFetchedSize +
...
begin
  ...
  PNativeInt(FpInfo + oCols.FFetchedSize +
    SizeOf(NativeInt) * oCols.FInvariantMap[AColumn])^ := NativeInt(AObj);
end;
...
begin
  ...
  dtTime:
    begin
      iDiff := PLongint(ABuff1)^ - PLongint(ABuff2)^;
    ...
    if (iType1 = iType2) and
      ...
      varOleStr:
        Result := FDCompareWideStr(
          ...
          FDWideStrLen(TVarData(V1).VOleStr), FDWideStrLen(TVarData(V2).VOleStr),
          Table.Locale, coNoCase in AOptions, coPartial in AOptions, coNoSymbols in AOptions);
{$IFNDEF NEXTGEN}
      varString:
...
begin
  ...
  if not GetData(AColumn, AVersion, pBuff, 0, iLen, False) then
    ...
    dtTime:
      ...
      if PLongint(pBuff)^ < 0 then
        Result := FDTime2DateTime(PLongint(pBuff)^)
      else
        Result := VarSQLTimeStampCreate(FDTime2SQLTimeStamp(PLongint(pBuff)^));
    dtDate:
      Result := VarSQLTimeStampCreate(FDDate2SQLTimeStamp(PLongint(pBuff)^));
    dtByteString:
      begin
        if iLen = 0 then
...
{$ENDIF}
    ...
    dtWideString, dtWideMemo, dtWideHMemo, dtXML:
      begin
        SetString(SW, PWideChar(pBuff), iLen);
        Result := SW;
      end;
    dtGUID:
...
var
  ...
  begin
    ADataLen := 0;
    FDException(Self, [S_FD_LDatS], er_FD_FixedLenDataMismatch, [oCol.Name]);
  end;
...
{-------------------------------------------------------------------------------}
function TFDDatSRow.GetValues(const ANames: String; ADataSetNative: Boolean): Variant;
var
  iFields, i, j: Integer;

  function Cnv2Dataset(const AValue: Variant): Variant;
  begin
    Result := AValue;
  end;
...
begin
  ...
  if iFields <= 1 then
    Result := Cnv2Dataset(GetData(ANames))
  else begin
    Result := VarArrayCreate([0, iFields - 1], varVariant);
    i := 1;
    for j := 0 to iFields - 1 do
      Result[j] := Cnv2Dataset(GetData(FDExtractFieldName(ANames, i)));
  end;
end;
...
begin
  ...
  dtArrayRef:
    begin
      ...
      Result := oObj = nil;
      if not Result then
        Result := (oObj as TFDDatSNestedRowList).Count = 0;
    end;
  else
...
begin
  ...
  if lRowExists and (DBLockID <> 0) then
    DBUnlock;
end;
...
{-------------------------------------------------------------------------------}
function TFDDatSRow.SetBlobLength(ABuffer: PByte; AColumn: Integer;
  ApData: PByte; ALength: Integer; out AZeroAddOn, AUnitSize: Integer;
  AForceNotNull: Boolean = False): PByte;
var
  oCols: TFDDatSColumnList;
...
begin
  ...
  dtAnsiString:
    begin
      AUnitSize := SizeOf(TFDAnsiChar);
      AZeroAddOn := SizeOf(TFDAnsiChar);
    end;
  dtWideString:
    begin
      AUnitSize := SizeOf(WideChar);
      AZeroAddOn := SizeOf(WideChar);
    end;
  ...
  dtMemo, dtHMemo:
    ...
    begin
      AUnitSize := SizeOf(Byte);
      AZeroAddOn := 0;
    end;
  end;
  if (pBlobData^.FData <> nil) and (ALength = 0) then begin
    FreeMem(pBlobData^.FData, pBlobData^.FLength * AUnitSize + AZeroAddOn);
    pBlobData^.FData := nil;
  end
  else if (pBlobData^.FData = nil) and (ALength > 0) then
    GetMem(pBlobData^.FData, ALength * AUnitSize + AZeroAddOn)
  else if (pBlobData^.FData <> nil) and (ALength > 0) and (pBlobData^.FLength <> ALength) then
    ReallocMem(pBlobData^.FData, ALength * AUnitSize + AZeroAddOn);
...
var
  ...
  p: PByte;
begin
  Result := SetBlobLength(ABuffer, AColumn, ApData, ALength, iZeroAddOn,
    iUnitSize, AForceNotNull);
  if (Result <> nil) and (ApData <> nil) and (ALength <> 0) then begin
    Move(ApData^, Result^, ALength * iUnitSize);
    if iZeroAddOn > 0 then begin
      p := PByte(Result) + ALength * iUnitSize;
...
{-------------------------------------------------------------------------------}
function TFDDatSRow.CheckWrite(AColumn: Integer): PByte;
...
var
  ...
  function RoundTime(AValue: LongInt; AScale: Integer): LongInt;
  var
    iRem: LongInt;
  begin
  ...
  begin
    ...
    // Special case - MSSQL scale for DATETIME or SMALLDATETIME
      ...
      AValue.Fractions := AValue.Fractions - AValue.Fractions mod LongWord(AScale)
    else if AScale < 60000 then begin
      AValue.Fractions := 0;
  ...
  begin
    if AScale = 1000 then
      ...
      AValue.Fractions := 0;
      AValue.Seconds := AValue.Seconds - AValue.Seconds mod LongWord(AScale div 1000);
    end;
  end;
  ...
  if caBlobData in oCol.Attributes then begin
    ...
    dtTime:
      begin
        ...
        if ABuff <> nil then
          if not lFetching and Table.Round2Scale and (oCol.Scale > 0) then
            PLongint(pSrc)^ := RoundTime(PLongint(ABuff)^, oCol.Scale)
          else
...
begin
  if AFinalBlobLen <> -1 then begin
    pBuff := CheckWrite(AColumn);
    SetBlobData(pBuff, AColumn, nil, AFinalBlobLen);
  end;
  ...
  try
    try
      if not VarIsNull(AValue) and not VarIsEmpty(AValue) then
        ...
        dtTime:
          begin
            i32 := FDSQLTimeStamp2Time(FDVar2SQLTimeStamp(AValue));
            pData := @i32;
          end;
        dtDate:
          begin
            i32 := FDSQLTimeStamp2Date(FDVar2SQLTimeStamp(AValue));
            pData := @i32;
          end;
        dtByteString,
        dtAnsiString,
...
begin
  if not (Table.DataHandleMode in [lmStandard, lmRefreshing]) then
    Exit;
  case AProposedState of
  rsInitializing,
...
var
  V: Variant;
begin
  case Table.Columns[AColumn].DataType of
  dtRowSetRef:
    Result := '<rowset>';
  dtCursorRef:
    Result := '<cursor>';
  dtRowRef:
    Result := '(' + NestedRow[AColumn].DumpRow(AWithColNames, AVersion) + ')';
  dtArrayRef:
    Result := '(' + NestedRow[AColumn].DumpRow(False, AVersion) + ')';
  dtParentRowRef:
    Result := '<parent>';
  else
    ...
    if VarIsNull(V) then
      Result := '<null>'
    else if Table.Columns[AColumn].DataType in [dtDateTime, dtDateTimeStamp] then
      Result := SQLTimeStampToStr('yyyy-mm-dd hh:nn:ss.zzz', VarToSQLTimeStamp(V))
    else if Table.Columns[AColumn].DataType <> dtTime then
      Result := VarToStr(V)
    else
...
{-------------------------------------------------------------------------------}
class function TFDDatSRow.GetStreamingName(AStorage: TFDStorage): String;
begin
  if (AStorage <> nil) and
...
{-------------------------------------------------------------------------------}
procedure TFDDatSRow.LoadRowVersion(AStorage: TFDStorage; const AName: String;
  var ABuff: PByte; ABuffKind: TFDDatSRowState);
var
  ...
  begin
    ...
    for i := 0 to oCols.Count - 1 do begin
      oCol := oCols.ItemsI[i];
      if oCol.DataType in ATypes then
        if AStorage.ReadValue(oCol.Name, i, oCol.DataType, pBuff, iLen) then
          SetData(i, pBuff, iLen);
    end;
  end;

begin
  if not AStorage.TestObject(AName) then begin
    if ABuff <> nil then
      FreeBuffer(ABuff);
    Exit;
  end;
  AStorage.ReadObjectBegin(AName);
  ...
  try
    if ABuff <> nil then
      ...
      ABuff := AllocBuffer;
    LoadTypes(C_FD_AllTypes - C_FD_BlobTypes - [dtByteString]);
    LoadTypes(C_FD_BlobTypes + [dtByteString]);
  finally
    FRowState := iPriorRowState;
    AStorage.ReadObjectEnd(AName);
  end;
end;

{-------------------------------------------------------------------------------}
procedure TFDDatSRow.InternalLoadFromStorage(AStorage: TFDStorage);
begin
  inherited InternalLoadFromStorage(AStorage);
  ...
  FRowPriorState := TFDDatSRowState(AStorage.ReadEnum('RowPriorState',
    TypeInfo(TFDDatSRowState), Integer(rsDetached)));
  if AStorage.TestObject('Exception') then begin
    AStorage.ReadObjectBegin('Exception');
    try
      RowError := AStorage.LoadObject as EFDException;
    finally
      AStorage.ReadObjectEnd('Exception');
    end;
  end;
...
{-------------------------------------------------------------------------------}
procedure TFDDatSRow.SaveRowVersion(AStorage: TFDStorage; const AName: String;
  ABuff: PByte; ABuffKind: TFDDatSRowState);
var
  iPriorRowState: TFDDatSRowState;
...
begin
  if ABuff = nil then
    Exit;
  AStorage.WriteObjectBegin(AName);
  iPriorRowState := RowState;
  FRowState := ABuffKind;
  try
    WriteTypes(C_FD_AllTypes - C_FD_BlobTypes - [dtByteString]);
    WriteTypes(C_FD_BlobTypes + [dtByteString]);
  finally
    FRowState := iPriorRowState;
    AStorage.WriteObjectEnd(AName);
  end;
end;

{-------------------------------------------------------------------------------}
procedure TFDDatSRow.InternalSaveToStorage(AStorage: TFDStorage);
begin
  inherited InternalSaveToStorage(AStorage);
  AStorage.WriteLongWord('RowID', Integer(RowID), $FFFFFFFF);
  AStorage.WriteEnum('RowState', TypeInfo(TFDDatSRowState),
    Integer(RowState), Integer(rsDetached));
  ...
  if FExtraInfo <> nil then begin
    AStorage.WriteObjectBegin('Exception');
    try
      AStorage.SaveObject(FExtraInfo^.FRowException);
    finally
      AStorage.WriteObjectEnd('Exception');
    end;
  end;
  SaveRowVersion(AStorage, 'Original', FpOriginal, rsImportingOriginal);
  SaveRowVersion(AStorage, 'Current', FpCurrent, rsImportingCurent);
  SaveRowVersion(AStorage, 'Proposed', FpProposed, rsImportingProposed);
...
{-------------------------------------------------------------------------------}
procedure TFDDatSAggregate.InternalLoadFromStorage(AStorage: TFDStorage);
var
  lActive: Boolean;
begin
  Active := False;
...
{-------------------------------------------------------------------------------}
procedure TFDDatSAggregate.InternalSaveToStorage(AStorage: TFDStorage);
begin
  inherited InternalSaveToStorage(AStorage);
  AStorage.WriteBoolean('Active', Active, False);
  AStorage.WriteString('Expression', Expression, '');
...
begin
  ...
  if FRowIDOrdered then begin
    iFirstNo := 0;
    iLastNo := FCount - 1;
    iObjRowId := TFDDatSRow(AObj).FRowID;
...
{-------------------------------------------------------------------------------}
procedure TFDDatSTableRowList.InternalLoadFromStorage(AStorage: TFDStorage);
var
  oRow: TFDDatSRow;
  sName: String;
begin
...
{-------------------------------------------------------------------------------}
procedure TFDDatSView.InternalLoadFromStorage(AStorage: TFDStorage);
var
  sName: String;
  lActive: Boolean;
begin
...
{-------------------------------------------------------------------------------}
procedure TFDDatSView.InternalSaveToStorage(AStorage: TFDStorage);
begin
  inherited InternalSaveToStorage(AStorage);
  AStorage.WriteBoolean('Active', Active, False);
  if SourceView <> nil then
...
{-------------------------------------------------------------------------------}
procedure TFDDatSViewList.InternalSaveToStorage(AStorage: TFDStorage);
var
  i: Integer;
begin
  for i := 0 to Count - 1 do
...
begin
  ...
  if AObj is TFDDatSTable then begin
    ...
    FRound2Scale := TFDDatSTable(AObj).UpdatesRegistry;
    ...
    if TFDDatSTable(AObj).FChanges <> nil then
      FDefaultView := Views.ViewByName(TFDDatSTable(AObj).FChanges.Name);
...
begin
  ...
  if Result then
    Result := (MinimumCapacity = TFDDatSTable(AObj).MinimumCapacity) and
      ...
      (Round2Scale = TFDDatSTable(AObj).UpdatesRegistry) and
...
{-------------------------------------------------------------------------------}
procedure TFDDatSTable.EndLoadData(const AState: TFDDatSLoadState);
var
  i, j: Integer;
  oChildTab: TFDDatSTable;
...
{-------------------------------------------------------------------------------}
procedure TFDDatSTable.MakeColumnMap(ASrcRow: TFDDatSRow; out AColumnMap: TFDArrayOfInteger);
var
  i: Integer;
...
begin
  ...
  for i := 0 to Columns.Count - 1 do
    if oSrcTab = Self then
      AColumnMap[i] := i
    else begin
      AColumnMap[i] := oSrcTab.Columns.IndexOfName(Columns[i].Name);
      if (AColumnMap[i] >= 0) and
         (Columns[i].DataType in [dtRowSetRef, dtCursorRef, dtRowRef,
                                  dtArrayRef, dtParentRowRef]) and
         (Columns[i].DataType <> oSrcTab.Columns[AColumnMap[i]].DataType) then
        AColumnMap[i] := -1;
    end;
end;
...
var
  ...
  procedure AssignColData(ABuffKind: TFDDatSRowState);
  var
      ...
      dtRowRef, dtArrayRef:
        begin
          ...
          if oSrcNestedRow <> nil then begin
            ...
            oDestNestedTab.MakeColumnMap(oSrcNestedRow, aMap);
      ...
      dtRowSetRef, dtCursorRef:
        begin
          ...
          if oSrcNestedRows.Count <> 0 then begin
            oDestNestedTab := Columns.ItemsI[iDestColInd].NestedTable;
            oDestNestedTab.MakeColumnMap(oSrcNestedRows[0], aMap);
...
{-------------------------------------------------------------------------------}
{$HINTS OFF}
procedure TFDDatSTable.InternalLoadFromStorage(AStorage: TFDStorage);
var
  iIndex: Integer;
  rState: TFDDatSLoadState;
begin
  ...
  if AStorage.IsStored(siData) then begin
...
{-------------------------------------------------------------------------------}
procedure TFDDatSTable.InternalSaveToStorage(AStorage: TFDStorage);
begin
  inherited InternalSaveToStorage(AStorage);
  if AStorage.IsStored(siMeta) then begin
  ...
  if AStorage.IsStored(siData) then
...
{-------------------------------------------------------------------------------}
procedure TFDDatSUpdatesJournal.InternalLoadFromStorage(AStorage: TFDStorage);
var
  sName: String;
  oRow: TFDDatSRow;
  oTab: TFDDatSTable;
...
begin
  ...
  try
    ...
    sName := GetStreamingName(AStorage);
    while not AStorage.IsObjectEnd(sName) do begin
      AStorage.ReadObjectBegin('Change');
      ...
      oView := oTab.Views.FindSortedView('#', [], []);
      if oView = nil then begin
        oView := oTab.Views.Add('');
        oView.Mechanisms.AddSort('#');
        oList.Add(oView);
      end;
      i := oView.Find([Integer(iRID)], []);
      if i <> -1 then begin
        oRow := oView.Rows.ItemsI[i];
        oRow.FChangeNumber := iChangeNumber;
        AppendRow(oRow);
      end;
      AStorage.ReadObjectEnd('Change');
    end;
  finally
    for i := 0 to oList.Count - 1 do
      FDFree(TFDDatSView(oList[i]));
    FDFree(oList);
  end;
end;

{-------------------------------------------------------------------------------}
procedure TFDDatSUpdatesJournal.InternalSaveToStorage(AStorage: TFDStorage);
var
  oRow: TFDDatSRow;
begin
  if not AStorage.IsStored(siDelta) then
  ...
  while oRow <> nil do begin
    AStorage.WriteObjectBegin('Change');
    ...
    AStorage.WriteObjectEnd('Change');
...
{-------------------------------------------------------------------------------}
procedure TFDDatSManager.Merge(ARow: TFDDatSRow; AOptions: TFDDatSMergeOptions);
begin
...
{-------------------------------------------------------------------------------}
procedure TFDDatSManager.Merge(AManager: TFDDatSManager; AOptions: TFDDatSMergeOptions);
begin
...
{-------------------------------------------------------------------------------}
procedure TFDDatSManager.Merge(ATable: TFDDatSTable; AOptions: TFDDatSMergeOptions);
begin
...
{-------------------------------------------------------------------------------}
procedure TFDDatSManager.InternalLoadFromStorage(AStorage: TFDStorage);
begin
  inherited InternalLoadFromStorage(AStorage);
  if AStorage.IsStored(siMeta) then begin
    CaseSensitive := AStorage.ReadBoolean('CaseSensitive', True);
...
{-------------------------------------------------------------------------------}
procedure TFDDatSManager.InternalSaveToStorage(AStorage: TFDStorage);
begin
  inherited InternalSaveToStorage(AStorage);
  if AStorage.IsStored(siMeta) then begin
    AStorage.WriteBoolean('CaseSensitive', CaseSensitive, True);
...
initialization
  ...
  oMan.RegisterClass(TFDDatSNestedRowList);
  oMan.RegisterClass(TFDDatSTableRowList);
  oMan.RegisterClass(TFDDatSViewRowList);
