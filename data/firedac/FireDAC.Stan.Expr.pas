...
{$I FireDAC.inc}
{$IFDEF WIN32}
  {$HPPEMIT '#pragma link "FireDAC.Stan.Expr.obj"'}
{$ELSE}
  {$HPPEMIT '#pragma link "FireDAC.Stan.Expr.o"'}
{$ENDIF}
...
type
  ...
  private
    FName: String;
    FCalls: array [ecANSI .. ecUTF16] of TFDExpressionCompare;
...
type
  TFDExpressionToken = (etEnd, etSymbol, etName, etLiteral,  etLParen,
...
{$ENDIF}
  ...
  private
    ...
    procedure SaveState(out ABmk: TFDExprParserBmk);
    procedure RestoreState(const ABmk: TFDExprParserBmk);
    procedure FixupFieldNode(ANode: PFDExpressionNode; const AColumnName: String);
    procedure Optimize(ANode: PFDExpressionNode);
  protected
...
{-------------------------------------------------------------------------------}
function IsNumeric(DataType: TFDDataType): Boolean;
begin
  Result := DataType in [
...
{-------------------------------------------------------------------------------}
function IsTemporal(DataType: TFDDataType): Boolean;
begin
...
{-------------------------------------------------------------------------------}
function IsString(DataType: TFDDataType): Boolean;
begin
  Result := DataType in [dtAnsiString, dtWideString, dtGUID];
end;

{-------------------------------------------------------------------------------}
function IsBLOB(DataType: TFDDataType): Boolean;
begin
...
{-------------------------------------------------------------------------------}
function TFDExpression.HandleNotification(AKind, AReason: Word; AParam1, AParam2: LongWord): Boolean;
begin
...
begin
  ...
  try
    ...
    if not (poAggregate in FParserOptions) and not (poDefaultExpr in FParserOptions) and
       (Root^.FDataType <> dtBoolean) then
...
{-------------------------------------------------------------------------------}
procedure TFDExpressionParser.Optimize(ANode: PFDExpressionNode);
begin
...
{-------------------------------------------------------------------------------}
procedure TFDExpressionParser.SaveState(out ABmk: TFDExprParserBmk);
begin
...
{-------------------------------------------------------------------------------}
procedure TFDExpressionParser.RestoreState(const ABmk: TFDExprParserBmk);
begin
...
begin
  inherited Create;
  FFunctions := TFDStringList.Create;
  FFunctions.Capacity := 100;
  FFunctions.Sorted := True;
  FFunctions.Duplicates := dupError;
  FCollations := TFDStringList.Create;
  FCollations.Sorted := True;
  FCollations.Duplicates := dupAccept;
...
begin
  for i := 0 to FFunctions.Count - 1 do
    FDFree(FFunctions.Objects[i]);
  for i := 0 to FCollations.Count - 1 do
    FDFree(FCollations.Objects[i]);
