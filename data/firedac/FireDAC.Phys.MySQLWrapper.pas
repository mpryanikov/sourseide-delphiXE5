...
uses
  FireDAC.Stan.Util, FireDAC.Stan.Intf, FireDAC.Stan.Storage, FireDAC.Stan.Error,
...
{$ENDIF}
    ...
    property ClientVersion: TFDVersion read FClientVersion;
...
{$ENDIF}
    ...
    property ServerOutput: Boolean read FServerOutput write FServerOutput;
    ...
    property Port: Longword read FPort;
...
{-------------------------------------------------------------------------------}
procedure TFDMySQLError.LoadFromStorage(AStorage: TFDStorage);
begin
...
{-------------------------------------------------------------------------------}
procedure TFDMySQLError.SaveToStorage(AStorage: TFDStorage);
begin
  ...
  if (AErrNo = ER_PARSE_ERROR) and Supports(AInitiator, IFDPhysCommand, oCommand) then begin
    ...
    if i1 <> 0 then begin
      ...
      while (i2 > i1) and (sMsg[i2] <> '''') do
        Dec(i2);
      iOff := Pos(FDFixCRLF(Copy(sMsg, i1, i2 - i1 - 1)), sSQL);
...
{-------------------------------------------------------------------------------}
function TMySQLSession.GetWarningCount: LongWord;
...
{$ENDIF}
  ...
  if FLib.mysql_real_query(FPMySQL, my_pchar(PByte(sCmd)), Length(sCmd)) = 0 then begin
    oRes := StoreResult;
    try
...
{$IFDEF FireDAC_MONITOR}
  if Tracing then Trace1;
{$ENDIF}
  iRes := FLib.mysql_real_query(FPMySQL, my_pchar(PByte(ACmd)), FDByteStrLength(ACmd));
...
{$ENDIF}
  ...
  if ServerOutput and (WarningCount <> 0) then
    GetServerOutput;
end;
...
begin
  ALen := PLongWord(NativeUInt(FpLengths) + NativeUInt(AIndex) * SizeOf(my_ulong))^;
...
{-------------------------------------------------------------------------------}
function MySQLNativeExceptionLoad(AStorage: TFDStorage): TObject;
begin
