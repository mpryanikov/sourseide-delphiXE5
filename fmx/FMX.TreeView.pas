...
uses
  ...
  System.Generics.Collections, FMX.TextLayout, FMX.Graphics;
...
function TTreeViewItem.IsHeightStored: Boolean;
begin
...
function TTreeViewItem.IsWidthStored: Boolean;
begin
  Result := False;
end;
...
begin
  ...
  try
    ...
    else if (TextObject is TText) then
    ...
    else if Assigned(FCheck) then
      FChildrenOffset := FCheck.Width
    else if Assigned(FButton) then
...
begin
  ...
  begin
    ...
    begin
      LTreeView.RealignContent;
      // Update position if we appeared abroad content
      if (not LTreeView.AniCalculations.Animation) then
        LTreeView.AniCalculations.ViewportPosition :=
          LTreeView.AniCalculations.ViewportPosition;
    end;
  end;
end;
...
var
  ...
  begin
    ...
    begin
      LCustomHeight := AItem.FCustomHeight;
      ...
      P := TPointF.Create(OffsetX + AItem.Margins.Left, CurY + AItem.Margins.Top);
      ...
      else if LCustomHeight then
        ItemSize.cy := AItem.Height
      else if ItemSize.Height <= 0 then
        ItemSize.cy := 19;
      ...
      AItem.FCustomHeight := LCustomHeight;
...
begin
  UpdateGlobalIndexes;
  if (not IsUpdating) and Assigned(ContentLayout) then
  begin
...
begin
  ...
  begin
    ...
    FHoveredItem.MouseClick(Button, Shift, P.X, P.Y);
  end;
end;
