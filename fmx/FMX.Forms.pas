...
uses
  ...
  FMX.ActnList, FMX.Messages, FMX.Styles, FMX.Controls, FMX.TextLayout, FMX.Graphics
{$IFDEF MSWINDOWS}
  , Winapi.Messages
{$ENDIF}
  ;
...
{$IFDEF MSWINDOWS}
  ...
  ['{ACCC9241-07E2-421B-8F4C-B70D1E4050AE}']
    ...
    procedure SetDesignerOSVersion(const Value: string);
    ...
    property OSVersion: string read GetDesignerOSVersion write SetDesignerOSVersion;
  end;
{$ELSE}
...
{$ENDIF}
  ...
  TFormsCreatedMessage = class (FMX.Messages.TMessage);
  ...
  TFmxFormState = (fsRecreating, fsModal,
                   fsReleased, fsInDesigner,
                   fsWasNotShown, fsShowing, fsUpdateBorder, fsActivation);
  ...
  TScaleChangedMessage = class(FMX.Messages.TMessage<TCommonCustomForm>)
  end;

  TOrientationChangedMessage = class(FMX.Messages.TMessage)
  end;
...
{ TCommonCustomForm }
  ...
  private type
    THandleState = (hsNormal, hsNeedRecreate, hsChanged);
  private
    ...
    procedure WriteDesignerOSVersion(Writer: TWriter);
...
//    function GetImeWindowRect: TRectF; virtual;
    ...
    property FormStyle: TFormStyle read FFormStyle write SetFormStyle default TFormStyle.fsNormal;
  ...
  public
    ...
    procedure ClosePopupForms(SaveForm: TCommonCustomForm);
...
var
  ...
  I : TInteractiveGesture;
begin
  GlobalNameSpace.BeginWrite;
  try
    inherited;
    if (csDesigning in ComponentState) or Supports(self, IDesignerForm, DesignerForm) then
      FFormState := FFormState + [TFMXFormState.fsInDesigner];
    FFormState := FFormState + [TFMXFormState.fsWasNotShown];
    ...
    TPlatformServices.Current.SupportsPlatformService(IFMXFullScreenWindowService, IInterface(FFullScreenWindowService));
  ...
  for I := Low(FGestureRecognizers) to High(FGestureRecognizers) do
    FGestureRecognizers[I] := 0;
...
begin
  ...
  if FGestureRecognizers[Recognizer] = 1 then
    if TPlatformServices.Current.SupportsPlatformService(IFMXGestureRecognizersService, IInterface(RecognizersService)) then
...
begin
  ...
  if FBorderStyle in [TFmxFormBorderStyle.bsNone] then
    FBorderStyle := TFmxFormBorderStyle.bsSizeable;
  try
    CreateHandle;
  finally
    ...
    begin
      ...
      FHandleState := THandleState.hsChanged;
    end;
  end;
...
var
  ...
  procedure Clear;
  begin
    if Assigned(FMainMenuNative) then
    begin
...
var
  ...
  I: Integer;
begin
  FHandleState := THandleState.hsNormal;
...
begin
  if ([csDesigning, csLoading, csDestroying] * ComponentState = []) and
     ([TFmxFormState.fsRecreating, TFmxFormState.fsReleased] * FormState = []) then
  begin
...
begin
  if ([csDesigning, csLoading, csDestroying, csUpdating] * ComponentState = []) and
     ([TFmxFormState.fsRecreating, TFmxFormState.fsReleased] * FormState = []) and
     (([TFmxFormState.fsWasNotShown] * FormState = []) or
      (FHandleState = THandleState.hsNeedRecreate) or
      ([TFmxFormState.fsShowing] * FormState <> [])) then
  begin
    if (FUpdating > 0) then
      Exit;
    ...
    try
      ...
      try
        ...
        FBorder.Recreate;
        CreateHandle;
        if FormStyle <> TFormStyle.fsPopup then
          FWinService.SetClientSize(Self, LClientSize);
        Realign;
        if Visible and ([TFmxFormState.fsShowing] * FormState = []) then
          FWinService.ShowWindow(Self);
      finally
        if Assigned(OldActiveForm) then
...
{$IFDEF MSWINDOWS}
  ...
  if (FFormState * [TFmxFormState.fsShowing]) <> [] then
    Exit;
  // Change the form state
  FFormState := FFormState + [TFmxFormState.fsShowing];
  try
    try
      if TFmxFormState.fsWasNotShown in FormState then
    ...
    try
      ...
      try
        ...
        begin
          ...
          // update position
          case LPosition of
            TFormPosition.poDesktopCenter:
              LPosition := TFormPosition.poDefault; // unsupported
            TFormPosition.poMainFormCenter:
              if not(Assigned(Application) and Assigned(Application.MainForm) and (Application.MainForm <> self)) then
                LPosition := TFormPosition.poScreenCenter;
            ...
            TFormPosition.poScreenCenter:
              begin
                ...
                if not(csDesigning in ComponentState) and (LScreenSize.cx > 0) and (LScreenSize.cy > 0) then
                  LScreenSize := Screen.Size;
                R := TRectF.Create(0, 0, LScreenSize.cx, LScreenSize.cy);
              end;
            TFormPosition.poMainFormCenter:
              R := TRectF.Create(Application.MainForm.Left, Application.MainForm.Top,
                Application.MainForm.Left + Application.MainForm.Width,
                Application.MainForm.Top + Application.MainForm.Height);
            TFormPosition.poOwnerFormCenter:
              R := TRectF.Create(ParentForm.Left, ParentForm.Top, ParentForm.Left + ParentForm.Width,
                ParentForm.Top + ParentForm.Height);
          end;
          // set bounds
          case LPosition of
            TFormPosition.poDesigned:
              SetBounds(Left, Top, Width, Height);
            TFormPosition.poDefaultPosOnly:
              SetBounds(Round(FDefaultWindowRect.Left), Round(FDefaultWindowRect.Top), Width, Height);
            TFormPosition.poDefaultSizeOnly:
              SetBounds(Left, Top, Round(FDefaultWindowRect.Width), Round(FDefaultWindowRect.Height));
            TFormPosition.poScreenCenter, TFormPosition.poMainFormCenter, TFormPosition.poOwnerFormCenter:
...
begin
  if (FHandleState in [THandleState.hsChanged, THandleState.hsNeedRecreate]) or
     (not Assigned(FHandle)) then
  begin
    FHandleState := THandleState.hsNeedRecreate;
    Recreate;
  end;
end;
...
begin
  ...
  Screen.RemoveForm(Self);
  FFormState := FFormState + [TFmxFormState.fsReleased];
  inherited Release;
end;
...
var
  NR, PR: TRectF;
  SizeChanged: Boolean;
  LFirstShowing: Boolean;
begin
  LFirstShowing := [TFmxFormState.fsWasNotShown, TFmxFormState.fsShowing] * FormState =
                   [TFmxFormState.fsWasNotShown, TFmxFormState.fsShowing];
  if (ALeft <> FLeft) or
     (ATop <> FTop) or
     (AWidth <> FWidth) or
     (AHeight <> FHeight) or
  ...
  begin
    ...
    FWidth := AWidth;
    FHeight := AHeight;
    if (not (TFmxFormState.fsWasNotShown in FormState)) or
...
begin
  ...
  if (FHandleState in [THandleState.hsNeedRecreate, THandleState.hsChanged]) or
...
begin
  ...
  begin
    ...
    begin
      Updated;
      if (FHandleState in [THandleState.hsNeedRecreate, THandleState.hsChanged]) then
        Recreate;
    end;
  end;
end;
...
begin
{$IFDEF WIN32}
  if Assigned(FDesigner) and
    ([csLoading, csDestroying] * ComponentState = []) and
    ([TFmxFormState.fsUpdateBorder, TFmxFormState.fsInDesigner] * FormState = [TFmxFormState.fsInDesigner]) and
    (([TFmxFormState.fsWasNotShown] * FormState = []) or ([TFmxFormState.fsShowing] * FormState <> [])) then
  begin
    FFormState := FFormState + [TFmxFormState.fsUpdateBorder];
    try
...
begin
  Result := ((csDesigning in ComponentState) or
             ((Assigned(Screen) and (Screen.FormCount > 0)) or
             (Application = nil) or
...
begin
  if (Msg is TStyleChangedMessage) and
     (not (csDestroying in ComponentState)) and
     ([TFmxFormState.fsWasNotShown] * FormState = []) then
    DoStyleChanged;
end;
...
var
  ...
  LFocused: IControl;
begin
  if (not (TFmxFormState.fsActivation in FFormState)) and
     (FormStyle <> TFormStyle.fsPopup) then
  begin
    if (Visible) and
       ((not Active) or (TFmxFormState.fsShowing in FFormState)) and
       (not (Released or (csDestroying in ComponentState))) and
       (ApplicationState = TApplicationState.asRunning) then
    begin
      FFormState := FFormState + [TFmxFormState.fsActivation];
      try
        OldActiveForm := Screen.ActiveForm;
        FActive := True;
        try
          ...
          if Assigned(FFocused) then
          begin
            if Supports(FFocused, ITextInput, TSC) and Assigned(TSC.GetTextService) then
...
var
  ...
  LFocused: IControl;
begin
  if (not Active) or (TFmxFormState.fsActivation in FFormState) then
    Exit;
  FFormState := FFormState + [TFmxFormState.fsActivation];
  try
    try
...
begin
  if (NewValue = TFormStyle.fsPopup) and
  ...
  begin
    if FFormStyle = TFormStyle.fsPopup then
      Result := TFormStyle.fsNormal
    else
      Result := FFormStyle;
  end
  else
...
begin
  ...
  begin
    OldPopup := FFormStyle = TFormStyle.fsPopup;
    NewPopup := V = TFormStyle.fsPopup;
    LFound := Assigned(Screen) and
             (Screen.Contains(self));
    if ([csLoading, csDestroying] * ComponentState = []) and
       ([TFmxFormState.fsRecreating, TFmxFormState.fsInDesigner,
         TFmxFormState.fsReleased, TFmxFormState.fsWasNotShown] * FormState = []) then
    begin
      OldFocused := Focused;
      OldActive := Active;
...
begin
  Result := Assigned(FFullScreenWindowService);
end;
...
begin
  ...
  Filer.DefineProperty('DesignerOSVersion', ReadDesignerOSVersion, WriteDesignerOSVersion, {$IFDEF MSWINDOWS}True{$ELSE}False{$ENDIF});
...
begin
  {$IFDEF MSWINDOWS}
  FDesignerOSVersion:= Reader.ReadString;
  {$ELSE}
  Reader.ReadString;
  {$ENDIF}
end;
...
procedure TCommonCustomForm.WriteDesignerOSVersion(Writer: TWriter);
begin
  {$IFDEF MSWINDOWS}
  Writer.WriteString(FDesignerOSVersion);
  {$ENDIF}
end;
...
begin
  ...
  if Assigned(Parent) and Parent.GetInterface(IDesignerForm, DesignerForm) then
    Parent := nil;
...
begin
  Result := nil;
  if Assigned(Value) and Assigned(Value.GetObject) then
  begin
    Parent := TFmxObject(Value.GetObject);
    while Assigned(Parent) and (not (Parent is TCommonCustomForm)) do
      Parent := Parent.Parent;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        FFocused.DoExit;
        try
          if Assigned(TextInput) and Assigned(TextInput.GetTextService) then
...
procedure TCommonCustomForm.SetDesignerOSVersion(const Value: String);
begin
  FDesignerOSVersion := Value;
end;
...
begin
  ...
  if Assigned(SB) and Assigned(SB.Style) then
    ResourceObject := TControl(SB.Style.FindStyleResource(FStyleLookup));

  if not Assigned(ResourceObject) then
    if Assigned(TStyleManager.ActiveStyleForScene(Self)) then
      ResourceObject := TControl(TStyleManager.ActiveStyleForScene(Self).FindStyleResource(FStyleLookup));

  if not Assigned(ResourceObject) then
  begin
    if not SameText(FStyleLookup, DefaultFormStyleLookup) then
    begin
    ...
    if not Assigned(ResourceObject) then
    begin
      StyleName := DefaultFormStyleLookup;
      ResourceObject := TControl(FindStyleResource(StyleName));
    end;
  end;
  if not Assigned(ResourceObject) and Assigned(TStyleManager.ActiveStyle(Self)) then
  begin
    if not SameText(FStyleLookup, DefaultFormStyleLookup) then
    begin
      StyleName := FStyleLookup;
      ResourceObject := TControl(TStyleManager.ActiveStyle(Self).FindStyleResource(StyleName));
    end;
    if not Assigned(ResourceObject) then
    begin
      StyleName := DefaultFormStyleLookup;
  ...
  if not Assigned(ResourceObject) then
    ResourceObject := FMX.Types.FindStyleResource(FStyleLookup);
...
procedure TScreen.ClosePopupForms(SaveForm: TCommonCustomForm);
var
  FSaveForm: TCommonCustomForm;
  Found: Boolean;
  I: Integer;
...
begin
  if Assigned(SaveForm) and (SaveForm.FormStyle = TFormStyle.fsPopup) then
    FSaveForm := SaveForm
  else
    FSaveForm := nil;
  Found := True;
  ...
  try
    ...
    begin
      Found := False;
        for I := PopupFormCount - 1 downto 0 do
        begin
          if (FSaveForm <> PopupForms[I]) and
             (PopupForms[I].Visible) and
             (not ClosedList.Contains(PopupForms[I])) and
             (not (csDesigning in PopupForms[I].ComponentState)) and
             (not IsParent(FSaveForm, PopupForms[I])) then
          begin
            Found := True;
            ClosedList.Add(PopupForms[I]);
            PopupForms[I].Close;
            Break;
          end;
        end;
...
begin
  ...
  begin
    ...
    if AForm.FormStyle = TFormStyle.fsPopup then
...
begin
  if not Assigned(FMouseSvc) then
  begin
    if not TPlatformServices.Current.SupportsPlatformService(IFMXMouseService, IInterface(FMouseSvc)) then
...
begin
  Result := False;
  if Assigned(AForm) and Assigned(AParent) then
  begin
    AForm := AForm.ParentForm;
    while Assigned(AForm) do
    begin
...
procedure TApplicationFormFactor.SetSupportedOrientations(
  AOrientations: TFormOrientations);
var
  AppService: IFMXScreenService;
begin
  inherited;
  if TPlatformServices.Current.SupportsPlatformService(IFMXScreenService, IInterface(AppService)) then
