...
uses
  ...
  FireDAC.Stan.Intf, FireDAC.Stan.Error, FireDAC.Stan.Option, FireDAC.Stan.Storage,
...
type
  ...
  public
    ...
    procedure LoadFB(const AVendorHome, AVendorLib: String);
    procedure LoadIB(const AVendorHome, AVendorLib: String);
    procedure Lock;
    procedure Unlock;
    function getb(p: PBStream): ISCSChar;
    function putb(x: ISCSChar; p: PBStream): Integer;
    function putbx(x: ISCSChar; p: PBStream): Integer;
  ...
  public
    ...
    property Status: ISCStatus read FStatus;
  end;
  ...
  public
    ...
    procedure AddDWord(ADWord: LongWord);
    ...
    property AsPByte: PISCSChar read GetAsPByte;
...
{$ENDIF}
  ...
  public
    ...
    constructor Create(AEnv: TIBEnv; AOwningObj: TObject);
    constructor CreateUsingHandle(AEnv: TIBEnv; AHandle: IscDbHandle;
      AOwningObj: TObject);
    destructor Destroy; override;
    procedure Attach(const ADBName: String; AParams: TStrings);
  ...
  public
    ...
    constructor Create(AEnv: TIBEnv; AOwningObj: TObject);
    constructor CreateUsingHandle(AEnv: TIBEnv; AHandle: IscTrHandle;
      AOwningObj: TObject);
    destructor Destroy; override;
  ...
  TIBEventSlice = class(TObject)
    FEvents: TIBEvents;
    FEventBuffer,
    FResultBuffer: PISCUChar;
    FBufferLength: ISC_LONG;
    FEventsID: ISC_LONG;
    FSysAlert: Boolean;
    FStatusVestor: TISCStatusVector;
    FBaseIndex: Integer;
    FEvent: TEvent;
    FThread: TThread;
  end;
  ...
  TIBEventsOnFired = procedure (AEvents: TIBEvents; ABaseIndex: Integer;
    const ACounts: TISCStatusVector) of Object;
  ...
  private
    ...
    function GetBaseXSQLVarV1: PXSQLVarV1;
    function GetBaseXSQLVarV2: PXSQLVarV2;
    function GetBindSize: LongWord;
    function GetDataXSQLVarV1(ARowIndex: LongWord): PXSQLVarV1;
    function GetDataXSQLVarV2(ARowIndex: LongWord): PXSQLVarV2;
  ...
  protected
    property BaseXSQLVarV2: PXSQLVarV2 read GetBaseXSQLVarV2;
    property BaseXSQLVarV1: PXSQLVarV1 read GetBaseXSQLVarV1;
    property DataXSQLVarV2[ARowIndex: LongWord]: PXSQLVarV2 read GetDataXSQLVarV2;
    property DataXSQLVarV1[ARowIndex: LongWord]: PXSQLVarV1 read GetDataXSQLVarV1;
  public
    ...
    procedure SetData(ARowIndex: LongWord; ApData: Pointer; ALen: LongWord;
      ADataType: TFDDataType = dtUnknown); overload;
    // helpful
    function GetData(var AData; out ALen: LongWord): Boolean; overload;
    function GetData(var AData): Boolean; overload;
    procedure SetData(const AData; ALen: LongWord = 0); overload;
{$IFDEF FireDAC_MONITOR}
    function DumpValue(ARowIndex: LongWord): UnicodeString;
...
{$ENDIF}
  ...
  private
    ...
    FRowCount: Word;
    FV2: Boolean;
    function GetVarCount: Smallint;
    procedure SetVarCount(ACount: Smallint);
    function GetItems(AIndex: LongWord): TIBVariable;
  ...
  public
    FXSQLDA: PXSQLDAV2;
    FXSQLVAR: PXSQLVarV2;
    FRowsAffected: array of LongWord;
  ...
  private
    ...
    FIsOpen: Boolean;
    F_type: LongWord;
    Fnum_segments: LongWord;
    Fmax_segment: LongWord;
    Ftotal_length: LongWord;
  ...
  public
    ...
    property num_segments: LongWord read Fnum_segments;
    property max_segment: LongWord read Fmax_segment;
    property total_length: LongWord read Ftotal_length;
    property _type: LongWord read F_type;
    ...
    property BlobID: TISCQuad read FBlobID write FBlobID;
    property National: Boolean read GetNational;
  end;
...
(*
    property sql_records             // see RowsXXXX prooperties
...
*)
  ...
  private
    ...
    FSEPassword: String;
  protected
    procedure GetServiceParams(ABld: TIBPBWriter); virtual; abstract;
    property Encoder: TFDEncoder read FEncoder;
  public
    FSvcHandle: IscSvcHandle;
    constructor Create(AEnv: TIBEnv; AOwningObj: TObject);
    destructor Destroy; override;
    procedure Attach;
    procedure Detach;
  ...
  TIBRestoreOptions = set of (roDeactivateIdx, roNoShadow, roNoValidity,
    roOneAtATime, roReplace, roCreate, roUseAllSpace, roValidate,
    roFixFSSData, roFixFSSMetaData, roMetaDataOnly);
...
{$IFNDEF NEXTGEN}
  System.AnsiStrings,
{$ENDIF}
  System.SysUtils, Data.FmtBCD, Data.SqlTimSt;
...
{$ENDIF}
  LoadBase(AVendorHome, AVendorLib {$IFNDEF FireDAC_IB_STATIC}, [C_GDS, C_TOGO] {$ENDIF});
end;
...
{$IFDEF USE_API}
  ...
  if Assigned(Fisc_get_client_version) then begin
    SetLength(sVer, 4000);
    Fisc_get_client_version(PISCSChar(PByte(sVer)));
    sVer2 := TFDEncoder.Deco(PByte(sVer), -1, ecANSI);
    i := 1;
    if Brand <> ibInterbase then begin
      sTmp := FDStrToken(sVer2, ' -.', i);   // platform
      sTmp := FDStrToken(sVer2, ' -.', i);   // build type, major ver
      sTmp := FDStrToken(sVer2, ' -.', i);   // minor ver
      sRev := FDStrToken(sVer2, ' -.', i);   // revision
      sBuild := FDStrToken(sVer2, ' -.', i); // build
      sTmp := FDStrToken(sVer2, ' -.', i);   // brand
      // product version (up to build kind)
      FVersion := FDVerStr2Int(FDStrToken(sVer2, ' ', i) + '.' + sRev + '.' + sBuild);
    end
    else
...
{-------------------------------------------------------------------------------}
function TIBLib.getb(p: PBStream): ISCSChar;
begin
  Dec(p^.bstr_cnt);
  if p^.bstr_cnt >= 0 then begin
    Result := ISCSChar(Integer(p^.bstr_ptr^) and $FF);
    Inc(p^.bstr_ptr);
  end
  else
    Result := ISCSChar(FBLOB_get(p));
end;

{-------------------------------------------------------------------------------}
function TIBLib.putb(x: ISCSChar; p: PBStream): Integer;
begin
  Dec(p^.bstr_cnt);
  if (x = ISCSChar(#10)) or (p^.bstr_cnt = 0) then
...
{-------------------------------------------------------------------------------}
function TIBLib.putbx(x: ISCSChar; p: PBStream): Integer;
begin
  Dec(p^.bstr_cnt);
...
{-------------------------------------------------------------------------------}
procedure TIBObject.Check(AStatus: ISCStatus; AInitiator: TObject);
begin
  if AStatus = 0 then
    Exit;
...
{-------------------------------------------------------------------------------}
procedure TFDIBError.LoadFromStorage(AStorage: TFDStorage);
begin
...
{-------------------------------------------------------------------------------}
procedure TFDIBError.SaveToStorage(AStorage: TFDStorage);
begin
...
var
  ...
  pStatusVector: PISCStatus;
  buff: array[0..512] of ISCSChar;
  ...
  try
    ...
    repeat
      if lObjName and (pStatusVector^ = isc_arg_gds) then
        if (PISCStatus(NativeUInt(pStatusVector) + 1 * SizeOf(ISCStatus))^ = isc_random) and
           (PISCStatus(NativeUInt(pStatusVector) + 2 * SizeOf(ISCStatus))^ in [isc_arg_string, isc_arg_cstring]) then
          iTakeObjName := 1
        else if (AError.FEnv.Lib.Brand = ibFirebird) and (AError.FEnv.Lib.Version >= ivFB020000) and
                ((PISCStatus(NativeUInt(pStatusVector) + 1 * SizeOf(ISCStatus))^ = isc_dyn_filter_not_found) or
                 (PISCStatus(NativeUInt(pStatusVector) + 1 * SizeOf(ISCStatus))^ = isc_dyn_func_not_found) or
                 (PISCStatus(NativeUInt(pStatusVector) + 1 * SizeOf(ISCStatus))^ = isc_dyn_index_not_found) or
                 (PISCStatus(NativeUInt(pStatusVector) + 1 * SizeOf(ISCStatus))^ = isc_dyn_view_not_found) or
                 (PISCStatus(NativeUInt(pStatusVector) + 1 * SizeOf(ISCStatus))^ = isc_dsql_view_not_found) or
                 (PISCStatus(NativeUInt(pStatusVector) + 1 * SizeOf(ISCStatus))^ = isc_dyn_domain_not_found) or
                 (PISCStatus(NativeUInt(pStatusVector) + 1 * SizeOf(ISCStatus))^ = isc_dyn_proc_not_found) or
                 (PISCStatus(NativeUInt(pStatusVector) + 1 * SizeOf(ISCStatus))^ = isc_dyn_exception_not_found) or
                 (PISCStatus(NativeUInt(pStatusVector) + 1 * SizeOf(ISCStatus))^ = isc_dyn_proc_param_not_found) or
                 (PISCStatus(NativeUInt(pStatusVector) + 1 * SizeOf(ISCStatus))^ = isc_dyn_trig_not_found) or
                 (PISCStatus(NativeUInt(pStatusVector) + 1 * SizeOf(ISCStatus))^ = isc_dyn_charset_not_found) or
                 (PISCStatus(NativeUInt(pStatusVector) + 1 * SizeOf(ISCStatus))^ = isc_dyn_collation_not_found) or
                 (PISCStatus(NativeUInt(pStatusVector) + 1 * SizeOf(ISCStatus))^ = isc_dyn_role_not_found) or
                 (PISCStatus(NativeUInt(pStatusVector) + 1 * SizeOf(ISCStatus))^ = isc_dyn_gen_not_found) or
                 (PISCStatus(NativeUInt(pStatusVector) + 1 * SizeOf(ISCStatus))^ = isc_dyn_table_not_found) or
                 (PISCStatus(NativeUInt(pStatusVector) + 1 * SizeOf(ISCStatus))^ = isc_dsql_table_not_found) or
                 (PISCStatus(NativeUInt(pStatusVector) + 1 * SizeOf(ISCStatus))^ = isc_dyn_package_not_found) or
                 (PISCStatus(NativeUInt(pStatusVector) + 1 * SizeOf(ISCStatus))^ = isc_dyn_schema_not_found)) and
                (PISCStatus(NativeUInt(pStatusVector) + 2 * SizeOf(ISCStatus))^ in [isc_arg_string, isc_arg_cstring]) then
          iTakeObjName := 2;
    ...
    oErr := TFDIBError(AppendError(1, AError.Status, sMsg, sObjName, eKind, -1,
      -1));
...
{-------------------------------------------------------------------------------}
procedure TIBError.Check(AStatus: ISCStatus; AInitiator: TObject = nil);
begin
  if AStatus = 0 then
    Exit;
  FStatus := AStatus;
...
begin
  p := CheckBufferSize(2);
  PWord(p)^ := Word(FLib.Fisc_portable_integer(PISCUChar(@AWord), 2));
end;

{-------------------------------------------------------------------------------}
procedure TIBPBWriter.AddDWord(ADWord: LongWord);
var
  p: PByte;
begin
  p := CheckBufferSize(4);
  PLongWord(p)^ := LongWord(FLib.Fisc_portable_integer(PISCUChar(@ADWord), 4));
end;
...
begin
  ...
  Move(PByte(sb)^, (PByte(FBuffer) + FFinalSize - l)^,
    l * SizeOf(TFDAnsiChar));
end;
...
{-------------------------------------------------------------------------------}
function TIBPBWriter.GetAsPByte: PISCSChar;
begin
  Result := PISCSChar(PByte(AsBlock));
end;
...
begin
  for j := 0 to AParams.Count - 1 do begin
    sCurStr := Trim(LowerCase(FDNameFromIndex(AParams, j)));
    sCurValue := Trim(FDValueFromIndex(AParams, j));
...
{-------------------------------------------------------------------------------}
function TIBPBReader.GetAsPByte: PISCSChar;
begin
  Result := PISCSChar(PByte(AsBlock));
end;
...
begin
  Result := Word(FLib.Fisc_vax_integer(PISCSChar(PByte(FBuffer) + FPos),
    SizeOf(Word)));
...
begin
  Result := Integer(FLib.Fisc_vax_integer(PISCSChar(PByte(FBuffer) + FPos),
    SizeOf(Integer)));
...
const
  ...
  DPBInfos: array[1 .. DPBInfo_Items] of TIBPBInfo = (
    ...
    (Name: 'garbage_collect';        ParamType: prIgno; Code: isc_dpb_garbage_collect), // not implemented
    ...
    (Name: 'no_garbage_collect';     ParamType: prIgno; Code: isc_dpb_no_garbage_collect),// not implemented
...
constructor TIBDatabase.CreateUsingHandle(AEnv: TIBEnv;
  AHandle: IscDbHandle; AOwningObj: TObject);
begin
...
{-------------------------------------------------------------------------------}
procedure TIBDatabase.Attach(const ADBName: String; AParams: TStrings);
...
{$ENDIF}
    Check(Lib.Fisc_attach_database(@Error.FStatusVector, Smallint(FDByteStrLength(sDBName)),
      PISCSChar(PByte(sDBName)), @FDBHandle, Smallint(Length(sDBParams)),
      PISCSChar(PByte(sDBParams))));
  finally
    Lib.Unlock;
  end;
end;
...
begin
  ...
  try
    ...
    if (Lib.Brand = ibFirebird) and (Lib.Version >= ivFB020500) then begin
      ...
      if i <> -1 then
        AParams.Add('set_db_charset=' + FDValueFromIndex(AParams, i));
      sDBParams := Env.BuildPB(Encoder, AParams, isc_tpb_version1, 0,
        PIBPBInfoArr(@DPBInfos), DPBInfo_Items, FilterDBParam);
{$IFDEF FireDAC_MONITOR}
      if Env.Tracing then Trace1;
{$ENDIF}
      Check(Lib.Fisc_create_database(@Error.FStatusVector, Smallint(FDByteStrLength(sDBName)),
        PISCSChar(PByte(sDBName)), @FDBHandle, Smallint(Length(sDBParams)),
        PISCSChar(PByte(sDBParams)), 0));
    end
    else begin
...
{$ENDIF}
      Check(Lib.Fisc_dsql_execute_immediate(@Error.FStatusVector, @FDBHandle,
        @hTr, Smallint(FDByteStrLength(sSQL)), PISCSChar(PByte(sSQL)),
...
{$ENDIF}
  ...
  Check(Lib.Fisc_database_info(@Error.FStatusVector, @FDBHandle, 1, @cInfo,
    SmallInt(ASize), PISCSChar(ABuff)));
...
begin
  ...
  TFDStringList(FParams).CaseSensitive := False;
...
constructor TIBTransaction.CreateUsingHandle(AEnv: TIBEnv;
  AHandle: IscTrHandle; AOwningObj: TObject);
begin
  Create(AEnv, AOwningObj);
  FTRHandle := AHandle;
  FOwnHandle := False;
...
{$ENDIF}
  if AMsg = '' then
    ...
    Check(Lib.Fisc_prepare_transaction2(@Error.FStatusVector, @FTrHandle,
      Smallint(FDByteStrLength(sb) * SizeOf(Byte)), PISCUChar(PByte(sb))));
  end;
end;
...
{$ENDIF}
  ...
  Check(Lib.Fisc_transaction_info(@Error.FStatusVector, @FTRHandle,
        Smallint(SizeOf(lb)), PISCSChar(@lb), Smallint(SizeOf(rb)), PISCSChar(@rb)));
  iLen := Lib.Fisc_vax_integer(@rb[1], 2);
  Result := Lib.Fisc_vax_integer(@rb[3], iLen);
end;
...
{$IFDEF FireDAC_MONITOR}
  if Env.Tracing then Trace1;
{$ENDIF}
  Lib.Fisc_event_counts(@ASlice.FStatusVestor, ASlice.FBufferLength,
    ASlice.FEventBuffer, ASlice.FResultBuffer);
  if not ASlice.FSysAlert and Assigned(FOnFired) then
    FOnFired(Self, ASlice.FBaseIndex, ASlice.FStatusVestor);
end;
...
var
  ...
  function E(AIndex: Integer): PISCUChar;
  begin
    if AIndex >= n then
      Result := nil
    else
      Result := PISCUChar(PByte(aNames[AIndex]));
  end;
...
{-------------------------------------------------------------------------------}
function TIBVariables.GetItems(AIndex: LongWord): TIBVariable;
begin
...
begin
  ...
  if lMultyRows then begin
    ASSERT(V2);
    pVar := FXSQLVAR;
    pBaseVar := PXSQLVarV2(@FXSQLDA^.sqlvar);
  end
  else begin
    pBaseVar := nil;
    if V2 then
      pVar := PXSQLVarV2(@FXSQLDA^.sqlvar)
    else
      pVar := PXSQLVarV1(@FXSQLDA^.sqlvar);
  end;
  ...
  for j := 0 to RowCount - 1 do
    if V2 then begin
      ...
      for i := 0 to VarCount - 1 do begin
        oVar := TIBVariable(FList[i]);
        PXSQLVarV2(pVar)^.sqlind := PSmallInt(pBuff);
        PSmallInt(pBuff)^ := -1;
        if oVar.SQLDataType = SQL_NULL then
          PXSQLVarV2(pVar)^.sqldata := nil
        else
          PXSQLVarV2(pVar)^.sqldata := PByte(PByte(pBuff) + SizeOf(SmallInt));
        PXSQLVarV2(pVar)^.sqllen := Smallint(oVar.DataSize);
        if oVar.SQLDataType = SQL_VARYING then
          PVary(PXSQLVarV2(pVar)^.sqldata).vary_length := Smallint(oVar.DataSize);
        if lMultyRows then begin
          PXSQLVarV2(pVar)^.sqltype := pBaseVar^.sqltype;
          PXSQLVarV2(pVar)^.sqlscale := pBaseVar^.sqlscale;
          PXSQLVarV2(pVar)^.sqlprecision := pBaseVar^.sqlprecision;
          PXSQLVarV2(pVar)^.sqlsubtype := pBaseVar^.sqlsubtype;
          pBaseVar := PXSQLVar(NativeUInt(pBaseVar) + SizeOf(TXSQLVARV2));
        end;
        pVar := PXSQLVar(NativeUInt(pVar) + SizeOf(TXSQLVARV2));
      ...
      for i := 0 to VarCount - 1 do begin
        oVar := TIBVariable(FList[i]);
        PXSQLVarV1(pVar)^.sqlind := PISCShort(pBuff);
        PSmallInt(pBuff)^ := -1;
        if oVar.SQLDataType = SQL_NULL then
          PXSQLVarV1(pVar)^.sqldata := nil
        else
          PXSQLVarV1(pVar)^.sqldata := PISCSChar(PByte(pBuff) + SizeOf(SmallInt));
        PXSQLVarV1(pVar)^.sqllen := Smallint(oVar.DataSize);
        if oVar.SQLDataType = SQL_VARYING then
          PVary(PXSQLVarV1(pVar)^.sqldata).vary_length := Smallint(oVar.DataSize);
        pVar := PXSQLVar(NativeUInt(pVar) + SizeOf(TXSQLVARV1));
        pBuff := PByte(PByte(pBuff) + oVar.BindSize);
      end;
    end;
end;
...
{-------------------------------------------------------------------------------}
function TIBVariables.GetBuffSize: LongWord;
begin
  CheckBuff;
  Result := FBuffSize;
end;
...
{-------------------------------------------------------------------------------}
function TIBVariable.GetBaseXSQLVarV1: PXSQLVarV1;
begin
  ASSERT(not FVars.V2);
  Result := PXSQLVarV1(NativeUInt(@FVars.FXSQLDA.sqlvar) + SizeOf(TXSQLVarV1) * FIndex);
end;

{-------------------------------------------------------------------------------}
function TIBVariable.GetBaseXSQLVarV2: PXSQLVarV2;
begin
  ASSERT(FVars.V2);
  Result := PXSQLVarV2(NativeUInt(@FVars.FXSQLDA.sqlvar) + SizeOf(TXSQLVarV2) * FIndex);
end;

{-------------------------------------------------------------------------------}
function TIBVariable.GetDataXSQLVarV1(ARowIndex: LongWord): PXSQLVarV1;
begin
...
{-------------------------------------------------------------------------------}
function TIBVariable.GetDataXSQLVarV2(ARowIndex: LongWord): PXSQLVarV2;
begin
  ASSERT(FVars.V2 and (ARowIndex <= $7FFFFFFF) and (ARowIndex < FVars.RowCount));
  if FVars.RowCount <= 1 then
    Result := BaseXSQLVarV2
  else
    Result := PXSQLVarV2(NativeUInt(FVars.FXSQLVAR) +
      Word(SizeOf(TXSQLVarV2)) * Word(FVars.VarCount) * ARowIndex +
      Word(SizeOf(TXSQLVarV2)) * FIndex);
end;
...
begin
  if FVars.V2 then begin
    ...
    Result := (pVarV2^.sqlind^ = -1) or (pVarV2^.sqldata = nil);
  end
  else begin
    pVarV1 := DataXSQLVarV1[ARowIndex];
    Result := (pVarV1^.sqlind^ = -1) or (pVarV1^.sqldata = nil);
  end;
end;
...
begin
  ...
  case SQLDataType of
  SQL_VARYING:    Inc(Result, SizeOf(TVary) + DataSize);
  SQL_TEXT:       Inc(Result, DataSize);
  SQL_SHORT:      Inc(Result, SizeOf(SmallInt));
  SQL_LONG:       Inc(Result, SizeOf(Integer));
  SQL_QUAD:       Inc(Result, SizeOf(TISCQuad));
  SQL_INT64:      Inc(Result, SizeOf(Int64));
  SQL_FLOAT:      Inc(Result, SizeOf(Single));
  SQL_D_FLOAT,
  SQL_DOUBLE:     Inc(Result, SizeOf(Double));
  SQL_BOOLEAN_IB: Inc(Result, SizeOf(WordBool));
  SQL_BOOLEAN_FB: Inc(Result, SizeOf(Boolean));
  SQL_TIMESTAMP:  Inc(Result, SizeOf(TISCTimeStamp));
  SQL_TYPE_TIME:  Inc(Result, SizeOf(ISCTime));
  SQL_TYPE_DATE:  Inc(Result, SizeOf(ISCDate));
  SQL_BLOB:       Inc(Result, SizeOf(TISCQuad));
  SQL_ARRAY:      Inc(Result, SizeOf(TISCQuad));
  SQL_NULL:       ;
  end;
end;
...
var
  ...
  begin
    ...
    FDInt2Str(pData, AIntSize, pBuff, iLen, -iScale);
...
begin
  ...
  if FVars.V2 then begin
    pVarV2 := DataXSQLVarV2[ARowIndex];
    Result := (pVarV2^.sqltype and 1 = 0) or (pVarV2^.sqlind^ <> -1);
    ...
    pVarV1 := DataXSQLVarV1[ARowIndex];
    Result := (pVarV1^.sqltype and 1 = 0) or (pVarV1^.sqlind^ <> -1);
    if not Result then begin
      ApData := nil;
      ALen := 0;
      Exit;
  ...
  SQL_TEXT:
    begin
      ...
      if iSubType = csIDOctets then begin
        if (SQLDataType = SQL_TEXT) and FVars.FStatement.StrsTrim then
          while (ALen > 0) and (PByte(pData)[ALen - 1] = 0) do
            Dec(ALen);
        if (ALen = 0) and FVars.FStatement.StrsEmpty2Null then begin
          Result := False;
          ALen := 0;
          if AByRef then
            ApData := nil;
  ...
  SQL_TIMESTAMP:
    if AByRef then
      ...
      FVars.FStatement.Env.Lib.Fisc_decode_timestamp(PISCTimeStamp(pData), @rCTimeStamp);
      PSQLTimeStamp(ApData).Year := Smallint(rCTimeStamp.tm_year + 1900);
      PSQLTimeStamp(ApData).Month := Smallint(rCTimeStamp.tm_mon + 1);
      PSQLTimeStamp(ApData).Day := Smallint(rCTimeStamp.tm_mday);
      PSQLTimeStamp(ApData).Hour := Smallint(rCTimeStamp.tm_hour);
      PSQLTimeStamp(ApData).Minute := Smallint(rCTimeStamp.tm_min);
      PSQLTimeStamp(ApData).Second := Smallint(rCTimeStamp.tm_sec);
      PSQLTimeStamp(ApData).Fractions := PISCTimeStamp(pData).timestamp_time mod
        ISC_TIME_SECONDS_PRECISION div 10;
    end;
  SQL_TYPE_TIME:
    if AByRef then
      ApData := pData
      ...
      FVars.FStatement.Env.Lib.Fisc_decode_sql_time(PISCTime(pData), @rCTimeStamp);
      PLongWord(ApData)^ := LongWord(rCTimeStamp.tm_sec + rCTimeStamp.tm_min * 60 +
        rCTimeStamp.tm_hour * 3600) * 1000 +
        PISCTime(pData)^ mod ISC_TIME_SECONDS_PRECISION div 10;
    end;
  SQL_TYPE_DATE:
    if AByRef then
      ApData := pData
      ...
      ALen := SizeOf(LongWord);
      FVars.FStatement.Env.Lib.Fisc_decode_sql_date(PISCDate(pData), @rCTimeStamp);
      PLongWord(ApData)^ := LongWord(Trunc(EncodeDate(Word(rCTimeStamp.tm_year + 1900),
  ...
  SQL_BLOB:
    if AByRef then
      ...
      PISCQuad(ApData)^ := PISCQuad(pData)^;
  SQL_ARRAY:
...
procedure TIBVariable.SetData(ARowIndex: LongWord; ApData: Pointer;
  ALen: LongWord; ADataType: TFDDataType = dtUnknown);
var
  lIsNull: Boolean;
  pData: Pointer;
  pInd: PSmallint;
  ...
  begin
    FDException(FVars.FStatement.OwningObj, [S_FD_LPhys, Lib.DriverID],
  ...
  var
    ...
    iLen: Integer;
  begin
    FDBCD2Str(aBuff, iLen, PBcd(ApData)^,
      Char(FormatSettings.DecimalSeparator));
...
begin
      ...
      if not lIsNull then begin
        if (SQLDataType = SQL_TEXT) and FVars.FStatement.StrsTrim then
          while (ALen > 0) and (PByte(ApData)[ALen - 1] = 0) do
            Dec(ALen);
        if FVars.FStatement.StrsEmpty2Null and (ALen = 0) then
          lIsNull := True;
      end;
      if not lIsNull then begin
  ...
  SQL_TIMESTAMP:
    if lIsNull then begin
      PISCTimeStamp(pData)^.timestamp_date := 0;
      PISCTimeStamp(pData)^.timestamp_time := 0;
    end
    else begin
      FillChar(rCTimeStamp, SizeOf(rCTimeStamp), 0);
      rCTimeStamp.tm_year := PSQLTimeStamp(ApData).Year - 1900;
      rCTimeStamp.tm_mon := PSQLTimeStamp(ApData).Month - 1;
      rCTimeStamp.tm_mday := PSQLTimeStamp(ApData).Day;
      rCTimeStamp.tm_hour := PSQLTimeStamp(ApData).Hour;
      rCTimeStamp.tm_min := PSQLTimeStamp(ApData).Minute;
      rCTimeStamp.tm_sec := PSQLTimeStamp(ApData).Second;
      FVars.FStatement.Env.Lib.Fisc_encode_timestamp(@rCTimeStamp, PISCTimeStamp(pData));
      PISCTimeStamp(pData)^.timestamp_time := PISCTimeStamp(pData)^.timestamp_time +
        PSQLTimeStamp(ApData).Fractions * 10;
    end;
  SQL_TYPE_TIME:
    if lIsNull then
      PISCTime(pData)^ := 0
      ...
      FVars.FStatement.Env.Lib.Fisc_encode_sql_time(@rCTimeStamp, PISCTime(pData));
      PISCTime(pData)^ := PISCTime(pData)^ + LongWord(ApData^) mod 1000 * 10;
    end;
  SQL_TYPE_DATE:
    if lIsNull then
      PISCDate(pData)^ := 0
      ...
      FVars.FStatement.Env.Lib.Fisc_encode_sql_date(@rCTimeStamp, PISCDate(pData));
    end;
  SQL_BLOB:
    if lIsNull then begin
      PISCQuad(pData)^.gds_quad_high := 0;
      PISCQuad(pData)^.gds_quad_low := 0;
    end
    else
      PISCQuad(pData)^ := PISCQuad(ApData)^;
  SQL_ARRAY:
...
{-------------------------------------------------------------------------------}
{$WARNINGS OFF}
// D2007 gives strange warning "Return value might be undefined"
function TIBVariable.DumpValue(ARowIndex: LongWord): UnicodeString;
var
  pData: Pointer;
  iLen: LongWord;
  s: String;
  ...
  if (SQLDataType = SQL_VARYING) or (SQLDataType = SQL_TEXT) then begin
    if not GetData(ARowIndex, pData, iLen, True) then
      ...
      if iLen > 1024 then begin
        ...
        SetString(sw, PWideChar(pData), iLen);
        Result := '''' + sw + '''';
      end
    else
      if iLen > 1024 then begin
    ...
    if not GetData(ARowIndex, pData, iLen, False) then
      ...
      SQL_SHORT:
        if iScale < -4 then begin
          FDBCD2Str(aBuff2, i, PBcd(pData)^,
            Char(FormatSettings.DecimalSeparator));
          SetString(s, ABuff2, i);
          Result := UnicodeString(s);
        end
        else if iScale < 0 then
          Result := FDCurr2Str(PCurrency(pData)^,
      ...
      SQL_FLOAT:
        Result := FDFloat2Str(PSingle(pData)^,
          FormatSettings.DecimalSeparator);
      SQL_D_FLOAT,
      SQL_DOUBLE:
        Result := FDFloat2Str(PDouble(pData)^,
          FormatSettings.DecimalSeparator);
      SQL_BOOLEAN_IB,
      SQL_BOOLEAN_FB:
      ...
      SQL_TYPE_TIME:
        Result := TimeToStr(FDTime2DateTime(PLongInt(pData)^));
      SQL_TYPE_DATE:
        Result := DateToStr(FDDate2DateTime(PLongInt(pData)^));
      SQL_BLOB:
        Result := '<BLOB>';
      SQL_ARRAY:
        Result := '<ARRAY>';
...
{$ENDIF}
    if ABuffLen > High(Word) then
      ...
      iSeg := Word(ABuffLen);
    Check(Lib.Fisc_put_segment(@Error.FStatusVector, @FBLHandle, iSeg, PISCSChar(ABuff)));
...
begin
  ...
  isc_info_sql_stmt_insert:
    begin
      ...
      // and operation is finished successfull, then one record is
      // inserted.
      FRowsInserted := 1;
      FRowsDeleted := 0;
      FRowsUpdated := 0;
...
{$IFDEF FireDAC_MONITOR}
  if Env.Tracing then Trace2;
{$ENDIF}
  s := Database.Encoder.Encode(ASQL);
  Check(Lib.Fisc_dsql_prepare(@Error.FStatusVector, @FTransaction.FTRHandle,
    @FSTHandle, FDByteStrLength(s), PISCSChar(PByte(s)), Dialect,
    OutVars.FXSQLDA));
...
{$ENDIF}
  ...
  Check(Lib.Fisc_dsql_execute_immediate(@Error.FStatusVector,
    @FDatabase.FDBHandle, @FTransaction.FTRHandle, Smallint(FDByteStrLength(sb)),
    PISCSChar(PByte(sb)), Dialect, InVars.FXSQLDA));
end;
...
begin
  ...
  try
    case FProtocol of
    0:   sSvcName := Encoder.Encode('service_mgr');
    1:   sSvcName := Encoder.Encode(Host + ':service_mgr');
    2:   sSvcName := Encoder.Encode('\\'+ Host + '\service_mgr');
    else SetLength(sSvcName, 0);
    end;

    oWrt.AddByte(isc_spb_version);
    oWrt.AddByte(isc_spb_current_version);
    if UserName <> '' then begin
      oWrt.AddByte(isc_spb_user_name);
      oWrt.AddShortString(UserName);
    end;
    if Password <> '' then begin
      oWrt.AddByte(isc_spb_password);
      oWrt.AddShortString(Password);
    end;
    if SEPassword <> '' then begin
      oWrt.AddByte(isc_dpb_sys_encrypt_password);
      oWrt.AddShortString(SEPassword);
    end;
    if SqlRoleName <> '' then begin
      oWrt.AddByte(isc_spb_sql_role_name);
      oWrt.AddShortString(SqlRoleName);
    end;
    if ConnectTimeout <> 0 then begin
      oWrt.AddByte(isc_spb_connect_timeout);
      oWrt.AddDWord(ConnectTimeout);
    end;
    if InstanceName <> '' then begin
      oWrt.AddByte(isc_spb_instance_name);
      oWrt.AddShortString(InstanceName);
    end;

{$IFDEF FireDAC_MONITOR}
    if Env.Tracing then Trace1;
{$ENDIF}
    Check(Lib.Fisc_service_attach(@Error.FStatusVector, Smallint(FDByteStrLength(sSvcName)),
      PISCSChar(PByte(sSvcName)), @FSvcHandle, oWrt.Len, oWrt.AsPByte));
  finally
    FDFree(oWrt);
    Lib.Unlock;
  end;
...
begin
  ...
  for i := 0 to BackupFiles.Count - 1 do
    if Trim(BackupFiles[i]) <> '' then begin
      ...
      ABld.AddLongString(FDNameFromIndex(BackupFiles, i));

      s := FDValueFromIndex(BackupFiles, i);
      if s <> '' then begin
...
begin
  ...
  if Options - [roFixFSSData, roFixFSSMetaData] <> [] then begin
    ABld.AddByte(isc_spb_options);
    iOpts := 0;
    ...
    if roReplace in Options then
      iOpts := iOpts or isc_spb_res_replace;
    if roCreate in Options then
      iOpts := iOpts or isc_spb_res_create;
...
var
  ...
  begin
    if EUADatabase <> '' then begin
      ...
      ABld.AddLongString(EUADatabase);
    end;
  end;

begin
  ABld.AddByte(C_Action2IB[Action]);
  if Action = saDisplayUsers then begin
    AddEUA;
    Exit;
  end;
...
{ TIBConfig                                                                     }
{-------------------------------------------------------------------------------}
procedure TIBConfig.SetPageBuffers(const AValue: LongWord);
begin
...
{-------------------------------------------------------------------------------}
procedure TIBConfig.SetSQLDialect(const AValue: LongWord);
begin
...
{-------------------------------------------------------------------------------}
procedure TIBConfig.SetSweepInterval(const AValue: LongWord);
begin
...
{-------------------------------------------------------------------------------}
procedure TIBConfig.ShutdownDB(AMode: TIBShutdownMode; ATimeout: LongWord);
begin
  case AMode of
...
// + TIBConfigService.CreateEncryptionKey
// + TIBConfigService.SetSystemEncryption
// + TIBConfigService.GrantEncryptionTo
// + TIBConfigService.DecryptDatabase
// TIBConfigService.DefaultDecryption
// + TIBConfigService.DropEncryptionKey
// + TIBConfigService.EncryptColumn
// + TIBConfigService.EncryptDatabase

// TIBConfigService.CreateJournal
// TIBConfigService.CreateJournalArchive
// TIBConfigService.DropJournal
// TIBConfigService.DropJournalArchive
// TIBConfigService.GetJournalInformation

// TIBConfigService.SetFlushInterval
// TIBConfigService.GetFlushInterval
// TIBConfigService.SetGroupCommit
// TIBConfigService.GetGroupCommit
// TIBConfigService.SetLingerInterval
// TIBConfigService.GetLingerInterval
// TIBConfigService.SetReclaimInterval
// TIBConfigService.GetReclaimInterval
// TIBConfigService.GetSweepInterval

// TIBConfigService.ReclaimMemory
// TIBConfigService.ReserveSpace
// - TIBConfigService.SweepDatabase
// TIBConfigService.DisableFlush
// TIBConfigService.FlushDatabase
...
{-------------------------------------------------------------------------------}
function IBNativeExceptionLoad(AStorage: TFDStorage): TObject;
begin
