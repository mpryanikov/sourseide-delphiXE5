...
uses
  ...
  FMX.Text, FMX.Forms, FMX.Controls, FMX.Graphics;
...
uses
  ...
  FMX.Dialogs, FMX.Menus, Macapi.Foundation, Macapi.QuartzCore, Macapi.CoreGraphics, Macapi.KeyCodes,
  Macapi.ObjCRuntime, Macapi.CoreFoundation, System.RegularExpressions, System.StrUtils,
  System.Variants, FMX.Consts,
  FMX.Canvas.Mac,
  FMX.Canvas.GPU,
  FMX.Context.Mac, FMX.KeyMapping,
...
{$M+}
  ...
  TPlatformCocoa = class(TInterfacedObject, IFMXApplicationService, IFMXSystemFontService, IFMXTimerService,
    IFMXWindowService, IFMXMenuService, IFMXDragDropService, IFMXClipboardService, IFMXCursorService, IFMXMouseService,
    IFMXScreenService, IFMXLocaleService, IFMXDialogService, IFMXTextService, IFMXContextService, IFMXCanvasService,
    IFMXStyleService, IFMXWindowBorderService, IFMXHideAppService, IFMXSystemInformationService, IFMXLoggingService,
    IFMXFullScreenWindowService, IFMXListingService)
  private type
...
begin
  ISFNkey := False;
  Result := PlatformKeyToVirtualKey(AKeyCode, Kind);
  ISFNkey := Kind <> TKeyKind.kkUsual;
end;
  ...
  try
    ...
    try
      if Assigned(Screen) then
        Screen.ClosePopupForms(FOwner.Wnd);
...
begin
  ...
  begin
    ...
    while Assigned(LParent) and
         (LParent <> WindowHandleToPlatform(Application.MainForm.Handle).Wnd) do
...
begin
  try
    if Wnd.FormStyle <> TFormStyle.fsPopup then
    begin
      if Assigned(Screen) and Assigned(PlatformCocoa) and (not PlatformCocoa.FDisableClosePopups) then
        Screen.ClosePopupForms(Wnd);
      Wnd.Activate;
    end;
  except
    Application.HandleException(Self);
...
begin
  AutoReleasePool := TNSAutoreleasePool.Create;
  try
    if (AForm.Owner is TPopup) or (AForm.FormStyle = TFormStyle.fsPopup) then
    ...
    case TOpenCustomForm(AForm).FormStyle of
      TFormStyle.fsPopup: NewLevel := kCGMinimumWindowLevelKey;
      TFormStyle.fsStayOnTop: NewLevel := kCGFloatingWindowLevelKey;
    else
      NewLevel := kCGBaseWindowLevelKey;
    end;
    if Assigned(AForm.ParentForm) and
       Assigned(AForm.ParentForm.Handle) then
    begin
      LParentWindow := WindowHandleToPlatform(AForm.ParentForm.Handle).Wnd;
      if Assigned(LParentWindow) then
      begin
    ...
    try
      ...
      begin
        ...
        try
          ...
          if (AForm.FormStyle = TFormStyle.fsPopup) and
              Assigned(AForm.ParentForm) and
             (AForm.ParentForm.FormStyle = TFormStyle.fsPopup) then
          begin
            Wnd := WindowHandleToPlatform(AForm.ParentForm.Handle).Wnd;
            if Assigned(Wnd) and Assigned(NSApp) then
...
var
  ...
  begin
    ...
    begin
      if CurrentView.Parent is TPopup then
        TPopup(CurrentView.Parent).IsOpen := False;
...
begin
  ...
  try
    ...
    try
      ...
      begin
        if Assigned(P.Root) and (P.Root.GetObject is TCommonCustomForm) then
  ...
  {  if not VarIsObject(Data.Data) and VarIsStr(Data.Data) then
    ...
    begin
      ...
      if Data.Source is TControl then
        ...
        Offset := TPointF.Create(ABitmap.Width div 2, ABitmap.Height div 2);
      Offset.X := -Offset.X;
...
function TPlatformCocoa.GetSystemStyle(const Context: TFmxObject): TFmxObject;
begin
  Result := FMX.Controls.Mac.GetSystemStyle(Context);
end;
