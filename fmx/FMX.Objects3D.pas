...
uses
  System.Classes, System.Types, System.UITypes, System.SysUtils, FMX.Types, FMX.Objects,
  FMX.Types3D, FMX.Controls3D, FMX.MaterialSources, FMX.Graphics;
...
uses System.UIConsts, System.Math, System.RTLConsts, FMX.Import, System.Generics.Collections, System.Generics.Defaults,
  FMX.Materials, FMX.TextLayout;
...
begin
  ...
  begin
    Context.DrawCube(NullVector3D, Vector3D(Width, Height, Depth), AbsoluteOpacity, $8060A799);
  end;
end;

function TDummy.DoRayCastIntersect(const RayPos, RayDir: TVector3D; var Intersection: TVector3D): Boolean;
begin
  Result := False;
...
begin
  ...
  begin
    ...
    Context.DrawLine(Vector3D(X, -Height / 2, 0), Vector3D(X, Height / 2, 0), AbsoluteOpacity, C);
    Context.DrawLine(Vector3D(-X, -Height / 2, 0), Vector3D(-X, Height / 2, 0), AbsoluteOpacity, C);
    X := X + FFrequency;
  end;
  while Y < Height / 2 do
  begin
    if (frac(Y) = 0) and (frac(Y / Marks) = 0) then
      ...
      C := MakeColor(FLineColor, AbsoluteOpacity);
    Context.DrawLine(Vector3D(-Width / 2, Y, 0), Vector3D(Width / 2, Y, 0), AbsoluteOpacity, C);
    Context.DrawLine(Vector3D(-Width / 2, -Y, 0), Vector3D(Width / 2, -Y, 0), AbsoluteOpacity, C);
    Y := Y + FFrequency;
  end;
end;
...
function TGrid3D.DoRayCastIntersect(const RayPos, RayDir: TVector3D; var Intersection: TVector3D): Boolean;
var
  IPoint: TVector3D;
begin
...
begin
  ...
  begin
    if Assigned(FMaterialSource) then
    begin
...
begin
  Context.DrawCube(Vector3D(0, 0, 0), Vector3D(Width, Height, Depth), AbsoluteOpacity, FColor);
end;
...
function TCustomMesh.DoRayCastIntersect(const RayPos, RayDir: TVector3D; var Intersection: TVector3D): Boolean;
begin
...
begin
  ...
  Context.DrawTriangles(FData.VertexBuffer, FData.IndexBuffer, TMaterialSource.ValidMaterial(FMaterialSource), AbsoluteOpacity);
end;
...
function TPlane.DoRayCastIntersect(const RayPos, RayDir: TVector3D; var Intersection: TVector3D): Boolean;
var
  IPoint: TVector3D;
begin
  if RayCastPlaneIntersect(RayPos, RayDir, Vector3D(0,0,0), Vector3D(0, 0, -1), IPoint) and (Abs(IPoint.X) < Width / 2) and (Abs(IPoint.Y) < Height / 2) then
...
function TDisk.DoRayCastIntersect(const RayPos, RayDir: TVector3D;
...
function TCube.DoRayCastIntersect(const RayPos, RayDir: TVector3D; var Intersection: TVector3D): Boolean;
var
...
function TSphere.DoRayCastIntersect(const RayPos, RayDir: TVector3D; var Intersection: TVector3D): Boolean;
var
  INear, IFar: TVector3D;
begin
  // Calling inherited will search through the MeshData for intersection. This is
...
begin
  ...
  begin
    if Assigned(FMaterialBackSource) then
    begin
...
begin
  ...
  begin
    if Assigned(FMaterialShaftSource) then
    begin
...
begin
  ...
  if (csDesigning in ComponentState) and not Locked then
    Context.DrawCube(Vector3D(Width / 2, Height / 2, 0), Vector3D(Width, Height, Depth), AbsoluteOpacity, $8060A799);
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          Result := Result + Vector(P.X - Points[i - 2].X, P.Y - Points[i - 2].Y).Length;
        end
        else
        begin
          Result := Result + Vector(P.X - Points[i - 1].X, P.Y - Points[i - 1].Y).Length;
        end;
      end;
    end;
    Result := Result / 10;
...
procedure TModel3D.ReadModel(Stream: TStream);
  ...
  begin
    ...
    begin
      ...
      FMeshCollection[i].Stored := False;
    end;
  end;
