...
type
  ...
  strict private
    ...
    FCustomTypeMap: TDictionary<Integer, TDSProxyParamAccess>;
  private
...
begin
  FCustomTypeMap.AddOrSetValue(ADbxDataType, AProxyParamRec);
end;
...
begin
  ...
  FCustomTypeMap := TDictionary<Integer, TDSProxyParamAccess>.Create;
...
begin
  FCustomTypeMap.Remove(ADbxDataType);
end;
...
var
  Name: string;
begin
  if FCustomTypeMap.ContainsKey(Param.DataType) then
    Exit(FCustomTypeMap.Items[Param.DataType].TypeIdentifier);
  Name := Param.TypeName;
  if not Name.IsEmpty then
    Exit(Name);
...
begin
  if FCustomTypeMap.ContainsKey(Param.DataType) then
    Exit(FCustomTypeMap.Items[Param.DataType].Getter);
  case Param.DataType of
    TDBXDataTypes.AnsiStringType:
...
begin
  ...
  if FCustomTypeMap.ContainsKey(Param.DataType) then
    Setter := FCustomTypeMap.Items[Param.DataType].Setter
...
constructor TDSProxyParamAccess.Create(const ATypeId, AGetter, ASetter: string);
begin
