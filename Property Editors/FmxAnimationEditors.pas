...
uses
  System.Classes, DesignEditors, DesignIntf, System.Generics.Collections, FMX.Types, FMX.Graphics, System.TypInfo,
...
uses
  ...
  System.RTLConsts, FMX.Consts, FmxBitmapItemEdit, FMX.MultiResBitmap;
...
begin
  ...
  GradientDialog.ShowStyles := GradientDialog.ShowStyles + [FMX.Graphics.TBrushKind.bkGradient];
  GradientDialog.ShowBrushList := false;
  GradientDialog.Brush.Kind := FMX.Graphics.TBrushKind.bkGradient;
...
begin
  ...
  D.ShowStyles := [FMX.Graphics.TBrushKind.bkSolid];
...
procedure TBitmapOfItemProperty.PropDrawValue(ACanvas: TCanvas;
  const ARect: TRect; ASelected: Boolean);
var
  LBitmap: FMX.Graphics.TBitmap;
  LRect, R: TRectF;
  ...
  procedure DrawCross(Color: TColor);
  begin
      ACanvas.Pen.Color := Color;
      ACanvas.MoveTo(ARect.Left + 2, ARect.Top + 2);
      ACanvas.LineTo(ARect.Left + 6, ARect.Top + 6);
      ACanvas.MoveTo(ARect.Left + 5, ARect.Top + 2);
      ACanvas.LineTo(ARect.Left + 1, ARect.Top + 6);
  end;
begin
  LBitmap := Bitmap;
  Item := nil;
