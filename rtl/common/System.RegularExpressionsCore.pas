...
begin
  ...
  SetLength(S, Len1 + Len2 + Len3);

  if Len2 > 0 then
  begin
    for I := Len3-1 downto 0 do
      ...
      case Char(S[J]) of
        ...
        '`':
          begin
            ...
            DeleteBytes(S, I, J+1-I);
            InsertBytes(TEncoding.UTF8.GetBytes(SubjectLeft), S, I);
            I := I + Offsets[0];
            Exit;
          end;
        '''':
          begin
            ...
            DeleteBytes(S, I, J+1-I);
            InsertBytes(TEncoding.UTF8.GetBytes(SubjectRight), S, I);
...
begin
  ...
  OffsetCount := pcre_exec(FPattern, FHints, M.AsRaw(FSubject).ToPointer, FStop, 0, Opts, @Offsets[0], High(Offsets));
  Result := OffsetCount > 0;
...
begin
  ...
  if FStart >= FStop then
    ...
    OffsetCount := pcre_exec(FPattern, FHints, M.AsRaw(FSubject).ToPointer, FStop, FStart, Opts, @Offsets[0], High(Offsets));
  Result := OffsetCount > 0;
  // Convert offsets into TBytes indices
  if Result then
  begin
...
begin
  ...
  DeleteBytes(FSubject, MatchedOffset, MatchedLength);
  if Result <> '' then
    InsertBytes(Tmp, FSubject, MatchedOffset);
  // Position to continue search
  FStart := FStart - MatchedLength +  Length(Tmp);
  FStop := FStop - MatchedLength + Length(Tmp);
...
begin
  if Value <> TEncoding.UTF8.GetString(FSubject) then
  begin
...
begin
  ...
  begin
    ...
    Count := 1;
    repeat
      Strings.Add(TEncoding.UTF8.GetString(CopyBytes(FSubject, Offset, MatchedOffset - Offset)));
      Inc(Count);
  ...
  begin
    ...
    repeat
      ...
      begin
        ...
        begin
          LUseOffset := False;
          Strings.Add(TEncoding.UTF8.GetString(CopyBytes(FSubject, 0, MatchedOffset)));
      ...
      begin
        Strings.Add(TEncoding.UTF8.GetString(CopyBytes(FSubject, LOffset, MatchedOffset - LOffset)));
        Inc(Count);
        if Self.GroupCount > 0 then
          Strings.Add(Groups[GroupCount]);
      end;
...
begin
  ARegEx.Subject := TEncoding.UTF8.GetString(FSubject);
  ARegEx.Start := FStart;
end;
