...
{$IFDEF MSWINDOWS}
  Winapi.Windows,
{$ENDIF}
  System.Classes, System.SyncObjs, Data.FmtBCD,
...
{$IFDEF MSWINDOWS}
  TSQLiteCollationCompareStringW = class;
{$ENDIF}
  TSQLiteBLOB = class;
  ...
  TSQLiteRTreeDoubleArray = array of Double;
...
*)
  ...
  private
    ...
    {$IFDEF AUTOREFCOUNT} [Weak] {$ENDIF} FDatabase: TSQLiteDatabase;
...
{$ENDIF}
  ...
  private
    ...
    {$IFDEF AUTOREFCOUNT} [Weak] {$ENDIF} FCurStatement: TSQLiteStatement;
...
{$ENDIF}
    ...
    function GetSQLHandlerList: TInterfaceList;
...
{$ENDIF}
    ...
    property SQLHandlerList: TInterfaceList read GetSQLHandlerList;
  ...
  private
    {$IFDEF AUTOREFCOUNT} [Weak] {$ENDIF} FDatabase: TSQLiteDatabase;
    {$IFDEF AUTOREFCOUNT} [Weak] {$ENDIF} FExtension: TSQLiteExtension;
  public
...
{$IFDEF MSWINDOWS}
  TSQLiteCollationCompareStringW = class(TSQLiteCollation)
  private
    FLocale: TFDLocalID;
    FFlags: LongWord;
  protected
  ...
  public
    ...
    property Locale: TFDLocalID read FLocale write FLocale;
...
{$ENDIF}
  ...
  private
    {$IFDEF AUTOREFCOUNT} [Weak] {$ENDIF} FStmt: TSQLiteStatement;
    FIndex: Integer;
    FName: String;
    FTabName: String;
    FColName: String;
  ...
  private
    function DoCalculate(geom: psqlite3_rtree_geometry; nCoord: Integer;
      aCoord: PDouble; var pRes: Integer): Integer;
  ...
  private
    {$IFDEF AUTOREFCOUNT} [Weak] {$ENDIF} FRTreeInst: TSQLiteRTreeInstance;
  ...
  FDsqlite3_vtab = packed record
    pVTab: sqlite3_vtab;
    pADModule: TSQLiteVModule;
    pADTable: TSQLiteVTable;
  end;
  PFDsqlite3_vtab = ^FDsqlite3_vtab;
  TSQLiteVArgs = array of UnicodeString;

  FDsqlite3_vtab_cursor = packed record
    pVTab: sqlite3_vtab;
    pADModule: TSQLiteVModule;
    pADTable: TSQLiteVTable;
    pADCursor: TSQLiteVCursor;
  end;
  ...
  protected
    ...
    procedure VTabRename(const ANewName: UnicodeString); virtual; abstract;
...
uses
  System.SysUtils, System.Variants, Data.SqlTimSt,
{$IFNDEF NEXTGEN}
  System.AnsiStrings,
{$ENDIF}
  System.Generics.Collections, System.Types,
  FireDAC.Stan.Storage, FireDAC.Phys.Intf;
...
begin
  ...
  Result := Trunc((X1 + X2 + FDay + B - 1524.5) * MSecsPerDay);
end;

{-------------------------------------------------------------------------------}
procedure j2time(const AValue: Int64; out AHour, AMin, ASec: Word; out AFsec: LongWord);
var
  s: Integer;
begin
...
{-------------------------------------------------------------------------------}
function time2j(AHour, AMin, ASec: Word; AFsec: LongWord): Int64;
begin
...
{$ENDIF}
  ...
  case iMainCode of
  SQLITE_ERROR:
    if Pos('no such', sMsg) = 1 then begin
      eKind := ekObjNotExists;
      i1 := Pos(':', sMsg);
      if i1 <> 0 then
        sObjName := Trim(Copy(sMsg, i1 + 1, MAXINT));
    end
    else if (Pos(': syntax error', sMsg) <> 0) and (AOwner is TSQLiteStatement) then begin
      i1 := Pos('near "', sMsg);
      i2 := FDPosEx('"', sMsg, i1 + 6);
      if (i1 <> 0) and (i1 <> 0) then begin
        ...
        iCmdOffset := Pos(FDFixCRLF(Copy(sMsg, i1 + 6, i2 - i1 - 6)), sSQL) +
          (oStmt.FCmdCurr - PByte(oStmt.FCmdText)) div SizeOf(Char);
      end;
    end;
  SQLITE_CONSTRAINT:
    begin
      i1 := Pos(' may not be NULL', sMsg);
      if i1 <> 0 then
        sObjName := Copy(sMsg, 1, i1 - 1)
      else begin
        i1 := Pos(' is not unique', sMsg);
        if i1 = 0 then
          i1 := Pos(' are not unique', sMsg);
        if i1 <> 0 then begin
          eKind := ekUKViolated;
          sObjName := Copy(sMsg, 1, i1 - 1);
        end
        else begin
          i1 := Pos('PRIMARY KEY must be unique', sMsg);
          if i1 <> 0 then
            eKind := ekUKViolated;
        end;
      end;
...
{$ENDIF}
  ...
  FVTabs := TFDStringList.Create;
  FVTabs.Sorted := True;
  FVTabs.CaseSensitive := False;
...
{$IFDEF FireDAC_MONITOR}
    if Tracing then Trace1;
{$ENDIF}
    Check(Lib.Fsqlite3_key(Handle, PByte(sKey), FDByteStrLength(sKey)));
  end;
end;
...
{$IFDEF FireDAC_MONITOR}
    if Tracing then Trace1;
{$ENDIF}
    Check(Lib.Fsqlite3_rekey(Handle, PByte(sKey), FDByteStrLength(sKey)));
  end;
end;
...
begin
  ...
  if Result then
    TSQLiteVTable(FVTabs.Objects[i]).DescribeColumn(AColName, ANotNull, AInPK,
      AAutoInc);
end;

{-------------------------------------------------------------------------------}
function TSQLiteDatabase.GetSQLHandlerList: TInterfaceList;
begin
  if FSQLHandlerList = nil then
    FSQLHandlerList := TInterfaceList.Create;
  Result := FSQLHandlerList;
end;
...
begin
  ...
  if FDByteStrLength(sBEntry) = 0 then
    pEntry := nil
  else
...
begin
  ...
  try
    for i := 0 to FExtensions.Count - 1 do
      LoadExtension(FDNameFromIndex(FExtensions, i), FDValueFromIndex(FExtensions, i));
  finally
...
{$IFDEF MSWINDOWS}
{-------------------------------------------------------------------------------}
{ TSQLiteCollationCompareStringW                                                }
{-------------------------------------------------------------------------------}
constructor TSQLiteCollationCompareStringW.Create(ALib: TSQLiteLib);
begin
...
{-------------------------------------------------------------------------------}
function TSQLiteCollationCompareStringW.Compare(len1: Integer; str1: Pointer;
  len2: Integer; str2: Pointer): Integer;
begin
  case CompareStringW(FLocale, FFlags,
    ...
    if (PChar(pVal) + 2)^ = ':' then begin
      ...
      if (PChar(pVal) + 5)^ = ':' then begin
        ...
        if (PChar(pVal) + 8)^ = '.' then begin
          ...
          if iMSecSize > 3 then
            iMSecSize := 3;
          FDStr2Int(PChar(pVal) + 9, iMSecSize, @lwMSec, SizeOf(LongWord), True);
        end;
      end;
    end
    else
  ...
  if (pVal = nil) and (iSize = 0) then begin
    ...
    etNumber:
      if AByRef then begin
        ...
        SQLITE_INTEGER:
          ADDouble2BCD(PInt64(pVal)^, PBcd(AValue)^);
        SQLITE_FLOAT:
          ADDouble2BCD(PDouble(pVal)^, PBcd(AValue)^);
        end;
        ASize := SizeOf(TBcd);
      end;
    etCurrency:
    ...
    etDate:
      if AByRef then begin
        ...
        if (wYear <> 0) and (wMonth <> 0) and (wDay <> 0) then
          PLongint(AValue)^ := FDDateTime2Date(EncodeDate(wYear, wMonth, wDay))
        else begin
          PLongint(AValue)^ := 0;
          Result := False;
        end;
    ...
    etTime:
      if AByRef then begin
        ...
        if wHour <> $FF then
          PLongint(AValue)^ := FDDateTime2Time(EncodeTime(wHour, wMin, wSec, lwMSec))
        else begin
          PLongint(AValue)^ := 0;
          Result := False;
        end;
...
begin
  ...
  if GetData(pVal, iSize, False, etDateTime) then
    Result := FDSQLTimeStamp2DateTime(rVal);
end;
  ...
  if (AValue = nil) and (ASize = 0) then begin
    ...
    etDate:
      begin
        ...
        etString:
          begin
            rTS := FDDate2SQLTimeStamp(PLongint(AValue)^);
        ...
        etDateTime:
          begin
            iSize := SizeOf(Double);
            PDouble(pVal)^ := FDDate2DateTime(PLongint(AValue)^);
            iType := SQLITE_FLOAT;
          end;
        end;
      end;
    etTime:
      begin
        ...
        etString:
          begin
            rTS := FDTime2SQLTimeStamp(PLongint(AValue)^);
            iSize := WideFormatBuf(PChar(pVal)^, C_FD_MaxFixedSize div SizeOf(Char),
        ...
        etDateTime:
          begin
            iSize := SizeOf(Double);
            PDouble(pVal)^ := FDTime2DateTime(PLongint(AValue)^);
            iType := SQLITE_FLOAT;
          end;
        end;
      end;
      ...
      begin
        ...
        if not lDate and not ltime then begin
          ...
          etDateTime:
            begin
              iSize := SizeOf(Double);
              PDouble(pVal)^ := FDSQLTimeStamp2DateTime(PSQLTimeStamp(AValue)^);
              iType := SQLITE_FLOAT;
            end;
          end;
        end;
...
begin
  ...
  SetData(PByte(sb), FDByteStrLength(sb), etString);
end;
...
var
  rDT: TSQLTimeStamp;
begin
  rDT := FDDateTime2SQLTimeStamp(AValue);
...
{$ENDIF}
      try
        ...
        on E: ESQLiteNativeException do begin
          if (E.Kind = ekObjNotExists) and (Database.FSQLHandlerList <> nil) then
            for i := 0 to Database.FSQLHandlerList.Count - 1 do begin
              if IFDPhysSQLHandler(Database.FSQLHandlerList[i]).DataSetNotFound(E.Errors[0].ObjName) then begin
                lDone := False;
                Break;
              end;
            end;
...
begin
  ...
  FCmdTailLen := FDByteStrLength(FCmdText);
  FBaseParamIndex := 0;
  FParamDefsCount := 0;
  FColumnDefsCount := 0;
  PrepareBase;
...
{$IFDEF FireDAC_MONITOR}
  if Database.Tracing then Trace1;
{$ENDIF}
  Database.Lib.Fsqlite3_result_error(Handle, PByte(sb), FDByteStrLength(sb));
end;
...
{$ENDIF}
    Database.Check(Database.Lib.Fsqlite3_create_function(Database.Handle,
      ...
      SQLITE_UTF16, Self, nil, FDsqlite3_func, FDsqlite3_final));
  end
  else begin
{$IFDEF FireDAC_MONITOR}
    if Database.Tracing then Trace2;
{$ENDIF}
    Database.Check(Database.Lib.Fsqlite3_create_function(Database.Handle,
      PByte(Database.Encoder.Encode(Func.Name)), Func.Args,
      SQLITE_UTF16, Self, FDsqlite3_func, nil, nil));
  end;
end;
...
begin
  for i := 0 to FDExpressionManager().FunctionCount - 1 do begin
    ...
    if (oDesc.ScopeKind in [ckUnknown, ckConst]) and (oDesc.ArgMax <> MAXINT) then begin
      ...
      if not lIgnore then
        for j := oDesc.ArgMin to oDesc.ArgMax do
          TSQLiteExpressionFunction.CreateExpr(ALib, oDesc, j);
    end;
  end;
end;
  ...
  if VarIsNull(V) then
    ...
    varString:   Output.AsAnsiString := TFDAnsiString(V);
      ...
      if VarIsFMTBcd(V) then
        ...
        Output.AsDateTime := FDSQLTimeStamp2DateTime(VarToSQLTimeStamp(V))
      else
        ASSERT(False);
    end;
end;
  ...
  try
    if geom^.pUser = nil then begin
      ...
      for i := 0 to geom^.nParam - 1 do
        TSQLiteRTreeData(geom^.pUser).FParams[i] := PDouble(PByte(geom^.aParam) + SizeOf(Double) * i)^;
      geom^.xDelUser := @FDsqlite3_rtree_xDelUser;
      SetLength(TSQLiteRTreeData(geom^.pUser).FColumns, nCoord);
    end;
    for i := 0 to nCoord - 1 do
      TSQLiteRTreeData(geom^.pUser).FColumns[i] := PDouble(PByte(aCoord) + SizeOf(Double) * i)^;
...
begin
  Result := SQLITE_OK;
  try
    if PFDsqlite3_vtab(pVTab)^.pADTable <> nil then
      PFDsqlite3_vtab(pVTab)^.pADTable.xDisconnect(pVTab);
  except
    Result := SQLITE_ERROR;
  end;
end;
...
begin
  ...
  try
    if PFDsqlite3_vtab(pVTab)^.pADTable <> nil then
      PFDsqlite3_vtab(pVTab)^.pADTable.xDestroy(pVTab);
  except
    Result := SQLITE_ERROR;
  end;
end;
...
var
  oTab: TSQLiteVTable;
begin
  oTab := PFDsqlite3_vtab(pVTab)^.pADTable;
...
begin
  Result := PFDsqlite3_vtab(pVTab)^.pADTable.xFindFunction(pVTab, nArg, zName,
    pxFunc, ppArg);
end;
...
begin
  Result := SQLITE_OK;
  try
    if PFDsqlite3_vtab_cursor(cursor)^.pADCursor <> nil then
      PFDsqlite3_vtab_cursor(cursor)^.pADCursor.FTable.xClose(cursor);
  except
    Result := SQLITE_ERROR;
  end;
end;
...
var
  oCrs: TSQLiteVCursor;
begin
  oCrs := PFDsqlite3_vtab_cursor(cursor)^.pADCursor;
...
begin
  Result := PFDsqlite3_vtab_cursor(cursor)^.pADCursor.xEof(cursor);
end;
...
begin
  ...
  try
    ...
    for i := 0 to argc - 1 do begin
      aArgs[i] := UnicodeString(FDatabase.Encoder.Decode(argv^, -1, ecUTF8));
...
{$ENDIF}
    ...
    if ppVTab = nil then begin
      oTab.FrTab.pADModule := Self;
      oTab.FrTab.pADTable := oTab;
      // +2 for ROWID's
...
begin
  if FrTab.pVTab.zErrMsg <> nil then
    FrTab.pADModule.FLib.Fsqlite3_free(FrTab.pVTab.zErrMsg);
...
begin
  oCursor := VTabOpen();
  try
    oCursor.FrCursor.pADModule := FrTab.pADModule;
    oCursor.FrCursor.pADTable := Self;
    oCursor.FrCursor.pADCursor := oCursor;
...
var
  oCursor: TSQLiteVCursor;
begin
  oCursor := PFDsqlite3_vtab_cursor(cursor)^.pADCursor;
  try
    ...
    PFDsqlite3_vtab_cursor(cursor)^.pADCursor := nil;
    FDFree(oCursor);
  end;
end;
...
{-------------------------------------------------------------------------------}
function SQLiteNativeExceptionLoad(AStorage: TFDStorage): TObject;
begin
...
initialization
  ...
  FDExtensionManager().AddExtension([TSQLiteExpressionFunction]);
