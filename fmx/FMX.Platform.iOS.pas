...
uses FMX.Types, FMX.Platform, FMX.Forms, FMX.Graphics, Macapi.ObjectiveC,
...
uses
  ...
  FMX.Layouts, FMX.StdCtrls, FMX.Objects, FMX.Messages, FMX.BehaviorManager,
  FMX.WebBrowser;
...
//    procedure applicationDidFinishLaunching(Sender: UIApplication); cdecl;
  ...
  TPlatformCocoaTouch = class(TInterfacedObject , IFMXApplicationService, IFMXSystemFontService, IFMXTimerService,
    ...
    IFMXImageGeneratorService, IFMXLoggingService, IFMXApplicationEventService, IFMXDefaultMetricsService,
...
{ TFMXTextEditActionsMenu }
  ...
  private
    ...
    procedure ClosePopups(SaveForm: TCommonCustomForm);
  protected
...
begin
  ...
  begin
    ...
    View.FReturnKeyType := TReturnKeyType.rktDefault;
  end;
  View.unmarkText;
  View.FText := '';
...
begin
  Result := [TCaretBehavior.cbDisableCaretInsideWords];
end;
...
var
  ScreenSize: TPointF;
begin
  ScreenSize := TPointF(FMainScreen.bounds.size);
  if GetScreenOrientation in [TScreenOrientation.soLandscape, TScreenOrientation.soInvertedLandscape] then
...
begin
  case Gesture of
    ...
    TInteractiveGesture.igTwoFingerTap:
      begin
...
begin
  ...
  begin
    //save the previous FGestureControl, if any
    if Assigned(FGestureControl) then
...
begin
  ...
  // It's a discreet gesture. Handle it only when it's state is UIGestureRecognizerStateEnded and
  // no other interactive gesture is in progress.
  if gestureRecognizer.state = UIGestureRecognizerStateEnded then
    if not Assigned(FGestureControl) then
    begin
      // Get the control from "under" the gesture.
      Obj := IControl(FForm.ObjectAtPoint(FForm.ClientToScreen(PointF(LPoint.X, LPoint.Y))));
      if Assigned(Obj) then
...
begin
  case FReturnKeyType of
    TReturnKeyType.rktDefault:
      Result := UIReturnKeyDefault;
    TReturnKeyType.rktDone:
      Result := UIReturnKeyDone;
    TReturnKeyType.rktGo:
      Result := UIReturnKeyGo;
    TReturnKeyType.rktNext:
      Result := UIReturnKeyNext;
    TReturnKeyType.rktSearch:
      Result := UIReturnKeySearch;
    TReturnKeyType.rktSend:
...
begin
  ...
  begin
    ...
    begin
      if FMarkRange.location > 0 then
        TempString := FText.Substring(0, FMarkRange.location - 1);
      FText := TempString + UTF8ToUnicodeString(markedText.UTF8String) + FText.Substring(FMarkRange.location + FMarkRange.length)
    end
    else
    ...
    if FSelectRange.length > 0 then
    begin
      FText := FText.Substring(0, FSelectRange.location - 1) +
...
begin
  FContextMenu.ShowSpellItems := Assigned(Form.Focused) and
...
procedure TFMXViewBase.ClosePopups(SaveForm: TCommonCustomForm);
begin
  if Assigned(Screen) then
  begin
    Screen.ClosePopupForms(SaveForm);
  end;
end;
...
begin
  ...
  if AAction = sel_getUid('spell1:') then
    Result := TStandartActionType.satSpell1
  else
  if AAction = sel_getUid('spell2:') then
    Result := TStandartActionType.satSpell2
  else
  if AAction = sel_getUid('spell3:') then
    Result := TStandartActionType.satSpell3
  else
...
begin
  ...
  begin
    ...
    begin
      ...
      FSpellItem1 := TUIMenuItem.Wrap(FSpellItem1.initWithTitle(NSSTR('Spell1'), sel_getUid('spell1:')));
      MenuItems.addObject((FSpellItem1 as ILocalObject).GetObjectID);
      FSpellItem2 := TUIMenuItem.Alloc;
      FSpellItem2 := TUIMenuItem.Wrap(FSpellItem2.initWithTitle(NSSTR('Spell2'), sel_getUid('spell2:')));
      MenuItems.addObject((FSpellItem2 as ILocalObject).GetObjectID);
      FSpellItem3 := TUIMenuItem.Alloc;
      FSpellItem3 := TUIMenuItem.Wrap(FSpellItem3.initWithTitle(NSSTR('Spell3'), sel_getUid('spell3:')));
...
begin
  ...
  begin
    FSpellItem1.setTitle(NSSTR(FSpells[0]));
    if Length(FSpells) > 1 then
    begin
      FSpellItem2.setTitle(NSSTR(FSpells[1]));
      if Length(FSpells) > 2 then
        FSpellItem3.setTitle(NSSTR(FSpells[2]));
    end;
  end;
end;
