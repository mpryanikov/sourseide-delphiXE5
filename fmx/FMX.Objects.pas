...
uses
  System.Types, System.UITypes, System.Classes, System.Rtti, FMX.Messages, FMX.Types, FMX.Controls, FMX.TextLayout,
...
{ TImage }
  ...
  protected
    ...
    procedure Paint; override;
    function DoGetBitmap(const IncludeEmpty: Boolean): TBitmapOfItem;
    function GetData: TValue; override;
    procedure SetData(const Value: TValue); override;
...
uses
  System.UIConsts, System.RTLConsts, System.SysUtils, System.Math, FMX.Consts, FMX.Forms;
...
begin
  ...
  begin
    ...
    FLayout.Text := ConvertText(Value);
    AdjustSize;
    Repaint;
  end;
end;
...
begin
  ...
  if Assigned(FImage) and
    ([csLoading, csDestroying] * FImage.ComponentState = []) then
  begin
...
begin
  Result := (FMultiResBitmap.TransparentColor <> TColors.SysNone) or
            (FMultiResBitmap.SizeKind <> skCustom) or
...
function TImage.DoGetBitmap(const IncludeEmpty: Boolean): TBitmapOfItem;
var
...
begin
  ...
  if (FScreenScale > 0) and Assigned(FMultiResBitmap) then
  begin
    Item := FMultiResBitmap.ItemByScale(FScreenScale, False, IncludeEmpty);
...
function TImage.GetBitmapByScale(const Scale: Single): TBitmap;
var
  Item: TCustomBitmapItem;
begin
  if Assigned(FMultiResBitmap) then
  begin
    Item := FMultiResBitmap.ItemByScale(Scale, False, False);
    if Assigned(Item) then
      Result := Item.Bitmap
    else
      Result := nil;
...
var
  B: TBitmap;
  function CurrentBitmapFound(var B: TBitmap; const IncludeEmpty: Boolean): Boolean;
  var
    Scale: Single;
  begin
    Scale := 0;
    B := DoGetBitmap(IncludeEmpty);
    if Assigned(B) then
      Scale := TBitmapOfItem(B).BitmapItem.Scale;

    if Assigned(B) and Assigned(B.ResourceBitmap) then
      B := B.ResourceBitmap;
    Result := Assigned(B) and (not B.IsEmpty);
    if Result then
      FCurrentScale := Scale;
  end;
begin
  ...
  begin
    ...
    try
      ...
      FCurrentBitmap := nil;
      B := nil;
      if CurrentBitmapFound(B, True) or CurrentBitmapFound(B, False) then
        FCurrentBitmap := B;
    finally
...
begin
  ...
  LR := RectF(0, 0, Trunc(Width * FScreenScale), Trunc(Height * FScreenScale));
  ...
  begin
    ...
    R := RectF(Width - FBitmapMargins.Right, Height - FBitmapMargins.Bottom,
      Width, Height);
    Canvas.DrawBitmap(FCurrentBitmap, RectF(FCurrentBitmap.Width - FBitmapMargins.Right,
      FCurrentBitmap.Height - FBitmapMargins.Bottom, FCurrentBitmap.Width, FCurrentBitmap.Height), R,
      AbsoluteOpacity, True);
    { lefttop }
    R := RectF(0, 0, FBitmapMargins.Left, FBitmapMargins.Top);
    Canvas.DrawBitmap(FCurrentBitmap, RectF(0, 0, FBitmapMargins.Left, FBitmapMargins.Top), R,
      AbsoluteOpacity, True);
    { righttop }
    R := RectF(Width - FBitmapMargins.Right, 0, Width, FBitmapMargins.Top);
    Canvas.DrawBitmap(FCurrentBitmap, RectF(FCurrentBitmap.Width - FBitmapMargins.Right, 0, FCurrentBitmap.Width,
      FBitmapMargins.Top), R, AbsoluteOpacity, True);
    { leftbottom }
    R := RectF(0, Height - FBitmapMargins.Bottom, FBitmapMargins.Left, Height);
    Canvas.DrawBitmap(FCurrentBitmap, RectF(0, FCurrentBitmap.Height - FBitmapMargins.Bottom,
      FBitmapMargins.Left, FCurrentBitmap.Height), R, AbsoluteOpacity, True);
    { top }
    if FMarginWrapMode = TImageWrapMode.iwTile then
    begin
      for I := 0 to Trunc((Width - FBitmapMargins.Left - FBitmapMargins.Right) / (FCurrentBitmap.Width -
        FBitmapMargins.Left - FBitmapMargins.Right)) + 1 do
      begin
        R := RectF(FBitmapMargins.Left, 0, FCurrentBitmap.Width - FBitmapMargins.Bottom, FBitmapMargins.Top);
        ...
        Canvas.DrawBitmap(FCurrentBitmap, RectF(0 + FBitmapMargins.Left, 0,
          FCurrentBitmap.Width - FBitmapMargins.Right, FBitmapMargins.Top),
    ...
    begin
      R := RectF(FBitmapMargins.Left, 0, Width - FBitmapMargins.Right,
        FBitmapMargins.Top);
      Canvas.DrawBitmap(FCurrentBitmap, RectF(FBitmapMargins.Left, 0,
        FCurrentBitmap.Width - FBitmapMargins.Right, FBitmapMargins.Top), R,
        AbsoluteOpacity, True);
    end;
    { left }
    if FMarginWrapMode = TImageWrapMode.iwTile then
    begin
      for J := 0 to Trunc((Height - FBitmapMargins.Bottom - FBitmapMargins.Top) / (FCurrentBitmap.Height -
        FBitmapMargins.Bottom - FBitmapMargins.Top)) + 1 do
      begin
        R := RectF(0, FBitmapMargins.Top, FBitmapMargins.Left, FCurrentBitmap.Height - FBitmapMargins.Bottom);
        ...
        Canvas.DrawBitmap(FCurrentBitmap, RectF(0, FBitmapMargins.Top,
    ...
    begin
      ...
      Canvas.DrawBitmap(FCurrentBitmap, RectF(0, FBitmapMargins.Top, FBitmapMargins.Left,
        FCurrentBitmap.Height - FBitmapMargins.Bottom), R, AbsoluteOpacity, True);
    end;
    { right }
    if FMarginWrapMode = TImageWrapMode.iwTile then
    begin
      for J := 0 to Trunc((Height - FBitmapMargins.Bottom - FBitmapMargins.Top) / (FCurrentBitmap.Height - FBitmapMargins.Bottom - FBitmapMargins.Top)) + 1 do
      begin
        R := RectF(Width - FBitmapMargins.Right, FBitmapMargins.Top, Width, FCurrentBitmap.Height - FBitmapMargins.Bottom);
        OffsetRect(R, 0, J * R.Height);
        IntersectRect(IntersectionRect, RectF(Width - FBitmapMargins.Right, FBitmapMargins.Top, Width, Height - FBitmapMargins.Bottom), R);
        Canvas.DrawBitmap(FCurrentBitmap, RectF(FCurrentBitmap.Width - FBitmapMargins.Right, FBitmapMargins.Top,
          FCurrentBitmap.Width, FBitmapMargins.Top + IntersectionRect.Height),
          IntersectionRect, AbsoluteOpacity, True)
      end;
    end
    else
    begin
      R := RectF(Width - FBitmapMargins.Right, FBitmapMargins.Top, Width,
        Height - FBitmapMargins.Bottom);
      Canvas.DrawBitmap(FCurrentBitmap, RectF(FCurrentBitmap.Width - FBitmapMargins.Right,
        FBitmapMargins.Top, FCurrentBitmap.Width, FCurrentBitmap.Height - FBitmapMargins.Bottom), R,
        AbsoluteOpacity, True);
    end;
    { bottom }
    if FMarginWrapMode = TImageWrapMode.iwTile then
    begin
      for I := 0 to Trunc((Width - FBitmapMargins.Left - FBitmapMargins.Right) / (FCurrentBitmap.Width - FBitmapMargins.Left - FBitmapMargins.Right)) + 1 do
      begin
        R := RectF(FBitmapMargins.Left, Height - FBitmapMargins.Bottom, FCurrentBitmap.Width - FBitmapMargins.Right, Height);
        OffsetRect(R, I * R.Width, 0);
        IntersectRect(IntersectionRect, RectF(FBitmapMargins.Left, Height - FBitmapMargins.Bottom, Width - FBitmapMargins.Right,
          Height), R);
        Canvas.DrawBitmap(FCurrentBitmap, RectF(FBitmapMargins.Left, FCurrentBitmap.Height - FBitmapMargins.Bottom,
          FCurrentBitmap.Width - FBitmapMargins.Right, FCurrentBitmap.Height),
          IntersectionRect, AbsoluteOpacity, True)
      end;
    end
    else
    begin
      ...
      Canvas.DrawBitmap(FCurrentBitmap, RectF(FBitmapMargins.Left,
        FCurrentBitmap.Height - FBitmapMargins.Bottom, FCurrentBitmap.Width - FBitmapMargins.Right,
        FCurrentBitmap.Height), R, AbsoluteOpacity, True);
    end;
    { center }
    ...
    begin
      for I := 0 to Trunc((Width - FBitmapMargins.Left - FBitmapMargins.Right) / (FCurrentBitmap.Width - FBitmapMargins.Left - FBitmapMargins.Right)) + 1 do
        for J := 0 to Trunc((Height - FBitmapMargins.Bottom - FBitmapMargins.Top) / (FCurrentBitmap.Height - FBitmapMargins.Bottom - FBitmapMargins.Top)) + 1 do
        begin
          R := FBitmapMargins.PaddingRect(RectF(0, 0, FCurrentBitmap.Width, FCurrentBitmap.Height));
          OffsetRect(R, I * R.Width, J * R.Height);
          IntersectRect(IntersectionRect, FBitmapMargins.PaddingRect(LocalRect), R);
          Canvas.DrawBitmap(FCurrentBitmap, RectF(FBitmapMargins.Left, FBitmapMargins.Top, FBitmapMargins.Left + IntersectionRect.Width, FBitmapMargins.Top + IntersectionRect.Height),
    ...
    begin
      ...
      Canvas.DrawBitmap(FCurrentBitmap, RectF(FBitmapMargins.Left, FBitmapMargins.Top,
        FCurrentBitmap.Width - FBitmapMargins.Right, FCurrentBitmap.Height - FBitmapMargins.Bottom), R,
        AbsoluteOpacity, True);
    end;
  end
  else
  begin
    case FWrapMode of
      TImageWrapMode.iwOriginal:
        begin
          R := RectF(0, 0, FCurrentBitmap.Width, FCurrentBitmap.Height);
          IntersectRect(IntersectionRect, LR, R);
          Canvas.DrawBitmap(FCurrentBitmap, RectF(0, 0, IntersectionRect.Width, IntersectionRect.Height),
            RectF(R.Left, R.Top, R.Left + IntersectionRect.Width / FScreenScale, R.Top + IntersectionRect.Height / FScreenScale), AbsoluteOpacity, DisableInterpolation)
        end;
      TImageWrapMode.iwFit:
        begin
          LR := RectF(0, 0, Width, Height);
          R := RectF(0, 0, FCurrentBitmap.Width / FCurrentBitmap.BitmapScale, FCurrentBitmap.Height / FCurrentBitmap.BitmapScale);
          R.Fit(LR);
          R.Left := Round(R.Left);
          R.Top := Round(R.Top);
          R.Right := Round(R.Right);
          R.Bottom := Round(R.Bottom);
          Canvas.DrawBitmap(FCurrentBitmap, RectF(0, 0, FCurrentBitmap.Width, FCurrentBitmap.Height), R, AbsoluteOpacity, DisableInterpolation);
        end;
      TImageWrapMode.iwStretch:
        begin
          Canvas.DrawBitmap(FCurrentBitmap, RectF(0, 0, FCurrentBitmap.Width, FCurrentBitmap.Height), RectF(0, 0, Width, Height), AbsoluteOpacity, DisableInterpolation)
        end;
      TImageWrapMode.iwTile:
        begin
          for I := 0 to Trunc(LR.Width / FCurrentBitmap.Width) + 1 do
            for J := 0 to Trunc(LR.Height / FCurrentBitmap.Height) + 1 do
            begin
              R := RectF(0, 0, FCurrentBitmap.Width, FCurrentBitmap.Height);
              OffsetRect(R, I * FCurrentBitmap.Width, J * FCurrentBitmap.Height);
              IntersectRect(IntersectionRect, LR, R);
              Canvas.DrawBitmap(FCurrentBitmap, RectF(0, 0, IntersectionRect.Width, IntersectionRect.Height),
                RectF(R.Left / FScreenScale, R.Top / FScreenScale, (R.Left + IntersectionRect.Width) / FScreenScale,
                  (R.Top + IntersectionRect.Height) / FScreenScale), AbsoluteOpacity, True)
            end;
        end;
...
begin
  ...
  Height := FGripSize * 2;
  SetAcceptsControls(False);
  FGripCenter := TPosition.Create(TPointF.Create(0,0));
end;
