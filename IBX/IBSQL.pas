...
uses
  System.SysUtils, System.Variants, System.Classes, IBHeader, IBExternals,
  IBDatabase, IBIntf, Data.FMTBcd;
...
uses Data.DB, Data.DBConsts, System.Types, IBBlob, IBSQLMonitor,
     IB, IBUtils, IBXConst, IBXMLHeader, IBErrorCodes, system.Character;
...
begin
  SetAsCPString(Value, FSQL.Database.CharacterSetCodePage);
end;
...
begin
  ...
  begin
    ...
    begin
      bt := TEncoding.ANSI.GetBytes(Value);
      SetStringValue;
    end
  end
  else
  begin
    ...
    begin
      ...
      begin
        ss := TBytesStream.Create(TEncoding.ANSI.GetBytes(Value));
        try
          LoadFromStream(ss);
        finally
          ss.Free;
...
begin
  ...
  begin
    ...
    st := st + FRowDelimiter;
    BytesWritten := FFile.Write(st[Low(st)], Length(st) * sizeof(Char));
...
var
  ...
  begin
    if (c = CR) and (Delimiter.IndexOf(LF) > 0) then {mbcs ok}
    begin
      BytesRead := FFile.Read(c, 1);
...
begin
  ...
  begin
    if FColDelimiter.IndexOf(c) > 0 then {mbcs ok}
    begin
      ...
      if FRowDelimiter.IndexOf(c) > 0 then {mbcs ok}
      begin
  ...
  try
    ...
    try
      if Assigned(Encoding) then
        ...
        bt := TEncoding.ANSI.GetBytes(FProcessedSQL.Text + #0);
      try
