...
uses
  ...
  FMX.Controls, FMX.Types, FMX.Objects, FMX.Graphics;
...
{ TMagnifierGlass }
  ...
  { Magnifying glass increasing area parent control. Allows to change
    scaling coefficient |LoupeScale|. Has two options of display - a circular and rectangular lens |LoupeMode|.
    For installation of style of a magnifying glass the following names of styles are used for |LoupeMode|:
      * lmCircle - 'magnifierglassstyle'
      * lmRectangle - 'magnifierglassrectanglestyle'
  ...
  begin
    case ZoomMode of
      TZoomMode.zmAbsolute:
        ZoomPoint := ZoomRegionCenter.Point * ScreenScale;
      TZoomMode.zmRelative:
        ZoomPoint := (Position.Point * ScreenScale) + (PointF(Width / 2, Height / 2) * ScreenScale) +
                     (ZoomRegionCenter.Point * ScreenScale);
    end;
