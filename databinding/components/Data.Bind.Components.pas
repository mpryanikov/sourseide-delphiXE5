...
type
  ...
  protected
    ...
    property BindListEditor: IBindListEditorCommon read FBindListEditor write FBindListEditor;
  public
  ...
  public
  type
    TArrayListExpression = TArray<TList<TBindingExpression>>;
  private
    FFormatControlExpressions: TExpressions;
    FClearControlExpressions: TExpressions;
    FFormatControlExpressionObjects: TList<TBindingExpression>;
    ...
    procedure GetBindingExpressions(const AEnumerator: IScopeRecordEnumerator; const AEditor: IBindListEditor;
      ACallback: TProc<TArrayListExpression>); overload;
    procedure GetBindingExpressions(const AEnumerator: IScopeRecordEnumerator; const AEditor: IBindListEditor;
      ACallback: TProc<TArrayListExpression, TArrayListExpression, TBindingExpression,
        TObject>); overload;
  ...
  private
    ...
    procedure GetBindingExpressions(const AEnumerator: IScopeRecordEnumerator;
      ...
      ACallback: TProc<TArray<TList<TBindingExpression>>>);
...
begin
  inherited;  // Set active false
  FBindListEditor := nil; // Release editor
end;
...
begin
  if Supports(SourceComponent, IScopeRecordEnumerable, LIntf) then
    ...
    raise TBindCompException.CreateFmt(sNoEnumerator, [Self.GetDisplayName, SafeClassName(SourceComponent)]);
end;
...
procedure TCustomBindGridList.GetBindingExpressions(const AEnumerator: IScopeRecordEnumerator; const AEditor: IBindListEditor;
  ACallback: TProc<TArrayListExpression>);
var
  LColumnLinkExpressionItem: TColumnFormatExpressionItem;
  LBindingExpressions: TObjectList<TList<TBindingExpression>>;
  LList: TList<TBindingExpression>;
  LExpressions: TExpressions;
  LExpression: TExpressionItem;
begin
  LBindingExpressions := TObjectList<TList<TBindingExpression>>.Create();
  try
    for LColumnLinkExpressionItem in FColumnExpressions do
    begin
      LList := TObjectList<TBindingExpression>.Create;
...
procedure TCustomBindGridList.GetBindingExpressions(const AEnumerator: IScopeRecordEnumerator; const AEditor: IBindListEditor;
  ...
  procedure Add(AColumnLinkExpressionItem: TColumnFormatExpressionItem;
    AExpressions: TExpressions; LList: TList<TBindingExpression>; const AItemScope: IScope;
...
var
  LColumnLinkExpressionItem: TColumnFormatExpressionItem;
  LBindingExpressions: TObjectList<TList<TBindingExpression>>;
  LHeaderBindingExpressions: TObjectList<TList<TBindingExpression>>;
  LList: TList<TBindingExpression>;
  LBreakExpression: TBindingExpression;
  LBindingExpressionArray: TArrayListExpression;
  LHeaderBindingExpressionArray: TArrayListExpression;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        begin
          LHeaderBindingExpressions := TObjectList<TList<TBindingExpression>>.Create();
        end;
        LList := TObjectList<TBindingExpression>.Create;
    ...
    begin
      ...
      LBreakExpression := TBindings.CreateExpression(
...
procedure LocalFill(AExpressions: TCustomBindGridList.TArrayListExpression);
var
  LExpression: TBindingExpression;
  LExpressions: TList<TBindingExpression>;
begin
  ...
  begin
    ...
    begin
      GetBindingExpressions(LEnumerator, AEditor,
        procedure(AExpressions, AHeaderExpressions: TArrayListExpression;     // Callback
          ABreakExpression: TBindingExpression;
          ...
          begin
            ...
            LGroupEditor.FillGroupList(LEnumerator,
                ...
                begin
                  ...
                  begin
                      LValue := ABreakExpression.Evaluate;
                      ...
                      begin
                        if LValue <> nil then
                        begin
                          LValueGetValue := LValue.GetValue;
//                          if LBOF then
//                            Result := True
//                          else
                          ...
                          begin
                            Result := not SameStr(LValueGetValue.ToString, LPrevValue.ToString);
                          end;
                          LPrevValue := LValueGetValue;
                        end
                        else
    ...
    begin
      GetBindingExpressions(LEnumerator, AEditor,
        procedure(AExpressions: TArrayListExpression)
        begin
          AEditor.FillList(LEnumerator,
            procedure(const ASourceScope, AEditorScope: IScope)
            begin
    ...
    LEnumerator := nil;
//    if Supports(AEditor, IBindListSynchEditor, LSynchEditor) then
//      LSynchEditor.ApplyFilter;
  end;
end;
...
var
  ...
  LCanCache: Boolean;
begin
  if FBindListEditor <> nil then
    AEditor := FBindListEditor
  else
  begin
    AEditor := nil;
    if ControlComponent <> nil then
    ...
    begin
      ...
      FBindListEditor := AEditor;  // Keep it around
        ...
        begin
          ...
          begin
            GetBindingExpressions(LEnumerator, LEditor,
              procedure(AExpressions: TArray<TList<TBindingExpression>>)
              begin
                LEditor.FillList(LEnumerator,
                  procedure(const ASourceScope, AEditorScope: IScope)
                  begin
...
procedure TCustomLinkFillControlToField.SetAutoBufferCount(
  const Value: Boolean);
begin
...
function TCustomLinkFillControlToField.UpdateBufferCount: Boolean;
var
  LNewBufferCount: Integer;
  LScopeBuffer: IScopeBuffer;
begin
  Result := False;
  if Supports(FillDataSource, IScopeBuffer, LScopeBuffer) then
    if LScopeBuffer.GetBufferCount(LNewBufferCount) then
    begin
      Result := True;
...
procedure TCustomLinkFillControlToProperty.SetAutoBufferCount(
  const Value: Boolean);
begin
  if FAutoBufferCount <> Value then
  begin
...
function TCustomLinkFillControlToProperty.UpdateBufferCount: Boolean;
var
  LNewBufferCount: Integer;
  LScopeBuffer: IScopeBuffer;
begin
...
procedure TCustomLinkListControlToField.SetAutoBufferCount(
  const Value: Boolean);
begin
  if FAutoBufferCount <> Value then
  begin
...
function TCustomLinkListControlToField.UpdateBufferCount: Boolean;
var
  LNewBufferCount: Integer;
  LScopeBuffer: IScopeBuffer;
begin
  Result := False;
  if Supports(DataSource, IScopeBuffer, LScopeBuffer) then
    if LScopeBuffer.GetBufferCount(LNewBufferCount) then
    begin
      Result := True;
      if LNewBufferCount > BufferCount then
...
begin
  Result := TValue.Empty;
  if Supports(FBindListEditor, IBindListEditorCommon, LEditor) then
  begin
