...
begin
  ...
  FWebView.setScalesPageToFit(True);
...
begin
  ...
  if Assigned(webView) then
  begin
    lRequest := webView.request;
    if Assigned(lRequest) then
      FWebBrowser.FURL := NSStrToStr(lRequest.mainDocumentURL.absoluteString);
  end ;
end;
