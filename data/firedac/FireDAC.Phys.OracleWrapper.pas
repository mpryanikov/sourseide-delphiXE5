...
{     FireDAC Oracle Call Interface wrapper classes     }
{                                                       }
...
{$ENDIF}
    ...
    class procedure GetOracleHomeList(AList: TStrings);
  ...
  private
    ...
    FOwningObj: TObject;
    FEnv: TOCIEnv;
    function Init(AType: ub4): pOCIHandle;
...
{$ENDIF}
  ...
  public
    constructor Create(ALib: TOCILib; AOwningObj: TObject
...
{$ENDIF}
  ...
  TOCIVarDataType = (otUnknown, otInteger, otSingle, otDouble, otNumber, otString,
    otChar, otNString, otNChar, otRaw, otDateTime, otLong, otNLong, otLongRaw,
    otROWID, otCursor, otNestedDataSet, otCLOB, otNCLOB, otBLOB, otCFile, otBFile,
    otTimeStamp, otIntervalYM, otIntervalDS, otBoolean);
  ...
  private
    ...
    procedure InitPiecedExchange(out pld: TOCIPieceLoopData);
    procedure DoPiecedExchange(var AData: TOCIPieceLoopData; ARes: sb4);
{$IFDEF FireDAC_MONITOR}
...
//      var BufLen: pUb4; var PieceStatus: ub1; var Ind: pointer): sb4;
  ...
  private
    ...
    procedure FreeLong(ApLong: POraLong);
    procedure InitLong(ApLong: POraLong);
    procedure GetLong(ApLong: POraLong; var AData: pUb1; var ASize: ub4);
    procedure SetLong(ApLong: POraLong; AData: pUb1; ASize: ub4);
    procedure AppendLong(ApLong: POraLong; AData: pUb1; ASize: ub4);
...
{$IFDEF FireDAC_MONITOR}
    function DumpValue(AIndex: ub4): UnicodeString;
{$ENDIF}
  ...
  private
    function Init(AType: ub4): pOCIDescriptor;
  public
    destructor Destroy; override;
  end;
  ...
  public
    ...
    constructor CreateUseHandle(AEnv: TOCIEnv; AHandle: pOCIHandle);
    destructor Destroy; override;
  ...
  public
    procedure SetValues(AYear: sb2; AMonth, FDay, AHour, AMinute, ASecond: ub1;
  ...
  public
    constructor Create(AEnv: TOCIEnv; AType: ub4);
    constructor CreateUseHandle(AEnv: TOCIEnv; AHandle: pOCIHandle; ADataType: TOCIVarDataType);
...
uses
  ...
  FireDAC.Stan.Storage, FireDAC.Stan.ResStrs;

{-------------------------------------------------------------------------------}
const
  nc2ociValueType: array[TOCIVarDataType] of ub2 = (0, SQLT_INT, SQLT_FLT, SQLT_FLT,
    SQLT_VNU, SQLT_CHR, SQLT_CHR, SQLT_CHR, SQLT_CHR, SQLT_BIN, SQLT_DAT, SQLT_LNG,
    SQLT_LNG, SQLT_LBI, SQLT_CHR, SQLT_RSET, SQLT_RSET, SQLT_CLOB, SQLT_CLOB, SQLT_BLOB,
    SQLT_CFILEE, SQLT_BFILEE, SQLT_TIMESTAMP_TZ, SQLT_INTERVAL_YM, SQLT_INTERVAL_DS,
    _SQLT_BOL);
  nc2ociValueSize: array[TOCIVarDataType] of sb4 = (0, SizeOf(Integer), SizeOf(Single),
    SizeOf(Double), OCI_NUMBER_SIZE, C_FD_DefStrSize * SizeOf(TFDAnsiChar),
    C_FD_DefStrSize * SizeOf(TFDAnsiChar), C_FD_DefStrSize * SizeOf(WideChar),
    C_FD_DefStrSize * SizeOf(WideChar), C_FD_DefStrSize, SizeOf(TOraDate),
    C_FD_DefLongSize * SizeOf(TFDAnsiChar), C_FD_DefLongSize * SizeOf(WideChar),
    C_FD_DefLongSize, IRowIdLen * SizeOf(TFDAnsiChar), SizeOf(pOCIStmt), SizeOf(pOCIStmt),
    SizeOf(pOCILobLocator), SizeOf(pOCILobLocator), SizeOf(pOCILobLocator),
    SizeOf(pOCILobLocator), SizeOf(pOCILobLocator), SizeOf(pOCIDateTime),
    SizeOf(pOCIInterval), SizeOf(pOCIInterval), SizeOf(WordBool));
...
{$IFDEF FireDAC_MONITOR}
  dataTypeName: array[TOCIVarDataType] of String =
    ('otUnknown', 'otInteger', 'otSingle', 'otDouble', 'otNumber', 'otString',
     'otChar', 'otNString', 'otNChar', 'otRaw', 'otDateTime', 'otLong', 'otNLong',
     'otLongRaw', 'otROWID', 'otCursor', 'otDataSet', 'otCLOB', 'otNCLOB',
     'otBLOB', 'otCFile', 'otBFile', 'otTimeStamp', 'otIntervalYM',
     'otIntervalDS', 'otBoolean');
...
begin
  ...
  _SQLT_REC: Result := '_SQLT_REC';
  _SQLT_TAB: Result := '_SQLT_TAB';
  _SQLT_BOL: Result := '_SQLT_BOL';
  // post 8.1
...
begin
  if ANLSLang <> '' then begin
    FDSetEnv('NLS_LANG', ANLSLang);
    FMessages.Value('NLS_LANG', ANLSLang);
  end;
  if ATNSAdmin <> '' then begin
    FDSetEnv('TNS_ADMIN', ATNSAdmin);
    FMessages.Value('TNS_ADMIN', ATNSAdmin);
  end;
...
var
  ...
  var
    ...
    begin
      ...
      if not FileExists(sFileName) then begin
        FMessages.Msg(Format('Warning: [%s] does not exists in Oracle Home [%s]',
          [sFileName, AKey]));
        Exit;
      end;

      // Ok, found something
      FMessages.Msg(Format('Found [%s], position [%d], in [reg: %s]',
        [sFileName, iBestPath, AKey]));

      // Check OCI architecture
      eMType := FDGetLibMachineType(sFileName);
      if not (eMType in [mtUnknown, mtNative]) then begin
        FMessages.Msg(Format('Warning: not supported architecture [%s]. Required [%s]',
    ...
    var
      ...
      lInPath: Boolean;
    begin
      FMessages.Start(Format('Checking Oracle Home at key [%s]', [AKey]));
      try
        Result := oReg.OpenKeyReadOnly(AKey);
        if Result then begin
          sRoot := GetRegPathVar(oReg, 'ORACLE_HOME');
          if sRoot <> '' then begin
            sBin := sRoot + '\bin';
              ...
              while i <= Length(sEnvPath) do
                if AnsiCompareText(sBin, NormalizePath(FDExtractFieldName(sEnvPath, i))) = 0 then begin
                  ...
                  if iBestPath >= i then begin
                    ...
                    FMessages.Msg(Format('Skipping, position [%d] is after [%d]', [i, iBestPath]));
                  Break;
                end;
              if not lInPath then
                FMessages.Msg(Format('Warning: bin folder [%s] is not in PATH', [sBin]));
            end;
          end;
        end;
      finally
  ...
  begin
    FMessages.Start('Searching for Oracle Home');
    iPrevInd := iBestPath;
  ...
  var
    ...
    begin
      ...
      // Ok, found something
      FMessages.Msg(Format('Found [%s], position [%d], in [%s]', [AFileName, ABestInd, AWhere]));
      ...
      if sVerStr <> '' then begin
        ...
        if iVer < cvOracle100000 then
          FMessages.Msg(Format('Warning: invalid OIC version [%s]. Minimal version [10.0]',
            [sVerStr]));
      end;
  ...
  begin
    FMessages.Start('Searching for Instant Client');
    iPrevInd := iBestPath;
    try
...
begin
  if AVendorHome <> '' then
    ...
    sExplicitHome := FDReadEnvValue('ORACLE_HOME');
  ...
  if AVendorLib <> '' then
    ...
    sExplicitLib := FDReadEnvValue('ORAOCI');
  if AVendorLib <> '' then
    FMessages.Value('Explicit OCI', AVendorLib);

  sEnvPath := FDReadEnvValue('PATH');
  iBestPath := MAXINT;
  FInstantClient := False;
  FOracleHome := '';
...
begin
  ...
  if AVendorHome <> '' then
    ...
    FOracleHome := FDReadEnvValue('ORACLE_HOME');
  if AVendorLib <> '' then
    Result := AVendorLib
  else begin
    Result := FDReadEnvValue('ORAOCI');
    if Result = '' then begin
...
const
  C_Tns_Names = 'TNS_NAMES';
  C_Tns_Admin = 'TNS_ADMIN';
  C_TnsNamesOra = 'tnsnames.ora';
{$IFDEF MSWINDOWS}
var
  oReg: TRegistry;
{$ENDIF}
begin
  FTNSNames := FDReadEnvValue(C_Tns_Names);
  if FTNSNames = '' then begin
    FTNSNames := FDReadEnvValue(C_Tns_Admin);
    if FTNSNames <> '' then
      FTNSNames := FDNormPath(FTNSNames) + C_TnsNamesOra
    else if FInstantClient then
      FTNSNames := FDNormPath(FOracleHome) + C_TnsNamesOra
{$IFDEF MSWINDOWS}
    else begin
      oReg := TRegistry.Create;
      try
        ...
        if FTNSNames = '' then begin
          ...
          if FTNSNames = '' then begin
            if Version >= cvOracle81000 then
              FTNSNames := GetRegPathVar(oReg, 'NETWORK')
            else
              FTNSNames := GetRegPathVar(oReg, 'NET80');
            if FTNSNames = '' then begin
              if Version >= cvOracle81000 then
                FTNSNames := FDNormPath(FOracleHome) + 'Network'
              else
                FTNSNames := FDNormPath(FOracleHome) + 'Net80';
            end;
            FTNSNames := FDNormPath(FTNSNames) + 'Admin';
          end;
          FTNSNames := FDNormPath(FTNSNames) + C_TnsNamesOra;
        end;
      finally
...
{$ENDIF}
begin
  FNLSLang := FDReadEnvValue('NLS_LANG');
{$IFDEF MSWINDOWS}
  if FNLSLang <> '' then
    Exit;
  oReg := TRegistry.Create;
  try
    oReg.RootKey := HKEY_LOCAL_MACHINE;
    oReg.OpenKeyReadOnly(FKey);
    FNLSLang := oReg.ReadString('NLS_LANG');
  finally
    FDFree(oReg);
  end;
{$ENDIF}
...
begin
  ...
  if Version >= cvOracle90000 then begin
    ...
    @OCIDateTimeAssign := nil;
    @OCIIntervalAssign := nil;
    @OCIIntervalCheck := nil;
  ...
  if Version >= cvOracle102000 then
...
begin
  ...
  if FTNSNames = '' then
    Exit;
  try
    f := TFDTextFile.Create(FTNSNames, True, False, ecUTF8, elDefault);
    try
      buff := f.ExtractString(1, f.Stream.Size);
    finally
      FDFree(f);
    end;
  except
...
{-------------------------------------------------------------------------------}
function TOCIHandle.Init(AType: ub4): pOCIHandle;
{$IFDEF FireDAC_MONITOR}
  procedure ProcTrace;
  begin
...
{$ENDIF}
  ...
  Check(Env.Lib.OCIAttrSet(FHandle, FType, PByte(sb),
    FDByteStrLength(sb), AAtrType, Env.Error.FHandle));
end;
...
{$ENDIF}
  ...
  Check(Env.Lib.OCIServerAttach(FHandle, Env.Error.FHandle, PByte(sb),
    FDByteStrLength(sb), OCI_DEFAULT));
  FAttached := True;
  Select;
end;
...
{$ENDIF}
  ...
  Check(Env.Lib.OCIPasswordChange(FOwner.FHandle, Env.Error.FHandle,
    PByte(sbUser), FDByteStrLength(sbUser),
    PByte(sbOldPwd), FDByteStrLength(sbOldPwd),
    PByte(sbNewPwd), FDByteStrLength(sbNewPwd), OCI_DEFAULT));
  FStarted := True;
end;
...
{-------------------------------------------------------------------------------}
procedure TOCIStatement.InitPiecedExchange(out pld: TOCIPieceLoopData);
begin
...
{-------------------------------------------------------------------------------}
procedure TOCIStatement.DoPiecedExchange(var AData: TOCIPieceLoopData; ARes: sb4);
var
  varHndl: pOCIHandle;
  iVarType: ub4;
  idx: ub4;
...
{$ENDIF}
  ...
  Check(Env.Lib.OCIStmtPrepare(FHandle, Env.Error.FHandle, PByte(sb),
    FDByteStrLength(sb), OCI_NTV_SYNTAX, OCI_DEFAULT));
...
begin
  ...
  FService.UpdateNonBlockingMode;
{$IFDEF FireDAC_MONITOR}
  if Env.Tracing then DumpParameters(ARows, AOffset, [odIn, odInOut]);
{$ENDIF}
  FLastRowCount := 0;
  FCanceled := False;
...
{$IFDEF FireDAC_MONITOR}
  if Env.Tracing then begin
    DumpParameters(ARows, AOffset, [odOut, odInOut, odRet]);
...
begin
  ...
  if (pDate^.century > 100) or (pDate^.century = 100) and (pDate^.year > 100) then
    D.DateTime := FDDateTime2MSecs(FDEncodeDateTime(
...
var
  ...
  procedure SetNibble(FDigit: Byte);
  begin
    if iNibbleDigit = 0 then
      pNibble^ := FDigit shl 4
    else
      pNibble^ := pNibble^ or FDigit;
  ...
  if (DataType in otVBlobs) and (VarType in [odIn, odOut, odInOut]) and
    ...
    if (DataType in [otRaw]) and (DataSize = C_FD_DefStrSize) or
      ...
      if TOCIStatement(FOwner).STMT_TYPE in [OCI_STMT_BEGIN, OCI_STMT_DECLARE] then
        ...
        otChar,
        otNString,
        otNChar:   iMaxSize := C_FD_MaxPLSQLDataSize div iBPC;
        otRaw:     iMaxSize := C_FD_MaxPLSQLDataSize;
        end
      else
        case DataType of
        otString:  iMaxSize := 4000 div iBPC;
        otChar:    iMaxSize := 2000 div iBPC;
        otRaw:     iMaxSize := 2000;
        otNString: iMaxSize := 4000 div iBPC;
        otNChar:   iMaxSize := 2000 div iBPC;
        end;
...
{$ENDIF}
      Check(Env.Lib.OCIDefineByPos(FOwner.FHandle, FHandle, Env.Error.FHandle,
        FPosition, FBindData^.FBuffer, sz, tp, FBindData^.FInd, FBindData^.FALen,
        FBindData^.FRCode, OCI_DEFAULT));
    end;
  end
...
{$ENDIF}
          Check(Env.Lib.OCIBindByPos(FOwner.FHandle, FHandle, Env.Error.FHandle,
            FPosition, FBindData^.FBuffer, sz, tp, FBindData^.FInd, FBindData^.FALen,
            FBindData^.FRCode, v1, v2, OCI_DEFAULT));
        end
      else begin
...
{$ENDIF}
          Check(Env.Lib.OCIBindByName(FOwner.FHandle, FHandle, Env.Error.FHandle,
            PByte(sbPlcHldr), FDByteStrLength(sbPlcHldr), nil, sz, tp,
            nil, nil, nil, v1, v2, OCI_DATA_AT_EXEC));
          TOCIStatement(FOwner).AddPieceVar(Self);
        end
        else begin
{$IFDEF FireDAC_MONITOR}
...
{$ENDIF}
          Check(Env.Lib.OCIBindByName(FOwner.FHandle, FHandle, Env.Error.FHandle,
            PByte(sbPlcHldr), FDByteStrLength(sbPlcHldr), FBindData^.FBuffer, sz, tp,
            FBindData^.FInd, FBindData^.FALen, FBindData^.FRCode, v1, v2, OCI_DEFAULT));
        end;
      end;
    end;
  end
...
procedure TOCIVariable.FreeLong(ApLong: POraLong);
begin
...
{-------------------------------------------------------------------------------}
procedure TOCIVariable.InitLong(ApLong: POraLong);
begin
  ApLong^.FData := nil;
  ApLong^.FSize := 0;
end;
...
{-------------------------------------------------------------------------------}
procedure TOCIVariable.GetLong(ApLong: POraLong; var AData: pUb1; var ASize: ub4);
begin
  ASize := ApLong^.FSize;
  AData := ApLong^.FData;
end;
...
{-------------------------------------------------------------------------------}
procedure TOCIVariable.SetLong(ApLong: POraLong; AData: pUb1; ASize: ub4);
begin
  FreeLong(ApLong);
...
{-------------------------------------------------------------------------------}
procedure TOCIVariable.AppendLong(ApLong: POraLong; AData: pUb1; ASize: ub4);
begin
  if ApLong^.FData = nil then
  ...
  if FBindData^.FInd[AIndex] = -1 then begin
    ...
    if ByteSemantic then
      ...
      otString, otChar, otLong:
        ASize := LongWord(ASize) div SizeOf(TFDAnsiChar);
      otNString, otNChar, otNLong:
  ...
  if (FBindData^.FInd[AIndex] = -1) and (DataType < otCursor) then begin
    ...
    case DataType of
      ...
      otRaw, otLongRaw:
        begin
          if LongData then
            GetLong(POraLong(pBuff), pBuff, ASize)
          else
            ASize := FBindData^.FALen[AIndex];
          ASize := LongWord(ASize);
        ...
        begin
          if TOCIStatement(FOwner).StrsEmpty2Null and (ASize = 0) then
            ...
            if (ASize > FValue_char_sz) and (
              ...
              ErrorDataTooLarge(FValue_char_sz, ASize);
            if LongData then begin
              SetLong(POraLong(Buffer[AIndex]), ABuff, ASize);
              FBindData^.FALen[AIndex] := ub2(SizeOf(TOraLong));
            end
            else begin
        ...
        begin
          ...
          if TOCIStatement(FOwner).StrsEmpty2Null and (ASize = 0) then
            ...
            if (ASize > FValue_char_sz) and (
              ...
              ErrorDataTooLarge(FValue_char_sz, ASize);
            if LongData then begin
              SetLong(POraLong(Buffer[AIndex]), ABuff, ASize * SizeOf(TFDAnsiChar));
              FBindData^.FALen[AIndex] := ub2(SizeOf(TOraLong));
            end
            else begin
              Move(PFDAnsiString(ABuff)^, Buffer[AIndex]^, ASize * SizeOf(TFDAnsiChar));
      ...
      otNString, otNChar, otNLong:
        begin
          if ASize = $FFFFFFFF then
            ASize := FDWideStrLen(PWideChar(ABuff));
          if (DataType = otNChar) and TOCIStatement(FOwner).StrsTrim then
          ...
          if TOCIStatement(FOwner).StrsEmpty2Null and (ASize = 0) then
            ...
            if (ASize > FValue_char_sz) and (
              ...
              ErrorDataTooLarge(FValue_char_sz, ASize);
            if LongData then begin
              SetLong(POraLong(Buffer[AIndex]), ABuff, ASize * SizeOf(WideChar));
              FBindData^.FALen[AIndex] := ub2(SizeOf(TOraLong));
            end
            else begin
...
begin
  ...
  ASSERT(LongData);
  ASSERT(FBindData <> nil);
  AppendLong(POraLong(Buffer[AIndex]), ABuff, ASize);
  FBindData^.FInd[AIndex] := 0;
  FBindData^.FALen[AIndex] := ub2(SizeOf(TOraLong));
end;
...
{$IFDEF FireDAC_MONITOR}
{-------------------------------------------------------------------------------}
function TOCIVariable.DumpValue(AIndex: ub4): UnicodeString;
var
  p: pUb1;
  sz: ub4;
  s: String;
    ...
    case DataType of
      otString, otChar, otLong:
        if sz > 1024 then begin
          s := Env.DataEncoder.Decode(p, 1024);
          Result := UnicodeString('(truncated at 1024) ''' + s + ' ...''');
        end
        else begin
          s := Env.DataEncoder.Decode(p, sz);
          Result := UnicodeString('''' + s + '''');
        end;
      otNString, otNChar, otNLong:
        if sz > 1024 then begin
          SetString(sw, PWideChar(p), 1024);
          Result := '(truncated at 1024) ''' + sw + ' ...''';
        end
        else begin
          SetString(sw, PWideChar(p), sz);
          Result := '''' + sw + '''';
        end;
      otROWID:
        begin
      ...
      otDateTime:
        begin
          ...
          rTS := FDDateTime2SQLTimeStamp(FDMSecs2DateTime(d.DateTime));
...
{ TOCIDescriptor                                                                }
{-------------------------------------------------------------------------------}
function TOCIDescriptor.Init(AType: ub4): pOCIDescriptor;
{$IFDEF FireDAC_MONITOR}
  procedure ProcTrace;
  begin
...
begin
  if FSCALE = $FFFFFFFF then
    FSCALE := GetUb4Attr(OCI_ATTR_SCALE);
  Result := FSCALE;
end;
...
{$ENDIF}
  ...
  Check(Env.Lib.OCILobFileSetName(Env.Handle, Env.Error.FHandle, FHandle,
    PByte(sbDir), ub2(FDByteStrLength(sbDir)),
    PByte(sbFileName), ub2(FDByteStrLength(sbFileName))));
end;
...
{$ENDIF}
      ...
      Result := Env.Lib.OCIDescribeAny(FOwner.FHandle, Env.Error.FHandle,
        PByte(sbName), FDByteStrLength(sbName), OCI_OTYPE_NAME,
...
var
  ...
  begin
    if Length(AName) > 30 then begin
      ...
      if i >= 0 then
        Result := FDValueFromIndex(oPars, i)
      else begin
  ...
  var
    ...
    procedure AddParam;
    begin
      AOnDefPar(sArg, vt, dt, sz, prc, scl, AIsTable, AIsResult);
      Inc(iParCnt);
    end;
  ...
  begin
    ...
    _SQLT_REC:
      begin
        ...
        BodyVar(sVar);
        sType := AItem.SCHEMA_NAME + '.' + AItem.TYPE_NAME + '.' + AItem.SUB_NAME;
        if sType = '.' then
...
{-------------------------------------------------------------------------------}
constructor TOCIOpaqueValue.CreateUseHandle(AEnv: TOCIEnv; AHandle: pOCIHandle);
begin
  inherited Create(AEnv);
  FOwner := AEnv;
  FHandle := AHandle;
...
{-------------------------------------------------------------------------------}
destructor TOCIOpaqueValue.Destroy;
begin
  if not FOwned then
    FHandle := nil;
  inherited Destroy;
...
{-------------------------------------------------------------------------------}
procedure TOCITimestamp.SetValues(AYear: sb2; AMonth, FDay, AHour,
...
begin
  ...
  Check(Env.Lib.OCIDateTimeConstruct(Owner.Handle, Env.Error.Handle, Handle,
    AYear, AMonth, FDay, AHour, AMinute, ASecond, ANanoSeconds, pTZ, iLen));
  FIsNull := False;
  FParsed := True;
end;
...
{-------------------------------------------------------------------------------}
constructor TOCITimeInterval.CreateUseHandle(AEnv: TOCIEnv; AHandle: pOCIHandle;
...
begin
  if ADataType = otIntervalYM then
    ...
    ASSERT(False);
  inherited CreateUseHandle(AEnv, AHandle);
end;
...
var
  ...
  function Cvt(AValue: LongWord): sb4;
  begin
    Result := sb4(AValue);
...
{-------------------------------------------------------------------------------}
function OCINativeExceptionLoad(AStorage: TFDStorage): TObject;
begin
