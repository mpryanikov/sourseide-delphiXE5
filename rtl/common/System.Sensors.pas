...
// region monitoring will not work
  ...
  public
    ...
    property CustomPropery[const Prop]: Variant read GetCustomProperty write SetCustomProperty;
  ...
  protected
    ...
    // initiates the underlying receiving of location change notifications
    function DoStart: Boolean; virtual; abstract;
    // stops the underlying receving of location change notifications
  ...
  public
    ...
    // start and stop the reception of location change notifications
    procedure Start;
    procedure Stop;
  ...
  protected
    ...
    // initiates the underlying receiving of motion change notifications
    function DoStart: Boolean; virtual; abstract;
    // stops the underlying receving of motion change notifications
    procedure DoStop; virtual; abstract;
    constructor Create(AManager: TSensorManager); override;
  public
    // start and stop the reception of motion change notifications
    procedure Start;
    procedure Stop;
    // indicates if the sensor is started, periodically receiving updates
    property Started: Boolean read FStarted;
  ...
  protected
    ...
    function GetUpdateInterval: Double; virtual; abstract;
    procedure SetUpdateInterval(AInterval: Double); virtual; abstract;
    // initiates the underlying receiving of orientation change notifications
    function DoStart: Boolean; virtual;
    // stops the underlying receving of orientation change notifications
  ...
  public
    ...
    property HumanPresense: Boolean index TProperty.HumanPresence read GetBooleanProperty;
...
procedure TCustomLocationSensor.Start;
begin
  if not Started then
    FStarted := DoStart;
...
procedure TCustomLocationSensor.Stop;
begin
  if Started then
  begin
    DoStop;
...
procedure TCustomMotionSensor.Start;
begin
  if not Started then
    FStarted := DoStart;
end;

procedure TCustomMotionSensor.Stop;
begin
  if Started then
  begin
    DoStop;
...
function TCustomOrientationSensor.DoStart: Boolean;
begin
  Result := True;
end;

procedure TCustomOrientationSensor.DoStop;
begin
  // Nothing
end;
...
procedure TCustomOrientationSensor.Start;
begin
  if not Started then
    FStarted := DoStart;
end;

procedure TCustomOrientationSensor.Stop;
begin
  if Started then
  begin
    DoStop;
