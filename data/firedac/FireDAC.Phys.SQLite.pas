...
{$I FireDAC.inc}
{$IFDEF WIN32}
  {$HPPEMIT '#pragma link "FireDAC.Phys.SQLite.obj"'}
{$ELSE}
  {$HPPEMIT '#pragma link "FireDAC.Phys.SQLite.o"'}
{$ENDIF}
...
type
  ...
  private
    FAction: TFDSQLiteValidateAction;
    FDatabase: String;
    FMaxErrors: Integer;
  ...
  private
    FAction: TFDSQLiteSecurityAction;
    FDatabase: String;
    FToPassword: String;
    FPassword: String;
    FEncryption: String;
  ...
  published
    ...
    property Locale: TFDLocalID read FLocale write SetLocale
      {$IFDEF MSWINDOWS} default LOCALE_USER_DEFAULT {$ENDIF};
...
{$IFDEF POSIX}
  Posix.Pthread,
{$ENDIF}
  System.Variants, Data.FmtBCD, Data.SQLTimSt, System.SysUtils, Data.DB,
  FireDAC.Stan.Param, FireDAC.Stan.Error, FireDAC.Stan.Consts, FireDAC.Stan.Cipher, 
    FireDAC.Stan.Factory,
  FireDAC.DatS,
  FireDAC.Phys.Intf, FireDAC.Phys.SQLGenerator, FireDAC.Phys.SQLiteCli, FireDAC.Phys.SQLiteMeta;
...
{$ENDIF}
  ...
  private
    ...
    function FD2SQLiteDataType(ADataType: TFDDataType): TSQLiteExtDataType;
    function SQLite2ADDataType(ADataType: TSQLiteExtDataType;
...
begin
  ...
  try
    if DatabaseObj = nil then begin
      oSrc.Open(FDExpandStr(Database), smReadOnly, scDefault);
...
begin
  ...
  oDef.DriverID := DriverLink.ActualDriverID;
...
{-------------------------------------------------------------------------------}
procedure TFDSQLiteCollation.SetLocale(const AValue: TFDLocalID);
begin
...
const
  ...
  C_Flags: array [TFDSQLiteCollationFlag] of LongWord = (LINGUISTIC_IGNORECASE,
...
{$IFDEF MSWINDOWS}
    FCollation := TSQLiteCollationCompareStringW.Create(CliObj as TSQLiteLib);
    TSQLiteCollationCompareStringW(FCollation).OwningObj := Self;
    TSQLiteCollationCompareStringW(FCollation).Locale := Locale;
    TSQLiteCollationCompareStringW(FCollation).Flags := 0;
    for eFlag := Low(TFDSQLiteCollationFlag) to High(TFDSQLiteCollationFlag) do
      if eFlag in Flags then
        TSQLiteCollationCompareStringW(FCollation).Flags :=
          TSQLiteCollationCompareStringW(FCollation).Flags or C_Flags[eFlag];
{$ELSE}
    FDCapabilityNotSupported(Self, [S_FD_LPhys, DriverLink.ActualDriverID]);
{$ENDIF}
  end
  else begin
...
begin
  ...
  FLib := TSQLiteLib.Create(FDPhysManagerObj);
end;
...
{-------------------------------------------------------------------------------}
function TFDPhysSQLiteDriver.GetConnParamCount(AKeys: TStrings): Integer;
begin
  Result := inherited GetConnParamCount(AKeys) + 16;
end;

{-------------------------------------------------------------------------------}
procedure TFDPhysSQLiteDriver.GetConnParams(AKeys: TStrings; AIndex: Integer;
...
begin
  ...
  if AIndex < inherited GetConnParamCount(AKeys) then begin
    ...
    if AName = S_FD_ConnParam_Common_Database then begin
      ALoginIndex := 0;
      AType := '@F:SQLite Database|*.sdb;*.db';
    end
    else if AName = S_FD_ConnParam_Common_UserName then
      ALoginIndex := -1
    else if AName = S_FD_ConnParam_Common_Password then
      ALoginIndex := 1;
  end
  else begin
    ADefVal := '';
    ...
    0:
      begin
        AName := S_FD_ConnParam_SQLite_OpenMode;
        AType := S_FD_CreateUTF8 + ';' + S_FD_CreateUTF16 + ';' + S_FD_ReadWrite + ';' + S_FD_ReadOnly;
        ADefVal := S_FD_CreateUTF8;
      end;
    1:
      begin
        AName := S_FD_ConnParam_SQLite_Encrypt;
        AType := S_FD_No + ';' + FDCipherGetClasses();
        ADefVal := S_FD_No;
      end;
    2:
      begin
        AName := S_FD_ConnParam_SQLite_BusyTimeout;
        AType := '@I';
        ADefVal := '10000';
      end;
    3:
      begin
        AName := S_FD_ConnParam_SQLite_CacheSize;
        AType := '@I';
        ADefVal := S_FD_CacheSize;
      end;
    4:
      begin
        AName := S_FD_ConnParam_SQLite_SharedCache;
        AType := '@L';
        ADefVal := S_FD_True;
      end;
    5:
      begin
        AName := S_FD_ConnParam_SQLite_LockingMode;
        AType := S_FD_Normal + ';' + S_FD_Exclusive;
        ADefVal := S_FD_Exclusive;
      end;
    6:
      begin
        AName := S_FD_ConnParam_SQLite_Synchronous;
        AType := S_FD_Full + ';' + S_FD_Normal + ';' + S_FD_Off;
        ADefVal := S_FD_Off;
      end;
    7:
      begin
        AName := S_FD_ConnParam_SQLite_JournalMode;
        AType := S_FD_Delete + ';' + S_FD_Truncate + ';' + S_FD_Persist + ';' +
          S_FD_Memory + ';' + S_FD_WAL + ';' + S_FD_Off;
        ADefVal := S_FD_Delete;
      end;
    8:
      begin
        AName := S_FD_ConnParam_SQLite_ForeignKeys;
        AType := S_FD_On + ';' + S_FD_Off;
        ADefVal := S_FD_On;
      end;
    9:
      begin
        AName := S_FD_ConnParam_SQLite_StringFormat;
        AType := S_FD_Choose + ';' + S_FD_Unicode + ';' + S_FD_ANSI;
        ADefVal := S_FD_Choose;
      end;
   10:
      begin
        AName := S_FD_ConnParam_SQLite_GUIDFormat;
        AType := S_FD_String + ';' + S_FD_Binary;
        ADefVal := S_FD_String;
      end;
   11:
      begin
        AName := S_FD_ConnParam_SQLite_DateTimeFormat;
        AType := S_FD_String + ';' + S_FD_Binary + ';' + S_FD_DateTime;
        ADefVal := S_FD_String;
      end;
   12:
      begin
        AName := S_FD_ConnParam_SQLite_Extensions;
        AType := '@S';
        ADefVal := S_FD_False;
      end;
   13:
      begin
        AName := S_FD_ConnParam_SQLite_Advanced;
        AType := '@S';
      end;
   14:
      begin
...
begin
  Result := APwd;
  s := GetConnectionDef.AsString[S_FD_ConnParam_SQLite_Encrypt];
...
procedure TFDPhysSQLiteConnection.InternalConnect;
  ...
  begin
    AList.Clear;
    if GetConnectionDef.HasValue(AParam) then begin
      sExt := GetConnectionDef.AsString[AParam];
      i := 1;
  ...
  var
    s: String;
  begin
    if GetConnectionDef.HasValue(AParam) then
      s := GetConnectionDef.AsString[AParam]
    else
      s := ADef;
...
begin
  ...
  if GetConnectionDef.HasValue(S_FD_ConnParam_SQLite_OpenMode) then begin
    s := GetConnectionDef.AsString[S_FD_ConnParam_SQLite_OpenMode];
    if CompareText(s, S_FD_CreateUTF8) = 0 then begin
      eMode := smCreate;
      eEncoding := ecUTF8;
    end
    else if CompareText(s, S_FD_CreateUTF16) = 0 then begin
      eMode := smCreate;
      eEncoding := ecUTF16;
    end
    else if CompareText(s, S_FD_ReadWrite) = 0 then
      eMode := smReadWrite
    else if CompareText(s, S_FD_ReadOnly) = 0 then
      eMode := smReadOnly;
  end;
  if GetConnectionDef.HasValue(S_FD_ConnParam_SQLite_SharedCache) then
    if GetConnectionDef.AsBoolean[S_FD_ConnParam_SQLite_SharedCache] then
...
{$ENDIF}
  ...
  FStringFormat := etUnknown;
  s := GetConnectionDef.AsString[S_FD_ConnParam_SQLite_StringFormat];
  if CompareText(s, S_FD_Unicode) = 0 then
    FStringFormat := etUString
  else if CompareText(s, S_FD_ANSI) = 0 then
    FStringFormat := etString;
  FGuidFormat := etString;
  if CompareText(GetConnectionDef.AsString[S_FD_ConnParam_SQLite_GUIDFormat], S_FD_Binary) = 0 then
    FGuidFormat := etBlob;
  s := GetConnectionDef.AsString[S_FD_ConnParam_SQLite_DateTimeFormat];
  FDateTimeFormat := etString;
  if CompareText(s, S_FD_Binary) = 0 then
    FDateTimeFormat := etInteger
  else if CompareText(s, S_FD_DateTime) = 0 then
    FDateTimeFormat := etDateTime;
  FDatabase.DateTimeFormat := FDateTimeFormat;

  if GetConnectionDef.HasValue(S_FD_ConnParam_SQLite_BusyTimeout) then
    FBusyTimeout := GetConnectionDef.AsInteger[S_FD_ConnParam_SQLite_BusyTimeout]
  else
    FBusyTimeout := 10000;
  ...
  if InternalGetSharedCliHandle() = nil then begin
    ...
    if GetConnectionDef.HasValue(S_FD_ConnParam_Common_Password) then
      FDatabase.Key(PreparePwd(GetConnectionDef.Password));
    ...
    if GetConnectionDef.HasValue(S_FD_ConnParam_SQLite_Advanced) then begin
      s := GetConnectionDef.AsString[S_FD_ConnParam_SQLite_Advanced];
      i := 1;
      while i <= Length(s) do
        InternalExecuteDirect('PRAGMA ' + FDExpandStr(FDExtractFieldName(s, i)), nil);
    end;

    if GetConnectionDef.IsSpecified(S_FD_ConnParam_Common_NewPassword) then
      FDatabase.ReKey(PreparePwd(GetConnectionDef.NewPassword));
  end;
end;
...
begin
  if (FDatabase = nil) or (FDatabase.Handle = nil) then begin
    ...
    oDB.Open(GetConnectionDef.ExpandedDatabase, smReadWrite, scPrivate);
  end
  else
    oDB := FDatabase;
  try
...
begin
  if GetHandler <> nil then begin
    ...
    GetHandler.HandleEvent(oMsg.FName, oMsg.FParams);
  end;
end;
...
{-------------------------------------------------------------------------------}
function TFDPhysSQLiteCommand.FD2SQLiteDataType(ADataType: TFDDataType): TSQLiteExtDataType;
begin
...
{-------------------------------------------------------------------------------}
function TFDPhysSQLiteCommand.SQLite2ADDataType(ADataType: TSQLiteExtDataType;
  AUnsigned: Boolean): TFDDataType;
begin
  ...
  for i := 0 to Length(FParInfos) - 1 do begin
    ...
    try
      ...
      oFmtOpts.ResolveFieldType('', oParam.DataType, oParam.ADDataType, oParam.Size,
        oParam.Precision, oParam.NumericScale, eDestFldType, pInfo^.FSize, pInfo^.FPrec,
        pInfo^.FScale, pInfo^.FSrcDataType, pInfo^.FDestDataType, False);
      pInfo^.FOutSQLDataType := FD2SQLiteDataType(pInfo^.FDestDataType);
      pInfo^.FOutDataType := SQLite2ADDataType(pInfo^.FOutSQLDataType,
  ...
  for i := 0 to Length(FColInfos) - 1 do begin
    ...
    try
      ...
      if GetMetaInfoKind = mkNone then
        oFmtOpts.ResolveDataType(pInfo^.FName, pInfo^.FSrcDataType, pInfo^.FSize,
          pInfo^.FPrec, pInfo^.FScale, pInfo^.FDestDataType, pInfo^.FSize, True)
      else
...
{-------------------------------------------------------------------------------}
function TFDPhysSQLiteCommand.InternalOpen: Boolean;
begin
...
{-------------------------------------------------------------------------------}
procedure TFDPhysSQLiteCommand.FetchRow(ATable: TFDDatSTable;
...
initialization
  FDPhysManager();
  FDPhysManagerObj.RegisterDriverClass(TFDPhysSQLiteDriver);
  FDExtensionManager().AddExtension([TFDPhysSQLitePostEventFunc]);

finalization
//  FDPhysManagerObj.UnRegisterDriverClass(TFDPhysSQLiteConnection);
