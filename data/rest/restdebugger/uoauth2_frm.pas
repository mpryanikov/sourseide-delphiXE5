...
begin
  ...
  begin
    MessageDlg('Please provide an Authentication-Endpoint', TMsgDlgType.mtError, [TMsgDlgBtn.mbOK], 0);
    EXIT;
  end;
  if (edt_OAuth2ClientID.Text = '') then
  begin
    MessageDlg('Please provide a Client-ID (sometimes called "App-Key")', TMsgDlgType.mtError, [TMsgDlgBtn.mbOK], 0);
    EXIT;
  end;
...
begin
  ...
  begin
    MessageDlg('Please provide an Authentication-Code. This value can be requested in Step #1 "Authorization" ',
      TMsgDlgType.mtError, [TMsgDlgBtn.mbOK], 0);
    EXIT;
  end;
  if (edt_OAuth2AccessTokenEndpoint.Text = '') then
  begin
    MessageDlg('Please provide a Token-Endpoint', TMsgDlgType.mtError, [TMsgDlgBtn.mbOK], 0);
    EXIT;
  end;
  if (edt_OAuth2ClientID.Text = '') OR (edt_OAuth2ClientSecret.Text = '') then
  begin
    MessageDlg('Please provide a Client-ID and a Client-Secret (sometimes called "App-Key" / "App-Secret")',
      TMsgDlgType.mtError, [TMsgDlgBtn.mbOK], 0);
    EXIT;
  end;
