...
type
  ...
  private
    ...
    function MatchRecordSetColumn(const AColumn: Variant; AColNameKind: TFDPhysNameKind): Boolean;
  protected
  ...
  private
    ...
    FOwner: TPersistent;
    function Find(const AColumn: Variant; ANameKind: TFDPhysNameKind): Integer;
  ...
  public
    ...
    function Lookup(const AColumn: Variant; ANameKind: TFDPhysNameKind): TFDDAptColumnMapping;
    ...
    procedure Remove(const AColumn: Variant; ANameKind: TFDPhysNameKind);
...
{-------------------------------------------------------------------------------}
function TFDDAptColumnMapping.MatchRecordSetColumn(const AColumn: Variant;
  AColNameKind: TFDPhysNameKind): Boolean;
var
  oCol: TFDDatSColumn;
begin
  case AColNameKind of
  nkID:
    Result := SourceColumnID = AColumn;
  nkSource:
    Result := {$IFDEF FireDAC_NOLOCALE_META} CompareText {$ELSE} AnsiCompareText {$ENDIF}
      (AColumn, SourceColumnName) = 0;
  nkDatS:
    Result := {$IFDEF FireDAC_NOLOCALE_META} CompareText {$ELSE} AnsiCompareText {$ENDIF}
      (AColumn, DatSColumnName) = 0;
  nkObj:
    begin
      oCol := TFDDatSColumn(LongWord(AColumn));
      Result :=
...
{-------------------------------------------------------------------------------}
function TFDDAptColumnMappings.Find(const AColumn: Variant; ANameKind: TFDPhysNameKind): Integer;
var
  i: Integer;
begin
  Result := -1;
...
{-------------------------------------------------------------------------------}
function TFDDAptColumnMappings.Lookup(const AColumn: Variant;
  ANameKind: TFDPhysNameKind): TFDDAptColumnMapping;
var
  i: Integer;
begin
...
{-------------------------------------------------------------------------------}
procedure TFDDAptColumnMappings.Remove(const AColumn: Variant;
  ANameKind: TFDPhysNameKind);
var
  i: Integer;
begin
  i := Find(AColumn, ANameKind);
