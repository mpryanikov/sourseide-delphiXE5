...
{$ENDIF}
  ...
  FireDAC.Stan.Intf, FireDAC.Stan.Storage, FireDAC.Stan.Error, FireDAC.Stan.Util,
...
{$ENDIF}
  protected
    ...
    procedure SetPtrAttribute(AAttr: SQLInteger; AValue: Pointer); virtual;
...
{$ENDIF}
  ...
  TODBCDriverKind = (dkOther, dkSQLSrv, dkSQLNC, dkFreeTDS, dkSQLSrvOther,
    dkDB2, dkMySQL, dkOracleOra, dkOracleMS, dkOracleOther, dkMSAccessJet,
    dkMSAccessACE, dkFirebird, dkInterbase, dkSQLAnywere, dkSQLite,
    dkPostgreSQL, dkAdvantage, dkNexus, dkSybaseASE, dkInformix,
    dkPWMicroFocus, dkDBF, dkSQLBase, dkElevateDB, dkDBISAM, dkSolidDB);
...
{$ENDIF}
    ...
    function GetInfoOptionSQLLen(AInfoType: Integer): SQLLen;
  ...
  protected
    ...
    procedure SetAttribute(AAttr: SQLInteger; ApValue: SQLPointer;
      ACharData: Boolean = False); override;
    procedure SetPtrAttribute(AAttr: SQLInteger; AValue: Pointer); override;
    procedure GetInfo(AInfoType: SQLUSmallint; var AInfoValue: String); overload;
{$IFDEF FireDAC_64}
    procedure GetInfo(AInfoType: SQLUSmallint; var AInfoValue: SQLLen); overload;
{$ENDIF}
...
{$ENDIF}
    ...
    property DESCRIBE_OUTPUT_LEVEL: String index SQL_ATTR_DESCRIBE_OUTPUT_LEVEL read GetCharAttribute write SetCharAttribute;
    ...
    property DRIVER_HENV: SQLLen index SQL_DRIVER_HENV read GetInfoOptionSQLLen;
...
{$ENDIF}
  ...
  public
    ...
    procedure Bind(APageBufferClass: TODBCPageBufferClass; ARowCount: SQLULen;
  ...
  protected
    ...
    procedure GetDataPtr(AVar: TODBCVariable; AIndex: SQLULen;
      out ApData: SQLPointer; out ApIndicator: PSQLLen); virtual;
    procedure ShiftBuffersPtr(ARowCount: SQLLen); virtual;
    procedure UnShift;
  ...
  private
    ...
    function FetchLateBindedColumn(AVariable: TODBCVariable; AIndex: Integer;
      ASize: SQLLen): SQLReturn;
    procedure SetPos(ARowNum: SQLULen; AOperation: SQLUSmallint;
...
{$ENDIF}
  ...
  private
    ...
    function PutLongParam(AIndex: Integer): SQLReturn;
{$IFDEF FireDAC_MONITOR}
...
{$IFDEF UNIX}
  dynlibs,
{$ENDIF}
  System.SysUtils, System.SyncObjs, Data.SqlTimSt, System.DateUtils,
...
{-------------------------------------------------------------------------------}
procedure TFDODBCNativeError.LoadFromStorage(AStorage: TFDStorage);
begin
...
{-------------------------------------------------------------------------------}
procedure TFDODBCNativeError.SaveToStorage(AStorage: TFDStorage);
begin
...
begin
  eKind := ekOther;
  iADCode := er_FD_ODBCGeneral;
  case AStatus of
  SQL_SUCCESS:           sRetCode := 'SQL_SUCCESS';
  SQL_SUCCESS_WITH_INFO: sRetCode := 'SQL_SUCCESS_WITH_INFO';
  SQL_NO_DATA:           begin sRetCode := 'SQL_NO_DATA';
                               eKind := ekNoDataFound;
                               iADCode := er_FD_AccExactMismatch;
                         end;
...
begin
  if AKind = ekOther then
    if ASQLState = 'HY008' then
      AKind := ekCmdAborted
            ...
            (ASQLState = '08003') or (ASQLState = '08004') or
            (AHandle.Connection <> nil) and (AHandle.Connection.DriverKind = dkInformix) and
              (ANativeError = -25580) then
      AKind := ekServerGone
  ...
  if (AHandle is TODBCCommandStatement) and (ARowIndex = -1) then begin
    ...
    if ARowIndex < 0 then
      ARowIndex := -1
    else if TODBCCommandStatement(AHandle).FParamList.FBuffer <> nil then
      Inc(ARowIndex, TODBCCommandStatement(AHandle).FParamList.FBuffer.Shifted);
  end;
    ...
    while not Terminated and FMonitor.Tracing do begin
      ...
      if not PeekNamedPipe(hPipe, nil, 0, nil, @dwTotalBytesAvail, nil) or
        ...
        if FLockOutput = 0 then begin
          pMsg := nil;
          iLen := LongWord(FEncoder.Decode(@aBuff, dwBytesRead, Pointer(pMsg)));
          i := 0;
          ...
          if pStr <> nil then begin
            Inc(pStr);
            iLen := iLen - i - LongWord(pStr - (pMsg + i));
          end
          else begin
...
begin
  ...
  FhDLL := FhCPLib;
  try
    @SQLConfigDataSource := GetODBCTextProcAddress('SQLConfigDataSource');
    @SQLInstallerError   := GetODBCTextProcAddress('SQLInstallerError');
  finally
    FhDLL := hPrev;
  end;
end;
  ...
  try
    ...
    SQL_SUCCESS_WITH_INFO:
      if oConn <> nil then begin
        ...
        if (oConn.RdbmsKind = mkMSSQL) and (
            // MSSQL:
        ...
        else if (oConn.RdbmsKind = mkDB2) and
            // DB2:
...
begin
  SetAttribute(AAttr, PSQLLen(AValue), False);
end;

{-------------------------------------------------------------------------------}
procedure TODBCHandle.SetSQLULenAttribute(AAttr: SQLInteger; AValue: SQLULen);
begin
  SetAttribute(AAttr, PSQLULen(AValue), False);
end;
...
begin
  ...
  if Match('SQLSRV') then
  ...
  else if Match('DB2CLI') or Match('LIBDB2') or Match('IVDB2') or Match('PBDB2') then
    FDriverKind := dkDB2
  ...
  else if Match('DBODBC') and (sDbms <> 'DBISAM') or Match('WOD') then
    FDriverKind := dkSQLAnywere
    ...
    FDriverKind := dkNexus
  else if Match('FDAPTIVE SERVER ENTERPRISE') or Match('SYODASE') or Match('SYSYBNT') then
  ...
  dkInterbase:
    FRdbmsKind := mkInterbase;
  dkFirebird:
    FRdbmsKind := mkFirebird;
  dkSQLAnywere:
    FRdbmsKind := mkASA;
  dkSQLite:
    FRdbmsKind := mkSQLite;
  dkPostgreSQL:
    FRdbmsKind := mkPostgreSQL;
  dkAdvantage:
    FRdbmsKind := mkADS;
  dkNexus:
    FRdbmsKind := mkNexus;
...
{$ENDIF}
  ...
  iOutConnStrLen := 0;
  Check(Lib.SQLDriverConnect(FHandle, {$IFDEF MSWINDOWS} GetDesktopWindow {$ENDIF}
                                      {$IFDEF POSIX} 0 {$ENDIF},
    PSQLChar(AConnString), SQL_NTS, PSQLChar(Result), C_RETURNED_STRING_MAXLEN,
    iOutConnStrLen, ADriverCompletion));
  FConnected := True;
  AfterConnect;
...
{-------------------------------------------------------------------------------}
{$IFDEF FireDAC_64}
procedure TODBCConnection.GetInfo(AInfoType: SQLUSmallint; var AInfoValue: SQLLen);
begin
  AInfoValue := 0;
...
{-------------------------------------------------------------------------------}
function TODBCConnection.GetInfoOptionSQLLen(AInfoType: Integer): SQLLen;
begin
  Result := 0;
  GetInfo(SQLUSmallint(AInfoType), Result);
end;
...
const
  C_DefKeywords =
    'ABSOLUTE,IS,ACTION,ISOLATION,FDA,JOIN,ADD,KEY,' +
  ...
  C_MSAccAdditionalKeywords =
    ...
    'VARP,YESNO';
begin
  // On Informix 3.70.TC5DE SQL_KEYWORDS leads to AV
  if FDriverKind = dkInformix then
    Result := C_DefKeywords
  else begin
    // "div" in next SetLength is needed to avoid SQL_ERROR on "SQL Anywhere 11"
    // driver in Unicode mode.
    SetLength(Result, $3FFF div SizeOf(Char));
    GetInfo(SQLUSmallint(SQL_KEYWORDS), Result);
    Result := C_DefKeywords + Result;
  end;
  // The following is a workaround for the MSOff 2007 Access ODBC driver,
  // which truncates the key word list.
  if (FRdbmsKind = mkMSAccess) and (Pos(',TEXT,', Result) = 0) then
...
var
  iStringLen: SQLInteger;
begin
  if ACharData and (ApValue <> nil) then
    iStringLen := StrLen(PChar(ApValue)) * SizeOf(Char)
  else
    iStringLen := 0;
...
{-------------------------------------------------------------------------------}
procedure TODBCConnection.SetPtrAttribute(AAttr: SQLInteger; AValue: Pointer);
begin
  Check(Lib.SQLSetConnectAttr(FHandle, AAttr, AValue, SQL_IS_POINTER));
end;
...
begin
  ...
  FStmt := AOwner;
  FPosition := AColNum;
  iColMaxLen := TODBCConnection(FStmt.Connection).MAX_COL_NAME_LEN;
  ...
  case FStmt.Connection.RdbmsKind of
  mkMSSQL:
...
procedure ODBCBcd2Numeric(const ABcd: TBcd; out ANum: TSQLNumericRec);
const
  MaxWords = SQL_MAX_NUMERIC_LEN div SizeOf(LongWord);
var
  i, j, k: Integer;
  iMulDer: Integer;
  iNibble: Integer;
  iDev, iVal: Int64;
  aMul: array [0..MaxWords - 1] of LongWord;
  aVal: array [0..MaxWords - 1] of LongWord;
  iMulSize: Integer;
  iValSize: Integer;
begin
  ...
  iValSize := 1;
  aMul[0] := 1;
  FillChar(aVal[0], MaxWords * SizeOf(LongWord), 0);
  ...
  for i := k downto 0 do begin
    ...
    for j := 0 to iValSize - 1 do begin
      ...
      aVal[j] := LongWord(iVal and $FFFFFFFF);
      iDev := iVal shr 32;
    end;
    ...
    if i > 0 then begin
      ...
      for j := 0 to iMulSize - 1 do begin
        ...
        aMul[j] := LongWord(iVal and $FFFFFFFF);
        iDev := iVal shr 32;
      end;
      if iDev <> 0 then begin
        aMul[iMulSize] := iDev;
  ...
  Move(aVal[0], ANum.Val[0], iValSize * SizeOf(LongWord));
...
begin
  ...
  if (oConn.DriverODBCVersion < ovODBC3) or
     (oConn.DriverKind in [dkElevateDB, dkFreeTDS]) then
    Exit;
  oStmt := FList.Owner;
  case CDataType of
  SQL_C_NUMERIC:
...
begin
  FDException(FList.FOwner.OwningObj, [S_FD_LPhys, S_FD_ODBCId],
...
begin
  ...
  if (pInd^ = SQL_NULL_DATA) or (pData = nil) then begin
      ...
      begin
        ...
        SQL_SS_TIME2:
          if AByRef then
            ...
            PLongWord(ApData)^ := (pTime2^.Second + pTime2^.Minute * 60 +
              pTime2^.Hour * 3600) * 1000 + pTime2^.Fraction div 1000000;
            ASize := SizeOf(LongWord);
          end;
        else
          begin
            if (SQLDataType = SQL_BINARY) and FList.FOwner.StrsTrim then begin
            ...
            if (ASize = 0) and FList.FOwner.StrsEmpty2Null then begin
              Result := False;
              ASize := 0;
              if AByRef then
                ApData := nil;
    ...
    SQL_C_CHAR:
      begin
        ...
        SQL_DECIMAL:
          ...
          begin
            if (SQLDataType = SQL_CHAR) and FList.FOwner.StrsTrim then begin
              pTail := PFDAnsiString(pData) + ASize - 1;
    ...
    SQL_C_WCHAR:
      begin
        if ASize = SQL_NTS then
          ASize := FDWideStrLen(PWideChar(pData))
        else
          ASize := ASize div SizeOf(WideChar);
        if ((SQLDataType = SQL_WCHAR) or (SQLDataType = SQL_GRAPHIC)) and FList.FOwner.StrsTrim then begin
        ...
        else if ((SQLDataType = SQL_XML) or (SQLDataType = SQL_SS_XML)) and (PWideChar(pData)^ = #$FEFF) then begin
          Inc(PWideChar(pData));
          Dec(ASize);
        end;
        if (ASize = 0) and FList.FOwner.StrsEmpty2Null then begin
    ...
    SQL_C_DATE:
      if AByRef then
        ...
        PLongWord(ApData)^ := LongWord(Trunc(EncodeDate(pDate^.Year,
    ...
    SQL_C_TIME:
      if AByRef then
        ...
        PLongWord(ApData)^ := (pTime^.Second + pTime^.Minute * 60 + pTime^.Hour * 3600) * 1000;
        ASize := SizeOf(LongWord);
      end;
...
begin
  if LongData and IsParameter then
    if (ParamType in [SQL_PARAM_INPUT_OUTPUT, SQL_PARAM_OUTPUT]) and (ASize < DataSize) then
  ...
  SQL_C_BINARY:
    if SQLDataType = SQL_SS_TIME2 then begin
      if lIsNull then
        ...
        pTime2^.Fraction := SQLUInteger(LongWord(ApData^) mod 1000 * 1000000);
      ...
      if lIsNull then begin
        ...
        iSize := ASize;
        if (SQLDataType = SQL_BINARY) and FList.FOwner.StrsTrim then
  ...
  SQL_C_WCHAR:
    if lIsNull then begin
      ...
      if ASize = SQL_NTS then
        iSize := FDWideStrLen(PWideChar(ApData))
      else
        iSize := ASize;
      if (iSize * SizeOf(WideChar) > DataSize) and not LongData then
        ErrorDataTooLarge(DataSize, iSize * SizeOf(WideChar));
      if ((SQLDataType = SQL_WCHAR) or (SQLDataType = SQL_GRAPHIC)) and FList.FOwner.StrsTrim then
...
begin
  ...
  if SQLDataType = SQL_SS_VARIANT then
    FMSSQLVariantBinary := Connection.MSSQLVariantBinary
  ...
  SQL_C_BINARY:
    ...
    SQL_VARCHAR:
      FLongData := (DataSize = MAXINT) or
        not IsParameter and (DataSize > FList.FOwner.MaxStringSize + SizeOf(SQLChar));
    SQL_BINARY,
    SQL_VARBINARY:
      FLongData := (DataSize = MAXINT) or
        not IsParameter and (DataSize > FList.FOwner.MaxStringSize);
    SQL_WCHAR,
    SQL_WVARCHAR,
    SQL_GRAPHIC,
    SQL_VARGRAPHIC,
    ...
    SQL_SS_XML:
      FLongData := (DataSize = MAXINT) or
        not IsParameter and (DataSize > FList.FOwner.MaxStringSize + SizeOf(SQLWChar));
    end;
  end;
...
{$IFDEF FireDAC_MONITOR}
{-------------------------------------------------------------------------------}
function TODBCVariable.DumpValue(AIndex: SQLULen; var ASize: SQLLen): UnicodeString;
const
...
begin
  ...
  if not GetData(AIndex, pData, ASize, True) then
    ...
    SQL_C_BINARY:
      ...
      else if ASize > 512 then begin
        s := FDBin2Hex(pData, 512);
        Result := UnicodeString('(truncated at 512) ' + s + ' ...');
      end
      else begin
        s := FDBin2Hex(pData, ASize);
        Result := UnicodeString(s);
      end;
    SQL_C_CHAR:
      if ASize > 1024 then begin
        s := Connection.FEncoder.Decode(pData, 1024, ecANSI);
        Result := UnicodeString('(truncated at 1024) ''' + s + ' ...''');
      end
      else begin
        s := Connection.FEncoder.Decode(pData, ASize, ecANSI);
        Result := UnicodeString('''' + s + '''');
      end;
    SQL_C_WCHAR:
      if ASize > 1024 then begin
        ...
        SetString(sw, PWideChar(pData), ASize);
        Result := '''' + sw + '''';
      end;
    SQL_C_TYPE_DATE,
    SQL_C_DATE:
...
{-------------------------------------------------------------------------------}
function TODBCVariable.DumpValue(AIndex: SQLULen): UnicodeString;
var
  iSize: SQLLen;
begin
...
begin
  if SQLDataType = SQL_REFCURSOR then
    Exit;
  ASSERT(FList.Owner is TODBCCommandStatement);
  FBinded := False;
  oStmt := FList.Owner;
  if not LongData then begin
    ...
    if (ColumnSize = MAXINT) and (CDataType <> SQL_C_WCHAR) then
      iColSize := ColumnSize
    else
      iColSize := MAXINT div 2;
    if ParamType = SQL_PARAM_INPUT then begin
      FBinded := True;
      oStmt.Check(oStmt.Lib.SQLBindParameter(oStmt.Handle, Position, ParamType,
        CDataType, SQLDataType, iColSize, 0, SQLPointer(Position), 0,
        LocalBufIndicator));
    end
...
procedure TODBCVariableList.Bind(APageBufferClass: TODBCPageBufferClass;
  ARowCount: SQLULen; AFixedLenOnly: Boolean);
begin
  ASSERT(Count > 0);
  if FBuffer = nil then
...
begin
  for i := 0 to Count - 1 do begin
    ...
    if not (AFixedLenOnly and ((oVar.CDataType = SQL_C_CHAR) or
       ...
       (oVar.ParamType in [SQL_PARAM_INPUT, SQL_PARAM_INPUT_OUTPUT]) then
      oVar.Bind;
  end;
end;
...
begin
  ASSERT(Parameters and (AVariable is TODBCParameter) or
         not Parameters and (AVariable is TODBCColumn));
  Result := AVariable;
  FList.Add(AVariable);
...
var
  ...
  oVar: TODBCVariable;
begin
  // Clear ROWS_FETCHED_PTR to workaround Advantage ODBC driver bug
  if not FVariableList.Parameters then
...
begin
  if FVariableList.Parameters then begin
    oCmd := TODBCCommandStatement(FVariableList.Owner);
...
procedure TODBCPageBuffer.GetDataPtr(AVar: TODBCVariable; AIndex: SQLULen;
  out ApData: SQLPointer; out ApIndicator: PSQLLen);
begin
  ASSERT(False);
end;
...
type
  ...
  protected
    ...
    procedure GetDataPtr(AVar: TODBCVariable; AIndex: SQLULen;
      out ApData: SQLPointer; out ApIndicator: PSQLLen); override;
    procedure ShiftBuffersPtr(ARowCount: SQLLen); override;
  end;
...
begin
  ...
  for i := 0 to FVariableList.Count - 1 do begin
    ...
    oVar := FVariableList[i];
    oVar.LocalBuffer := oVar.GetDataInd(0, pBuffer, pInd);
    oVar.LocalBufIndicator := pInd;
    if oVar.LongData then
      for j := 0 to FRowCount - 1 do
        oVar.GetDataInd(j, pBuffer, pInd)^ := nil;
  end;
end;
...
procedure TODBCRowWiseBuffer.GetDataPtr(AVar: TODBCVariable;
  AIndex: SQLULen; out ApData: SQLPointer; out ApIndicator: PSQLLen);
var
  i: Integer;
  iOffset: Integer;
...
begin
  ...
  for i := 0 to FVariableList.Count - 1 do begin
    ...
    if oVarCur = AVar then
      Break;
    if not oVarCur.LongData then
      Inc(iOffset, oVarCur.DataSize + SizeOf(SQLLen))
    else
      Inc(iOffset, SizeOf(Pointer) + SizeOf(SQLLen));
  end;

  ApData := SQLPointer(NativeInt(FCommonBuffer) + iOffset);
  if not AVar.LongData then
    ApIndicator := PSQLLen(NativeInt(ApData) + AVar.DataSize)
  else
    ApIndicator := PSQLLen(NativeInt(ApData) + SizeOf(Pointer));
end;
...
begin
  if FVariableList.Parameters then begin
    if TODBCCommandStatement(FVariableList.Owner).ParamSetSupported then
      TODBCCommandStatement(FVariableList.Owner).PARAM_BIND_TYPE := RowSize;
  end
  else
    FVariableList.Owner.ROW_BIND_TYPE := RowSize;
  inherited SetStmtAttributes;
end;

{-------------------------------------------------------------------------------}
procedure TODBCRowWiseBuffer.ShiftBuffersPtr(ARowCount: SQLLen);
var
  iOffset: SQLLen;
  i: Integer;
  oVar: TODBCVariable;
begin
  ...
  for i := 0 to FVariableList.Count - 1 do begin
    oVar := FVariableList[i];
    Inc(NativeInt(oVar.FLocalBuffer), iOffset);
    Inc(NativeInt(oVar.FLocalBufIndicator), iOffset);
  end;
...
begin
  ...
  ApIndicator := PSQLLen(NativeInt(AVar.LocalBufIndicator) + AIndex * SizeOf(SQLLen));
end;
...
{-------------------------------------------------------------------------------}
procedure TODBCColWiseBuffer.ShiftBuffersPtr(ARowCount: SQLLen);
var
  i: Integer;
  oVar: TODBCVariable;
begin
...
{-------------------------------------------------------------------------------}
function TODBCStatementBase.FetchLateBindedColumn(AVariable: TODBCVariable;
...
begin
  ...
  try
    AVariable.GetDataInd(AIndex, pBuffer, pInd);
    if AVariable.CDataType = SQL_C_NUMERIC then
      iDataType := SQL_ARD_TYPE
    else if AVariable.MSSQLVariantBinary then
      if (AVariable.FCVariantSbType = 0) or (ASize = 0) then
        iDataType := AVariable.CDataType
      else if AVariable.FCVariantSbType = SQL_C_NUMERIC then
        iDataType := SQL_C_CHAR
      else
        iDataType := AVariable.FCVariantSbType
    else
      iDataType := AVariable.CDataType;
    Result := Lib.SQLGetData(FHandle, AVariable.Position, iDataType, pBuffer, ASize, pInd);
...
{-------------------------------------------------------------------------------}
procedure TODBCStatementBase.SetPos(ARowNum: SQLULen; AOperation: SQLUSmallint;
  ALocktype: SQLUSmallint);
begin
  ...
  for i := 0 to FColumnList.Count - 1 do begin
    ...
    if oVar.LateBinding then begin
      ...
      if oBuffer.RowsProcessed > 1 then
        SetPos(AIndex + 1, SQL_POSITION, SQL_LOCK_NO_CHANGE);

      // MS SQL Server VARIANT data type fetching
      if oVar.MSSQLVariantBinary then begin
        ppBuffer := oVar.GetDataPtr(AIndex, pBuffer, iCurSize, pInd);
        ppBuffer^ := @iDummy;
        FetchLateBindedColumn(oVar, AIndex, 0);
        ppBuffer^ := pBuffer;
        if pInd^ <> SQL_NULL_DATA then begin
          iVarType := 0;
          ColAttributeIntSilent(oVar.Position, SQL_CA_SS_VARIANT_TYPE, iVarType);
          oVar.FCVariantSbType := iVarType;
          if iVarType = SQL_NUMERIC then
            Inc(pInd^, 2);
          if iCurSize < pInd^ then
            oVar.AllocLongData(AIndex, pInd^);
          FetchLateBindedColumn(oVar, AIndex, pInd^);
        end;
      end

      // BLOB data
      else if oVar.LongData then begin
        case oVar.CDataType of
        SQL_C_CHAR:   iZeroBytes := SizeOf(TFDAnsiChar);
        SQL_C_WCHAR:  iZeroBytes := SizeOf(WideChar);
        else          iZeroBytes := 0;
        end;

        oVar.GetDataPtr(AIndex, pBuffer, iCurSize, pInd);
        iIteration := 0;
        iCurSize := 0;
        iFetchSize := FPieceBuffLen;
        iBufferLen := iFetchSize;
        ppBuffer := oVar.AllocLongData(AIndex, iBufferLen);
        pBuffer := ppBuffer^;
        ...
        try
          try
            repeat
              ...
              Inc(iIteration);
              iRet := FetchLateBindedColumn(oVar, AIndex, iFetchSize);
              lGetAll :=
                (iRet = SQL_NO_DATA) or
      ...
      // other late binding data
        ...
        FetchLateBindedColumn(oVar, AIndex, oVar.DataSize);
    end;
  end;
end;
  ...
  try
    ...
    repeat
      ...
      if iRet = SQL_NO_DATA then
        ...
        // RowsProcessed = 1 - 1 row fetched only - immediatelly FetchLateBindedColumn,
...
begin
  if (FParamList <> nil) and (FParamList.Count > 0) then begin
    FParamList.Clear;
...
{-------------------------------------------------------------------------------}
function TODBCCommandStatement.PutLongParam(AIndex: Integer): SQLReturn;
var
...
begin
  ...
  if not (TODBCConnection(Owner).DriverKind in [dkSQLSrv, dkSQLNC]) or
...
var
  ...
  begin
    ...
    if (ARowCount > 1) and (oBuffer <> nil) then begin
      ...
      if (TODBCConnection(Owner).DriverKind in [dkSQLSrv, dkSQLNC]) and
         (iTimes = 1) then begin
      ...
      else if oBuffer.RowsProcessed <> 0 then
        for i := 0 to ARowCount - oBuffer.Shifted - 1 do
          if oBuffer.FRowStatusArr[i] in [SQL_PARAM_SUCCESS, SQL_INFX_PARAM_SUCCESS, SQL_PARAM_SUCCESS_WITH_INFO] then
            Inc(iRes);
    end

    // MSSQL: if SET NOCOUNT ON, then there is no count
    else if (TODBCConnection(Owner).DriverKind in [dkSQLSrv, dkSQLNC]) and
            (SS_NOCOUNT_STATUS = SQL_NC_ON) then
      iRes := -1
      ...
      if TODBCConnection(Owner).RdbmsKind = mkMSAccess then
...
{$ENDIF}
  ...
  try
        ...
        try
          if iRet = SQL_NEED_DATA then
            if VarBindingKind = bkColumnWise then
              ...
              iRet := PutLongParam(iCurRow);
              Inc(iCurRow);
              if ARowCount > iCurRow + AOffset then
                oBuffer.ShiftBuffersPtr(1);
            end;
          if (iRet <> SQL_NO_DATA) or AExact or (iTimes > 1) then
            Check(iRet);
        except
          on E: EODBCNativeException do begin
            AdjustRowIndex(E);
    ...
    until (VarBindingKind = bkColumnWise) or
          (VarBindingKind = bkRowWise) and (iTimes = iCurRow);
...
begin
  if Tracing then begin
    ...
    for i := 0 to FParamList.Count - 1 do begin
      oVar := FParamList.Items[i];
      if AInput and ((oVar.ParamType = SQL_PARAM_INPUT) or (oVar.ParamType = SQL_PARAM_INPUT_OUTPUT)) or
         not AInput and ((oVar.ParamType = SQL_PARAM_INPUT_OUTPUT) or (oVar.ParamType = SQL_PARAM_OUTPUT) or
                         (oVar.ParamType = SQL_RETURN_VALUE)) then begin
        iLen := 0;
...
{-------------------------------------------------------------------------------}
function ODBCNativeExceptionLoad(AStorage: TFDStorage): TObject;
begin
